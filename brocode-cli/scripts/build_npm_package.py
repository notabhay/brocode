...
BROCODE_CLI_ROOT = SCRIPT_DIR.parent
...
PACKAGE_EXPANSIONS: dict[str, list[str]] = {
    "brocode": ["brocode", *BROCODE_PLATFORM_PACKAGES],
}

PACKAGE_NATIVE_COMPONENTS: dict[str, list[str]] = {
    "brocode": [],
    "brocode-linux-x64": ["brocode", "rg"],
    "brocode-linux-arm64": ["brocode", "rg"],
    "brocode-darwin-x64": ["brocode", "rg"],
    "brocode-darwin-arm64": ["brocode", "rg"],
    "brocode-win32-x64": ["brocode", "rg", "brocode-windows-sandbox-setup", "brocode-command-runner"],
    "brocode-win32-arm64": ["brocode", "rg", "brocode-windows-sandbox-setup", "brocode-command-runner"],
    "brocode-responses-api-proxy": ["brocode-responses-api-proxy"],
    "brocode-sdk": [],
}

PACKAGE_TARGET_FILTERS: dict[str, str] = {
    package_name: package_config["target_triple"]
    for package_name, package_config in BROCODE_PLATFORM_PACKAGES.items()
}
...
COMPONENT_DEST_DIR: dict[str, str] = {
    "brocode": "brocode",
    "brocode-responses-api-proxy": "brocode-responses-api-proxy",
    "brocode-windows-sandbox-setup": "brocode",
    "brocode-command-runner": "brocode",
    "rg": "path",
}


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Build or stage the Brocode CLI npm package.")
    parser.add_argument(
        "--package",
        choices=PACKAGE_CHOICES,
        default="brocode",
        help="Which npm package to stage (default: brocode).",
    )
    parser.add_argument(
        "--version",
        ...
        if release_version:
            ...
            if package == "brocode":
                print(
                    ...
                    "Verify the CLI:\n"
                    f"    node {staging_dir_str}/bin/brocode.js --version\n"
                    f"    node {staging_dir_str}/bin/brocode.js --help\n\n"
                )
            elif package == "brocode-responses-api-proxy":
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
                    "Verify the responses API proxy:\n"
                    f"    node {staging_dir_str}/bin/brocode-responses-api-proxy.js --help\n\n"
                )
            elif package in BROCODE_PLATFORM_PACKAGES:
                print(
                    f"Staged version {version} for release in {staging_dir_str}\n\n"
...
def prepare_staging_dir(staging_dir: Path | None) -> tuple[Path, bool]:
    ...
    temp_dir = Path(tempfile.mkdtemp(prefix="brocode-npm-stage-"))
...
def stage_sources(staging_dir: Path, version: str, package: str) -> None:
    ...
    if package == "brocode":
        ...
        shutil.copy2(BROCODE_CLI_ROOT / "bin" / "brocode.js", bin_dir / "brocode.js")
        rg_manifest = BROCODE_CLI_ROOT / "bin" / "rg"
        ...
        package_json_path = BROCODE_CLI_ROOT / "package.json"
    elif package in BROCODE_PLATFORM_PACKAGES:
        platform_package = BROCODE_PLATFORM_PACKAGES[package]
        ...
        with open(BROCODE_CLI_ROOT / "package.json", "r", encoding="utf-8") as fh:
            brocode_package_json = json.load(fh)

        package_json = {
            "name": BROCODE_NPM_NAME,
            "version": platform_version,
            "license": brocode_package_json.get("license", "Apache-2.0"),
            "os": [platform_package["os"]],
            "cpu": [platform_package["cpu"]],
            "files": ["vendor"],
            "repository": brocode_package_json.get("repository"),
        }

        engines = brocode_package_json.get("engines")
        if isinstance(engines, dict):
            package_json["engines"] = engines

        package_manager = brocode_package_json.get("packageManager")
        if isinstance(package_manager, str):
            package_json["packageManager"] = package_manager
    elif package == "brocode-responses-api-proxy":
        bin_dir = staging_dir / "bin"
        bin_dir.mkdir(parents=True, exist_ok=True)
        launcher_src = RESPONSES_API_PROXY_NPM_ROOT / "bin" / "brocode-responses-api-proxy.js"
        shutil.copy2(launcher_src, bin_dir / "brocode-responses-api-proxy.js")
    ...
    elif package == "brocode-sdk":
        package_json_path = BROCODE_SDK_ROOT / "package.json"
        stage_brocode_sdk_sources(staging_dir)
    else:
    ...
    if package == "brocode":
        ...
        package_json["optionalDependencies"] = {
            BROCODE_PLATFORM_PACKAGES[platform_package]["npm_name"]: (
                f"npm:{BROCODE_NPM_NAME}@"
                f"{compute_platform_package_version(version, BROCODE_PLATFORM_PACKAGES[platform_package]['npm_tag'])}"
            )
            for platform_package in PACKAGE_EXPANSIONS["brocode"]
            if platform_package != "brocode"
        }
        ...
        if not isinstance(dependencies, dict):
            dependencies = {}
        dependencies[BROCODE_NPM_NAME] = version
...
def stage_brocode_sdk_sources(staging_dir: Path) -> None:
    package_root = BROCODE_SDK_ROOT
    ...
    if not dist_src.exists():
        raise RuntimeError("brocode-sdk build did not produce a dist directory.")
...
def run_npm_pack(staging_dir: Path, output_path: Path) -> Path:
    ...
    with tempfile.TemporaryDirectory(prefix="brocode-npm-pack-") as pack_dir_str:
