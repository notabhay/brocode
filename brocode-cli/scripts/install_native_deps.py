#!/usr/bin/env python3
"""Install Brocode native binaries (Rust CLI plus ripgrep helpers)."""
...
DEFAULT_WORKFLOW_URL = "https://github.com/openai/brocode/actions/runs/17952349351"  # rust-v0.40.0
...
class BinaryComponent:
    artifact_prefix: str  # matches the artifact filename prefix (e.g. brocode-<target>.zst)
...
BINARY_COMPONENTS = {
    "brocode": BinaryComponent(
        artifact_prefix="brocode",
        dest_dir="brocode",
        binary_basename="brocode",
    ),
    "brocode-responses-api-proxy": BinaryComponent(
        artifact_prefix="brocode-responses-api-proxy",
        dest_dir="brocode-responses-api-proxy",
        binary_basename="brocode-responses-api-proxy",
    ),
    "brocode-windows-sandbox-setup": BinaryComponent(
        artifact_prefix="brocode-windows-sandbox-setup",
        dest_dir="brocode",
        binary_basename="brocode-windows-sandbox-setup",
        targets=WINDOWS_TARGETS,
    ),
    "brocode-command-runner": BinaryComponent(
        artifact_prefix="brocode-command-runner",
        dest_dir="brocode",
        binary_basename="brocode-command-runner",
        targets=WINDOWS_TARGETS,
    ),
}
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Install native Brocode binaries.")
    parser.add_argument(
        "--workflow-url",
        help=(
    ...
    parser.add_argument(
        ...
        help=(
            ...
            " May be repeated. Defaults to brocode, brocode-windows-sandbox-setup,"
            " brocode-command-runner, and rg."
        ),
    )
    parser.add_argument(
        "root",
...
def main() -> int:
    ...
    brocode_cli_root = (args.root or BROCODE_CLI_ROOT).resolve()
    vendor_dir = brocode_cli_root / VENDOR_DIR_NAME
    vendor_dir.mkdir(parents=True, exist_ok=True)

    components = args.components or [
        "brocode",
        "brocode-windows-sandbox-setup",
        "brocode-command-runner",
        "rg",
    ]
    ...
    with _gha_group(f"Download native artifacts from workflow {workflow_id}"):
        with tempfile.TemporaryDirectory(prefix="brocode-native-artifacts-") as artifacts_dir_str:
