...
use brocode_utils_home_dir::find_brocode_home;
#[cfg(unix)]
...
#[cfg(unix)]
const EXECVE_WRAPPER_ARG0: &str = "brocode-execve-wrapper";
const LOCK_FILENAME: &str = ".lock";
const TOKIO_WORKER_STACK_SIZE_BYTES: usize = 16 * 1024 * 1024;

#[derive(Clone, Debug, Default, Eq, PartialEq)]
pub struct Arg0DispatchPaths {
    /// Stable path to the current Brocode executable for child re-execs.
    ///
    ...
    /// instead of the real Brocode CLI.
...
pub fn arg0_dispatch() -> Option<Arg0PathEntryGuard> {
    ...
    if exe_name == EXECVE_WRAPPER_ARG0 {
        ...
        let exit_code = runtime
            .block_on(brocode_shell_escalation::run_shell_escalation_execve_wrapper(file, argv));
        match exit_code {
    ...
    if exe_name == BROCODE_LINUX_SANDBOX_ARG0 {
        // Safety: [`run_main`] never returns.
        brocode_linux_sandbox::run_main();
    } else if exe_name == APPLY_PATCH_ARG0 || exe_name == MISSPELLED_APPLY_PATCH_ARG0 {
        brocode_apply_patch::main();
    }

    let argv1 = args.next().unwrap_or_default();
    if argv1 == BROCODE_CORE_APPLY_PATCH_ARG1 {
        let patch_arg = args.next().and_then(|s| s.to_str().map(str::to_owned));
        let exit_code = match patch_arg {
            Some(patch_arg) => {
                let mut stdout = std::io::stdout();
                let mut stderr = std::io::stderr();
                match brocode_apply_patch::apply_patch(&patch_arg, &mut stdout, &mut stderr) {
                    Ok(()) => 0,
                    Err(_) => 1,
                }
            }
            None => {
                eprintln!(
                    "Error: {BROCODE_CORE_APPLY_PATCH_ARG1} requires a UTF-8 PATCH argument."
                );
                1
            }
        };
    ...
    match prepend_path_entry_for_brocode_aliases() {
        Ok(path_entry) => Some(path_entry),
        Err(err) => {
            // It is possible that Brocode will proceed successfully even if
...
/// While we want to deploy the Brocode CLI as a single executable for simplicity,
...
/// `brocode-linux-sandbox` we *directly* execute
/// [`brocode_linux_sandbox::run_main`] (which never returns). Otherwise we:
///
/// 1.  Load `.env` values from `~/.brocode/.env` before creating any threads.
/// 2.  Construct a Tokio multi-thread runtime.
/// 3.  Capture the current executable path and derive the
///     `brocode-linux-sandbox` helper path (falling back to the current
    ...
    Fut: Future<Output = anyhow::Result<()>>,
{
    ...
    runtime.block_on(async move {
        ...
        let paths = Arg0DispatchPaths {
            brocode_self_exe: current_exe.clone(),
            brocode_linux_sandbox_exe: if cfg!(target_os = "linux") {
...
fn linux_sandbox_exe_path(
    ...
    // Prefer the `brocode-linux-sandbox` alias when available so callers can
    // re-exec through a path whose basename still triggers arg0 dispatch on
    // bubblewrap builds that do not support `--argv0`.
    path_entry_guard
        .and_then(|path_entry| path_entry.paths().brocode_linux_sandbox_exe.clone())
        .or(current_exe)
}
...
const ILLEGAL_ENV_VAR_PREFIX: &str = "BROCODE_";

/// Load env vars from ~/.brocode/.env.
///
/// Security: Do not allow `.env` files to create or modify any variables
/// with names starting with `BROCODE_`.
fn load_dotenv() {
    if let Ok(brocode_home) = find_brocode_home()
        && let Ok(iter) = dotenvy::from_path_iter(brocode_home.join(".env"))
    {
        set_filtered(iter);
    }
}

/// Helper to set vars from a dotenvy iterator while filtering out `BROCODE_` keys.
...
///   with the "secret" --brocode-run-as-apply-patch flag.
///
...
pub fn prepend_path_entry_for_brocode_aliases() -> std::io::Result<Arg0PathEntryGuard> {
    ...
    #[cfg(not(debug_assertions))]
    {
        ...
        if brocode_home.starts_with(&temp_root) {
            return Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                format!(
                    "Refusing to create helper binaries under temporary dir {temp_root:?} (brocode_home: {brocode_home:?})"
                ),
            ));
        }
    }
...
"#,
    ...
    let paths = Arg0DispatchPaths {
        brocode_self_exe: std::env::current_exe().ok(),
        brocode_linux_sandbox_exe: {
            #[cfg(target_os = "linux")]
            {
                Some(path.join(BROCODE_LINUX_SANDBOX_ARG0))
            }
...
mod tests {
    ...
    #[test]
    fn linux_sandbox_exe_path_prefers_brocode_linux_sandbox_alias() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let lock_file = create_lock(temp_dir.path())?;
        let alias_path = temp_dir.path().join("brocode-linux-sandbox");
        let path_entry = Arg0PathEntryGuard::new(
            temp_dir,
            lock_file,
            Arg0DispatchPaths {
                brocode_self_exe: Some(PathBuf::from("/usr/bin/brocode")),
                brocode_linux_sandbox_exe: Some(alias_path.clone()),
                main_execve_wrapper_exe: None,
            },
        );

        assert_eq!(
            linux_sandbox_exe_path(Some(&path_entry), Some(PathBuf::from("/usr/bin/brocode"))),
            Some(alias_path),
        );
        Ok(())
    }
