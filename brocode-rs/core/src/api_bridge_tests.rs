...
fn map_api_error_maps_server_overloaded() {
    ...
    assert!(matches!(err, BrocodeErr::ServerOverloaded));
}
...
fn map_api_error_maps_usage_limit_limit_name_header() {
    ...
    headers.insert(
        ACTIVE_LIMIT_HEADER,
        http::HeaderValue::from_static("brocode_other"),
    );
    headers.insert(
        "x-brocode-other-limit-name",
        http::HeaderValue::from_static("brocode_other"),
    );
    let body = serde_json::json!({
        "error": {
    ...
    let BrocodeErr::UsageLimitReached(usage_limit) = err else {
        panic!("expected BrocodeErr::UsageLimitReached, got {err:?}");
    };
    assert_eq!(
        usage_limit
            .rate_limits
...
fn map_api_error_extracts_identity_auth_details_from_headers() {
    ...
    let BrocodeErr::UnexpectedStatus(err) = err else {
        panic!("expected BrocodeErr::UnexpectedStatus, got {err:?}");
    };
