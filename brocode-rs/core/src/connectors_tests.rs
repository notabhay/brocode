...
fn brocode_app_tool(
    ...
    let tool_namespace = connector_name
        .map(sanitize_name)
        .map(|connector_name| format!("mcp__{BROCODE_APPS_MCP_SERVER_NAME}__{connector_name}"))
        .unwrap_or_else(|| BROCODE_APPS_MCP_SERVER_NAME.to_string());
...
async fn refresh_accessible_connectors_cache_from_mcp_tools_writes_latest_installed_apps() {
    let brocode_home = tempdir().expect("tempdir should succeed");
    ...
    let tools = HashMap::from([
        ...
        (
            "mcp__brocode_apps__openai_hidden".to_string(),
            brocode_app_tool(
                "openai_hidden",
...
async fn cloud_requirements_disable_connector_applies_without_user_apps_table() {
    let brocode_home = tempdir().expect("tempdir should succeed");
    std::fs::write(brocode_home.path().join(CONFIG_TOML_FILE), "").expect("write config");
...
async fn local_requirements_disable_connector_overrides_user_apps_config() {
    ...
    let config_toml_path =
        AbsolutePathBuf::try_from(brocode_home.path().join(CONFIG_TOML_FILE)).expect("abs path");
    let mut config = ConfigBuilder::default()
        .brocode_home(brocode_home.path().to_path_buf())
        .fallback_cwd(Some(brocode_home.path().to_path_buf()))
        .build()
