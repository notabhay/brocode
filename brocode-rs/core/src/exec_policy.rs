...
use brocode_execpolicy::AmendError;
use brocode_execpolicy::Decision;
use brocode_execpolicy::Error as ExecPolicyRuleError;
...
use brocode_execpolicy::PolicyParser;
use brocode_execpolicy::RuleMatch;
use brocode_execpolicy::blocking_append_allow_prefix_rule;
use brocode_execpolicy::blocking_append_network_rule;
...
pub(crate) fn child_uses_parent_exec_policy(parent_config: &Config, child_config: &Config) -> bool {
    fn exec_policy_config_folders(config: &Config) -> Vec<AbsolutePathBuf> {
        config
            ...
            .into_iter()
            .filter_map(brocode_config::ConfigLayerEntry::config_folder)
            .collect()
    }
...
pub enum ExecPolicyError {
    ...
    ParsePolicy {
        path: String,
        source: brocode_execpolicy::Error,
    },
}
...
impl ExecPolicyManager {
    ...
    pub(crate) async fn append_amendment_and_update(
        ...
        let policy_path = default_policy_path(brocode_home);
        spawn_blocking({
...
fn exec_policy_message_for_display(source: &brocode_execpolicy::Error) -> String {
...
fn default_policy_path(brocode_home: &Path) -> PathBuf {
    brocode_home.join(RULES_DIR_NAME).join(DEFAULT_POLICY_FILE)
}
...
/// - Note: we only use this amendment when the command fails to run in sandbox and brocode prompts the user to run outside the sandbox
