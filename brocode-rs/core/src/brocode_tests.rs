...
use brocode_protocol::models::FunctionCallOutputBody;
use brocode_protocol::models::FunctionCallOutputPayload;
use brocode_protocol::permissions::FileSystemAccessMode;
use brocode_protocol::permissions::FileSystemPath;
use brocode_protocol::permissions::FileSystemSandboxEntry;
use brocode_protocol::permissions::FileSystemSandboxPolicy;
use brocode_protocol::permissions::FileSystemSpecialPath;
...
use brocode_execpolicy::Decision;
use brocode_execpolicy::NetworkRuleProtocol;
use brocode_execpolicy::Policy;
use brocode_network_proxy::NetworkProxyConfig;
...
use brocode_protocol::protocol::Submission;
...
use brocode_protocol::mcp::CallToolResult as McpCallToolResult;
...
#[path = "brocode_tests_guardian.rs"]
mod guardian_tests;

use brocode_protocol::models::function_call_output_content_items_to_text;
...
fn test_model_client_session() -> crate::client::ModelClientSession {
    crate::client::ModelClient::new(
        ...
        brocode_protocol::protocol::SessionSource::Exec,
...
fn make_mcp_tool(
    ...
    let tool_namespace = if server_name == BROCODE_APPS_MCP_SERVER_NAME {
        connector_name
...
async fn get_base_instructions_no_user_content() {
    ...
    let test_cases = vec![
        ...
        InstructionsTestCase {
            slug: "gpt-5.1-brocode",
            expects_apply_patch_instructions: false,
        },
        InstructionsTestCase {
            slug: "gpt-5.1-brocode-max",
            expects_apply_patch_instructions: false,
        },
    ];
...
async fn reload_user_config_layer_updates_effective_apps_config() {
    let (session, _turn_context) = make_session_and_context().await;
    let brocode_home = session.brocode_home().await;
    std::fs::create_dir_all(&brocode_home).expect("create brocode home");
    let config_toml_path = brocode_home.join(CONFIG_TOML_FILE);
    std::fs::write(
        &config_toml_path,
...
fn non_app_mcp_tools_remain_visible_without_search_selection() {
    let mcp_tools = HashMap::from([
        (
            "mcp__brocode_apps__calendar_create_event".to_string(),
            make_mcp_tool(
...
#[test]
fn search_tool_selection_keeps_brocode_apps_tools_without_mentions() {
...
#[test]
fn apps_mentions_add_brocode_apps_tools_to_search_selected_set() {
...
async fn fork_startup_context_then_first_turn_diff_snapshot() -> anyhow::Result<()> {
    ...
    let mut builder = test_brocode().with_config(|config| {
        config.permissions.approval_policy =
            brocode_config::Constrained::allow_any(AskForApproval::OnRequest);
    });
    ...
    initial
        ...
        .await?;
    wait_for_event(&initial.brocode, |ev| {
        matches!(ev, EventMsg::TurnComplete(_))
    })
    .await;
    // Forking reads the persisted rollout JSONL, so force the completed source turn to disk
    // before snapshotting from it.
    initial.brocode.ensure_rollout_materialized().await;
    initial.brocode.flush_rollout().await;

    let mut fork_config = initial.config.clone();
    fork_config.permissions.approval_policy =
        brocode_config::Constrained::allow_any(AskForApproval::UnlessTrusted);
    let forked = initial
        .thread_manager
        .fork_thread(
            usize::MAX,
    ...
    settings.bind(|| {
        insta::assert_snapshot!(
            "brocode_core__brocode_tests__fork_startup_context_then_first_turn_diff",
            snapshot
        );
    });
...
async fn record_initial_history_forked_hydrates_previous_turn_settings() {
    ...
    let rollout_items = vec![
        ...
        RolloutItem::EventMsg(EventMsg::UserMessage(
            brocode_protocol::protocol::UserMessageEvent {
        ...
        RolloutItem::EventMsg(EventMsg::TurnComplete(
            brocode_protocol::protocol::TurnCompleteEvent {
                turn_id,
...
async fn thread_rollback_fails_without_persisted_rollout_path() {
    ...
    assert_eq!(
        error_event.brocode_error_info,
        Some(BrocodeErrorInfo::ThreadRollbackFailed)
    );
...
async fn set_rate_limits_retains_previous_credits() {
    ...
    let config = build_test_config(brocode_home.path()).await;
    ...
    let initial = RateLimitSnapshot {
        ...
        plan_type: Some(brocode_protocol::account::PlanType::Plus),
    };
...
async fn set_rate_limits_updates_plan_type_when_present() {
    ...
    let update = RateLimitSnapshot {
        ...
        secondary: None,
        credits: None,
        plan_type: Some(brocode_protocol::account::PlanType::Pro),
    };
    state.set_rate_limits(update.clone());
    ...
    loop {
        ...
        match evt.msg {
            EventMsg::Error(payload)
                if payload.brocode_error_info == Some(BrocodeErrorInfo::ThreadRollbackFailed) =>
            {
                return payload;
            }
            _ => continue,
...
async fn build_test_config(brocode_home: &Path) -> Config {
...
async fn session_configuration_apply_preserves_split_file_system_policy_on_cwd_only_update() {
    ...
    session_configuration.sandbox_policy =
        brocode_config::Constrained::allow_any(SandboxPolicy::WorkspaceWrite {
...
async fn new_default_turn_uses_config_aware_skills_for_role_overrides() {
    ...
    let brocode_home = parent_config.brocode_home.clone();
    let skill_dir = brocode_home.join("skills").join("demo");
    ...
    let role_path = brocode_home.join("skills-role.toml");
    std::fs::write(
        &role_path,
        format!(
...
async fn session_new_fails_when_zsh_fork_enabled_without_zsh_path() {
    let brocode_home = tempfile::tempdir().expect("create temp dir");
    let mut config = build_test_config(brocode_home.path()).await;
    config
        .features
    ...
    let plugins_manager = Arc::new(PluginsManager::new(config.brocode_home.clone()));
...
pub(crate) async fn make_session_and_context() -> (Session, TurnContext) {
    ...
    let environment = Arc::new(
        brocode_exec_server::Environment::create(/*exec_server_url*/ None)
            .await
        ...
        .notify_request_permissions_response(
            ...
            brocode_protocol::request_permissions::RequestPermissionsResponse {
                permissions: RequestPermissionProfile {
                    network: Some(brocode_protocol::models::NetworkPermissions {
                        enabled: Some(true),
                    }),
...
async fn request_permissions_emits_event_when_granular_policy_allows_requests() {
    ...
    *session.active_turn.lock().await = Some(ActiveTurn::default());
    ...
    let expected_response = brocode_protocol::request_permissions::RequestPermissionsResponse {
        permissions: RequestPermissionProfile {
            network: Some(brocode_protocol::models::NetworkPermissions {
                enabled: Some(true),
            }),
        ...
        async move {
            session
                .request_permissions(
                    turn_context.as_ref(),
                    call_id,
                    brocode_protocol::request_permissions::RequestPermissionsArgs {
...
async fn submit_with_id_captures_current_span_trace_context() {
    ...
    let _trace_test_context = install_test_tracing("brocode-core-tests");
...
async fn spawn_task_turn_span_inherits_dispatch_trace_context() {
    ...
    let submission_context =
        brocode_otel::context_from_w3c_trace_context(&submission_trace).expect("submission");
    let task_context =
        brocode_otel::context_from_w3c_trace_context(&task_trace).expect("task trace");
...
async fn shutdown_and_wait_allows_multiple_waiters() {
    ...
    let waiter_1 = {
        let brocode = Arc::clone(&brocode);
        tokio::spawn(async move { brocode.shutdown_and_wait().await })
    };
    let waiter_2 = {
        let brocode = Arc::clone(&brocode);
        tokio::spawn(async move { brocode.shutdown_and_wait().await })
...
async fn shutdown_and_wait_shuts_down_tracked_ephemeral_guardian_review() {
    ...
    parent_session
        .guardian_review_session
        .register_ephemeral_for_test(child_brocode)
        .await;
...
async fn handle_output_item_done_records_image_save_history_message() {
    ...
    let expected_saved_path = crate::stream_events_utils::image_generation_artifact_path(
        turn_context.config.brocode_home.as_path(),
