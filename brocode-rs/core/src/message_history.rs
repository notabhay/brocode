...
//!
//! The history is stored at `~/.brocode/history.jsonl` with **one JSON object per
...
/// Filename that stores the message history inside `~/.brocode`.
...
fn history_filepath(config: &Config) -> PathBuf {
    let mut path = config.brocode_home.clone();
...
pub async fn append_entry(text: &str, conversation_id: &ThreadId, config: &Config) -> Result<()> {
    ...
    // Resolve `~/.brocode/history.jsonl` and ensure the parent directory exists.
