...
use brocode_hooks::PostToolUseOutcome;
use brocode_hooks::PostToolUseRequest;
use brocode_hooks::PreToolUseOutcome;
use brocode_hooks::PreToolUseRequest;
use brocode_hooks::SessionStartOutcome;
use brocode_hooks::UserPromptSubmitOutcome;
use brocode_hooks::UserPromptSubmitRequest;
...
use brocode_protocol::protocol::HookCompletedEvent;
use brocode_protocol::protocol::HookRunSummary;
...
pub(crate) async fn run_pending_session_start_hooks(
    ...
    let request = brocode_hooks::SessionStartRequest {
        ...
        assert_eq!(
            messages
                ...
                .map(|message| match message {
                    brocode_protocol::models::ResponseItem::Message { role, content, .. } => {
                        let text = content
                            .iter()
                            .map(|item| match item {
