...
fn emit_feedback_request_tags_records_sentry_feedback_fields() {
    ...
    assert_eq!(
        tags.get("auth_env_brocode_api_key_present")
            .map(String::as_str),
        Some("false")
    );
    assert_eq!(
        tags.get("auth_env_brocode_api_key_enabled")
            .map(String::as_str),
        Some("true")
    );
    assert_eq!(
...
fn emit_feedback_request_tags_preserves_auth_env_fields_for_legacy_emitters() {
    ...
    let auth_env = AuthEnvTelemetry {
        openai_api_key_env_present: true,
        brocode_api_key_env_present: true,
        brocode_api_key_env_enabled: true,
        provider_env_key_name: Some("configured".to_string()),
        provider_env_key_present: Some(true),
        refresh_token_url_override_present: true,
...
fn resume_command_prefers_name_over_id() {
    ...
    assert_eq!(command, Some("brocode resume my-thread".to_string()));
}
...
fn resume_command_quotes_thread_name_when_needed() {
    ...
    assert_eq!(
        command,
        Some("brocode resume -- -starts-with-dash".to_string())
    );

    let command = resume_command(Some("two words"), None);
    assert_eq!(command, Some("brocode resume 'two words'".to_string()));

    let command = resume_command(Some("quote'case"), None);
    assert_eq!(command, Some("brocode resume \"quote'case\"".to_string()));
}
