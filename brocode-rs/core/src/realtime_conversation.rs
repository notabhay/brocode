...
use brocode_api::Provider as ApiProvider;
use brocode_api::RealtimeAudioFrame;
use brocode_api::RealtimeEvent;
use brocode_api::RealtimeEventParser;
use brocode_api::RealtimeSessionConfig;
use brocode_api::RealtimeSessionMode;
use brocode_api::RealtimeWebsocketClient;
use brocode_api::endpoint::realtime_websocket::RealtimeWebsocketEvents;
use brocode_api::endpoint::realtime_websocket::RealtimeWebsocketWriter;
...
use brocode_protocol::protocol::RealtimeHandoffRequested;
use http::HeaderMap;
...
impl RealtimeConversationManager {
    ...
    ) -> BrocodeResult<(Receiver<RealtimeEvent>, Arc<AtomicBool>)> {
        let previous_state = {
    ...
    pub(crate) async fn audio_in(&self, frame: RealtimeAudioFrame) -> BrocodeResult<()> {
        let sender = {
        ...
        match sender.try_send(frame) {
            ...
            Err(TrySendError::Closed(_)) => Err(BrocodeErr::InvalidRequest(
                "conversation is not running".to_string(),
            )),
        }
    }

    pub(crate) async fn text_in(&self, text: String) -> BrocodeResult<()> {
        let sender = {
            let guard = self.state.lock().await;
        ...
        sender
            .send(text)
            .await
            .map_err(|_| BrocodeErr::InvalidRequest("conversation is not running".to_string()))?;
        Ok(())
    }

    pub(crate) async fn handoff_out(&self, output_text: String) -> BrocodeResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
        ...
        *handoff.last_output_text.lock().await = Some(output_text.clone());
        if matches!(handoff.session_kind, RealtimeSessionKind::V1) {
            handoff
                ...
                .await
                .map_err(|_| {
                    BrocodeErr::InvalidRequest("conversation is not running".to_string())
                })?;
        }
        Ok(())
    }

    pub(crate) async fn handoff_complete(&self) -> BrocodeResult<()> {
        let handoff = {
            let guard = self.state.lock().await;
    ...
    pub(crate) async fn shutdown(&self) -> BrocodeResult<()> {
        let state = {
            let mut guard = self.state.lock().await;
            guard.take()
        };
...
async fn prepare_realtime_start(
    sess: &Arc<Session>,
    params: ConversationStartParams,
) -> BrocodeResult<PreparedRealtimeConversationStart> {
...
pub(crate) async fn handle_audio(
    ...
    if let Err(err) = sess.conversation.audio_in(params.frame).await {
        ...
        if sess.conversation.running_state().await.is_some() {
            ...
            send_conversation_error(sess, sub_id, err.to_string(), BrocodeErrorInfo::BadRequest)
                .await;
        }
    }
}
...
fn realtime_api_key(
    ...
    if let Some(api_key) = auth.and_then(BrocodeAuth::api_key) {
...
fn realtime_request_headers(
    ...
    api_key: &str,
) -> BrocodeResult<Option<HeaderMap>> {
    ...
    let auth_value = HeaderValue::from_str(&format!("Bearer {api_key}")).map_err(|err| {
        BrocodeErr::InvalidRequest(format!("invalid realtime api key header: {err}"))
    })?;
...
async fn send_conversation_error(
    ...
    sub_id: String,
    message: String,
    brocode_error_info: BrocodeErrorInfo,
) {
    sess.send_event_raw(Event {
        id: sub_id,
