...
use brocode_otel::config::OtelTlsConfig as OtelTlsSettings;
use std::error::Error;
...
/// Filter predicate for exporting only Brocode-owned events via OTEL.
/// Keeps events that originated from brocode_otel module
pub fn brocode_export_filter(meta: &tracing::Metadata<'_>) -> bool {
    meta.target().starts_with("brocode_otel")
}
