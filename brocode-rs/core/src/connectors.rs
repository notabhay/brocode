...
pub use brocode_app_server_protocol::AppBranding;
pub use brocode_app_server_protocol::AppInfo;
...
pub async fn list_accessible_connectors_from_mcp_tools_with_options_and_status(
    ...
    let refreshed_tools = if force_refetch {
        match mcp_connection_manager
            .hard_refresh_brocode_apps_tools_cache()
            .await
        {
            Ok(tools) => Some(tools),
            Err(err) => {
                warn!(
                    "failed to force-refresh tools for MCP server '{BROCODE_APPS_MCP_SERVER_NAME}', using cached/startup tools: {err:#}"
                );
                None
            }
        }
    ...
    let brocode_apps_ready = if refreshed_tools_succeeded {
        true
    } else if let Some(cfg) = mcp_servers.get(BROCODE_APPS_MCP_SERVER_NAME) {
        let immediate_ready = mcp_connection_manager
            .wait_for_server_ready(BROCODE_APPS_MCP_SERVER_NAME, Duration::ZERO)
            .await;
        if immediate_ready {
            true
            ...
            let ready = mcp_connection_manager
                .wait_for_server_ready(BROCODE_APPS_MCP_SERVER_NAME, timeout)
                .await;
    ...
    if brocode_apps_ready || !accessible_connectors.is_empty() {
        write_cached_accessible_connectors(cache_key, &accessible_connectors);
    }
    let accessible_connectors =
...
fn tool_suggest_connector_ids(config: &Config) -> HashSet<String> {
    let mut connector_ids = PluginsManager::new(config.brocode_home.clone())
...
async fn list_directory_connectors_for_tool_suggest_with_auth(
    ...
    brocode_connectors::list_all_connectors_with_options(
        cache_key,
        is_workspace_account,
        /*force_refetch*/ false,
        |path| {
...
pub(crate) fn brocode_app_tool_is_enabled(
    config: &Config,
    tool_info: &crate::mcp_connection_manager::ToolInfo,
) -> bool {
    if tool_info.server_name != BROCODE_APPS_MCP_SERVER_NAME {
        return true;
    }
...
fn plugin_app_to_app_info(connector_id: AppConnectorId) -> AppInfo {
    ...
    // connector_name values from brocode_apps tool discovery.
