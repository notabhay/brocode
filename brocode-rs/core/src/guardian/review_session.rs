...
use crate::brocode_delegate::run_brocode_thread_interactive;
...
impl GuardianReviewSessionReuseKey {
    fn from_spawn_config(spawn_config: &Config) -> Self {
        Self {
            ...
            brocode_linux_sandbox_exe: spawn_config.brocode_linux_sandbox_exe.clone(),
...
impl GuardianReviewSession {
    async fn shutdown(&self) {
        ...
        let _ = self.brocode.shutdown_and_wait().await;
    }
    ...
    async fn refresh_last_committed_rollout_items(&self) {
        match load_rollout_items_for_fork(&self.brocode.session).await {
            Ok(Some(items)) => {
...
impl GuardianReviewSessionManager {
    ...
    #[cfg(test)]
    pub(crate) async fn cache_for_test(&self, brocode: Brocode) {
        let reuse_key = GuardianReviewSessionReuseKey::from_spawn_config(
            brocode.session.get_config().await.as_ref(),
        );
    ...
    #[cfg(test)]
    pub(crate) async fn register_ephemeral_for_test(&self, brocode: Brocode) {
        let reuse_key = GuardianReviewSessionReuseKey::from_spawn_config(
            brocode.session.get_config().await.as_ref(),
        );
        self.state
...
async fn spawn_guardian_review_session(
    ...
    let brocode = run_brocode_thread_interactive(
        spawn_config,
        ...
        Box::pin(async {
            params
                ...
                .sync_session_approved_hosts_to(
                    &review_session.brocode.session.services.network_approval,
                )
                .await;
...
async fn append_guardian_followup_reminder(review_session: &GuardianReviewSession) {
    let turn_context = review_session.brocode.session.new_default_turn().await;
...
async fn wait_for_guardian_review(
    ...
    loop {
        tokio::select! {
            _ = &mut timeout => {
                let keep_review_session = interrupt_and_drain_turn(&review_session.brocode).await.is_ok();
            ...
            event = review_session.brocode.next_event() => {
                match event {
...
pub(crate) fn build_guardian_review_session_config(
    ...
    live_network_config: Option<brocode_network_proxy::NetworkProxyConfig>,
    active_model: &str,
...
async fn interrupt_and_drain_turn(brocode: &Brocode) -> anyhow::Result<()> {
