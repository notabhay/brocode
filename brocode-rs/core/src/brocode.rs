...
use brocode_analytics::AnalyticsEventsClient;
use brocode_analytics::AppInvocation;
use brocode_analytics::InvocationType;
use brocode_analytics::build_track_events_context;
use brocode_app_server_protocol::McpServerElicitationRequest;
use brocode_app_server_protocol::McpServerElicitationRequestParams;
use brocode_exec_server::Environment;
use brocode_exec_server::EnvironmentManager;
use brocode_features::FEATURES;
use brocode_features::Feature;
use brocode_features::unstable_features_warning_event;
use brocode_hooks::HookEvent;
use brocode_hooks::HookEventAfterAgent;
use brocode_hooks::HookPayload;
use brocode_hooks::HookResult;
use brocode_hooks::Hooks;
use brocode_hooks::HooksConfig;
use brocode_network_proxy::NetworkProxy;
use brocode_network_proxy::NetworkProxyAuditMetadata;
use brocode_network_proxy::normalize_host;
use brocode_otel::current_span_trace_id;
use brocode_otel::current_span_w3c_trace_context;
...
use brocode_protocol::dynamic_tools::DynamicToolResponse;
use brocode_protocol::dynamic_tools::DynamicToolSpec;
use brocode_protocol::items::PlanItem;
use brocode_protocol::items::TurnItem;
use brocode_protocol::items::UserMessageItem;
use brocode_protocol::items::build_hook_prompt_message;
use brocode_protocol::mcp::CallToolResult;
use brocode_protocol::models::BaseInstructions;
use brocode_protocol::models::PermissionProfile;
use brocode_protocol::models::format_allow_prefixes;
use brocode_protocol::openai_models::ModelInfo;
...
use brocode_protocol::protocol::TurnContextNetworkItem;
use brocode_protocol::protocol::W3cTraceContext;
use brocode_protocol::request_permissions::PermissionGrantScope;
use brocode_protocol::request_permissions::RequestPermissionProfile;
use brocode_protocol::request_permissions::RequestPermissionsArgs;
use brocode_protocol::request_permissions::RequestPermissionsEvent;
use brocode_protocol::request_permissions::RequestPermissionsResponse;
use brocode_protocol::request_user_input::RequestUserInputArgs;
use brocode_protocol::request_user_input::RequestUserInputResponse;
...
use brocode_utils_stream_parser::ProposedPlanSegment;
use brocode_utils_stream_parser::extract_proposed_plan_text;
use brocode_utils_stream_parser::strip_citations;
use chrono::Local;
use chrono::Utc;
...
use crate::brocode_thread::ThreadConfigSnapshot;
...
use brocode_config::CONFIG_TOML_FILE;
    ...
    fn to_error_event(&self) -> ErrorEvent {
        match self {
            Self::NoActiveTurn(_) => ErrorEvent {
                ...
                brocode_error_info: Some(BrocodeErrorInfo::BadRequest),
            },
...
use crate::mcp::with_brocode_apps_mcp;
use crate::mcp_connection_manager::McpConnectionManager;
use crate::mcp_connection_manager::brocode_apps_tools_cache_key;
use crate::mcp_connection_manager::filter_non_brocode_apps_mcp_tools_only;
use crate::memories;
...
use brocode_async_utils::OrCancelExt;
use brocode_git_utils::get_git_repo_root;
use brocode_otel::SessionTelemetry;
use brocode_otel::TelemetryAuthMode;
use brocode_otel::metrics::names::THREAD_STARTED_METRIC;
use brocode_protocol::config_types::CollaborationMode;
use brocode_protocol::config_types::Personality;
use brocode_protocol::config_types::ReasoningSummary as ReasoningSummaryConfig;
...
use brocode_protocol::models::DeveloperInstructions;
...
use brocode_utils_readiness::ReadinessFlag;

/// The high-level interface to the Brocode system.
...
/// Wrapper returned by [`Brocode::spawn`] containing the spawned [`Brocode`],
/// the submission id for the initial `ConfigureSession` request and the
/// unique session id.
pub struct BrocodeSpawnOk {
    pub brocode: Brocode,
...
pub(crate) struct BrocodeSpawnArgs {
...
impl Brocode {
    /// Spawn a new [`Brocode`] and initialize the session.
    pub(crate) async fn spawn(args: BrocodeSpawnArgs) -> BrocodeResult<BrocodeSpawnOk> {
        let parent_trace = match args.parent_trace {
            Some(trace) => {
                if brocode_otel::context_from_w3c_trace_context(&trace).is_some() {
                    Some(trace)
                } else {
        ...
        Self::spawn_internal(BrocodeSpawnArgs {
            parent_trace,
            ..args
        })
    ...
    async fn spawn_internal(args: BrocodeSpawnArgs) -> BrocodeResult<BrocodeSpawnOk> {
        let BrocodeSpawnArgs {
            mut config,
            auth_manager,
            models_manager,
            environment_manager,
        ...
        let exec_policy = if crate::guardian::is_guardian_reviewer_source(&session_source) {
            ...
            Arc::new(
                ExecPolicyManager::load(&config.config_layer_stack)
                    .await
                    .map_err(|err| BrocodeErr::Fatal(format!("failed to load rules: {err}")))?,
            )
        };
        ...
        let persisted_tools = if dynamic_tools.is_empty() {
            ...
            match thread_id {
                Some(thread_id) => {
                    ...
                    state_db::get_dynamic_tools(state_db_ctx.as_deref(), thread_id, "brocode_spawn")
                        .await
                }
                None => None,
            }
        ...
        // Generate a unique ID for the lifetime of this Brocode session.
        ...
        .map_err(|e| {
            ...
            map_session_init_error(&e, &config.brocode_home)
        })?;
    ...
    /// Use sparingly: prefer `submit()` so Brocode is responsible for generating
    /// unique IDs for each submission.
    pub async fn submit_with_id(&self, mut sub: Submission) -> BrocodeResult<()> {
        ...
        self.tx_sub
            .send(sub)
            .await
            .map_err(|_| BrocodeErr::InternalAgentDied)?;
        Ok(())
    }
    ...
    pub async fn shutdown_and_wait(&self) -> BrocodeResult<()> {
        ...
        match self.submit(Op::Shutdown).await {
            Ok(_) => {}
            Err(BrocodeErr::InternalAgentDied) => {}
...
pub(crate) struct TurnContext {
    ...
    pub(crate) brocode_self_exe: Option<PathBuf>,
    pub(crate) brocode_linux_sandbox_exe: Option<PathBuf>,
...
impl TurnContext {
    ...
    pub(crate) async fn with_model(&self, model: String, models_manager: &ModelsManager) -> Self {
        ...
        Self {
            ...
            brocode_self_exe: self.brocode_self_exe.clone(),
            brocode_linux_sandbox_exe: self.brocode_linux_sandbox_exe.clone(),
...
pub(crate) struct SessionConfiguration {
    ...
    cwd: AbsolutePathBuf,
    /// Directory containing all Brocode state for this session.
...
impl SessionConfiguration {
    pub(crate) fn brocode_home(&self) -> &PathBuf {
        &self.brocode_home
    }
...
impl Session {
    /// Builds the `x-brocode-beta-features` header value for this session.
    ///
    ...
    async fn start_managed_network_proxy(
        ...
        exec_policy: &brocode_execpolicy::Policy,
        sandbox_policy: &SandboxPolicy,
        network_policy_decider: Option<Arc<dyn brocode_network_proxy::NetworkPolicyDecider>>,
        blocked_request_observer: Option<Arc<dyn brocode_network_proxy::BlockedRequestObserver>>,
    ...
    pub(crate) async fn brocode_home(&self) -> PathBuf {
        let state = self.state.lock().await;
        state.session_configuration.brocode_home().clone()
    }
    ...
    fn make_turn_context(
        ...
        TurnContext {
            ...
            brocode_self_exe: per_turn_config.brocode_self_exe.clone(),
            brocode_linux_sandbox_exe: per_turn_config.brocode_linux_sandbox_exe.clone(),
            tool_call_gate: Arc::new(ReadinessFlag::new()),
    ...
    async fn new(
        ...
        let config_path = config.brocode_home.join(CONFIG_TOML_FILE);
        ...
        let auth_mode = auth
            .map(BrocodeAuth::auth_mode)
            .map(TelemetryAuthMode::from);
        let account_id = auth.and_then(BrocodeAuth::get_account_id);
        let account_email = auth.and_then(BrocodeAuth::get_account_email);
        let originator = crate::default_client::originator().value;
        let terminal_type = user_agent();
        let session_model = session_configuration.collaboration_mode.model().to_string();
        let auth_env_telemetry = collect_auth_env_telemetry(
            &session_configuration.provider,
            auth_manager.brocode_api_key_env_enabled(),
        );
        ...
        let thread_name =
            match session_index::find_thread_name_by_id(&config.brocode_home, &conversation_id)
                .instrument(info_span!(
        ...
        let hooks = Hooks::new(HooksConfig {
            ...
            feature_enabled: config.features.enabled(Feature::BrocodeHooks),
        ...
        let sandbox_state = SandboxState {
            sandbox_policy: session_configuration.sandbox_policy.get().clone(),
            brocode_linux_sandbox_exe: config.brocode_linux_sandbox_exe.clone(),
        ...
        let (mcp_connection_manager, cancel_token) = McpConnectionManager::new(
            ...
            sandbox_state,
            config.brocode_home.clone(),
            brocode_apps_tools_cache_key(auth),
        ...
        let session_start_source = match &initial_history {
            InitialHistory::Resumed(_) => brocode_hooks::SessionStartSource::Resume,
            InitialHistory::New | InitialHistory::Forked(_) => {
                brocode_hooks::SessionStartSource::Startup
            }
        };
                    ...
                    .filter(|model| *model != curr)
                {
                    ...
                    self.send_event(
                        ...
                        EventMsg::Warning(WarningEvent {
                         ...
                         Consider switching back to `{prev}` as it may affect Brocode performance."
                            ),
                        }),
                    )
                    .await;
    ...
    pub(crate) async fn update_settings(
        ...
        match state.session_configuration.apply(&updates) {
            Ok(updated) => {
                ...
                let brocode_home = updated.brocode_home.clone();
        ...
        let (
            ...
            match state.session_configuration.clone().apply(&updates) {
                Ok(next) => {
                    ...
                    let brocode_home = next.brocode_home.clone();
    ...
    pub(crate) async fn persist_execpolicy_amendment(
        ...
        self.services
            .exec_policy
            .append_amendment_and_update(&brocode_home, amendment)
            .await?;
    ...
    pub async fn request_mcp_server_elicitation(
        ...
        let id = match request_id {
            rmcp::model::NumberOrString::String(value) => {
                brocode_protocol::mcp::RequestId::String(value.to_string())
            }
    ...
    pub(crate) async fn record_model_warning(&self, message: impl Into<String>, ctx: &TurnContext) {
        self.services
            .session_telemetry
            .counter("brocode.model_warning", /*inc*/ 1, &[]);
    ...
    pub(crate) async fn build_initial_context(
        ...
        if turn_context.features.enabled(Feature::MemoryTool)
            ...
            && let Some(memory_prompt) =
                build_memory_tool_developer_instructions(&turn_context.config.brocode_home).await
        {
        ...
        if turn_context.features.enabled(Feature::BrocodeGitCommit)
    ...
    pub(crate) async fn notify_stream_error(
        ...
        turn_context: &TurnContext,
        message: impl Into<String>,
        brocode_error: BrocodeErr,
    ) {
        let additional_details = brocode_error.to_string();
        let brocode_error_info = BrocodeErrorInfo::ResponseStreamDisconnected {
            http_status_code: brocode_error.http_status_code_value(),
        };
        let event = EventMsg::StreamError(StreamErrorEvent {
            message: message.into(),
            brocode_error_info: Some(brocode_error_info),
    ...
    pub(crate) async fn take_pending_session_start_source(
        &self,
    ) -> Option<brocode_hooks::SessionStartSource> {
        let mut state = self.state.lock().await;
    ...
    async fn refresh_mcp_servers_inner(
        ...
        let mcp_servers = with_brocode_apps_mcp(
            mcp_servers,
        ...
        let sandbox_state = SandboxState {
            ...
            brocode_linux_sandbox_exe: turn_context.brocode_linux_sandbox_exe.clone(),
        ...
        let (refreshed_manager, cancel_token) = McpConnectionManager::new(
            ...
            sandbox_state,
            config.brocode_home.clone(),
            brocode_apps_tools_cache_key(auth.as_ref()),
            tool_plugin_provenance,
        )
        .await;
        {
...
mod handlers {
    ...
    use crate::brocode::SessionSettingsUpdate;
    use crate::brocode::SteerInputError;

    use crate::SkillError;
    use crate::brocode::spawn_review_thread;
    use crate::config::Config;
    ...
    use brocode_protocol::protocol::ReviewRequest;
    use brocode_protocol::protocol::RolloutItem;
    use brocode_protocol::protocol::SkillsListEntry;
    use brocode_protocol::protocol::ThreadNameUpdatedEvent;
    ...
    use brocode_protocol::config_types::Settings;
    use brocode_protocol::dynamic_tools::DynamicToolResponse;
    use brocode_protocol::mcp::RequestId as ProtocolRequestId;
    use brocode_protocol::user_input::UserInput;
    use brocode_rmcp_client::ElicitationAction;
    use brocode_rmcp_client::ElicitationResponse;
    use serde_json::Value;
    use std::path::PathBuf;
    use std::sync::Arc;
    ...
    pub async fn resolve_elicitation(
        ...
        decision: brocode_protocol::approvals::ElicitationAction,
        content: Option<Value>,
        meta: Option<Value>,
    ) {
        let action = match decision {
            brocode_protocol::approvals::ElicitationAction::Accept => ElicitationAction::Accept,
            brocode_protocol::approvals::ElicitationAction::Decline => ElicitationAction::Decline,
            brocode_protocol::approvals::ElicitationAction::Cancel => ElicitationAction::Cancel,
        };
            ...
            let event = Event {
                ...
                msg: EventMsg::GetHistoryEntryResponse(
                    crate::protocol::GetHistoryEntryResponseEvent {
                        offset,
                        log_id,
                        entry: entry_opt.map(|e| brocode_protocol::message_history::HistoryEntry {
    ...
    pub async fn list_skills(
        ...
        let brocode_home = sess.brocode_home().await;
    ...
    pub async fn drop_memories(sess: &Arc<Session>, config: &Arc<Config>, sub_id: String) {
        ...
        let memory_root = crate::memories::memory_root(&config.brocode_home);
        ...
        if num_turns == 0 {
            sess.send_event_raw(Event {
                ...
                msg: EventMsg::Error(ErrorEvent {
                    ...
                    brocode_error_info: Some(BrocodeErrorInfo::ThreadRollbackFailed),
                }),
            })
            .await;
            return;
    ...
    pub async fn set_thread_name(sess: &Arc<Session>, sub_id: String, name: String) {
        ...
        let brocode_home = sess.brocode_home().await;
        if let Err(e) =
            session_index::append_thread_name(&brocode_home, sess.conversation_id, &name).await
        {
            let event = Event {
                id: sub_id,
                msg: EventMsg::Error(ErrorEvent {
    ...
    pub async fn shutdown(sess: &Arc<Session>, sub_id: String) -> bool {
        ...
        sess.services
            ...
            .await;
        sess.guardian_review_session.shutdown().await;
        info!("Shutting down Brocode instance");
        ...
        sess.services.session_telemetry.counter(
            "brocode.conversation.turn.count",
...
async fn spawn_review_thread(
    ...
    let review_turn_context = TurnContext {
        ...
        final_output_json_schema: None,
        brocode_self_exe: parent_turn_context.brocode_self_exe.clone(),
        brocode_linux_sandbox_exe: parent_turn_context.brocode_linux_sandbox_exe.clone(),
        tool_call_gate: Arc::new(ReadinessFlag::new()),
...
pub(crate) async fn run_turn(
    ...
    // Although from the perspective of brocode.rs, TurnDiffTracker has the lifecycle of a Task which contains
        ...
        .await
        {
            Ok(sampling_request_output) => {
                ...
                if !needs_follow_up {
                    ...
                    .to_string();
                    let stop_request = brocode_hooks::StopRequest {
            ...
            Err(BrocodeErr::TurnAborted) => {
                // Aborted turn is reported via a different event.
                break;
            }
            Err(BrocodeErr::InvalidImageRequest()) => {
                let mut state = sess.state.lock().await;
                error_or_panic(
...
fn filter_brocode_apps_mcp_tools(
    ...
    mcp_tools
        .iter()
        .filter(|(_, tool)| {
            if tool.server_name != BROCODE_APPS_MCP_SERVER_NAME {
                return false;
            }
            let Some(connector_id) = brocode_apps_connector_id(tool) else {
                return false;
            };
            allowed.contains(connector_id) && connectors::brocode_app_tool_is_enabled(config, tool)
        })
        .map(|(name, tool)| (name.clone(), tool.clone()))
        .collect()
}

fn brocode_apps_connector_id(tool: &crate::mcp_connection_manager::ToolInfo) -> Option<&str> {
...
async fn run_sampling_request(
    ...
    cancellation_token: CancellationToken,
) -> BrocodeResult<SamplingRequestResult> {
    ...
    loop {
        ...
        .await
        {
            ...
            Err(BrocodeErr::ContextWindowExceeded) => {
                sess.set_total_tokens_full(&turn_context).await;
                return Err(BrocodeErr::ContextWindowExceeded);
            }
            Err(BrocodeErr::UsageLimitReached(e)) => {
                let rate_limits = e.rate_limits.clone();
                if let Some(rate_limits) = rate_limits {
                    sess.update_rate_limits(&turn_context, *rate_limits).await;
                }
                return Err(BrocodeErr::UsageLimitReached(e));
            }
            Err(err) => err,
        };
        ...
        if retries < max_retries {
            retries += 1;
            let delay = match &err {
                BrocodeErr::Stream(_, requested_delay) => {
...
pub(crate) async fn built_tools(
    ...
    let app_tools = connectors.as_ref().map(|connectors| {
        filter_brocode_apps_mcp_tools(&mcp_tools, connectors, &turn_context.config)
    });
    ...
    if let Some(connectors) = connectors.as_ref() {
        ...
        let mut selected_mcp_tools = filter_non_brocode_apps_mcp_tools_only(&mcp_tools);
        selected_mcp_tools.extend(filter_brocode_apps_mcp_tools(
            &mcp_tools,
...
fn agent_message_text(item: &brocode_protocol::items::AgentMessageItem) -> String {
    item.content
        .iter()
        .map(|entry| match entry {
            brocode_protocol::items::AgentMessageContent::Text { text } => text.as_str(),
        })
        .collect()
}
...
async fn emit_agent_message_in_plan_mode(
    sess: &Session,
    turn_context: &TurnContext,
    agent_message: brocode_protocol::items::AgentMessageItem,
    state: &mut PlanModeStreamState,
) {
        ...
        .contains(&agent_message_id)
    {
        let start_item = state
            ...
            .unwrap_or_else(|| {
                TurnItem::AgentMessage(brocode_protocol::items::AgentMessageItem {
...
async fn drain_in_flight(
    in_flight: &mut FuturesOrdered<BoxFuture<'static, BrocodeResult<ResponseInputItem>>>,
    sess: Arc<Session>,
    turn_context: Arc<TurnContext>,
) -> BrocodeResult<()> {
...
async fn try_run_sampling_request(
    ...
    let mut stream = client_session
        ...
        .or_cancel(&cancellation_token)
        .await??;
    let mut in_flight: FuturesOrdered<BoxFuture<'static, BrocodeResult<ResponseInputItem>>> =
        FuturesOrdered::new();
    ...
    let outcome: BrocodeResult<SamplingRequestResult> = loop {
        ...
        {
            Ok(event) => event,
            Err(brocode_async_utils::CancelErr::Cancelled) => break Err(BrocodeErr::TurnAborted),
        };

        let event = match event {
            Some(res) => res?,
            None => {
                break Err(BrocodeErr::Stream(
                        ...
                        && let Some(raw_text) = raw_assistant_output_text_from_item(&item)
                    {
                        ...
                        if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                            agent_message.content =
                                vec![brocode_protocol::items::AgentMessageContent::Text {
                                    text: if plan_mode {
                                        String::new()
                                    } else {
    ...
    if cancellation_token.is_cancelled() {
        return Err(BrocodeErr::TurnAborted);
    }
