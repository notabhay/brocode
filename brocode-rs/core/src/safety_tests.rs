...
fn external_sandbox_auto_approves_in_on_request() {
    ...
    let policy = SandboxPolicy::ExternalSandbox {
        network_access: brocode_protocol::protocol::NetworkAccess::Enabled,
    };
...
#[test]
fn missing_project_dot_brocode_config_requires_approval() {
    let tmp = TempDir::new().unwrap();
    let cwd = tmp.path().to_path_buf();
    let action = ApplyPatchAction::new_add_for_test(
        &cwd.join(".brocode").join("config.toml"),
        "".to_string(),
    );
