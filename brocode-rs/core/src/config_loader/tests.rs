...
async fn returns_config_error_for_schema_error_in_user_config() {
    ...
    let err = ConfigBuilder::default()
        .brocode_home(tmp.path().to_path_buf())
    ...
    let config_error = config_error_from_io(&err);
    let _guard = brocode_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let expected_config_error =
        brocode_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
            .expect("schema error");
    assert_eq!(config_error, &expected_config_error);
}
...
fn schema_error_points_to_feature_value() {
    ...
    let _guard = brocode_utils_absolute_path::AbsolutePathBufGuard::new(tmp.path());
    let error = brocode_config::config_error_from_typed_toml::<ConfigToml>(&config_path, contents)
        .expect("schema error");
...
async fn project_layers_prefer_closest_cwd() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".brocode")).await?;
    tokio::fs::create_dir_all(project_root.join(".brocode")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;

    tokio::fs::write(
        project_root.join(".brocode").join(CONFIG_TOML_FILE),
        "foo = \"root\"\n",
    )
    .await?;
    tokio::fs::write(
        nested.join(".brocode").join(CONFIG_TOML_FILE),
        "foo = \"child\"\n",
    )
    .await?;
    ...
    let project_layers: Vec<_> = layers
        ...
        .filter_map(|layer| match &layer.name {
            super::ConfigLayerSource::Project { dot_brocode_folder } => Some(dot_brocode_folder),
            _ => None,
        })
        .collect();
    ...
    assert_eq!(
        ...
        nested.join(".brocode").as_path()
    );
    assert_eq!(
        project_layers[1].as_path(),
        project_root.join(".brocode").as_path()
    );
...
#[tokio::test]
async fn project_paths_resolve_relative_to_dot_brocode_and_override_in_order() -> std::io::Result<()>
{
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
...
"#;
    ...
    .await?;
    tokio::fs::write(nested.join(".brocode").join(CONFIG_TOML_FILE), nested_cfg).await?;
    tokio::fs::write(
        project_root.join(".brocode").join("root.txt"),
        "root instructions",
    )
    .await?;
    tokio::fs::write(
        nested.join(".brocode").join("child.txt"),
        "child instructions",
    )
    .await?;
...
async fn cli_override_model_instructions_file_sets_base_instructions() -> std::io::Result<()> {
    ...
    let brocode_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&brocode_home).await?;
    tokio::fs::write(brocode_home.join(CONFIG_TOML_FILE), "").await?;
...
#[tokio::test]
async fn project_layer_is_added_when_dot_brocode_exists_without_config_toml() -> std::io::Result<()>
{
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    ...
    assert_eq!(
        vec![&ConfigLayerEntry {
            name: super::ConfigLayerSource::Project {
                dot_brocode_folder: AbsolutePathBuf::from_absolute_path(
                    project_root.join(".brocode")
                )?,
            },
            config: TomlValue::Table(toml::map::Map::new()),
            raw_toml: None,
...
#[tokio::test]
async fn brocode_home_is_not_loaded_as_project_layer_from_home_dir() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let home_dir = tmp.path().join("home");
    let brocode_home = home_dir.join(".brocode");
    tokio::fs::create_dir_all(&brocode_home).await?;
    tokio::fs::write(brocode_home.join(CONFIG_TOML_FILE), "foo = \"user\"\n").await?;
...
#[tokio::test]
async fn brocode_home_within_project_tree_is_not_double_loaded() -> std::io::Result<()> {
    let tmp = tempdir()?;
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let project_dot_brocode = project_root.join(".brocode");
    let nested_dot_brocode = nested.join(".brocode");

    tokio::fs::create_dir_all(&nested_dot_brocode).await?;
    tokio::fs::create_dir_all(project_root.join(".git")).await?;
    tokio::fs::write(
        nested_dot_brocode.join(CONFIG_TOML_FILE),
        "foo = \"child\"\n",
    )
    .await?;

    tokio::fs::create_dir_all(&project_dot_brocode).await?;
    make_config_for_test(
        &project_dot_brocode,
        &project_root,
        TrustLevel::Trusted,
    ...
    assert_eq!(
        vec![&ConfigLayerEntry {
            name: super::ConfigLayerSource::Project {
                dot_brocode_folder: AbsolutePathBuf::from_absolute_path(&nested_dot_brocode)?,
            },
...
async fn project_layers_disabled_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    let brocode_home_untrusted = tmp.path().join("home_untrusted");
    tokio::fs::create_dir_all(&brocode_home_untrusted).await?;
    make_config_for_test(
    ...
    let brocode_home_unknown = tmp.path().join("home_unknown");
    tokio::fs::create_dir_all(&brocode_home_unknown).await?;
    tokio::fs::write(
        brocode_home_unknown.join(CONFIG_TOML_FILE),
        "foo = \"user\"\n",
    )
    .await?;

    let layers_unknown = load_config_layers_state(
        &brocode_home_unknown,
        Some(cwd),
        &[] as &[(String, TomlValue)],
        LoaderOverrides::default(),
        CloudRequirementsLoader::default(),
...
-> std::io::Result<()> {
    ...
    let project_root = tmp.path().join("project");
    let nested = project_root.join("child");
    let dot_brocode = project_root.join(".brocode");
    let brocode_home = tmp.path().join("home");
    tokio::fs::create_dir_all(&nested).await?;
    tokio::fs::create_dir_all(&dot_brocode).await?;
    tokio::fs::create_dir_all(&brocode_home).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(
        dot_brocode.join(CONFIG_TOML_FILE),
        r#"
[mcp_servers.sentry]
...
async fn invalid_project_config_ignored_when_untrusted_or_unknown() -> std::io::Result<()> {
    ...
    tokio::fs::create_dir_all(nested.join(".brocode")).await?;
    tokio::fs::write(project_root.join(".git"), "gitdir: here").await?;
    tokio::fs::write(nested.join(".brocode").join(CONFIG_TOML_FILE), "foo =").await?;
    ...
    for (name, trust_level) in cases {
        let brocode_home = tmp.path().join(format!("home_{name}"));
        tokio::fs::create_dir_all(&brocode_home).await?;
...
mod requirements_exec_policy_tests {
    ...
    use brocode_config::RequirementsExecPolicyDecisionToml;
    use brocode_config::RequirementsExecPolicyParseError;
    use brocode_config::RequirementsExecPolicyPatternTokenToml;
    use brocode_config::RequirementsExecPolicyPrefixRuleToml;
    use brocode_config::RequirementsExecPolicyToml;
    use brocode_execpolicy::Decision;
    use brocode_execpolicy::Evaluation;
    use brocode_execpolicy::RuleMatch;
    use brocode_utils_absolute_path::AbsolutePathBuf;
    use pretty_assertions::assert_eq;
    use std::path::Path;
    use tempfile::tempdir;
    ...
    fn config_stack_for_dot_brocode_folder_with_requirements(
        dot_brocode_folder: &Path,
        requirements: ConfigRequirements,
    ) -> ConfigLayerStack {
        let dot_brocode_folder = AbsolutePathBuf::from_absolute_path(dot_brocode_folder)
            .expect("absolute dot_brocode_folder");
...
"#;
    ...
    async fn loads_requirements_exec_policy_without_rules_files() -> anyhow::Result<()> {
        ...
        let config_stack =
            config_stack_for_dot_brocode_folder_with_requirements(temp_dir.path(), requirements);
