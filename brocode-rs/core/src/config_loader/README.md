# `brocode-core` config loader

This module is the canonical place to **load and describe Brocode configuration layers** (user config, CLI/session overrides, managed config, and MDM-managed preferences) and to produce:
...
Exported from `brocode_core::config_loader`:

- `load_config_layers_state(brocode_home, cwd_opt, cli_overrides, overrides, cloud_requirements) -> ConfigLayerStack`
- `ConfigLayerStack`
...
```rust
use brocode_core::config_loader::{load_config_layers_state, LoaderOverrides};
