...
use brocode_config::ConfigRequirementsWithSources;
...
pub use brocode_config::AppRequirementToml;
pub use brocode_config::AppsRequirementsToml;
pub use brocode_config::CloudRequirementsLoadError;
pub use brocode_config::CloudRequirementsLoadErrorCode;
pub use brocode_config::CloudRequirementsLoader;
pub use brocode_config::ConfigError;
pub use brocode_config::ConfigLayerEntry;
pub use brocode_config::ConfigLayerStack;
pub use brocode_config::ConfigLayerStackOrdering;
pub use brocode_config::ConfigLoadError;
pub use brocode_config::ConfigRequirements;
pub use brocode_config::ConfigRequirementsToml;
pub use brocode_config::ConstrainedWithSource;
pub use brocode_config::FeatureRequirementsToml;
pub use brocode_config::LoaderOverrides;
...
pub use brocode_config::Sourced;
pub use brocode_config::TextPosition;
pub use brocode_config::TextRange;
pub use brocode_config::WebSearchModeRequirement;
pub(crate) use brocode_config::build_cli_overrides_layer;
pub(crate) use brocode_config::config_error_from_toml;
pub use brocode_config::default_project_root_markers;
pub use brocode_config::format_config_error;
pub use brocode_config::format_config_error_with_source;
pub(crate) use brocode_config::io_error_from_config_error;
pub use brocode_config::merge_toml_values;
pub use brocode_config::project_root_markers_from_config;
#[cfg(test)]
pub(crate) use brocode_config::version_for_toml;

/// On Unix systems, load default settings from this file path, if present.
/// Note that /etc/brocode/ is treated as a "config folder," so subfolders such
/// as skills/ and rules/ will also be honored.
pub const SYSTEM_CONFIG_TOML_FILE_UNIX: &str = "/etc/brocode/config.toml";

#[cfg(windows)]
const DEFAULT_PROGRAM_DATA_DIR_WINDOWS: &str = r"C:\ProgramData";

pub(crate) async fn first_layer_config_error(layers: &ConfigLayerStack) -> Option<ConfigError> {
    brocode_config::first_layer_config_error::<ConfigToml>(layers, CONFIG_TOML_FILE).await
}

pub(crate) async fn first_layer_config_error_from_entries(
    layers: &[ConfigLayerEntry],
) -> Option<ConfigError> {
    brocode_config::first_layer_config_error_from_entries::<ConfigToml>(layers, CONFIG_TOML_FILE)
        .await
}
...
/// - system    `/etc/brocode/requirements.toml` (Unix) or
///   `%ProgramData%\OpenAI\Brocode\requirements.toml` (Windows)
///
...
pub async fn load_config_layers_state(
    ...
    let loaded_config_layers =
        layer_io::load_config_layers_internal(brocode_home, overrides).await?;
    ...
    if let Some(config) = managed_config_from_mdm {
        ...
        // value for base_dir, so brocode_home is as good a value as any.
        let managed_config =
            resolve_relative_paths_in_config_toml(config.managed_config, brocode_home)?;
...
fn system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    AbsolutePathBuf::from_absolute_path(Path::new("/etc/brocode/requirements.toml"))
}
...
#[cfg(windows)]
fn windows_brocode_system_dir() -> PathBuf {
    ...
    program_data.join("OpenAI").join("Brocode")
}

#[cfg(windows)]
fn windows_system_requirements_toml_file() -> io::Result<AbsolutePathBuf> {
    let requirements_toml_file = windows_brocode_system_dir().join("requirements.toml");
    AbsolutePathBuf::try_from(requirements_toml_file)
}

#[cfg(windows)]
fn windows_system_config_toml_file() -> io::Result<AbsolutePathBuf> {
    let config_toml_file = windows_brocode_system_dir().join("config.toml");
...
fn project_layer_entry(
    ...
    let source = ConfigLayerSource::Project {
        dot_brocode_folder: dot_brocode_folder.clone(),
    };
...
async fn load_project_layers(
    ...
    let brocode_home_abs = AbsolutePathBuf::from_absolute_path(brocode_home)?;
    let brocode_home_normalized = normalize_path(brocode_home_abs.as_path())
        .unwrap_or_else(|_| brocode_home_abs.to_path_buf());
    let mut dirs = cwd
        .as_path()
        .ancestors()
        .scan(false, |done, a| {
    ...
    for dir in dirs {
        let dot_brocode = dir.join(".brocode");
        if !tokio::fs::metadata(&dot_brocode)
            .await
        ...
        let dot_brocode_abs = AbsolutePathBuf::from_absolute_path(&dot_brocode)?;
        let dot_brocode_normalized = normalize_path(dot_brocode_abs.as_path())
            .unwrap_or_else(|_| dot_brocode_abs.to_path_buf());
        if dot_brocode_abs == brocode_home_abs || dot_brocode_normalized == brocode_home_normalized
        {
            continue;
        }
        let config_file = dot_brocode_abs.join(CONFIG_TOML_FILE)?;
        match tokio::fs::read_to_string(&config_file).await {
            Ok(contents) => {
                ...
                let config =
                    resolve_relative_paths_in_config_toml(config, dot_brocode_abs.as_path())?;
...
/// from a file like `/etc/brocode/managed_config.toml` that has the same
...
impl From<LegacyManagedConfigToml> for ConfigRequirementsToml {
    fn from(legacy: LegacyManagedConfigToml) -> Self {
        ...
        if let Some(sandbox_mode) = sandbox_mode {
            ...
            // Allowing read-only is a requirement for Brocode to function correctly.
    ...
    fn windows_system_config_toml_file_uses_expected_suffix() {
        ...
        assert!(
            windows_system_config_toml_file()
                .expect("config.toml path")
                .as_path()
                .ends_with(Path::new("OpenAI").join("Brocode").join("config.toml"))
        );
    }
}
