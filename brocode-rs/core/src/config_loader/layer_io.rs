...
use brocode_config::config_error_from_toml;
use brocode_config::io_error_from_config_error;
...
#[cfg(unix)]
const BROCODE_MANAGED_CONFIG_SYSTEM_PATH: &str = "/etc/brocode/managed_config.toml";
...
pub(super) struct LoadedConfigLayers {
    /// If present, data read from a file such as `/etc/brocode/managed_config.toml`.
...
pub(super) async fn load_config_layers_internal(
    ...
    let managed_config_path = AbsolutePathBuf::from_absolute_path(
        managed_config_path.unwrap_or_else(|| managed_config_default_path(brocode_home)),
    )?;
...
pub(super) fn managed_config_default_path(brocode_home: &Path) -> PathBuf {
    #[cfg(unix)]
    {
        let _ = brocode_home;
        PathBuf::from(BROCODE_MANAGED_CONFIG_SYSTEM_PATH)
    }

    #[cfg(not(unix))]
    {
        brocode_home.join("managed_config.toml")
    }
}
