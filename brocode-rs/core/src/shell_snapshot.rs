...
impl ShellSnapshot {
    ...
    fn spawn_snapshot_task(
        ...
        tokio::spawn(
            async move {
                let timer =
                    session_telemetry.start_timer("brocode.shell_snapshot.duration_ms", &[]);
                ...
                session_telemetry.counter("brocode.shell_snapshot", /*inc*/ 1, &counter_tags);
    ...
    async fn try_new(
        ...
        let nonce = SystemTime::now()
            ...
            .unwrap_or(0);
        let path = brocode_home
            .join(SNAPSHOT_DIR)
            .join(format!("{session_id}.{nonce}.{extension}"));
        let temp_path = brocode_home
            .join(SNAPSHOT_DIR)
        ...
        tokio::spawn(async move {
            if let Err(err) = cleanup_stale_snapshots(&brocode_home, cleanup_session_id).await {
...
pub async fn cleanup_stale_snapshots(
    ...
    while let Some(entry) = entries.next_entry().await? {
        ...
        let rollout_path = find_thread_path_by_id_str(brocode_home, session_id).await?;
