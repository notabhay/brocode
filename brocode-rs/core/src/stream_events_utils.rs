...
use brocode_utils_stream_parser::strip_proposed_plan_blocks;
use futures::Future;
use tracing::debug;
...
fn strip_hidden_assistant_markup_and_parse_memory_citation(
    ...
    String,
    Option<brocode_protocol::memory_citation::MemoryCitation>,
) {
        ...
        && role == "assistant"
    {
        let combined = content
            ...
            .filter_map(|ci| match ci {
                brocode_protocol::models::ContentItem::OutputText { text } => Some(text.as_str()),
                _ => None,
            })
            .collect::<String>();
...
async fn save_image_generation_result(
    ...
    let bytes = BASE64_STANDARD
        ...
        .map_err(|err| {
            BrocodeErr::InvalidRequest(format!("invalid image generation payload: {err}"))
        })?;
    let path = image_generation_artifact_path(brocode_home, session_id, call_id);
...
pub(crate) async fn handle_output_item_done(
    ...
    match ToolRouter::build_tool_call(ctx.sess.as_ref(), item.clone()).await {
        ...
        Err(FunctionCallError::Fatal(message)) => {
            return Err(BrocodeErr::Fatal(message));
        }
    }
            ...
            if let TurnItem::AgentMessage(agent_message) = &mut turn_item {
                let combined = agent_message
                    ...
                    .map(|entry| match entry {
                        brocode_protocol::items::AgentMessageContent::Text { text } => {
                            text.as_str()
                        }
                    })
                    .collect::<String>();
                ...
                agent_message.content =
                    vec![brocode_protocol::items::AgentMessageContent::Text { text: stripped }];
                ...
                .await
                {
                    Ok(path) => {
                        ...
                        let image_output_dir = image_output_path
                            .parent()
                            .unwrap_or(turn_context.config.brocode_home.as_path());
