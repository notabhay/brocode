...
use crate::brocode_thread::BrocodeThread;
...
use brocode_app_server_protocol::ThreadHistoryBuilder;
...
struct TempBrocodeHomeGuard {
    path: PathBuf,
}

impl Drop for TempBrocodeHomeGuard {
    fn drop(&mut self) {
...
/// Represents a newly created Brocode thread (formerly called a conversation), including the first event
/// (which is [`EventMsg::SessionConfigured`]).
pub struct NewThread {
    pub thread_id: ThreadId,
    pub thread: Arc<BrocodeThread>,
...
pub struct ThreadManager {
    ...
    _test_brocode_home_guard: Option<TempBrocodeHomeGuard>,
}
...
pub(crate) struct ThreadManagerState {
    threads: Arc<RwLock<HashMap<ThreadId, Arc<BrocodeThread>>>>,
...
impl ThreadManager {
    pub fn new(
        ...
        Self {
            ...
            _test_brocode_home_guard: None,
        }
    }

    /// Construct with a dummy AuthManager containing the provided BrocodeAuth.
    ...
    pub(crate) fn with_models_provider_for_tests(
        ...
        let brocode_home = std::env::temp_dir().join(format!(
            "brocode-thread-manager-test-{}",
            uuid::Uuid::new_v4()
        ));
        std::fs::create_dir_all(&brocode_home)
            .unwrap_or_else(|err| panic!("temp brocode home dir create failed: {err}"));
        ...
        manager._test_brocode_home_guard = Some(TempBrocodeHomeGuard { path: brocode_home });
        manager
    }

    /// Construct with a dummy AuthManager containing the provided BrocodeAuth and brocode home.
    /// Used for integration tests: should not be used by ordinary business logic.
    ...
    pub async fn get_thread(&self, thread_id: ThreadId) -> BrocodeResult<Arc<BrocodeThread>> {
        self.state.get_thread(thread_id).await
    }

    pub async fn start_thread(&self, config: Config) -> BrocodeResult<NewThread> {
    ...
    pub async fn start_thread_with_tools(
        &self,
        config: Config,
        dynamic_tools: Vec<brocode_protocol::dynamic_tools::DynamicToolSpec>,
        persist_extended_history: bool,
    ) -> BrocodeResult<NewThread> {
    ...
    /// as `Arc<BrocodeThread>`, it is possible that other references to it exist elsewhere.
    /// Returns the thread if the thread was found and removed.
    pub async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<BrocodeThread>> {
...
impl ThreadManagerState {
    ...
    pub(crate) async fn get_thread(
        &self,
        thread_id: ThreadId,
    ) -> BrocodeResult<Arc<BrocodeThread>> {
    ...
    pub(crate) async fn send_op(&self, thread_id: ThreadId, op: Op) -> BrocodeResult<String> {
    ...
    pub(crate) async fn remove_thread(&self, thread_id: &ThreadId) -> Option<Arc<BrocodeThread>> {
    ...
    pub(crate) async fn spawn_thread_with_source(
        ...
        let BrocodeSpawnOk {
            brocode, thread_id, ..
        } = Brocode::spawn(BrocodeSpawnArgs {
            config,
            auth_manager,
        ...
        .await?;
        self.finalize_thread_spawn(brocode, thread_id, watch_registration)
            .await
    }
    ...
    async fn finalize_thread_spawn(
        ...
        let event = brocode.next_event().await?;
        let session_configured = match event {
            ...
            _ => {
                return Err(BrocodeErr::SessionConfiguredNotFirstEvent);
            }
        };

        let thread = Arc::new(BrocodeThread::new(
            brocode,
