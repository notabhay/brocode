...
///
/// 1. The process was spawned by Brocode as part of a shell tool call.
/// 2. NetworkSandboxPolicy is restricted for the tool call.
///
/// We may try to have just one environment variable for all sandboxing
/// attributes, so this may change in the future.
pub const BROCODE_SANDBOX_NETWORK_DISABLED_ENV_VAR: &str = "BROCODE_SANDBOX_NETWORK_DISABLED";
...
pub const BROCODE_SANDBOX_ENV_VAR: &str = "BROCODE_SANDBOX";
...
/// `BROCODE_SANDBOX_NETWORK_DISABLED_ENV_VAR` environment variable.
...
pub(crate) async fn spawn_child_async(request: SpawnChildRequest<'_>) -> std::io::Result<Child> {
    ...
    // If this Brocode process dies (including being killed via SIGKILL), we want
    ...
    unsafe {
        ...
        cmd.pre_exec(move || {
            if detach_from_tty {
                brocode_utils_pty::process_group::detach_from_tty()?;
            }
            ...
            #[cfg(target_os = "linux")]
            {
                ...
                // current parent dies."
                brocode_utils_pty::process_group::set_parent_death_signal(parent_pid)?;
            }
            Ok(())
        });
    }
