...
fn has_plugins_clone_dirs(brocode_home: &Path) -> bool {
    let Ok(entries) = std::fs::read_dir(brocode_home.join(".tmp")) else {
        return false;
    };
...
#[test]
fn curated_plugins_repo_path_uses_brocode_home_tmp_dir() {
...
if [ "$1" = "clone" ]; then
  dest="$5"
  mkdir -p "$dest/.git" "$dest/.agents/plugins" "$dest/plugins/gmail/.brocode-plugin"
...
EOF
  printf '%s\n' '{{"name":"gmail"}}' > "$dest/plugins/gmail/.brocode-plugin/plugin.json"
  exit 0
fi
...
async fn sync_openai_plugins_repo_falls_back_to_http_when_git_is_unavailable() {
    ...
    assert!(
        repo_path
            .join("plugins/gmail/.brocode-plugin/plugin.json")
            .is_file()
    );
    assert_eq!(read_curated_plugins_sha(tmp.path()).as_deref(), Some(sha));
}
...
"#,
  ...
  {"id":"1","name":"linear","marketplace_name":"openai-curated","version":"1.0.0","enabled":true}
    ...
    let auth_manager =
        AuthManager::from_auth_for_testing(BrocodeAuth::create_dummy_chatgpt_auth_for_testing());
...
fn curated_repo_zipball_bytes(sha: &str) -> Vec<u8> {
  ...
  "plugins": [
    ...
    writer
        .start_file(
            format!("{root}/plugins/gmail/.brocode-plugin/plugin.json"),
            options,
        )
