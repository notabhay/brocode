...
pub(crate) fn write_curated_plugin_sha(brocode_home: &Path) {
    write_curated_plugin_sha_with(brocode_home, TEST_CURATED_PLUGIN_SHA);
}

pub(crate) fn write_curated_plugin_sha_with(brocode_home: &Path, sha: &str) {
    write_file(&brocode_home.join(".tmp/plugins.sha"), &format!("{sha}\n"));
}

pub(crate) fn write_plugins_feature_config(brocode_home: &Path) {
    write_file(
...
pub(crate) async fn load_plugins_config(brocode_home: &Path) -> crate::config::Config {
    ConfigBuilder::default()
        .brocode_home(brocode_home.to_path_buf())
        .fallback_cwd(Some(brocode_home.to_path_buf()))
        .build()
        .await
