...
use brocode_app_server_protocol::MergeStrategy;
use brocode_features::Feature;
use brocode_plugin::AppConnectorId;
use brocode_plugin::PluginCapabilitySummary;
use brocode_plugin::PluginId;
use brocode_plugin::PluginIdError;
use brocode_plugin::PluginTelemetryMetadata;
use brocode_plugin::prompt_safe_plugin_description;
...
impl PluginsManager {
    pub fn new(brocode_home: PathBuf) -> Self {
        Self::new_with_restriction_product(brocode_home, Some(Product::Brocode))
    }
    ...
    pub fn new_with_restriction_product(
        ...
        // Product restrictions are enforced at marketplace admission time for a given BROCODE_HOME:
        ...
        // contents of that BROCODE_HOME and does not re-filter configured plugins by product, so
        // already-admitted plugins may continue exposing MCP servers/tools from shared local state.
        //
        // This assumes a single BROCODE_HOME is only used by one product.
        Self {
            brocode_home: brocode_home.clone(),
            store: PluginStore::new(brocode_home),
    ...
    async fn install_resolved_plugin(
        ...
        let plugin_version =
            if resolved.plugin_id.marketplace_name == OPENAI_CURATED_MARKETPLACE_NAME {
                Some(
                    read_curated_plugins_sha(self.brocode_home.as_path()).ok_or_else(|| {
        ...
        ConfigService::new_with_defaults(self.brocode_home.clone())
    ...
    async fn uninstall_plugin_id(&self, plugin_id: PluginId) -> Result<(), PluginUninstallError> {
        let plugin_telemetry = self
            ...
            .map(|_| installed_plugin_telemetry_metadata(self.brocode_home.as_path(), &plugin_id));
        let store = self.store.clone();
        ...
        ConfigEditsBuilder::new(&self.brocode_home)
    ...
    pub async fn sync_plugins_from_remote(
        ...
        let curated_marketplace_root = curated_plugins_repo_path(self.brocode_home.as_path());
        ...
        let curated_plugin_version = read_curated_plugins_sha(self.brocode_home.as_path())
            .ok_or_else(|| {
                PluginStoreError::Invalid(
                    "local curated marketplace sha is not available".to_string(),
                )
    ...
    pub fn read_plugin_for_config(
        ...
        let manifest = load_plugin_manifest(source_path.as_path()).ok_or_else(|| {
            MarketplaceError::InvalidPlugin(
                "missing or invalid .brocode-plugin/plugin.json".to_string(),
            )
        })?;
    ...
    fn start_curated_repo_sync(self: &Arc<Self>) {
        ...
        let manager = Arc::clone(self);
        let brocode_home = self.brocode_home.clone();
        if let Err(err) = std::thread::Builder::new()
            .name("plugins-curated-repo-sync".to_string())
            .spawn(
                move || match sync_openai_plugins_repo(brocode_home.as_path()) {
                    Ok(curated_plugin_version) => {
                        let configured_curated_plugin_ids =
                            configured_curated_plugin_ids_from_brocode_home(brocode_home.as_path());
    ...
    fn marketplace_roots(&self, additional_roots: &[AbsolutePathBuf]) -> Vec<AbsolutePathBuf> {
        ...
        let curated_repo_root = curated_plugins_repo_path(self.brocode_home.as_path());
...
fn refresh_curated_plugin_cache(
    ...
    let store = PluginStore::new(brocode_home.to_path_buf());
    let curated_marketplace_path = AbsolutePathBuf::try_from(
        curated_plugins_repo_path(brocode_home).join(".agents/plugins/marketplace.json"),
    )
...
fn configured_curated_plugin_ids_from_brocode_home(brocode_home: &Path) -> Vec<PluginId> {
...
fn load_plugin(
    ...
    let Some(manifest) = load_plugin_manifest(plugin_root.as_path()) else {
        loaded_plugin.error = Some("missing or invalid .brocode-plugin/plugin.json".to_string());
        return loaded_plugin;
    };
...
fn normalize_plugin_mcp_server_value(
        ...
        && oauth.contains_key("callbackPort")
    {
        warn!(
            plugin = %plugin_root.display(),
            "plugin MCP server OAuth callbackPort is ignored; Brocode uses global MCP OAuth callback settings"
        );
    }
