...
use brocode_plugin::validate_plugin_segment;
use brocode_utils_absolute_path::AbsolutePathBuf;
use brocode_utils_plugins::PLUGIN_MANIFEST_PATH;
use std::fs;
use std::io;
use std::path::Path;
...
impl PluginStore {
    pub fn new(brocode_home: PathBuf) -> Self {
        Self {
            root: AbsolutePathBuf::try_from(brocode_home.join(PLUGINS_CACHE_DIR))
