...
use brocode_otel::metrics::names::CURATED_PLUGINS_STARTUP_SYNC_FINAL_METRIC;
use brocode_otel::metrics::names::CURATED_PLUGINS_STARTUP_SYNC_METRIC;
use reqwest::Client;
...
// Keep this comfortably above a normal sync attempt so we do not race another Brocode process.
...
pub(crate) fn curated_plugins_repo_path(brocode_home: &Path) -> PathBuf {
    brocode_home.join(CURATED_PLUGINS_RELATIVE_DIR)
}

pub(crate) fn read_curated_plugins_sha(brocode_home: &Path) -> Option<String> {
    read_sha_file(brocode_home.join(CURATED_PLUGINS_SHA_FILE).as_path())
}

pub(crate) fn sync_openai_plugins_repo(brocode_home: &Path) -> Result<String, String> {
    sync_openai_plugins_repo_with_transport_overrides(brocode_home, "git", GITHUB_API_BASE_URL)
}
...
fn sync_openai_plugins_repo_with_transport_overrides(
    ...
    match sync_openai_plugins_repo_via_git(brocode_home, git_binary) {
        Ok(remote_sha) => {
        ...
        Err(err) => {
            ...
            let result = sync_openai_plugins_repo_via_http(brocode_home, api_base_url);
...
fn sync_openai_plugins_repo_via_git(
    ...
    let repo_path = curated_plugins_repo_path(brocode_home);
    let sha_path = brocode_home.join(CURATED_PLUGINS_SHA_FILE);
...
pub(super) fn start_startup_remote_plugin_sync_once(
    ...
    let marker_path = startup_remote_plugin_sync_marker_path(brocode_home.as_path());
    ...
    tokio::spawn(async move {
        ...
        if !wait_for_startup_remote_plugin_sync_prerequisites(brocode_home.as_path()).await {
            warn!(
                brocode_home = %brocode_home.display(),
            ...
            .await
        {
            Ok(sync_result) => {
                ...
                if let Err(err) =
                    write_startup_remote_plugin_sync_marker(brocode_home.as_path()).await
                {
                    warn!(
                        error = %err,
...
fn startup_remote_plugin_sync_marker_path(brocode_home: &Path) -> PathBuf {
    brocode_home.join(STARTUP_REMOTE_PLUGIN_SYNC_MARKER_FILE)
}

fn startup_remote_plugin_sync_prerequisites_ready(brocode_home: &Path) -> bool {
    brocode_home
        .join(".tmp/plugins/.agents/plugins/marketplace.json")
        .is_file()
        && brocode_home.join(".tmp/plugins.sha").is_file()
}

async fn wait_for_startup_remote_plugin_sync_prerequisites(brocode_home: &Path) -> bool {
    let deadline = tokio::time::Instant::now() + STARTUP_REMOTE_PLUGIN_SYNC_PREREQUISITE_TIMEOUT;
    loop {
        if startup_remote_plugin_sync_prerequisites_ready(brocode_home) {
            return true;
        }
...
async fn write_startup_remote_plugin_sync_marker(brocode_home: &Path) -> std::io::Result<()> {
    let marker_path = startup_remote_plugin_sync_marker_path(brocode_home);
...
fn emit_curated_plugins_startup_sync_counter(
    ...
    let Some(metrics) = brocode_otel::metrics::global() else {
        return;
    };
