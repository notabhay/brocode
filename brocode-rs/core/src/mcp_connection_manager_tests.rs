use super::*;
use brocode_protocol::protocol::GranularApprovalConfig;
...
fn create_test_tool(server_name: &str, tool_name: &str) -> ToolInfo {
    ToolInfo {
        ...
        tool_namespace: if server_name == BROCODE_APPS_MCP_SERVER_NAME {
...
fn create_brocode_apps_tools_cache_context(
    brocode_home: PathBuf,
    account_id: Option<&str>,
    chatgpt_user_id: Option<&str>,
) -> BrocodeAppsToolsCacheContext {
    BrocodeAppsToolsCacheContext {
        brocode_home,
...
#[test]
fn brocode_apps_tools_cache_is_overwritten_by_last_write() {
    let brocode_home = tempdir().expect("tempdir");
    let cache_context = create_brocode_apps_tools_cache_context(
        brocode_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let tools_gateway_1 = vec![create_test_tool(BROCODE_APPS_MCP_SERVER_NAME, "one")];
    let tools_gateway_2 = vec![create_test_tool(BROCODE_APPS_MCP_SERVER_NAME, "two")];

    write_cached_brocode_apps_tools(&cache_context, &tools_gateway_1);
    let cached_gateway_1 =
        read_cached_brocode_apps_tools(&cache_context).expect("cache entry exists for first write");
    assert_eq!(cached_gateway_1[0].tool_name, "one");

    write_cached_brocode_apps_tools(&cache_context, &tools_gateway_2);
    let cached_gateway_2 = read_cached_brocode_apps_tools(&cache_context)
        .expect("cache entry exists for second write");
    assert_eq!(cached_gateway_2[0].tool_name, "two");
}

#[test]
fn brocode_apps_tools_cache_is_scoped_per_user() {
    let brocode_home = tempdir().expect("tempdir");
    let cache_context_user_1 = create_brocode_apps_tools_cache_context(
        brocode_home.path().to_path_buf(),
        Some("account-one"),
        Some("user-one"),
    );
    let cache_context_user_2 = create_brocode_apps_tools_cache_context(
        brocode_home.path().to_path_buf(),
        Some("account-two"),
        Some("user-two"),
    );
    let tools_user_1 = vec![create_test_tool(BROCODE_APPS_MCP_SERVER_NAME, "one")];
    let tools_user_2 = vec![create_test_tool(BROCODE_APPS_MCP_SERVER_NAME, "two")];

    write_cached_brocode_apps_tools(&cache_context_user_1, &tools_user_1);
    write_cached_brocode_apps_tools(&cache_context_user_2, &tools_user_2);

    let read_user_1 =
        read_cached_brocode_apps_tools(&cache_context_user_1).expect("cache entry for user one");
    let read_user_2 =
        read_cached_brocode_apps_tools(&cache_context_user_2).expect("cache entry for user two");
...
#[test]
fn brocode_apps_tools_cache_filters_disallowed_connectors() {
    let brocode_home = tempdir().expect("tempdir");
    let cache_context = create_brocode_apps_tools_cache_context(
        brocode_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let cached =
        read_cached_brocode_apps_tools(&cache_context).expect("cache entry exists for user");
...
#[test]
fn brocode_apps_tools_cache_is_ignored_when_schema_version_mismatches() {
    let brocode_home = tempdir().expect("tempdir");
    let cache_context = create_brocode_apps_tools_cache_context(
        brocode_home.path().to_path_buf(),
        Some("account-one"),
    ...
    let bytes = serde_json::to_vec_pretty(&serde_json::json!({
        "schema_version": BROCODE_APPS_TOOLS_CACHE_SCHEMA_VERSION + 1,
        "tools": [create_test_tool(BROCODE_APPS_MCP_SERVER_NAME, "one")],
    }))
    .expect("serialize");
    std::fs::write(cache_path, bytes).expect("write");

    assert!(read_cached_brocode_apps_tools(&cache_context).is_none());
}

#[test]
fn brocode_apps_tools_cache_is_ignored_when_json_is_invalid() {
    let brocode_home = tempdir().expect("tempdir");
    let cache_context = create_brocode_apps_tools_cache_context(
        brocode_home.path().to_path_buf(),
        Some("account-one"),
...
#[test]
fn startup_cached_brocode_apps_tools_loads_from_disk_cache() {
    let brocode_home = tempdir().expect("tempdir");
    let cache_context = create_brocode_apps_tools_cache_context(
        brocode_home.path().to_path_buf(),
        Some("account-one"),
    ...
    write_cached_brocode_apps_tools(&cache_context, &cached_tools);
    ...
    assert_eq!(startup_tools[0].server_name, BROCODE_APPS_MCP_SERVER_NAME);
...
async fn list_all_tools_uses_startup_snapshot_while_client_is_pending() {
    ...
    manager.clients.insert(
        BROCODE_APPS_MCP_SERVER_NAME.to_string(),
        AsyncManagedClient {
    ...
    let tool = tools
        .get("mcp__brocode_apps__calendar_create_event")
        .expect("tool from startup cache");
    assert_eq!(tool.server_name, BROCODE_APPS_MCP_SERVER_NAME);
...
#[test]
fn elicitation_capability_enabled_only_for_brocode_apps() {
    let brocode_apps_capability = elicitation_capability_for_server(BROCODE_APPS_MCP_SERVER_NAME);
    assert!(matches!(
        brocode_apps_capability,
