...
use brocode_network_proxy::NetworkProxyConstraintError;
    ...
    stack
        ...
        .filter_map(|layer| {
            let path = match &layer.name {
                ...
                ConfigLayerSource::Project { dot_brocode_folder } => dot_brocode_folder
...
fn upsert_network_domain(
    config: &mut NetworkProxyConfig,
    host: String,
    permission: brocode_network_proxy::NetworkDomainPermission,
) {
    config
        .network
