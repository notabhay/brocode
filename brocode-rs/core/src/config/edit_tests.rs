...
"#,
    ...
    let raw = std::fs::read_to_string(brocode_home.join(CONFIG_TOML_FILE)).expect("read config");
...
fn blocking_set_model_writes_through_symlink_chain() {
    ...
    let link_path = brocode_home.join("config-link.toml");
...
fn blocking_set_model_replaces_symlink_on_cycle() {
    let tmp = tempdir().expect("tmpdir");
    let brocode_home = tmp.path();
    let link_a = brocode_home.join("a.toml");
    let link_b = brocode_home.join("b.toml");
    let config_path = brocode_home.join(CONFIG_TOML_FILE);
...
"#;
    std::fs::write(brocode_home.join(CONFIG_TOML_FILE), original).expect("seed config");
...
#[test]
fn blocking_set_hide_gpt_5_1_brocode_max_migration_prompt_preserves_table() {
    let tmp = tempdir().expect("tmpdir");
    let brocode_home = tmp.path();
    std::fs::write(
        brocode_home.join(CONFIG_TOML_FILE),
...
"#,
    ...
    let expected = r#"[notice]
existing = "value"
"hide_gpt-5.1-brocode-max_migration_prompt" = true
"#;
    assert_eq!(contents, expected);
}
...
fn blocking_clear_path_noop_when_missing() {
    ...
    assert!(
        !brocode_home.join(CONFIG_TOML_FILE).exists(),
