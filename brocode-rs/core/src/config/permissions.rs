...
use brocode_network_proxy::NetworkDomainPermission as ProxyNetworkDomainPermission;
use brocode_network_proxy::NetworkMode;
use brocode_network_proxy::NetworkProxyConfig;
use brocode_network_proxy::NetworkUnixSocketPermission as ProxyNetworkUnixSocketPermission;
...
/// for Brocode to function. These should always be added to the
/// `FileSystemSandboxPolicy` for a thread.
pub(crate) fn get_readable_roots_required_for_brocode_runtime(
    brocode_home: &Path,
    zsh_path: Option<&PathBuf>,
    main_execve_wrapper_exe: Option<&PathBuf>,
) -> Vec<AbsolutePathBuf> {
    let arg0_root = AbsolutePathBuf::from_absolute_path(brocode_home.join("tmp").join("arg0")).ok();
...
// Adding a new `:special_path` must not make older Brocode versions reject the
...
fn missing_filesystem_entries_warning(profile_name: &str) -> String {
    format!(
        "Permissions profile `{profile_name}` does not define any recognized filesystem entries for this version of Brocode. Filesystem access will remain restricted. Upgrade Brocode if this profile expects filesystem permissions."
    )
}
...
fn maybe_push_unknown_special_path_warning(
    ...
    push_warning(
        ...
        match subpath.as_deref() {
            Some(subpath) => format!(
                "Configured filesystem path `{path}` with nested entry `{}` is not recognized by this version of Brocode and will be ignored. Upgrade Brocode if this path is required.",
                subpath.display()
            ),
            None => format!(
                "Configured filesystem path `{path}` is not recognized by this version of Brocode and will be ignored. Upgrade Brocode if this path is required."
            ),
        },
    );
}
