...
pub async fn apply(
    ...
    task::spawn_blocking(move || apply_blocking(&brocode_home, profile.as_deref(), &edits))
        .await
...
impl ConfigEditsBuilder {
    pub fn new(brocode_home: &Path) -> Self {
        Self {
    ...
    pub fn apply_blocking(self) -> anyhow::Result<()> {
        apply_blocking(&self.brocode_home, self.profile.as_deref(), &self.edits)
    }
