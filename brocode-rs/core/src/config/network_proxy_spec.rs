...
use brocode_network_proxy::BlockedRequestObserver;
use brocode_network_proxy::ConfigReloader;
use brocode_network_proxy::ConfigState;
use brocode_network_proxy::NetworkDecision;
use brocode_network_proxy::NetworkPolicyDecider;
...
use brocode_network_proxy::NetworkProxyConstraints;
use brocode_network_proxy::NetworkProxyHandle;
use brocode_network_proxy::NetworkProxyState;
use brocode_network_proxy::build_config_state;
use brocode_network_proxy::host_and_port_from_network_addr;
use brocode_network_proxy::normalize_host;
use brocode_network_proxy::validate_policy_against_constraints;
    ...
    fn apply_requirements(
        ...
        if requirements.unix_sockets.is_some() {
            let allow_unix_sockets = requirements
                .unix_sockets
                .as_ref()
                .map(brocode_config::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
                .unwrap_or_default();
            config
                .network
    ...
    for host in hosts {
        if incoming.insert(host.clone()) {
            config.network.upsert_domain_permission(
                host,
                if allow {
                    brocode_network_proxy::NetworkDomainPermission::Allow
                } else {
                    brocode_network_proxy::NetworkDomainPermission::Deny
                },
                normalize_host,
            );
        }
