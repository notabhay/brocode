...
fn restricted_read_implicitly_allows_helper_executables() -> std::io::Result<()> {
    ...
    let brocode_home = temp_dir.path().join(".brocode");
    let zsh_path = temp_dir.path().join("runtime").join("zsh");
    let arg0_root = brocode_home.join("tmp").join("arg0");
    let allowed_arg0_dir = arg0_root.join("brocode-arg0-session");
    let sibling_arg0_dir = arg0_root.join("brocode-arg0-other-session");
    let execve_wrapper = allowed_arg0_dir.join("brocode-execve-wrapper");
...
fn network_toml_overlays_unix_socket_permissions_by_path() {
    ...
    assert_eq!(
        ...
        Some(brocode_network_proxy::NetworkUnixSocketPermissions {
            entries: BTreeMap::from([
                (
                    "/tmp/base.sock".to_string(),
