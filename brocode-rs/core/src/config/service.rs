...
use brocode_app_server_protocol::Config as ApiConfig;
use brocode_app_server_protocol::ConfigBatchWriteParams;
use brocode_app_server_protocol::ConfigLayerMetadata;
...
use brocode_app_server_protocol::OverriddenMetadata;
...
impl ConfigService {
    ...
    pub fn new_with_defaults(brocode_home: PathBuf) -> Self {
        Self {
            brocode_home,
    ...
    pub async fn load_user_saved_config(
        &self,
    ) -> Result<brocode_app_server_protocol::UserSavedConfig, ConfigServiceError> {
        let layers = self
            .load_thread_agnostic_config()
            .await
    ...
    async fn apply_edits(
        ...
        let allowed_path =
            AbsolutePathBuf::resolve_path_against_base(CONFIG_TOML_FILE, &self.brocode_home)
        ...
        let user_config_toml =
            deserialize_config_toml_with_base(user_config.clone(), &self.brocode_home).map_err(
                |err| {
                    ConfigServiceError::write(
                        ConfigWriteErrorCode::ConfigValidationError,
                        format!("Invalid configuration: {err}"),
    ...
    /// include any in-repo .brocode/ folders because there is no cwd/project root
...
fn override_message(layer: &ConfigLayerSource) -> String {
    match layer {
        ...
        ConfigLayerSource::Project { dot_brocode_folder } => format!(
            "Overridden by project config: {}/{CONFIG_TOML_FILE}",
            dot_brocode_folder.display(),
        ),
