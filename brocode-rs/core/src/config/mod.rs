...
use brocode_app_server_protocol::Tools;
use brocode_app_server_protocol::UserSavedConfig;
use brocode_features::Feature;
use brocode_features::FeatureConfigSource;
use brocode_features::FeatureOverrides;
use brocode_features::Features;
use brocode_features::FeaturesToml;
use brocode_git_utils::resolve_root_git_project_for_trust;
...
use brocode_protocol::config_types::WebSearchConfig;
...
use brocode_utils_absolute_path::AbsolutePathBufGuard;
...
use crate::config::permissions::get_readable_roots_required_for_brocode_runtime;
...
pub mod service;
pub mod types;
pub use brocode_config::Constrained;
...
pub use brocode_git_utils::GhostSnapshotConfig;
...
pub struct Config {
    ...
    ///
    /// - `None`: use default attribution (`Brocode <noreply@openai.com>`)
    /// - `Some("")` or whitespace-only: disable commit attribution
    /// - `Some("...")`: use the provided attribution text verbatim
    pub commit_attribution: Option<String>,

    /// Optional external notifier command. When set, Brocode will spawn this
    /// program after each completed *turn* (i.e. when the agent finishes
    /// processing a user submission). The value must be the full command
    /// broken into argv tokens **without** the trailing JSON argument - Brocode
    /// appends one extra argument containing a JSON payload describing the
    /// event.
    ///
    /// Example `~/.brocode/config.toml` snippet:
    ///
    /// ```toml
    /// notify = ["notify-send", "Brocode"]
    /// ```
    ///
    /// which will be invoked as:
    ///
    /// ```shell
    /// notify-send Brocode '{"type":"agent-turn-complete","turn-id":"12345"}'
    /// ```
    ///
    ...
    /// file (default): Use a file in the Brocode home directory.
    /// keyring: Use an OS-specific keyring service.
    /// auto: Use the OS-specific keyring service if available, otherwise use a file.
    pub cli_auth_credentials_store_mode: AuthCredentialsStoreMode,

    /// Definition for MCP servers that Brocode can reach out to for tool calls.
    ...
    ///
    /// When unset, Brocode will bind to an ephemeral port chosen by the OS.
    ...
    /// Path to the current Brocode executable. This cannot be set in the config
    /// file: it must be set in code via [`ConfigOverrides`].
    pub brocode_self_exe: Option<PathBuf>,

    /// Path to the `brocode-linux-sandbox` executable. This must be set if
    /// [`brocode_sandboxing::SandboxType::LinuxSeccomp`] is used. Note that this
    /// cannot be set in the config file: it must be set in code via
    /// [`ConfigOverrides`].
    ///
    /// When this program is invoked, arg0 will be set to `brocode-linux-sandbox`.
    pub brocode_linux_sandbox_exe: Option<PathBuf>,

    /// Path to the `brocode-execve-wrapper` executable used for shell
    ...
    /// When `true`, checks for Brocode updates on startup and surfaces update prompts.
    /// Set to `false` only if your Brocode updates are centrally managed.
    /// Defaults to `true`.
    ...
    /// When `false`, disables analytics across Brocode product surfaces in this machine.
    /// Voluntarily left as Optional because the default value might depend on the client.
    pub analytics_enabled: Option<bool>,

    /// When `false`, disables feedback collection across Brocode product surfaces.
    /// Defaults to `true`.
...
impl ConfigBuilder {
    pub fn brocode_home(mut self, brocode_home: PathBuf) -> Self {
        self.brocode_home = Some(brocode_home);
        self
    }
    ...
    pub async fn build(self) -> std::io::Result<Config> {
        ...
        let brocode_home = brocode_home.map_or_else(find_brocode_home, std::io::Result::Ok)?;
...
impl Config {
    ...
    pub fn load_default_with_cli_overrides(
        ...
        let config_toml = deserialize_config_toml_with_base(merged, &brocode_home)?;
    ...
    /// ignores user settings. For example, the `brocode exec` subcommand is
    /// designed to use [AskForApproval::Never] exclusively.
    ///
    /// Further, [ConfigOverrides] contains some options that are not supported
    /// in [ConfigToml], such as `cwd`, `brocode_self_exe`, `brocode_linux_sandbox_exe`, and
...
pub async fn load_config_as_toml_with_cli_overrides(
    ...
    let merged_toml = config_layer_stack.effective_config();
    let cfg = deserialize_config_toml_with_base(merged_toml, brocode_home).map_err(|e| {
...
pub async fn load_global_mcp_servers(
    ...
    // MCP servers defined in in-repo .brocode/ folders.
...
pub fn set_default_oss_provider(brocode_home: &Path, provider: &str) -> std::io::Result<()> {
...
/// Base config deserialized from ~/.brocode/config.toml.
...
pub struct ConfigToml {
    ...
    /// sanctioned by Brocode will likely degrade model performance.
    ...
    /// directories for `.brocode` folders. Defaults to [".git"] when unset.
    #[serde(default)]
...
pub use brocode_protocol::protocol::RealtimeConversationVersion as RealtimeWsVersion;
                    ...
                    if p.is_trusted() || p.is_untrusted() {
                        if cfg!(target_os = "windows")
                            && windows_sandbox_level
                                == brocode_protocol::config_types::WindowsSandboxLevel::Disabled
                        {
...
impl Config {
    ...
    fn load_from_base_config_with_overrides(
        ...
        Self::load_config_with_layer_stack(cfg, overrides, brocode_home, config_layer_stack)
    }
    ...
    pub(crate) fn load_config_with_layer_stack(
        ...
        let user_instructions = Self::load_instructions(Some(&brocode_home));
        ...
        let memories_root = memory_root(&brocode_home);
        ...
        let sqlite_home = cfg
            ...
            .unwrap_or_else(|| brocode_home.to_path_buf());
        ...
        let helper_readable_roots = get_readable_roots_required_for_brocode_runtime(
            &brocode_home,
            zsh_path.as_ref(),
    ...
    fn load_instructions(brocode_dir: Option<&Path>) -> Option<String> {
        let base = brocode_dir?;
...
/// Returns the path to the folder where Brocode logs are stored. Does not verify
