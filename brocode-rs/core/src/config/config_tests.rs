...
fn load_config_normalizes_relative_cwd_override() -> std::io::Result<()> {
    ...
    let config = Config::load_from_base_config_with_overrides(
        ...
        brocode_home.abs().into_path_buf(),
    )?;
...
fn default_permissions_profile_populates_runtime_sandbox_policy() -> std::io::Result<()> {
    ...
    let memories_root = brocode_home.path().join("memories").abs();
    assert_eq!(
...
fn permissions_profiles_allow_unknown_special_paths() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` is not recognized by this version of Brocode and will be ignored."
        )),
        "{:?}",
...
fn permissions_profiles_allow_unknown_special_paths_with_nested_entries() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Configured filesystem path `:future_special_path` with nested entry `docs` is not recognized by this version of Brocode and will be ignored."
        )),
        "{:?}",
        config.startup_warnings
    );
...
fn permissions_profiles_allow_missing_filesystem_with_warning() -> std::io::Result<()> {
    ...
    assert!(
        config.startup_warnings.iter().any(|warning| warning.contains(
            "Permissions profile `workspace` does not define any recognized filesystem entries for this version of Brocode."
        )),
        "{:?}",
        config.startup_warnings
    );
...
#[test]
fn sqlite_home_defaults_to_brocode_home_for_workspace_write() -> std::io::Result<()> {
    let brocode_home = TempDir::new()?;
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml::default(),
        ConfigOverrides {
    ...
    assert_eq!(config.sqlite_home, brocode_home.path().to_path_buf());
...
fn workspace_write_always_includes_memories_root_once() -> std::io::Result<()> {
    let brocode_home = TempDir::new()?;
    let memories_root = brocode_home.path().join("memories");
    let config = Config::load_from_base_config_with_overrides(
        ConfigToml {
...
async fn project_profile_overrides_user_profile() -> std::io::Result<()> {
    ...
    std::fs::write(
        brocode_home.path().join(CONFIG_TOML_FILE),
        format!(
            r#"
profile = "global"
...
"#,
    ...
    let project_config_dir = workspace.path().join(".brocode");
...
async fn managed_config_overrides_oauth_store_mode() -> anyhow::Result<()> {
    let brocode_home = TempDir::new()?;
    let managed_path = brocode_home.path().join("managed_config.toml");
    let config_path = brocode_home.path().join(CONFIG_TOML_FILE);
...
async fn load_global_mcp_servers_returns_empty_if_missing() -> anyhow::Result<()> {
    ...
    let servers = load_global_mcp_servers(brocode_home.path()).await?;
...
async fn replace_mcp_servers_round_trips_entries() -> anyhow::Result<()> {
    ...
    let loaded = load_global_mcp_servers(brocode_home.path()).await?;
...
"#,
    ...
    let err = load_global_mcp_servers(brocode_home.path())
        .await
...
async fn replace_mcp_servers_serializes_cwd() -> anyhow::Result<()> {
    ...
    let cwd_path = PathBuf::from("/tmp/brocode-mcp");
    let servers = BTreeMap::from([(
        "docs".to_string(),
        McpServerConfig {
            transport: McpServerTransportConfig::Stdio {
    ...
    let serialized = std::fs::read_to_string(&config_path)?;
    assert!(
        serialized.contains(r#"cwd = "/tmp/brocode-mcp""#),
    ...
    match &docs.transport {
        McpServerTransportConfig::Stdio { cwd, .. } => {
            assert_eq!(cwd.as_deref(), Some(Path::new("/tmp/brocode-mcp")));
        }
        other => panic!("unexpected transport {other:?}"),
    }
...
async fn set_model_updates_defaults() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(brocode_home.path())
        .set_model(Some("gpt-5.1-brocode"), Some(ReasoningEffort::High))
        .apply()
        .await?;

    let serialized = tokio::fs::read_to_string(brocode_home.path().join(CONFIG_TOML_FILE)).await?;
    let parsed: ConfigToml = toml::from_str(&serialized)?;

    assert_eq!(parsed.model.as_deref(), Some("gpt-5.1-brocode"));
...
async fn set_model_updates_profile() -> anyhow::Result<()> {
    ...
    ConfigEditsBuilder::new(brocode_home.path())
        ...
        .set_model(Some("gpt-5.1-brocode"), Some(ReasoningEffort::Medium))
        .apply()
        .await?;
    ...
    assert_eq!(profile.model.as_deref(), Some("gpt-5.1-brocode"));
    assert_eq!(
...
impl PrecedenceTestFixture {
    ...
    fn brocode_home(&self) -> PathBuf {
        self.brocode_home.path().to_path_buf()
    }
}
...
fn loads_compact_prompt_from_file() -> std::io::Result<()> {
    let brocode_home = TempDir::new()?;
    let workspace = brocode_home.path().join("workspace");
...
fn load_config_uses_requirements_guardian_developer_instructions() -> std::io::Result<()> {
    ...
    let config = Config::load_config_with_layer_stack(
        ConfigToml::default(),
        ConfigOverrides {
            cwd: Some(brocode_home.path().to_path_buf()),
            ..Default::default()
        },
        brocode_home.path().to_path_buf(),
        config_layer_stack,
...
fn load_config_rejects_missing_agent_role_config_file() -> std::io::Result<()> {
    let brocode_home = TempDir::new()?;
    let missing_path = brocode_home.path().join("agents").join("researcher.toml");
    let cfg = ConfigToml {
...
async fn agent_role_relative_config_file_resolves_against_config_toml() -> std::io::Result<()> {
    let brocode_home = TempDir::new()?;
    let role_config_path = brocode_home.path().join("agents").join("researcher.toml");
...
"#
    ...
    let standalone_agents_dir = repo_root.path().join(".brocode").join("agents");
...
async fn loads_legacy_split_agent_roles_from_config_toml() -> std::io::Result<()> {
    let brocode_home = TempDir::new()?;
    let researcher_path = brocode_home.path().join("agents").join("researcher.toml");
    let reviewer_path = brocode_home.path().join("agents").join("reviewer.toml");
    tokio::fs::create_dir_all(
        researcher_path
            .parent()
            .expect("role config should have a parent directory"),
...
"#
    ...
    let home_agents_dir = brocode_home.path().join("agents");
...
fn model_catalog_json_loads_from_path() -> std::io::Result<()> {
    let brocode_home = TempDir::new()?;
    let catalog_path = brocode_home.path().join("catalog.json");
...
"#;
    ...
    let brocode_home_temp_dir = TempDir::new().unwrap();
    ...
    Ok(PrecedenceTestFixture {
        cwd: cwd_temp_dir,
        brocode_home: brocode_home_temp_dir,
        cfg,
        model_provider_map,
        openai_provider,
...
fn test_precedence_fixture_with_o3_profile() -> std::io::Result<()> {
    ...
    assert_eq!(
        Config {
            ...
            brocode_home: fixture.brocode_home(),
            sqlite_home: fixture.brocode_home(),
            log_dir: fixture.brocode_home().join("log"),
            config_layer_stack: Default::default(),
            startup_warnings: Vec::new(),
            history: History::default(),
            ephemeral: false,
            file_opener: UriBasedFileOpener::VsCode,
            brocode_self_exe: None,
...
projects = { "/Users/mbolin/code/brocode4" = { trust_level = "trusted", foo = "bar" } , "/Users/mbolin/code/brocode3" = { trust_level = "trusted" } }
model = "foo""#;
    let mut doc = initial.parse::<DocumentMut>()?;

    // Approve a new directory
    let new_project = Path::new("/Users/mbolin/code/brocode2");
...
[projects."/Users/mbolin/code/brocode4"]
trust_level = "trusted"
foo = "bar"

[projects."/Users/mbolin/code/brocode3"]
trust_level = "trusted"

[projects."/Users/mbolin/code/brocode2"]
trust_level = "trusted"
"#;
    assert_eq!(contents, expected);
...
fn test_set_default_oss_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(brocode_home, "invalid_provider");
...
fn test_set_default_oss_provider_rejects_legacy_ollama_chat_provider() -> std::io::Result<()> {
    ...
    let result = set_default_oss_provider(brocode_home, LEGACY_OLLAMA_CHAT_PROVIDER_ID);
    assert!(result.is_err());
    let error = result.unwrap_err();
    assert_eq!(error.kind(), std::io::ErrorKind::InvalidInput);
    assert!(
...
async fn feature_requirements_reject_collab_legacy_alias() {
    let brocode_home = TempDir::new().expect("tempdir");
