...
use brocode_utils_output_truncation::approx_token_count;
use core_test_support::get_remote_test_env;
use core_test_support::skip_if_sandbox;
use core_test_support::test_brocode::test_env as remote_test_env;
...
async fn unified_exec_persists_across_requests() -> anyhow::Result<()> {
    ...
    write_stdin(
        &session,
        process_id,
        "export BROCODE_INTERACTIVE_SHELL_VAR=brocode\n",
        2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        &session,
        process_id,
        "echo $BROCODE_INTERACTIVE_SHELL_VAR\n",
        2_500,
    )
    .await?;
    assert!(
        out_2.truncated_output().contains("brocode"),
...
async fn multi_unified_exec_sessions() -> anyhow::Result<()> {
    ...
    .await?;
    assert!(
        out_3.truncated_output().contains("brocode"),
...
async fn unified_exec_timeouts() -> anyhow::Result<()> {
    ...
    write_stdin(
        &session,
        process_id,
        format!("export BROCODE_INTERACTIVE_SHELL_VAR={TEST_VAR_VALUE}\n").as_str(),
        2_500,
    )
    .await?;
    ...
    let out_2 = write_stdin(
        &session,
        process_id,
        "sleep 5 && echo $BROCODE_INTERACTIVE_SHELL_VAR\n",
        10,
    )
    .await?;
    assert!(
...
async fn requests_with_large_timeout_are_capped() -> anyhow::Result<()> {
    ...
    let result = exec_command(&session, &turn, "echo brocode", 120_000, None).await?;

    assert!(result.process_id.is_some());
    assert!(result.truncated_output().contains("brocode"));
...
async fn completed_commands_do_not_persist_sessions() -> anyhow::Result<()> {
    let (session, turn) = test_session_and_turn().await;
    let result = exec_command(&session, &turn, "echo brocode", 2_500, None).await?;
...
async fn completed_pipe_commands_preserve_exit_code() -> anyhow::Result<()> {
    ...
    let environment = brocode_exec_server::Environment::default();
