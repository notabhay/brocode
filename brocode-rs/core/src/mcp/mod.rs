...
pub(crate) const BROCODE_APPS_MCP_SERVER_NAME: &str = "brocode_apps";
const BROCODE_CONNECTORS_TOKEN_ENV_VAR: &str = "BROCODE_CONNECTORS_TOKEN";
...
fn brocode_apps_mcp_bearer_token_env_var() -> Option<String> {
    match env::var(BROCODE_CONNECTORS_TOKEN_ENV_VAR) {
        Ok(value) if !value.trim().is_empty() => Some(BROCODE_CONNECTORS_TOKEN_ENV_VAR.to_string()),
        Ok(_) => None,
        Err(env::VarError::NotPresent) => None,
        Err(env::VarError::NotUnicode(_)) => Some(BROCODE_CONNECTORS_TOKEN_ENV_VAR.to_string()),
    }
}

fn brocode_apps_mcp_bearer_token(auth: Option<&BrocodeAuth>) -> Option<String> {
...
fn brocode_apps_mcp_http_headers(auth: Option<&BrocodeAuth>) -> Option<HashMap<String, String>> {
    let mut headers = HashMap::new();
    if let Some(token) = brocode_apps_mcp_bearer_token(auth) {
        headers.insert("Authorization".to_string(), format!("Bearer {token}"));
    }
    if let Some(account_id) = auth.and_then(BrocodeAuth::get_account_id) {
...
fn normalize_brocode_apps_base_url(base_url: &str) -> String {
...
fn brocode_apps_mcp_url_for_base_url(base_url: &str) -> String {
    let base_url = normalize_brocode_apps_base_url(base_url);
    if base_url.contains("/backend-api") {
        format!("{base_url}/wham/apps")
    } else if base_url.contains("/api/brocode") {
        format!("{base_url}/apps")
    } else {
        format!("{base_url}/api/brocode/apps")
    }
}

pub(crate) fn brocode_apps_mcp_url(config: &Config) -> String {
    brocode_apps_mcp_url_for_base_url(&config.chatgpt_base_url)
}

fn brocode_apps_mcp_server_config(config: &Config, auth: Option<&BrocodeAuth>) -> McpServerConfig {
    let bearer_token_env_var = brocode_apps_mcp_bearer_token_env_var();
    let http_headers = if bearer_token_env_var.is_some() {
        None
    } else {
        brocode_apps_mcp_http_headers(auth)
    };
    let url = brocode_apps_mcp_url(config);
...
pub(crate) fn with_brocode_apps_mcp(
    ...
    if connectors_enabled {
        servers.insert(
            ...
            brocode_apps_mcp_server_config(config, auth),
        );
    } else {
        servers.remove(BROCODE_APPS_MCP_SERVER_NAME);
    }
    servers
}
