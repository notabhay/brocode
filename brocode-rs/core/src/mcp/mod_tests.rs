...
fn brocode_apps_mcp_url_for_base_url_keeps_existing_paths() {
    ...
    assert_eq!(
        brocode_apps_mcp_url_for_base_url("http://localhost:8080/api/brocode"),
        "http://localhost:8080/api/brocode/apps"
    );
    assert_eq!(
...
fn brocode_apps_server_config_uses_legacy_brocode_apps_path() {
    ...
    let mut servers = with_brocode_apps_mcp(
        HashMap::new(),
