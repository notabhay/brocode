...
//!
//! `ModelClient` is intended to live for the lifetime of a Brocode session and holds the stable
...
//! per-turn state such as the `x-brocode-turn-state` token used for sticky routing.
//!
...
use brocode_api::CompactClient as ApiCompactClient;
use brocode_api::CompactionInput as ApiCompactionInput;
use brocode_api::MemoriesClient as ApiMemoriesClient;
use brocode_api::MemorySummarizeInput as ApiMemorySummarizeInput;
use brocode_api::MemorySummarizeOutput as ApiMemorySummarizeOutput;
use brocode_api::RawMemory as ApiRawMemory;
use brocode_api::RequestTelemetry;
use brocode_api::ReqwestTransport;
use brocode_api::ResponseCreateWsRequest;
use brocode_api::ResponsesApiRequest;
use brocode_api::ResponsesClient as ApiResponsesClient;
use brocode_api::ResponsesOptions as ApiResponsesOptions;
use brocode_api::ResponsesWebsocketClient as ApiWebSocketResponsesClient;
use brocode_api::ResponsesWebsocketConnection as ApiWebSocketConnection;
use brocode_api::SseTelemetry;
use brocode_api::TransportError;
use brocode_api::WebsocketTelemetry;
use brocode_api::build_conversation_headers;
use brocode_api::common::Reasoning;
use brocode_api::common::ResponsesWsRequest;
use brocode_api::create_text_param_for_request;
use brocode_api::error::ApiError;
use brocode_api::requests::responses::Compression;
use brocode_api::response_create_client_metadata;
...
use brocode_protocol::config_types::Verbosity as VerbosityConfig;
...
use brocode_tools::create_tools_json_for_responses_api;
...
use crate::flags::BROCODE_RS_SSE_FIXTURE;
...
pub const X_BROCODE_TURN_STATE_HEADER: &str = "x-brocode-turn-state";
pub const X_BROCODE_TURN_METADATA_HEADER: &str = "x-brocode-turn-metadata";
...
struct CurrentClientSetup {
    auth: Option<BrocodeAuth>,
    api_provider: brocode_api::Provider,
...
///
/// This holds configuration and state that should be shared across turns within a Brocode session
...
/// - The `x-brocode-turn-state` sticky-routing token, which must be replayed for all requests within
///   the same turn.
///
/// Create a fresh `ModelClientSession` for each Brocode turn. Reusing it across turns would replay
...
pub struct ModelClientSession {
    ...
    /// on turn start via the `x-brocode-turn-state` response header. Once set, this value
    /// should be sent back to the server in the `x-brocode-turn-state` request header for
...
impl ModelClient {
    ...
    ///
    /// All arguments are expected to be stable for the lifetime of a Brocode session. Per-turn values
    ...
    pub fn new(
        ...
        let brocode_api_key_env_enabled = auth_manager
            .as_ref()
            .is_some_and(|manager| manager.brocode_api_key_env_enabled());
        let auth_env_telemetry = collect_auth_env_telemetry(&provider, brocode_api_key_env_enabled);
        Self {
    ...
    pub(crate) fn force_http_fallback(
        ...
        if activated {
            ...
            session_telemetry.counter(
                "brocode.transport.fallback_to_http",
                /*inc*/ 1,
    ...
    pub async fn compact_conversation_history(
        ...
        let request_telemetry = Self::build_request_telemetry(
            ...
            AuthRequestTelemetryContext::new(
                client_setup.auth.as_ref().map(BrocodeAuth::auth_mode),
    ...
    pub fn responses_websocket_enabled(&self) -> bool {
        if !self.state.provider.supports_websockets
            ...
            || (*BROCODE_RS_SSE_FIXTURE).is_some()
        {
            return false;
        }
    ...
    async fn current_client_setup(&self) -> Result<CurrentClientSetup> {
        ...
        let api_provider = self
            .state
            .provider
            .to_api_provider(auth.as_ref().map(BrocodeAuth::auth_mode))?;
...
impl ModelClientSession {
    ...
    fn responses_request_compression(
        &self,
        auth: Option<&crate::auth::BrocodeAuth>,
    ) -> Compression {
        if self.client.state.enable_request_compression
            && auth.is_some_and(BrocodeAuth::is_chatgpt_auth)
    ...
    async fn stream_responses_api(
        ...
        if let Some(path) = &*BROCODE_RS_SSE_FIXTURE {
            warn!(path, "Streaming from fixture");
            let stream = brocode_api::stream_from_fixture(
                path,
    ...
    /// Permanently disables WebSockets for this Brocode session and resets WebSocket state.
    ///
...
fn build_ws_client_metadata(turn_metadata_header: Option<&str>) -> Option<HashMap<String, String>> {
    ...
    client_metadata.insert(X_BROCODE_TURN_METADATA_HEADER.to_string(), turn_metadata);
    Some(client_metadata)
}

/// Builds the extra headers attached to Responses API requests.
///
/// These headers implement Brocode-specific conventions:
///
/// - `x-brocode-beta-features`: comma-separated beta feature keys enabled for the session.
/// - `x-brocode-turn-state`: sticky routing token captured earlier in the turn.
/// - `x-brocode-turn-metadata`: optional per-turn metadata for observability.
fn build_responses_headers(
    ...
    {
        headers.insert("x-brocode-beta-features", header_value);
    }
    if let Some(turn_state) = turn_state
        && let Some(state) = turn_state.get()
        && let Ok(header_value) = HeaderValue::from_str(state)
    {
        headers.insert(X_BROCODE_TURN_STATE_HEADER, header_value);
    }
    if let Some(header_value) = turn_metadata_header {
        headers.insert(X_BROCODE_TURN_METADATA_HEADER, header_value.clone());
    }
    headers
}
...
/// the mapped `BrocodeErr` is returned to the caller.
        ...
        && recovery.has_next()
    {
        ...
        return match recovery.next().await {
            ...
            Err(RefreshTokenError::Permanent(failed)) => {
                ...
                Err(BrocodeErr::RefreshTokenFailed(failed))
            }
