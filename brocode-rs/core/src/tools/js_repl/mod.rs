...
pub(crate) const JS_REPL_PRAGMA_PREFIX: &str = "// brocode-js-repl:";
...
impl JsReplManager {
    ...
    async fn start_kernel(
        ...
        env.insert(
            "BROCODE_JS_TMP_DIR".to_string(),
            self.tmp_dir.path().to_string_lossy().to_string(),
        );
        let node_module_dirs_key = "BROCODE_JS_REPL_NODE_MODULE_DIRS";
        ...
        let exec_env = sandbox
            .transform(SandboxTransformRequest {
                ...
                brocode_linux_sandbox_exe: turn.brocode_linux_sandbox_exe.as_ref(),
...
pub(crate) async fn resolve_compatible_node(config_path: Option<&Path>) -> Result<PathBuf, String> {
    let node_path = resolve_node(config_path).ok_or_else(|| {
        "Node runtime not found; install Node or set BROCODE_JS_REPL_NODE_PATH".to_string()
    })?;
...
pub(crate) fn resolve_node(config_path: Option<&Path>) -> Option<PathBuf> {
    if let Some(path) = std::env::var_os("BROCODE_JS_REPL_NODE_PATH") {
