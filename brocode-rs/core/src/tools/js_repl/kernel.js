...
const internalBindingSalt = (() => {
  const raw = process.env.BROCODE_THREAD_ID ?? "";
...
const tmpDir = process.env.BROCODE_JS_TMP_DIR || cwd;
const homeDir = process.env.HOME ?? null;
const nodeModuleDirEnv = process.env.BROCODE_JS_REPL_NODE_MODULE_DIRS ?? "";
...
function getRequireForBase(base) {
  ...
  if (!req) {
    req = createRequire(path.join(base, "__brocode_js_repl__.cjs"));
...
function nextInternalBindingName() {
  ...
  return `__brocode_internal_commit_${internalBindingSalt}_${internalBindingCounter++}`;
}
...
async function buildModuleSource(code) {
  ...
  helperDeclarations.push(
    ...
    `const ${markCommittedFnName} = import.meta.__brocodeInternalMarkCommittedBindings;`,
    `const ${markPreludeCompletedFnName} = import.meta.__brocodeInternalMarkPreludeCompleted;`,
    "delete import.meta.__brocodeInternalMarkCommittedBindings;",
    "delete import.meta.__brocodeInternalMarkPreludeCompleted;",
  );
...
function encodeByteImage(bytes, mimeType, detail) {
  if (bytes.byteLength === 0) {
    throw new Error("brocode.emitImage expected non-empty bytes");
  }
  if (typeof mimeType !== "string" || !mimeType) {
    throw new Error("brocode.emitImage expected a non-empty mimeType");
  }
...
function parseImageDetail(detail) {
  ...
  if (typeof detail !== "string" || !detail) {
    throw new Error("brocode.emitImage expected detail to be a non-empty string");
  }
  if (detail !== "original") {
    throw new Error(
      'brocode.emitImage only supports detail "original"; omit detail for default behavior',
    );
  }
  return detail;
}
...
function normalizeEmitImageUrl(value) {
  if (typeof value !== "string" || !value) {
    throw new Error("brocode.emitImage expected a non-empty image_url");
  }
  if (!/^data:/i.test(value)) {
    throw new Error("brocode.emitImage only accepts data URLs");
  }
  return value;
}
...
function parseContentItems(items) {
  ...
  for (const item of items) {
    if (!isPlainObject(item) || typeof item.type !== "string") {
      throw new Error("brocode.emitImage received malformed content items");
    }
    ...
    throw new Error(
      `brocode.emitImage does not support content item type "${item.type}"`,
    );
  }
...
function parseByteImageValue(value) {
  ...
  if (!bytes) {
    throw new Error(
      "brocode.emitImage expected bytes to be Buffer, Uint8Array, ArrayBuffer, or ArrayBufferView",
    );
  }
...
function parseToolOutput(output) {
  ...
  throw new Error("brocode.emitImage received an unsupported tool output shape");
}

function normalizeMcpImageData(data, mimeType) {
  if (typeof data !== "string" || !data) {
    throw new Error("brocode.emitImage expected MCP image data");
  }
  if (/^data:/i.test(data)) {
    return data;
  }
...
function parseMcpToolResult(result) {
  ...
  if (!isPlainObject(result)) {
    throw new Error("brocode.emitImage received an unsupported MCP result");
  }
  ...
  if (!isPlainObject(ok) || !Array.isArray(ok.content)) {
    throw new Error("brocode.emitImage received malformed MCP content");
  }
  ...
  for (const item of ok.content) {
    ...
    throw new Error(
      `brocode.emitImage does not support MCP content type "${item.type}"`,
    );
  }
...
function requireSingleImage(parsed) {
  if (parsed.textCount > 0) {
    throw new Error("brocode.emitImage does not accept mixed text and image content");
  }
  if (parsed.images.length !== 1) {
    throw new Error("brocode.emitImage expected exactly one image");
  }
...
function normalizeEmitImageValue(value) {
  ...
  if (!isPlainObject(value)) {
    throw new Error("brocode.emitImage received an unsupported value");
  }
...
const brocode = {
  ...
  tool(toolName, args) {
    ...
    if (typeof toolName !== "string" || !toolName) {
      return Promise.reject(new Error("brocode.tool expects a tool name string"));
    }
    ...
    await execContextStorage.run(execState, async () => {
      await withCapturedConsole(context, async (logs) => {
        const cellIdentifier = path.join(
          cwd,
          `.brocode_js_repl_cell_${cellCounter++}.mjs`,
        );
        module = new SourceTextModule(source, {
          context,
          ...
          initializeImportMeta(meta, mod) {
            ...
            meta.__brocodeInternalMarkCommittedBindings = markCommittedBindings;
            meta.__brocodeInternalMarkPreludeCompleted = markPreludeCompleted;
          },
          importModuleDynamically(specifier, referrer) {
            return importResolved(resolveSpecifier(specifier, referrer?.identifier));
          },
