...
fn maybe_wrap_shell_lc_with_snapshot_preserves_unset_override_variables() {
    ...
    std::fs::write(
        &snapshot_path,
        "# Snapshot file\nexport BROCODE_TEST_UNSET_OVERRIDE='snapshot-value'\n",
    )
    .expect("write snapshot");
    let session_shell = shell_with_snapshot(
        ShellType::Bash,
    ...
    let command = vec![
            "/bin/bash".to_string(),
            "-lc".to_string(),
            "if [ \"${BROCODE_TEST_UNSET_OVERRIDE+x}\" = x ]; then printf 'set:%s' \"$BROCODE_TEST_UNSET_OVERRIDE\"; else printf 'unset'; fi".to_string(),
        ];
    let explicit_env_overrides = HashMap::from([(
        "BROCODE_TEST_UNSET_OVERRIDE".to_string(),
    ...
    let output = Command::new(&rewritten[0])
        .args(&rewritten[1..])
        .env_remove("BROCODE_TEST_UNSET_OVERRIDE")
        .output()
        .expect("run rewritten command");
    assert!(output.status.success(), "command failed: {output:?}");
