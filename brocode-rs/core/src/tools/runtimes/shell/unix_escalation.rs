...
use brocode_execpolicy::MatchOptions;
...
use brocode_shell_command::bash::parse_shell_lc_plain_commands;
use brocode_shell_command::bash::parse_shell_lc_single_command_prefix;
use brocode_shell_escalation::EscalateServer;
use brocode_shell_escalation::EscalationDecision;
use brocode_shell_escalation::EscalationExecution;
use brocode_shell_escalation::EscalationPermissions;
use brocode_shell_escalation::EscalationPolicy;
use brocode_shell_escalation::EscalationSession;
use brocode_shell_escalation::ExecParams;
use brocode_shell_escalation::ExecResult;
use brocode_shell_escalation::Permissions as EscalatedPermissions;
use brocode_shell_escalation::PreparedExec;
use brocode_shell_escalation::ShellCommandExecutor;
use brocode_shell_escalation::Stopwatch;
...
pub(super) async fn try_run_zsh_fork(
    ...
    let sandbox_exec_request = attempt
        ...
        .map_err(|err| ToolError::Brocode(err.into()))?;
    ...
    let command_executor = CoreShellCommandExecutor {
        ...
        brocode_linux_sandbox_exe: ctx.turn.brocode_linux_sandbox_exe.clone(),
...
struct CoreShellActionProvider {
    ...
    session: Arc<crate::brocode::Session>,
    turn: Arc<crate::brocode::TurnContext>,
    call_id: String,
...
struct CoreShellCommandExecutor {
    ...
    network: Option<brocode_network_proxy::NetworkProxy>,
...
impl ShellCommandExecutor for CoreShellCommandExecutor {
    async fn run(
        ...
        for var in ["BROCODE_ESCALATE_SOCKET", "EXEC_WRAPPER"] {
...
impl CoreShellCommandExecutor {
    ...
    fn prepare_sandboxed_exec(
        ...
        let exec_request = sandbox_manager.transform(SandboxTransformRequest {
            ...
            brocode_linux_sandbox_exe: self.brocode_linux_sandbox_exe.as_ref(),
...
fn map_exec_result(
    ...
    if result.timed_out {
        return Err(ToolError::Brocode(BrocodeErr::Sandbox(
            SandboxErr::Timeout {
