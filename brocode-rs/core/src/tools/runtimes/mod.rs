...
fn build_override_exports(explicit_env_overrides: &HashMap<String, String>) -> (String, String) {
    ...
    let captures = keys
        ...
        .map(|(idx, key)| {
            format!(
                "__BROCODE_SNAPSHOT_OVERRIDE_SET_{idx}=\"${{{key}+x}}\"\n__BROCODE_SNAPSHOT_OVERRIDE_{idx}=\"${{{key}-}}\""
            )
        })
        .collect::<Vec<_>>()
        .join("\n");
    let restores = keys
        ...
        .map(|(idx, key)| {
            format!(
                "if [ -n \"${{__BROCODE_SNAPSHOT_OVERRIDE_SET_{idx}}}\" ]; then export {key}=\"${{__BROCODE_SNAPSHOT_OVERRIDE_{idx}}}\"; else unset {key}; fi"
            )
        })
        .collect::<Vec<_>>()
        .join("\n");
