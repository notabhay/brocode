...
//! `brocode --brocode-run-as-apply-patch`, and runs under the current
...
impl ApplyPatchRuntime {
    ...
    fn build_sandbox_command(
        ...
        Ok(Self::build_sandbox_command_with_program(
            req,
            brocode_windows_sandbox::resolve_current_exe_for_launch(brocode_home, "brocode.exe"),
        ))
    }
    ...
    fn build_sandbox_command(
        req: &ApplyPatchRequest,
        brocode_self_exe: Option<&PathBuf>,
    ) -> Result<SandboxCommand, ToolError> {
        let exe = Self::resolve_apply_patch_program(brocode_self_exe)?;
    ...
    fn resolve_apply_patch_program(
        ...
        if let Some(path) = brocode_self_exe {
            return Ok(path.clone());
        }

        std::env::current_exe()
            .map_err(|e| ToolError::Rejected(format!("failed to determine brocode exe: {e}")))
    }
    ...
    fn build_sandbox_command_with_program(req: &ApplyPatchRequest, exe: PathBuf) -> SandboxCommand {
        SandboxCommand {
            ...
            args: vec![
                BROCODE_CORE_APPLY_PATCH_ARG1.to_string(),
...
impl ToolRuntime<ApplyPatchRequest, ExecToolCallOutput> for ApplyPatchRuntime {
    async fn run(
        ...
        #[cfg(target_os = "windows")]
        let command = Self::build_sandbox_command(req, &ctx.turn.config.brocode_home)?;
        #[cfg(not(target_os = "windows"))]
        let command = Self::build_sandbox_command(req, ctx.turn.brocode_self_exe.as_ref())?;
