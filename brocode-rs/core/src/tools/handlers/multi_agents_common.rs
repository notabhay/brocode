...
pub(crate) fn collab_spawn_error(err: BrocodeErr) -> FunctionCallError {
    match err {
        BrocodeErr::UnsupportedOperation(message) if message == "thread manager dropped" => {
            FunctionCallError::RespondToModel("collab manager unavailable".to_string())
        }
        BrocodeErr::UnsupportedOperation(message) => FunctionCallError::RespondToModel(message),
        err => FunctionCallError::RespondToModel(format!("collab spawn failed: {err}")),
    }
}

pub(crate) fn collab_agent_error(agent_id: ThreadId, err: BrocodeErr) -> FunctionCallError {
    match err {
        BrocodeErr::ThreadNotFound(id) => {
...
pub(crate) fn apply_spawn_agent_runtime_overrides(
    ...
    config.brocode_linux_sandbox_exe = turn.brocode_linux_sandbox_exe.clone();
...
fn find_spawn_agent_model_name(
    available_models: &[brocode_protocol::openai_models::ModelPreset],
    requested_model: &str,
