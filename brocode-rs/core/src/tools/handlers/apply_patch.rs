...
use brocode_apply_patch::ApplyPatchAction;
use brocode_apply_patch::ApplyPatchFileChange;
use brocode_protocol::models::FileSystemPermissions;
use brocode_protocol::models::PermissionProfile;
use brocode_sandboxing::policy_transforms::effective_file_system_sandbox_policy;
use brocode_sandboxing::policy_transforms::merge_permission_profiles;
use brocode_sandboxing::policy_transforms::normalize_additional_permissions;
...
fn write_permissions_for_paths(
    ...
    file_system_sandbox_policy: &brocode_protocol::permissions::FileSystemSandboxPolicy,
    cwd: &Path,
...
async fn effective_patch_permissions(
    ...
    brocode_protocol::permissions::FileSystemSandboxPolicy,
) {
...
impl ToolHandler for ApplyPatchHandler {
    ...
    async fn handle(&self, invocation: ToolInvocation) -> Result<Self::Output, FunctionCallError> {
        ...
        match brocode_apply_patch::maybe_parse_apply_patch_verified(&command, &cwd) {
            brocode_apply_patch::MaybeApplyPatchVerified::Body(changes) => {
            ...
            brocode_apply_patch::MaybeApplyPatchVerified::CorrectnessError(parse_error) => {
                Err(FunctionCallError::RespondToModel(format!(
                    "apply_patch verification failed: {parse_error}"
                )))
            }
            brocode_apply_patch::MaybeApplyPatchVerified::ShellParseError(error) => {
                tracing::trace!("Failed to parse apply_patch input, {error:?}");
                Err(FunctionCallError::RespondToModel(
                    "apply_patch handler received invalid patch input".to_string(),
                ))
            }
            brocode_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => {
                Err(FunctionCallError::RespondToModel(
...
pub(crate) async fn intercept_apply_patch(
    ...
    match brocode_apply_patch::maybe_parse_apply_patch_verified(command, cwd) {
        ...
        brocode_apply_patch::MaybeApplyPatchVerified::NotApplyPatch => Ok(None),
    }
}
