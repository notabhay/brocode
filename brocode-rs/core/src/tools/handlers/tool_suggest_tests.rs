...
#[test]
fn filter_tool_suggest_discoverable_tools_for_brocode_tui_omits_plugins() {
    ...
    assert_eq!(
        filter_tool_suggest_discoverable_tools_for_client(discoverable_tools, Some("brocode-tui"),),
...
async fn verified_plugin_suggestion_completed_requires_installed_plugin() {
    ...
    let curated_root = crate::plugins::curated_plugins_repo_path(brocode_home.path());
    write_openai_curated_marketplace(&curated_root, &["sample"]);
    write_curated_plugin_sha(brocode_home.path());
    write_plugins_feature_config(brocode_home.path());

    let config = load_plugins_config(brocode_home.path()).await;
    let plugins_manager = PluginsManager::new(brocode_home.path().to_path_buf());
    ...
    let refreshed_config = load_plugins_config(brocode_home.path()).await;
    assert!(verified_plugin_suggestion_completed(
        "sample@openai-curated",
        &refreshed_config,
        &plugins_manager,
