...
use brocode_protocol::models::ShellCommandToolCallParams;
...
fn shell_payload_command(payload: &ToolPayload) -> Option<String> {
    match payload {
        ToolPayload::Function { arguments } => parse_arguments::<ShellToolCallParams>(arguments)
            .ok()
            .map(|params| brocode_shell_command::parse_command::shlex_join(&params.command)),
        ToolPayload::LocalShell { params } => Some(
            brocode_shell_command::parse_command::shlex_join(&params.command),
        ),
        _ => None,
    }
}
...
impl ShellHandler {
    async fn run_exec_like(args: RunExecLikeArgs) -> Result<FunctionToolOutput, FunctionCallError> {
        ...
        if effective_additional_permissions
            ...
            && !matches!(
                turn.approval_policy.value(),
                brocode_protocol::protocol::AskForApproval::OnRequest
            )
        {
        ...
        let exec_approval_requirement = session
            ...
            .create_exec_approval_requirement_for_command(ExecApprovalRequest {
                ...
                sandbox_permissions: if effective_additional_permissions.permissions_preapproved {
                    brocode_protocol::models::SandboxPermissions::UseDefault
                } else {
                    effective_additional_permissions.sandbox_permissions
                },
                prefix_rule,
        ...
        Ok(FunctionToolOutput {
            body: vec![
                brocode_protocol::models::FunctionCallOutputContentItem::InputText {
                    text: content,
                },
            ],
            success: Some(true),
