        ...
        | ResponseInputItem::CustomToolCallOutput { output, .. } => {
            let content = match output.body {
                ...
                FunctionCallOutputBody::ContentItems(items) => {
                    brocode_protocol::models::function_call_output_content_items_to_text(&items)
                        .unwrap_or_default()
                }
            };
...
async fn multi_agent_v2_list_agents_returns_completed_status_and_last_task_message() {
    ...
    let child_thread = manager
        ...
        .await
        .expect("child thread should exist");
    let child_turn = child_thread.brocode.session.new_default_turn().await;
    child_thread
        .brocode
        .session
        .send_event(
...
async fn multi_agent_v2_send_message_interrupts_busy_child_without_triggering_turn() {
    ...
    let active_turn = thread.brocode.session.new_default_turn().await;
    thread
        .brocode
        .session
        .spawn_task(
...
async fn resume_agent_restores_closed_agent_and_accepts_send_input() {
    ...
    let thread = manager
        .resume_thread_with_history(
            ...
            AuthManager::from_auth_for_testing(BrocodeAuth::from_api_key("dummy")),
            false,
            None,
        )
        .await
...
async fn multi_agent_v2_wait_agent_does_not_return_completed_content() {
    ...
    let thread = manager.start_thread(config).await.expect("start thread");
    let agent_id = thread.thread_id;
    let child_turn = thread.thread.brocode.session.new_default_turn().await;
    thread
        .thread
        .brocode
        .session
...
async fn tool_handlers_cascade_close_and_resume_and_keep_explicitly_closed_subtrees_closed() {
    ...
    let parent_session = parent.thread.brocode.session.clone();
    ...
    let child_thread = manager
        ...
        .await
        .expect("child thread should exist");
    let child_session = child_thread.brocode.session.clone();
    ...
    let operator_session = operator.thread.brocode.session.clone();
    let _ = manager
        .agent_control()
    ...
    let parent_resume_output = ResumeAgentHandler
        .handle(invocation(
            operator_session,
            operator.thread.brocode.session.new_default_turn().await,
            "resume_agent",
...
async fn build_agent_spawn_config_uses_turn_context_values() {
    ...
    turn.brocode_linux_sandbox_exe = Some(PathBuf::from("/bin/echo"));
    ...
    expected.brocode_linux_sandbox_exe = turn.brocode_linux_sandbox_exe.clone();
