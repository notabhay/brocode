...
use brocode_code_mode::CodeModeTurnHost;
use brocode_code_mode::RuntimeResponse;
...
pub(crate) const PUBLIC_TOOL_NAME: &str = brocode_code_mode::PUBLIC_TOOL_NAME;
pub(crate) const WAIT_TOOL_NAME: &str = brocode_code_mode::WAIT_TOOL_NAME;
pub(crate) const DEFAULT_WAIT_YIELD_TIME_MS: u64 = brocode_code_mode::DEFAULT_WAIT_YIELD_TIME_MS;
...
pub(crate) struct CodeModeService {
    inner: brocode_code_mode::CodeModeService,
}

impl CodeModeService {
    pub(crate) fn new(_js_repl_node_path: Option<PathBuf>) -> Self {
        Self {
            inner: brocode_code_mode::CodeModeService::new(),
        }
    }
    ...
    pub(crate) async fn execute(
        &self,
        request: brocode_code_mode::ExecuteRequest,
    ...
    pub(crate) async fn wait(
        &self,
        request: brocode_code_mode::WaitRequest,
    ) -> Result<RuntimeResponse, String> {
    ...
    ) -> Option<brocode_code_mode::CodeModeTurnWorker> {
...
pub(super) async fn build_enabled_tools(
    exec: &ExecContext,
) -> Vec<brocode_code_mode::ToolDefinition> {
...
fn tool_kind_for_spec(spec: &ToolSpec) -> brocode_code_mode::CodeModeToolKind {
    if matches!(spec, ToolSpec::Freeform(_)) {
        brocode_code_mode::CodeModeToolKind::Freeform
    } else {
        brocode_code_mode::CodeModeToolKind::Function
    }
}
...
fn tool_kind_for_name(
    ...
    tool_name: &str,
) -> Result<brocode_code_mode::CodeModeToolKind, String> {
    spec.as_ref()
