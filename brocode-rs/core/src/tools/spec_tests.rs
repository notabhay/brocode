...
use brocode_protocol::models::VIEW_IMAGE_TOOL_NAME;
...
use brocode_tools::AdditionalProperties;
use brocode_tools::CommandToolOptions;
use brocode_tools::ConfiguredToolSpec;
use brocode_tools::DiscoverablePluginInfo;
use brocode_tools::DiscoverableTool;
use brocode_tools::FreeformTool;
use brocode_tools::ResponsesApiTool;
use brocode_tools::ResponsesApiWebSearchFilters;
use brocode_tools::ResponsesApiWebSearchUserLocation;
use brocode_tools::SpawnAgentToolOptions;
use brocode_tools::ViewImageToolOptions;
use brocode_tools::WaitAgentTimeoutOptions;
use brocode_tools::create_close_agent_tool_v1;
use brocode_tools::create_close_agent_tool_v2;
use brocode_tools::create_exec_command_tool;
use brocode_tools::create_request_permissions_tool;
use brocode_tools::create_request_user_input_tool;
use brocode_tools::create_resume_agent_tool;
use brocode_tools::create_send_input_tool_v1;
use brocode_tools::create_send_message_tool;
use brocode_tools::create_spawn_agent_tool_v1;
use brocode_tools::create_spawn_agent_tool_v2;
use brocode_tools::create_view_image_tool;
use brocode_tools::create_wait_agent_tool_v1;
use brocode_tools::create_wait_agent_tool_v2;
use brocode_tools::create_write_stdin_tool;
use brocode_tools::mcp_tool_to_deferred_responses_api_tool;
...
fn deferred_responses_api_tool_serializes_with_defer_loading() {
    ...
    let serialized = serde_json::to_value(ToolSpec::Function(
        mcp_tool_to_deferred_responses_api_tool(
            "mcp__brocode_apps__lookup_order".to_string(),
            &tool,
        )
    ...
    assert_eq!(
        ...
        serde_json::json!({
            "type": "function",
            "name": "mcp__brocode_apps__lookup_order",
...
fn model_provided_unified_exec_is_blocked_for_windows_sandboxed_policies() {
    let mut model_info = model_info_from_models_json("gpt-5-brocode");
...
#[test]
fn test_full_toolset_specs_for_gpt5_brocode_unified_exec_web_search() {
    let model_info = model_info_from_models_json("gpt-5-brocode");
...
fn web_search_config_is_forwarded_to_tool_spec() {
    ...
    let web_search_config = WebSearchConfig {
        filters: Some(brocode_protocol::config_types::WebSearchFilters {
            allowed_domains: Some(vec!["example.com".to_string()]),
        }),
        user_location: Some(brocode_protocol::config_types::WebSearchUserLocation {
            r#type: brocode_protocol::config_types::WebSearchUserLocationType::Approximate,
            country: Some("US".to_string()),
            region: Some("California".to_string()),
            city: Some("San Francisco".to_string()),
            timezone: Some("America/Los_Angeles".to_string()),
        }),
        search_context_size: Some(brocode_protocol::config_types::WebSearchContextSize::High),
    };
...
#[test]
fn test_build_specs_gpt5_brocode_default() {
    let features = Features::with_defaults();
...
#[test]
fn test_build_specs_gpt51_brocode_default() {
    let features = Features::with_defaults();
    assert_default_model_tools(
        "gpt-5.1-brocode",
        &features,
...
#[test]
fn test_build_specs_gpt5_brocode_unified_exec_web_search() {
    let mut features = Features::with_defaults();
    features.enable(Feature::UnifiedExec);
    assert_model_tools(
        "gpt-5-brocode",
...
#[test]
fn test_build_specs_gpt51_brocode_unified_exec_web_search() {
    let mut features = Features::with_defaults();
    features.enable(Feature::UnifiedExec);
    assert_model_tools(
        "gpt-5.1-brocode",
...
#[test]
fn test_gpt_5_1_brocode_max_defaults() {
    let features = Features::with_defaults();
    assert_default_model_tools(
        "gpt-5.1-brocode-max",
        &features,
...
#[test]
fn test_brocode_5_1_mini_defaults() {
    let features = Features::with_defaults();
    assert_default_model_tools(
...
#[test]
fn test_gpt_5_1_brocode_max_unified_exec_web_search() {
    let mut features = Features::with_defaults();
    features.enable(Feature::UnifiedExec);
    assert_model_tools(
        "gpt-5.1-brocode-max",
    ...
    assert_eq!(
        tools_config
            .with_unified_exec_shell_mode_for_session(
                ...
                Some(&PathBuf::from(if cfg!(windows) {
                    r"C:\opt\brocode\brocode-execve-wrapper"
                } else {
                    "/opt/brocode/brocode-execve-wrapper"
                })),
            )
            .unified_exec_shell_mode,
        if cfg!(unix) {
            UnifiedExecShellMode::ZshFork(ZshForkConfig {
                shell_zsh_path: AbsolutePathBuf::from_absolute_path("/opt/brocode/zsh").unwrap(),
...
#[test]
fn search_tool_description_lists_each_brocode_apps_connector_once() {
    ...
    let (tools, _) = build_specs(
        ...
        Some(HashMap::from([
            (
                "mcp__brocode_apps__calendar_create_event".to_string(),
                ToolInfo {
                    server_name: crate::mcp::BROCODE_APPS_MCP_SERVER_NAME.to_string(),
                    tool_name: "_create_event".to_string(),
                    tool_namespace: "mcp__brocode_apps__calendar".to_string(),
                    tool: mcp_tool(
            ...
            (
                "mcp__brocode_apps__calendar_list_events".to_string(),
                ToolInfo {
                    server_name: crate::mcp::BROCODE_APPS_MCP_SERVER_NAME.to_string(),
            ...
            (
                "mcp__brocode_apps__gmail_search_threads".to_string(),
                ToolInfo {
                    server_name: crate::mcp::BROCODE_APPS_MCP_SERVER_NAME.to_string(),
                    tool_name: "_search_threads".to_string(),
                    tool_namespace: "mcp__brocode_apps__gmail".to_string(),
                    tool: mcp_tool(
...
fn search_tool_registers_namespaced_app_tool_aliases() {
    ...
    let alias = tool_handler_key("_create_event", Some("mcp__brocode_apps__calendar"));
