use base64::Engine;
use brocode_api::AuthProvider as ApiAuthProvider;
use brocode_api::TransportError;
use brocode_api::error::ApiError;
use brocode_api::rate_limits::parse_promo_message;
use brocode_api::rate_limits::parse_rate_limit_for_limit;
...
pub(crate) fn map_api_error(err: ApiError) -> BrocodeErr {
    match err {
        ApiError::ContextWindowExceeded => BrocodeErr::ContextWindowExceeded,
        ApiError::QuotaExceeded => BrocodeErr::QuotaExceeded,
        ApiError::UsageNotIncluded => BrocodeErr::UsageNotIncluded,
        ApiError::Retryable { message, delay } => BrocodeErr::Stream(message, delay),
        ApiError::Stream(msg) => BrocodeErr::Stream(msg, None),
        ApiError::ServerOverloaded => BrocodeErr::ServerOverloaded,
        ApiError::Api { status, message } => {
            BrocodeErr::UnexpectedStatus(UnexpectedResponseError {
                status,
                body: message,
                url: None,
                cf_ray: None,
        ...
        ApiError::InvalidRequest { message } => BrocodeErr::InvalidRequest(message),
        ApiError::Transport(transport) => match transport {
            TransportError::Http {
                ...
                {
                    return BrocodeErr::ServerOverloaded;
                }
                ...
                if status == http::StatusCode::BAD_REQUEST {
                        ...
                        .contains("The image data you provided does not represent a valid image")
                    {
                        ...
                        BrocodeErr::InvalidRequest(body_text)
                    }
                    ...
                    if let Ok(err) = serde_json::from_str::<UsageErrorResponse>(&body_text) {
                        if err.error.error_type.as_deref() == Some("usage_limit_reached") {
                            ...
                            return BrocodeErr::UsageLimitReached(UsageLimitReachedError {
                            ...
                            return BrocodeErr::UsageNotIncluded;
                        }
                    }

                    BrocodeErr::RetryLimit(RetryLimitReachedError {
                        status,
            ...
            TransportError::RetryLimit => BrocodeErr::RetryLimit(RetryLimitReachedError {
                status: http::StatusCode::INTERNAL_SERVER_ERROR,
                request_id: None,
            }),
            TransportError::Timeout => BrocodeErr::Timeout,
            TransportError::Network(msg) | TransportError::Build(msg) => {
                BrocodeErr::Stream(msg, None)
            }
        },
        ApiError::RateLimit(msg) => BrocodeErr::Stream(msg, None),
    }
}

const ACTIVE_LIMIT_HEADER: &str = "x-brocode-active-limit";
