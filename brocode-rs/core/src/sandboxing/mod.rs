...
command transformation live in the brocode-sandboxing crate; this module keeps
...
use crate::spawn::BROCODE_SANDBOX_NETWORK_DISABLED_ENV_VAR;
use brocode_network_proxy::NetworkProxy;
use brocode_protocol::config_types::WindowsSandboxLevel;
pub use brocode_protocol::models::SandboxPermissions;
...
use brocode_sandboxing::SandboxExecRequest;
...
impl ExecRequest {
    ...
    pub(crate) fn from_sandbox_exec_request(
        ...
        if !network_sandbox_policy.is_enabled() {
            env.insert(
                BROCODE_SANDBOX_NETWORK_DISABLED_ENV_VAR.to_string(),
                "1".to_string(),
            );
        }
        #[cfg(target_os = "macos")]
