...
//!
//! The [`McpConnectionManager`] owns one [`brocode_rmcp_client::RmcpClient`] per
...
use brocode_config::Constrained;
use brocode_protocol::approvals::ElicitationRequest;
...
use brocode_protocol::protocol::McpStartupFailure;
...
use brocode_rmcp_client::RmcpClient;
use brocode_rmcp_client::SendElicitation;
...
use crate::brocode::INITIAL_SUBMIT_ID;
...
const BROCODE_APPS_TOOLS_CACHE_SCHEMA_VERSION: u8 = 1;
const BROCODE_APPS_TOOLS_CACHE_DIR: &str = "cache/brocode_apps_tools";
const MCP_TOOLS_LIST_DURATION_METRIC: &str = "brocode.mcp.tools.list.duration_ms";
const MCP_TOOLS_FETCH_UNCACHED_DURATION_METRIC: &str =
    "brocode.mcp.tools.fetch_uncached.duration_ms";
const MCP_TOOLS_CACHE_WRITE_DURATION_METRIC: &str = "brocode.mcp.tools.cache_write.duration_ms";
...
pub(crate) fn brocode_apps_tools_cache_key(
    auth: Option<&crate::BrocodeAuth>,
) -> BrocodeAppsToolsCacheKey {
    ...
    I: IntoIterator<Item = ToolInfo>,
{
    ...
    for tool in tools {
        let qualified_name_raw = if tool.server_name != BROCODE_APPS_MCP_SERVER_NAME {
            format!(
                "mcp{}{}{}{}",
...
pub(crate) struct BrocodeAppsToolsCacheKey {
...
#[derive(Clone)]
struct BrocodeAppsToolsCacheContext {
    brocode_home: PathBuf,
    user_key: BrocodeAppsToolsCacheKey,
}

impl BrocodeAppsToolsCacheContext {
    fn cache_path(&self) -> PathBuf {
        let user_key_json = serde_json::to_string(&self.user_key).unwrap_or_default();
        let user_key_hash = sha1_hex(&user_key_json);
        self.brocode_home
            .join(BROCODE_APPS_TOOLS_CACHE_DIR)
            .join(format!("{user_key_hash}.json"))
    }
}

#[derive(Debug, Clone, Serialize, Deserialize)]
struct BrocodeAppsToolsDiskCache {
    schema_version: u8,
    tools: Vec<ToolInfo>,
}

enum CachedBrocodeAppsToolsLoad {
    Hit(Vec<ToolInfo>),
    Missing,
    Invalid,
}
...
struct ManagedClient {
    ...
    brocode_apps_tools_cache_context: Option<BrocodeAppsToolsCacheContext>,
}

impl ManagedClient {
    fn listed_tools(&self) -> Vec<ToolInfo> {
        let total_start = Instant::now();
        if let Some(cache_context) = self.brocode_apps_tools_cache_context.as_ref()
            && let CachedBrocodeAppsToolsLoad::Hit(tools) =
                load_cached_brocode_apps_tools(cache_context)
        {
            emit_duration(
        ...
        if self.brocode_apps_tools_cache_context.is_some() {
            emit_duration(
                MCP_TOOLS_LIST_DURATION_METRIC,
                total_start.elapsed(),
                &[("cache", "miss")],
...
impl AsyncManagedClient {
    ...
    fn new(
        ...
        let startup_snapshot = load_startup_cached_brocode_apps_tools_snapshot(
            &server_name,
            brocode_apps_tools_cache_context.as_ref(),
        )
            ...
            let outcome = async {
                ...
                match start_server_task(
                    ...
                    StartServerTaskParams {
                        ...
                        brocode_apps_tools_cache_context,
                    },
                )
...
pub const MCP_SANDBOX_STATE_CAPABILITY: &str = "brocode/sandbox-state";

/// Custom MCP request to push sandbox state updates.
/// When used, the `params` field of the notification is [`SandboxState`].
pub const MCP_SANDBOX_STATE_METHOD: &str = "brocode/sandbox-state/update";

#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(rename_all = "camelCase")]
pub struct SandboxState {
    pub sandbox_policy: SandboxPolicy,
    pub brocode_linux_sandbox_exe: Option<PathBuf>,
...
impl McpConnectionManager {
    ...
    pub async fn new(
        ...
        brocode_home: PathBuf,
        brocode_apps_tools_cache_key: BrocodeAppsToolsCacheKey,
        ...
        for (server_name, cfg) in mcp_servers.into_iter().filter(|(_, cfg)| cfg.enabled) {
            ...
            .await;
            let brocode_apps_tools_cache_context = if server_name == BROCODE_APPS_MCP_SERVER_NAME {
                Some(BrocodeAppsToolsCacheContext {
                    brocode_home: brocode_home.clone(),
                    user_key: brocode_apps_tools_cache_key.clone(),
                })
            } else {
                None
            };
    ...
    /// Force-refresh brocode apps tools by bypassing the in-process cache.
    ///
    ...
    pub async fn hard_refresh_brocode_apps_tools_cache(&self) -> Result<HashMap<String, ToolInfo>> {
        let managed_client = self
            .clients
            .get(BROCODE_APPS_MCP_SERVER_NAME)
            .ok_or_else(|| anyhow!("unknown MCP server '{BROCODE_APPS_MCP_SERVER_NAME}'"))?
            .client()
            .await
            .context("failed to get client")?;
        ...
        let tools = list_tools_for_client_uncached(
            BROCODE_APPS_MCP_SERVER_NAME,
            &managed_client.client,
            managed_client.tool_timeout,
        )
        .await
        .with_context(|| {
            format!("failed to refresh tools for MCP server '{BROCODE_APPS_MCP_SERVER_NAME}'")
        })?;
        emit_duration(
        ...
        write_cached_brocode_apps_tools_if_needed(
            BROCODE_APPS_MCP_SERVER_NAME,
            managed_client.brocode_apps_tools_cache_context.as_ref(),
            &tools,
        );
        emit_duration(
            MCP_TOOLS_LIST_DURATION_METRIC,
...
pub(crate) fn filter_non_brocode_apps_mcp_tools_only(
    mcp_tools: &HashMap<String, ToolInfo>,
) -> HashMap<String, ToolInfo> {
    mcp_tools
        .iter()
        .filter(|(_, tool)| tool.server_name != BROCODE_APPS_MCP_SERVER_NAME)
        .map(|(name, tool)| (name.clone(), tool.clone()))
        .collect()
}

fn normalize_brocode_apps_tool_title(
    server_name: &str,
    connector_name: Option<&str>,
    value: &str,
) -> String {
    if server_name != BROCODE_APPS_MCP_SERVER_NAME {
...
fn normalize_brocode_apps_tool_name(
    server_name: &str,
    tool_name: &str,
...
fn normalize_brocode_apps_namespace(server_name: &str, connector_name: Option<&str>) -> String {
    if server_name != BROCODE_APPS_MCP_SERVER_NAME {
...
async fn start_server_task(
    ...
    let params = InitializeRequestParams {
        ...
        client_info: Implementation {
            name: "brocode-mcp-client".to_owned(),
            version: env!("CARGO_PKG_VERSION").to_owned(),
            title: Some("Brocode".into()),
            description: None,
            icons: None,
            website_url: None,
        },
...
fn write_cached_brocode_apps_tools_if_needed(
    server_name: &str,
    cache_context: Option<&BrocodeAppsToolsCacheContext>,
    tools: &[ToolInfo],
) {
    if server_name != BROCODE_APPS_MCP_SERVER_NAME {
        return;
    ...
    if let Some(cache_context) = cache_context {
        ...
        write_cached_brocode_apps_tools(cache_context, tools);
        emit_duration(
...
fn load_startup_cached_brocode_apps_tools_snapshot(
    server_name: &str,
    cache_context: Option<&BrocodeAppsToolsCacheContext>,
) -> Option<Vec<ToolInfo>> {
    if server_name != BROCODE_APPS_MCP_SERVER_NAME {
    ...
    match load_cached_brocode_apps_tools(cache_context) {
        CachedBrocodeAppsToolsLoad::Hit(tools) => Some(tools),
        CachedBrocodeAppsToolsLoad::Missing | CachedBrocodeAppsToolsLoad::Invalid => None,
    }
}

#[cfg(test)]
fn read_cached_brocode_apps_tools(
    cache_context: &BrocodeAppsToolsCacheContext,
) -> Option<Vec<ToolInfo>> {
    match load_cached_brocode_apps_tools(cache_context) {
        CachedBrocodeAppsToolsLoad::Hit(tools) => Some(tools),
        CachedBrocodeAppsToolsLoad::Missing | CachedBrocodeAppsToolsLoad::Invalid => None,
...
fn load_cached_brocode_apps_tools(
    cache_context: &BrocodeAppsToolsCacheContext,
) -> CachedBrocodeAppsToolsLoad {
    let cache_path = cache_context.cache_path();
    let bytes = match std::fs::read(cache_path) {
        Ok(bytes) => bytes,
        Err(err) if err.kind() == std::io::ErrorKind::NotFound => {
            return CachedBrocodeAppsToolsLoad::Missing;
        }
        Err(_) => return CachedBrocodeAppsToolsLoad::Invalid,
    };
    let cache: BrocodeAppsToolsDiskCache = match serde_json::from_slice(&bytes) {
        Ok(cache) => cache,
        Err(_) => return CachedBrocodeAppsToolsLoad::Invalid,
    };
    if cache.schema_version != BROCODE_APPS_TOOLS_CACHE_SCHEMA_VERSION {
        return CachedBrocodeAppsToolsLoad::Invalid;
    }
    CachedBrocodeAppsToolsLoad::Hit(filter_disallowed_brocode_apps_tools(cache.tools))
}

fn write_cached_brocode_apps_tools(
    cache_context: &BrocodeAppsToolsCacheContext,
    tools: &[ToolInfo],
) {
    let cache_path = cache_context.cache_path();
    ...
    let tools = filter_disallowed_brocode_apps_tools(tools.to_vec());
    let Ok(bytes) = serde_json::to_vec_pretty(&BrocodeAppsToolsDiskCache {
        schema_version: BROCODE_APPS_TOOLS_CACHE_SCHEMA_VERSION,
        tools,
    }) else {
        return;
    };
...
fn filter_disallowed_brocode_apps_tools(tools: Vec<ToolInfo>) -> Vec<ToolInfo> {
    tools
        .into_iter()
        .filter(|tool| {
            tool.connector_id
...
fn emit_duration(metric: &str, duration: Duration, tags: &[(&str, &str)]) {
    if let Some(metrics) = brocode_otel::metrics::global() {
...
async fn list_tools_for_client_uncached(
    ...
    let tools = resp
        ...
        .into_iter()
        .map(|tool| {
            let tool_name = normalize_brocode_apps_tool_name(
                server_name,
                &tool.tool.name,
            ...
            let tool_namespace =
                normalize_brocode_apps_namespace(server_name, tool.connector_name.as_deref());
            let connector_name = tool.connector_name;
            let connector_description = tool.connector_description;
            let mut tool_def = tool.tool;
            if let Some(title) = tool_def.title.as_deref() {
                let normalized_title = normalize_brocode_apps_tool_title(
                    server_name,
        ...
        .collect();
    if server_name == BROCODE_APPS_MCP_SERVER_NAME {
        return Ok(filter_disallowed_brocode_apps_tools(tools));
    }
    Ok(tools)
}
...
fn mcp_init_error_display(
    ...
    {
        format!(
            "GitHub MCP does not support OAuth. Log in by adding a personal access token (https://github.com/settings/personal-access-tokens) to your environment and config.toml:\n[mcp_servers.{server_name}]\nbearer_token_env_var = BROCODE_GITHUB_PERSONAL_ACCESS_TOKEN"
        )
    } else if is_mcp_client_auth_required_error(err) {
        format!(
            "The {server_name} MCP server is not logged in. Run `brocode mcp login {server_name}`."
        )
