...
const BROCODE_ARC_MONITOR_ENDPOINT_OVERRIDE: &str = "BROCODE_ARC_MONITOR_ENDPOINT_OVERRIDE";
const BROCODE_ARC_MONITOR_TOKEN: &str = "BROCODE_ARC_MONITOR_TOKEN";
...
struct ArcMonitorMetadata {
    brocode_thread_id: String,
    brocode_turn_id: String,
    #[serde(skip_serializing_if = "Option::is_none")]
...
pub(crate) async fn monitor_action(
    ...
    let token = if let Some(token) = read_non_empty_env_var(BROCODE_ARC_MONITOR_TOKEN) {
        token
    } else {
    ...
    let url = read_non_empty_env_var(BROCODE_ARC_MONITOR_ENDPOINT_OVERRIDE).unwrap_or_else(|| {
        format!(
            "{}/brocode/safety/arc",
    ...
    if let Some(account_id) = auth
        .as_ref()
        .and_then(crate::auth::BrocodeAuth::get_account_id)
    {
...
async fn build_arc_monitor_request(
    ...
    ArcMonitorRequest {
        metadata: ArcMonitorMetadata {
            brocode_thread_id: conversation_id.clone(),
            brocode_turn_id: turn_context.sub_id.clone(),
