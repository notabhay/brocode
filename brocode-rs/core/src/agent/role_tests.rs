...
async fn apply_explorer_role_sets_model_and_adds_session_flags_layer() {
    ...
    assert_eq!(config.model.as_deref(), Some("gpt-5.1-brocode-mini"));
...
async fn apply_role_preserves_unspecified_keys() {
    ...
    .await;
    config.brocode_linux_sandbox_exe = Some(PathBuf::from("/tmp/brocode-linux-sandbox"));
    config.main_execve_wrapper_exe = Some(PathBuf::from("/tmp/brocode-execve-wrapper"));
    let role_path = write_role_config(
        &home,
        "effort-only.toml",
    ...
    assert_eq!(
        config.brocode_linux_sandbox_exe,
        Some(PathBuf::from("/tmp/brocode-linux-sandbox"))
    );
    assert_eq!(
        config.main_execve_wrapper_exe,
        Some(PathBuf::from("/tmp/brocode-execve-wrapper"))
    );
}
