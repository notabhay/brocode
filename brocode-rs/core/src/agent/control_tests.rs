...
impl AgentControlHarness {
    ...
    async fn start_thread(&self) -> (ThreadId, Arc<BrocodeThread>) {
        let new_thread = self
            .manager
...
async fn wait_for_subagent_notification(parent_thread: &Arc<BrocodeThread>) -> bool {
    let wait = async {
        loop {
...
async fn persist_thread_for_tree_resume(thread: &Arc<BrocodeThread>, message: &str) {
    thread
...
async fn send_input_errors_when_thread_missing() {
    ...
    assert_matches!(err, BrocodeErr::ThreadNotFound(id) if id == thread_id);
}
...
async fn send_inter_agent_communication_without_turn_queues_message_without_triggering_turn() {
    ...
    timeout(Duration::from_secs(5), async {
        loop {
            if thread.brocode.session.has_pending_input().await {
                break;
            }
...
async fn spawn_agent_can_fork_parent_thread_history() {
    ...
    parent_thread
        ...
        .await;
    let turn_context = parent_thread.brocode.session.new_default_turn().await;
    ...
    let history = child_thread.brocode.session.clone_history().await;
...
async fn spawn_agent_respects_max_threads_limit() {
    ...
    let BrocodeErr::AgentLimitReached {
        max_threads: seen_max_threads,
    } = err
    else {
        panic!("expected BrocodeErr::AgentLimitReached");
    };
...
async fn spawn_agent_limit_shared_across_clones() {
    ...
    let BrocodeErr::AgentLimitReached { max_threads } = err else {
        panic!("expected BrocodeErr::AgentLimitReached");
    };
...
async fn multi_agent_v2_completion_ignores_dead_direct_parent() {
    ...
    let tester_turn = tester_thread.brocode.session.new_default_turn().await;
    tester_thread
        .brocode
        .session
        .send_event(
