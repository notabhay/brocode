...
use brocode_sandboxing::landlock::allow_network_for_proxy;
...
/// (brocode-linux-sandbox), which defaults to bubblewrap for filesystem
    ...
    P: AsRef<Path>,
{
    ...
    let brocode_linux_sandbox_exe = brocode_linux_sandbox_exe.as_ref();
    ...
    let arg0 = if brocode_linux_sandbox_exe
        .file_name()
        ...
        == Some(BROCODE_LINUX_SANDBOX_ARG0)
    {
        ...
        brocode_linux_sandbox_exe.to_string_lossy().into_owned()
    } else {
        BROCODE_LINUX_SANDBOX_ARG0.to_string()
    };
    spawn_child_async(SpawnChildRequest {
        program: brocode_linux_sandbox_exe.to_path_buf(),
        args,
        arg0: Some(&arg0),
        cwd: command_cwd,
        network_sandbox_policy,
