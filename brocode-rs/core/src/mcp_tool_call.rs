...
use brocode_app_server_protocol::McpElicitationObjectType;
use brocode_app_server_protocol::McpElicitationSchema;
...
use brocode_otel::sanitize_metric_tag_value;
...
const MCP_CALL_COUNT_METRIC: &str = "brocode.mcp.call";
const MCP_CALL_DURATION_METRIC: &str = "brocode.mcp.call.duration_ms";
...
pub(crate) async fn handle_mcp_tool_call(
    ...
    let app_tool_policy = if server == BROCODE_APPS_MCP_SERVER_NAME {
    ...
    let approval_mode = if server == BROCODE_APPS_MCP_SERVER_NAME {
        app_tool_policy.approval
    } else {
        custom_mcp_tool_approval_mode(turn_context.as_ref(), &server, &tool_name)
    };

    if server == BROCODE_APPS_MCP_SERVER_NAME && !app_tool_policy.enabled {
    ...
    .await;
    maybe_track_brocode_app_used(sess.as_ref(), turn_context.as_ref(), &server, &tool_name).await;
...
async fn maybe_track_brocode_app_used(
    sess: &Session,
    turn_context: &TurnContext,
    server: &str,
    tool_name: &str,
) {
    if server != BROCODE_APPS_MCP_SERVER_NAME {
        return;
    }
...
pub(crate) struct McpToolApprovalMetadata {
    ...
    brocode_apps_meta: Option<serde_json::Map<String, serde_json::Value>>,
}

const MCP_TOOL_BROCODE_APPS_META_KEY: &str = "_brocode_apps";
...
fn build_mcp_tool_call_request_meta(
    ...
    if let Some(turn_metadata) = turn_context.turn_metadata_state.current_meta_value() {
        request_meta.insert(
            crate::X_BROCODE_TURN_METADATA_HEADER.to_string(),
            turn_metadata,
        );
    }

    if server == BROCODE_APPS_MCP_SERVER_NAME
        && let Some(brocode_apps_meta) =
            metadata.and_then(|metadata| metadata.brocode_apps_meta.clone())
    {
        request_meta.insert(
            MCP_TOOL_BROCODE_APPS_META_KEY.to_string(),
            serde_json::Value::Object(brocode_apps_meta),
        );
    }
...
pub(crate) const MCP_TOOL_APPROVAL_DECLINE_SYNTHETIC: &str = "__brocode_mcp_decline__";
const MCP_TOOL_APPROVAL_ACCEPT_AND_REMEMBER: &str = "Allow and don't ask me again";
const MCP_TOOL_APPROVAL_CANCEL: &str = "Cancel";
const MCP_TOOL_APPROVAL_KIND_KEY: &str = "brocode_approval_kind";
...
fn session_mcp_tool_approval_key(
    ...
    if invocation.server == BROCODE_APPS_MCP_SERVER_NAME && connector_id.is_none() {
        return None;
    }
...
pub(crate) async fn lookup_mcp_tool_metadata(
    ...
    let connector_description = if server == BROCODE_APPS_MCP_SERVER_NAME {
    ...
    Some(McpToolApprovalMetadata {
        ...
        brocode_apps_meta: tool_info
            .tool
            .meta
            .as_ref()
            .and_then(|meta| meta.get(MCP_TOOL_BROCODE_APPS_META_KEY))
...
async fn maybe_persist_mcp_tool_approval(
    ...
    let persist_result = if key.server == BROCODE_APPS_MCP_SERVER_NAME {
        let Some(connector_id) = key.connector_id.clone() else {
            remember_mcp_tool_approval(sess, key).await;
            return;
        };
        persist_brocode_app_tool_approval(
            &turn_context.config.brocode_home,
            &connector_id,
            &tool_name,
        )
        .await
...
async fn persist_brocode_app_tool_approval(
    brocode_home: &Path,
    connector_id: &str,
    tool_name: &str,
) -> anyhow::Result<()> {
...
async fn persist_custom_mcp_tool_approval(
        ...
        project_mcp_tool_approval_config_folder(config, server)
    {
        ...
        let servers = load_global_mcp_servers(&config.brocode_home).await?;
