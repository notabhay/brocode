...
use brocode_protocol::permissions::FileSystemSandboxKind;
use brocode_protocol::permissions::FileSystemSandboxPolicy;
use brocode_protocol::permissions::NetworkSandboxPolicy;
use brocode_sandboxing::SandboxCommand;
use brocode_sandboxing::SandboxManager;
use brocode_sandboxing::SandboxTransformRequest;
use brocode_sandboxing::SandboxType;
use brocode_sandboxing::SandboxablePreference;
use brocode_utils_absolute_path::AbsolutePathBuf;
use brocode_utils_pty::DEFAULT_OUTPUT_BYTES_CAP;
use brocode_utils_pty::process_group::kill_child_process_group;
...
pub struct ExecParams {
    ...
    pub windows_sandbox_level: brocode_protocol::config_types::WindowsSandboxLevel,
...
fn select_process_exec_tool_sandbox_type(
    ...
    windows_sandbox_level: brocode_protocol::config_types::WindowsSandboxLevel,
...
pub async fn process_exec_tool_call(
    ...
    network_sandbox_policy: NetworkSandboxPolicy,
    sandbox_cwd: &Path,
    brocode_linux_sandbox_exe: &Option<PathBuf>,
...
pub fn build_exec_request(
    ...
    let mut exec_req = manager
        .transform(SandboxTransformRequest {
            ...
            brocode_linux_sandbox_exe: brocode_linux_sandbox_exe.as_ref(),
            use_legacy_landlock,
            windows_sandbox_level,
            windows_sandbox_private_desktop,
        })
        .map(|request| ExecRequest::from_sandbox_exec_request(request, options))
        .map_err(BrocodeErr::from)?;
...
fn record_windows_sandbox_spawn_failure(
    ...
    let level = if matches!(
        windows_sandbox_level,
        brocode_protocol::config_types::WindowsSandboxLevel::Elevated
    ) {
        "elevated"
    } else {
        "legacy"
    ...
    if let Some(metrics) = brocode_otel::metrics::global() {
        let _ = metrics.counter(
            "brocode.windows_sandbox.createprocessasuserw_failed",
            /*inc*/ 1,
            &[
...
async fn exec_windows_sandbox(
    ...
    use crate::config::find_brocode_home;
    use brocode_windows_sandbox::run_windows_sandbox_capture_elevated;
    use brocode_windows_sandbox::run_windows_sandbox_capture_with_extra_deny_write_paths;
    ...
    let policy_str = serde_json::to_string(sandbox_policy).map_err(|err| {
        BrocodeErr::Io(io::Error::other(format!(
            "failed to serialize Windows sandbox policy: {err}"
        )))
    })?;
    let sandbox_cwd = cwd.clone();
    let brocode_home = find_brocode_home().map_err(|err| {
        BrocodeErr::Io(io::Error::other(format!(
            "windows sandbox: failed to resolve brocode_home: {err}"
        )))
    })?;
        ...
        if use_elevated {
            run_windows_sandbox_capture_elevated(
                brocode_windows_sandbox::ElevatedSandboxCaptureRequest {
                    ...
                    sandbox_policy_cwd: &sandbox_cwd,
                    brocode_home: brocode_home.as_ref(),
                    command,
                    cwd: &cwd,
                    env_map: env,
                    timeout_ms,
    ...
    let capture = match spawn_res {
        ...
        Ok(Err(err)) => {
            ...
            return Err(BrocodeErr::Io(io::Error::other(format!(
                "windows sandbox: {err}"
            ))));
        }
        Err(join_err) => {
...
fn finalize_exec_result(
    raw_output_result: std::result::Result<RawExecToolCallOutput, BrocodeErr>,
    ...
    match raw_output_result {
        Ok(raw_output) => {
            ...
            #[cfg(target_family = "unix")]
            {
                if let Some(signal) = raw_output.exit_status.signal() {
                    if signal == TIMEOUT_CODE {
                        ...
                        return Err(BrocodeErr::Sandbox(SandboxErr::Signal(signal)));
                    }
                }
            }
            ...
            if timed_out {
                return Err(BrocodeErr::Sandbox(SandboxErr::Timeout {
                    output: Box::new(exec_output),
                }));
            }

            if is_likely_sandbox_denied(sandbox_type, &exec_output) {
                return Err(BrocodeErr::Sandbox(SandboxErr::Denied {
                    output: Box::new(exec_output),
...
pub(crate) mod errors {
    use super::BrocodeErr;
    use brocode_sandboxing::SandboxTransformError;

    impl From<SandboxTransformError> for BrocodeErr {
        fn from(err: SandboxTransformError) -> Self {
            match err {
                ...
                SandboxTransformError::SeatbeltUnavailable => BrocodeErr::UnsupportedOperation(
