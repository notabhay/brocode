...
fn build_stage_one_input_message_truncates_rollout_using_model_context_window() {
    ...
    let mut model_info = model_info_from_slug("gpt-5.2-brocode");
...
async fn build_memory_tool_developer_instructions_renders_embedded_template() {
    ...
    let brocode_home = temp.path();
    let memories_dir = brocode_home.join("memories");
    ...
    let instructions = build_memory_tool_developer_instructions(brocode_home)
        .await
        .unwrap();
