...
use brocode_state::Stage1Output;
use chrono::TimeZone;
use chrono::Utc;
...
fn memory_root_uses_shared_global_path() {
    ...
    let brocode_home = dir.path().join("brocode");
    assert_eq!(memory_root(&brocode_home), brocode_home.join("memories"));
}
...
keywords: brocode-state, migrations
---
### Task 1: migration-test
task: add-migration-test
task_group: brocode-state
...
- Confirmed no ordering regressions."
    ...
    assert!(raw_memories.contains("task_group: brocode-state"));
...
mod phase2 {
    ...
    use brocode_state::Phase2JobClaimOutcome;
    use brocode_state::Stage1Output;
    ...
    struct DispatchHarness {
        ...
        state_db: Arc<brocode_state::StateRuntime>,
    }

    impl DispatchHarness {
        async fn new() -> Self {
            let brocode_home = tempfile::tempdir().expect("create temp brocode home");
            let mut config = test_config();
            config.brocode_home = brocode_home.path().to_path_buf();
            config.cwd = config.brocode_home.abs();
            ...
            let manager = ThreadManager::with_models_provider_and_home_for_tests(
                BrocodeAuth::from_api_key("dummy"),
                config.model_provider.clone(),
                config.brocode_home.clone(),
                std::sync::Arc::new(brocode_exec_server::EnvironmentManager::new(
        ...
        async fn seed_stage1_output(&self, source_updated_at: i64) {
            ...
            let ownership_token = match claim {
                brocode_state::Stage1JobClaimOutcome::Claimed { ownership_token } => {
                    ownership_token
                }
    ...
    async fn dispatch_reclaims_stale_global_lock_and_starts_consolidation() {
        ...
        pretty_assertions::assert_eq!(
            config_snapshot.cwd,
            memory_root(&harness.config.brocode_home)
        );
        match config_snapshot.sandbox_policy {
            SandboxPolicy::WorkspaceWrite { writable_roots, .. } => {
                assert!(
                    writable_roots
                        .iter()
                        .any(|root| root.as_path() == harness.config.brocode_home.as_path()),
                    "consolidation subagent should have brocode_home as writable root"
                );
            }
            other => panic!("unexpected sandbox policy: {other:?}"),
        }
        subagent.brocode.session.ensure_rollout_materialized().await;
        subagent.brocode.session.flush_rollout().await;
    ...
    async fn dispatch_with_empty_stage1_outputs_rebuilds_local_artifacts() {
        let harness = DispatchHarness::new().await;
        let root = memory_root(&harness.config.brocode_home);
    ...
    async fn dispatch_marks_job_for_retry_when_spawn_agent_fails() {
        ...
        let claim = state_db
            ...
            .expect("claim stage-1 job");
        let ownership_token = match claim {
            brocode_state::Stage1JobClaimOutcome::Claimed { ownership_token } => ownership_token,
            other => panic!("unexpected stage-1 claim outcome: {other:?}"),
        };
        assert!(
            state_db
