...
impl RequestTelemetry for ModelsRequestTelemetry {
    fn on_request(
        ...
        tracing::event!(
            target: "brocode_otel.log_only",
            tracing::Level::INFO,
            event.name = "brocode.api_request",
            ...
            auth.env_brocode_api_key_present = self.auth_env.brocode_api_key_env_present,
            auth.env_brocode_api_key_enabled = self.auth_env.brocode_api_key_env_enabled,
        ...
        tracing::event!(
            target: "brocode_otel.trace_safe",
            tracing::Level::INFO,
            event.name = "brocode.api_request",
            duration_ms = %duration.as_millis(),
            http.response.status_code = status,
...
impl ModelsManager {
    ...
    ///
    /// Uses `brocode_home` to store cached model metadata and initializes with bundled catalog
    ...
    pub fn new_with_provider(
        ...
        let cache_path = brocode_home.join(MODEL_CACHE_FILE);
    ...
    fn construct_model_info_from_candidates(
        ...
        // retry for namespaced slugs like `custom/gpt-5.3-brocode`.
    ...
    async fn fetch_and_update_models(&self) -> CoreResult<()> {
        let _timer =
            brocode_otel::start_global_timer("brocode.remote_models.fetch_update.duration_ms", &[]);
        let auth = self.auth_manager.auth().await;
        let auth_mode = auth.as_ref().map(BrocodeAuth::auth_mode);
        let api_provider = self.provider.to_api_provider(auth_mode)?;
        let api_auth = auth_provider_from_auth(auth.clone(), &self.provider)?;
        let auth_env = collect_auth_env_telemetry(
            &self.provider,
            self.auth_manager.brocode_api_key_env_enabled(),
        );
    ...
    async fn try_load_cache(&self) -> bool {
        let _timer =
            brocode_otel::start_global_timer("brocode.remote_models.load_cache.duration_ms", &[]);
        let client_version = crate::models_manager::client_version_to_whole();
