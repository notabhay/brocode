...
fn sandbox_detection_ignores_network_policy_text_in_non_sandbox_mode() {
    let output = make_exec_output(
        ...
        "",
        "",
        r#"BROCODE_NETWORK_POLICY_DECISION {"decision":"ask","reason":"not_allowed","source":"decider","protocol":"http","host":"google.com","port":80}"#,
    );
    assert!(!is_likely_sandbox_denied(SandboxType::None, &output));
}
...
fn sandbox_detection_ignores_network_policy_text_with_zero_exit_code() {
    let output = make_exec_output(
        ...
        "",
        "",
        r#"BROCODE_NETWORK_POLICY_DECISION {"decision":"ask","source":"decider","protocol":"http","host":"google.com","port":80}"#,
    );
...
fn windows_restricted_token_skips_external_sandbox_policies() {
    let policy = SandboxPolicy::ExternalSandbox {
        network_access: brocode_protocol::protocol::NetworkAccess::Restricted,
    };
...
fn windows_restricted_token_allows_legacy_workspace_write_policies() {
    let policy = SandboxPolicy::WorkspaceWrite {
        writable_roots: vec![],
        read_only_access: brocode_protocol::protocol::ReadOnlyAccess::FullAccess,
...
fn windows_restricted_token_rejects_split_only_filesystem_policies() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        brocode_protocol::permissions::FileSystemSandboxEntry {
            path: brocode_protocol::permissions::FileSystemPath::Special {
                value:
                    brocode_protocol::permissions::FileSystemSpecialPath::CurrentWorkingDirectory,
            },
            access: brocode_protocol::permissions::FileSystemAccessMode::Write,
        },
        brocode_protocol::permissions::FileSystemSandboxEntry {
            path: brocode_protocol::permissions::FileSystemPath::Path {
                path: brocode_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&docs)
                    .expect("absolute docs"),
            },
            access: brocode_protocol::permissions::FileSystemAccessMode::Read,
        },
    ]);
...
fn windows_restricted_token_rejects_root_write_read_only_carveouts() {
    ...
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        brocode_protocol::permissions::FileSystemSandboxEntry {
            path: brocode_protocol::permissions::FileSystemPath::Special {
                value: brocode_protocol::permissions::FileSystemSpecialPath::Root,
            },
            access: brocode_protocol::permissions::FileSystemAccessMode::Write,
        },
        brocode_protocol::permissions::FileSystemSandboxEntry {
...
fn windows_restricted_token_supports_full_read_split_write_read_carveouts() {
    ...
    let expected_deny_write_paths = vec![
        brocode_utils_absolute_path::AbsolutePathBuf::from_absolute_path(cwd.join(".brocode"))
            .expect("absolute .brocode"),
        brocode_utils_absolute_path::AbsolutePathBuf::from_absolute_path(&docs)
            .expect("absolute docs"),
    ];
    #[cfg(not(windows))]
    let expected_deny_write_paths = vec![
...
fn process_exec_tool_call_uses_platform_sandbox_for_network_only_restrictions() {
    let expected = brocode_sandboxing::get_platform_sandbox(/*windows_sandbox_enabled*/ false)
...
async fn kill_child_process_group_kills_grandchildren_on_timeout() -> Result<()> {
    ...
    let params = ExecParams {
        ...
        network: None,
        sandbox_permissions: SandboxPermissions::UseDefault,
        windows_sandbox_level: brocode_protocol::config_types::WindowsSandboxLevel::Disabled,
        windows_sandbox_private_desktop: false,
        justification: None,
        arg0: None,
    };
...
async fn process_exec_tool_call_respects_cancellation_token() -> Result<()> {
    ...
    let output = match result {
        Err(BrocodeErr::Sandbox(SandboxErr::Timeout { output })) => output,
