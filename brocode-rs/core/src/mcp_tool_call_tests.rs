...
use crate::brocode::make_session_and_context_with_rx;
...
async fn approval_elicitation_request_uses_message_override_and_preserves_tool_params_keys() {
    ...
    let request = build_mcp_tool_approval_elicitation_request(
        ...
        McpToolApprovalElicitationRequest {
            server: BROCODE_APPS_MCP_SERVER_NAME,
    ...
    assert_eq!(
        ...
        McpServerElicitationRequestParams {
            ...
            server_name: BROCODE_APPS_MCP_SERVER_NAME.to_string(),
...
#[test]
fn brocode_apps_tool_question_uses_fallback_app_label() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        BROCODE_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn trusted_brocode_apps_tool_question_offers_always_allow() {
    let question = build_mcp_tool_approval_question(
        "q".to_string(),
        BROCODE_APPS_MCP_SERVER_NAME,
        "run_action",
...
#[test]
fn brocode_apps_tool_question_without_elicitation_omits_always_allow() {
    let session_key = McpToolApprovalKey {
        server: BROCODE_APPS_MCP_SERVER_NAME.to_string(),
...
#[test]
fn brocode_apps_connectors_support_persistent_approval() {
    let invocation = McpInvocation {
        server: BROCODE_APPS_MCP_SERVER_NAME.to_string(),
...
async fn mcp_tool_call_request_meta_includes_turn_metadata_for_custom_server() {
    ...
    assert_eq!(
        meta,
        serde_json::json!({
            crate::X_BROCODE_TURN_METADATA_HEADER: expected_turn_metadata,
        })
    );
}

#[tokio::test]
async fn brocode_apps_tool_call_request_meta_includes_turn_metadata_and_brocode_apps_meta() {
    let (_, turn_context) = make_session_and_context().await;
    let expected_turn_metadata = serde_json::from_str::<serde_json::Value>(
        &turn_context
            .turn_metadata_state
    ...
    let metadata = McpToolApprovalMetadata {
        ...
        brocode_apps_meta: Some(
            ...
            .as_object()
            .cloned()
            .expect("_brocode_apps metadata should be an object"),
        ),
    };
    ...
    assert_eq!(
        ...
        Some(serde_json::json!({
            crate::X_BROCODE_TURN_METADATA_HEADER: expected_turn_metadata,
            MCP_TOOL_BROCODE_APPS_META_KEY: {
                "resource_uri": "connector://calendar/tools/calendar_create_event",
                "contains_mcp_source": true,
                "connector_id": "calendar",
            },
...
#[test]
fn approval_elicitation_meta_includes_connector_source_for_brocode_apps() {
    assert_eq!(
        build_mcp_tool_approval_elicitation_meta(
            BROCODE_APPS_MCP_SERVER_NAME,
            Some(&approval_metadata(
...
#[tokio::test]
async fn persist_brocode_app_tool_approval_writes_tool_override() {
    let tmp = tempdir().expect("tempdir");

    persist_brocode_app_tool_approval(tmp.path(), "calendar", "calendar/list_events")
        .await
...
async fn maybe_persist_mcp_tool_approval_writes_project_config_for_project_server() {
    ...
    let project_brocode_dir = project_dir.path().join(".brocode");
    std::fs::create_dir_all(&project_brocode_dir).expect("create project .brocode dir");
    std::fs::write(
        project_brocode_dir.join(CONFIG_TOML_FILE),
        "[mcp_servers.docs]\ncommand = \"docs-server\"\n",
    )
    ...
    ConfigEditsBuilder::new(&brocode_home)
        .set_project_trust_level(
            project_dir.path(),
            brocode_protocol::config_types::TrustLevel::Trusted,
        )
        .apply()
        .await
    ...
    let contents = std::fs::read_to_string(project_brocode_dir.join(CONFIG_TOML_FILE))
...
async fn approve_mode_blocks_when_arc_returns_interrupt_for_model() {
    ...
    turn_context.auth_manager = Some(crate::test_support::auth_manager_from_auth(
        crate::BrocodeAuth::create_dummy_chatgpt_auth_for_testing(),
    ));
    let mut config = (*turn_context.config).clone();
