...
This crate implements the business logic for Brocode. It is designed to be used by the various Brocode UIs written in Rust.

## Dependencies

Note that `brocode-core` makes some assumptions about certain helper utilities being available in the environment. Currently, this support matrix is:
...
pointer file), the resolved `gitdir:` target, and `.brocode` read-only.
...
Expects the binary containing `brocode-core` to run the equivalent of `brocode sandbox linux` (legacy alias: `brocode debug landlock`) when `arg0` is `brocode-linux-sandbox`. See the `brocode-arg0` crate for details.
...
the binary and Brocode surfaces a startup warning through its normal notification
...
Expects the binary containing `brocode-core` to simulate the virtual `apply_patch` CLI when `arg1` is `--brocode-run-as-apply-patch`. See the `brocode-arg0` crate for details.
