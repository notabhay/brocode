...
use crate::test_brocode::TestBrocode;
use crate::test_brocode::test_brocode;
...
pub fn zsh_fork_runtime(test_name: &str) -> Result<Option<ZshForkRuntime>> {
    ...
    let Ok(main_execve_wrapper_exe) = brocode_utils_cargo_bin::cargo_bin("brocode-execve-wrapper")
    else {
        eprintln!("skipping {test_name}: unable to resolve `brocode-execve-wrapper` binary");
        return Ok(None);
    };
