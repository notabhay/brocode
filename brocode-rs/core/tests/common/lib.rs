...
use brocode_utils_cargo_bin::CargoBinError;
use ctor::ctor;
...
pub mod test_brocode;
pub mod test_brocode_exec;
pub mod tracing;
pub mod zsh_fork;
...
fn enable_deterministic_unified_exec_process_ids_for_tests() {
    brocode_core::test_support::set_thread_manager_test_mode(/*enabled*/ true);
    brocode_core::test_support::set_deterministic_process_ids(/*enabled*/ true);
}

#[ctor]
fn configure_arg0_dispatch_for_test_binaries() {
    let _ = TEST_ARG0_PATH_ENTRY.get_or_init(brocode_arg0::arg0_dispatch);
}
...
fn configure_insta_workspace_root_for_snapshot_tests() {
    ...
    let workspace_root = brocode_utils_cargo_bin::repo_root()
        .ok()
        .map(|root| root.join("brocode-rs"));
...
pub fn test_tmp_path() -> AbsolutePathBuf {
    test_absolute_path_with_windows("/tmp", Some(r"C:\Users\brocode\AppData\Local\Temp"))
}
...
/// avoids clobbering a developer’s real `~/.brocode`.
pub async fn load_default_config_for_test(brocode_home: &TempDir) -> Config {
...
fn default_test_overrides() -> ConfigOverrides {
    ConfigOverrides {
        brocode_linux_sandbox_exe: Some(
            find_brocode_linux_sandbox_exe().expect("should find binary for brocode-linux-sandbox"),
        ),
...
#[cfg(target_os = "linux")]
pub fn find_brocode_linux_sandbox_exe() -> Result<PathBuf, CargoBinError> {
    ...
    brocode_utils_cargo_bin::cargo_bin("brocode-linux-sandbox")
}
...
pub async fn wait_for_event<F>(
    brocode: &BrocodeThread,
    predicate: F,
) -> brocode_protocol::protocol::EventMsg
where
    F: FnMut(&brocode_protocol::protocol::EventMsg) -> bool,
{
    use tokio::time::Duration;
    wait_for_event_with_timeout(brocode, predicate, Duration::from_secs(1)).await
}

pub async fn wait_for_event_match<T, F>(brocode: &BrocodeThread, matcher: F) -> T
where
    F: Fn(&brocode_protocol::protocol::EventMsg) -> Option<T>,
{
    let ev = wait_for_event(brocode, |ev| matcher(ev).is_some()).await;
    matcher(&ev).unwrap()
}
    ...
    F: FnMut(&brocode_protocol::protocol::EventMsg) -> bool,
{
    ...
    loop {
        ...
        let ev = timeout(wait_time.max(Duration::from_secs(10)), brocode.next_event())
            .await
...
pub fn sandbox_env_var() -> &'static str {
    brocode_core::spawn::BROCODE_SANDBOX_ENV_VAR
}

pub fn sandbox_network_env_var() -> &'static str {
    brocode_core::spawn::BROCODE_SANDBOX_NETWORK_DISABLED_ENV_VAR
}

const REMOTE_ENV_ENV_VAR: &str = "BROCODE_TEST_REMOTE_ENV";
...
pub fn format_with_current_shell(command: &str) -> Vec<String> {
    brocode_core::shell::default_user_shell()
...
pub fn stdio_server_bin() -> Result<String, CargoBinError> {
    brocode_utils_cargo_bin::cargo_bin("test_stdio_server").map(|p| p.to_string_lossy().to_string())
}
...
#[macro_export]
macro_rules! brocode_linux_sandbox_exe_or_skip {
    () => {{
        #[cfg(target_os = "linux")]
        {
            match $crate::find_brocode_linux_sandbox_exe() {
                Ok(path) => Some(path),
                Err(err) => {
                    eprintln!("brocode-linux-sandbox binary not available, skipping test: {err}");
                    return;
                }
            }
        }
