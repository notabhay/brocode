...
async fn emits_deprecation_notice_for_legacy_feature_flag() -> anyhow::Result<()> {
    ...
    let TestBrocode { brocode, .. } = builder.build(&server).await?;

    let notice = wait_for_event_match(&brocode, |event| match event {
    ...
    assert_eq!(
        details.as_deref(),
        Some(
            "Enable it with `--enable unified_exec` or `[features].unified_exec` in config.toml. See https://developers.openai.com/brocode/config-basic#feature-flags for details."
        ),
    );
