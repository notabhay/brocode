...
use brocode_core::config_loader::NetworkRequirementsToml;
...
use brocode_protocol::approvals::NetworkApprovalProtocol;
...
impl TargetPath {
    fn resolve_for_patch(self, test: &TestBrocode) -> (PathBuf, String) {
        match self {
...
impl Expectation {
    fn verify(&self, test: &TestBrocode, result: &CommandResult) -> Result<()> {
        match self {
...
async fn expect_exec_approval(
    ...
    let event = wait_for_event(&test.brocode, |event| {
        matches!(
            event,
...
async fn wait_for_completion_without_approval(test: &TestBrocode) {
    let event = wait_for_event(&test.brocode, |event| {
        matches!(
            event,
            EventMsg::ExecApprovalRequest(_) | EventMsg::TurnComplete(_)
...
async fn wait_for_completion(test: &TestBrocode) {
...
async fn wait_for_spawned_thread(test: &TestBrocode) -> Result<Arc<BrocodeThread>> {
...
async fn run_scenario(scenario: &ScenarioSpec) -> Result<()> {
    ...
    let mut builder = test_brocode().with_model(model).with_config(move |config| {
...
async fn invalid_requested_prefix_rule_falls_back_for_compound_command() -> Result<()> {
    ...
    let command = "touch /tmp/brocode-fallback-rule-test.txt && echo hello > /tmp/brocode-fallback-rule-test.txt";
...
"#,
    ...
    let sandbox_policy_for_config = sandbox_policy.clone();
    let mut builder = test_brocode().with_home(home).with_config(move |config| {
        config.permissions.approval_policy = Constrained::allow_any(approval_policy);
        config.permissions.sandbox_policy = Constrained::allow_any(sandbox_policy_for_config);
        let layers = config
            .config_layer_stack
