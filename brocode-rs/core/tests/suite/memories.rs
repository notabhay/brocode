...
async fn memories_startup_phase2_tracks_added_and_removed_inputs_across_runs() -> Result<()> {
    ...
    let first = build_test_brocode(&server, home.clone()).await?;
    ...
    shutdown_test_brocode(&first).await?;
    ...
    let second = build_test_brocode(&server, home.clone()).await?;
    ...
    shutdown_test_brocode(&second).await?;
    Ok(())
}
...
async fn web_search_pollution_moves_selected_thread_into_removed_phase2_inputs() -> Result<()> {
    ...
    shutdown_test_brocode(&initial).await?;
    ...
    shutdown_test_brocode(&resumed).await?;
    Ok(())
}

async fn build_test_brocode(
    server: &wiremock::MockServer,
    home: Arc<TempDir>,
) -> Result<TestBrocode> {
    #[allow(clippy::expect_used)]
    let mut builder = test_brocode().with_home(home).with_config(|config| {
        config
            .features
            .enable(Feature::Sqlite)
            .expect("test config should allow feature update");
...
async fn init_state_db(home: &Arc<TempDir>) -> Result<Arc<brocode_state::StateRuntime>> {
    let db = brocode_state::StateRuntime::init(home.path().to_path_buf(), "test-provider".into())
        .await?;
...
async fn seed_stage1_output(
    ...
    let mut metadata_builder = brocode_state::ThreadMetadataBuilder::new(
        thread_id,
        brocode_home.join(format!("rollout-{thread_id}.jsonl")),
        updated_at,
        SessionSource::Cli,
    );
    metadata_builder.cwd = brocode_home.join(format!("workspace-{rollout_slug}"));
...
async fn shutdown_test_brocode(test: &TestBrocode) -> Result<()> {
    test.brocode.submit(Op::Shutdown {}).await?;
    wait_for_event(&test.brocode, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
    Ok(())
}
