...
use brocode_core::compact::SUMMARIZATION_PROMPT;
use brocode_core::compact::SUMMARY_PREFIX;
...
async fn assert_compaction_uses_turn_lifecycle_id(
    brocode: &std::sync::Arc<brocode_core::BrocodeThread>,
) {
    ...
    while turn_completed_id.is_none() {
        let event = brocode.next_event().await.expect("next event");
        match event.msg {
...
async fn summarize_context_three_requests_and_instructions() {
    ...
    // Build config pointing to the mock server and spawn Brocode.
    ...
    brocode.submit(Op::Compact).await.unwrap();
    let warning_event = wait_for_event(&brocode, |ev| matches!(ev, EventMsg::Warning(_))).await;
    ...
    // Shut down Brocode to flush rollout entries before inspecting the file.
    brocode.submit(Op::Shutdown).await.unwrap();
    wait_for_event(&brocode, |ev| matches!(ev, EventMsg::ShutdownComplete)).await;
...
async fn manual_compact_uses_custom_prompt() {
    ...
    brocode.submit(Op::Compact).await.expect("trigger compact");
    let warning_event = wait_for_event(&brocode, |ev| matches!(ev, EventMsg::Warning(_))).await;
    let EventMsg::Warning(WarningEvent { message }) = warning_event else {
        panic!("expected warning event after compact");
    };
...
async fn manual_compact_emits_api_and_local_token_usage_events() {
    ...
    let brocode = builder.build(&server).await.unwrap().brocode;
    ...
    let first = wait_for_event_match(&brocode, |ev| match ev {
    ...
    let last = wait_for_event_match(&brocode, |ev| match ev {
        EventMsg::TokenCount(tc) => tc
            .info
            .as_ref()
            .map(|info| info.last_token_usage.total_tokens),
...
async fn manual_compact_emits_context_compaction_items() {
    ...
    brocode
        ...
        .await
        .unwrap();
    wait_for_event(&brocode, |event| matches!(event, EventMsg::TurnComplete(_))).await;
    ...
    {
        let event = brocode.next_event().await.unwrap();
        match event.msg {
            EventMsg::ItemStarted(ItemStartedEvent {
...
async fn auto_compact_runs_after_resume_when_token_usage_is_over_limit() {
    ...
    let compacted_history = vec![
        brocode_protocol::models::ResponseItem::Message {
            id: None,
        ...
        brocode_protocol::models::ResponseItem::Compaction {
    ...
    let mut resume_builder = test_brocode().with_config(move |config| {
        set_test_compact_prompt(config);
        config.model_auto_compact_token_limit = Some(limit);
    });
    ...
    wait_for_event(&resumed.brocode, |event| {
...
async fn pre_sampling_compact_runs_on_switch_to_smaller_context_model() {
    ...
    let previous_model = "gpt-5.2-brocode";
    ...
    let test = builder.build(&server).await.expect("build test brocode");
    ...
    test.brocode
        ...
        .await
        .expect("submit first user turn");
    wait_for_event(&test.brocode, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    assert_compaction_uses_turn_lifecycle_id(&test.brocode).await;
...
async fn pre_sampling_compact_runs_after_resume_and_switch_to_smaller_model() {
    ...
    let model_provider = non_openai_model_provider(&server);
    let mut initial_builder = test_brocode()
        .with_auth(BrocodeAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model(previous_model)
        .with_config(move |config| {
            config.model_provider = model_provider;
    ...
    let initial = initial_builder
        .build(&server)
        .await
        .expect("build initial test brocode");
    let home = initial.home.clone();
    let rollout_path = initial
        .session_configured
        .rollout_path
    ...
    wait_for_event(&initial.brocode, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    let model_provider = non_openai_model_provider(&server);
    let mut resumed_builder = test_brocode()
        .with_auth(BrocodeAuth::create_dummy_chatgpt_auth_for_testing())
        .with_model(previous_model)
        .with_config(move |config| {
            config.model_provider = model_provider;
    ...
    assert_compaction_uses_turn_lifecycle_id(&resumed.brocode).await;
...
async fn manual_compact_retries_after_context_window_error() {
    ...
    let EventMsg::BackgroundEvent(event) =
        wait_for_event(&brocode, |ev| matches!(ev, EventMsg::BackgroundEvent(_))).await
    else {
...
async fn manual_compact_non_context_failure_retries_then_emits_task_error() {
    ...
    let reconnect_message = wait_for_event_match(&brocode, |event| match event {
    ...
    let task_error_message = wait_for_event_match(&brocode, |event| match event {
...
async fn auto_compact_clamps_config_limit_to_context_window() {
    ...
    let brocode = builder.build(&server).await.unwrap();

    brocode.submit_turn("OVER_LIMIT_TURN").await.unwrap();
    brocode.submit_turn("FOLLOW_UP_AFTER_CLAMP").await.unwrap();
...
async fn snapshot_request_shape_pre_turn_compaction_strips_incoming_model_switch() {
    ...
    let server = start_mock_server().await;
    let previous_model = "gpt-5.1-brocode-max";
    let next_model = "gpt-5.2-brocode";
...
async fn snapshot_request_shape_pre_turn_compaction_context_window_exceeded() {
    ...
    let error_message = wait_for_event_match(&brocode, |event| match event {
        EventMsg::Error(err) => Some(err.message.clone()),
        _ => None,
    })
    .await;
...
async fn snapshot_request_shape_manual_compact_without_previous_user_messages() {
    ...
    brocode.submit(Op::Compact).await.expect("run /compact");
    wait_for_event(&brocode, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
