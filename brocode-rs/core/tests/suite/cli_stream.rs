...
use brocode_core::auth::BROCODE_API_KEY_ENV_VAR;
use brocode_git_utils::collect_git_info;
use brocode_protocol::protocol::GitInfo;
use brocode_utils_cargo_bin::find_resource;
...
fn repo_root() -> std::path::PathBuf {
    ...
    brocode_utils_cargo_bin::repo_root().expect("failed to resolve repo root")
}
...
async fn responses_mode_stream_cli() {
    ...
    let bin = brocode_utils_cargo_bin::cargo_bin("brocode").unwrap();
    ...
    //     10,
    //     None,
    //     brocode_core::ThreadSortKey::UpdatedAt,
    //     &[],
...
async fn exec_cli_applies_model_instructions_file() {
    ...
    // Brocode to use the Responses API with the dummy env var.
    let provider_override = format!(
        "model_providers.mock={{ name = \"mock\", base_url = \"{}/v1\", env_key = \"PATH\", wire_api = \"responses\" }}",
        server.uri()
    );
...
/// Verify that `brocode exec --profile ...` preserves the active profile when it
...
/// 2. Configures brocode to read from this fixture via BROCODE_RS_SSE_FIXTURE env var
...
async fn integration_creates_and_checks_session_file() -> anyhow::Result<()> {
    ...
    // 4. Run the brocode CLI and invoke `exec`, which is what records a session.
    let bin = brocode_utils_cargo_bin::cargo_bin("brocode").unwrap();
    let mut cmd = AssertCommand::new(bin);
    cmd.arg("exec")
        .arg("--skip-git-repo-check")
    ...
    let bin2 = brocode_utils_cargo_bin::cargo_bin("brocode").unwrap();
    ...
    assert!(output2.status.success(), "resume brocode-cli run failed");
