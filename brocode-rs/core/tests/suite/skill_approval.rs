...
fn skill_script_command(test: &TestBrocode, script_name: &str) -> Result<String> {
...
async fn wait_for_exec_approval_request(test: &TestBrocode) -> Option<ExecApprovalRequestEvent> {
    wait_for_event_match(test.brocode.as_ref(), |event| match event {
...
async fn wait_for_turn_complete(test: &TestBrocode) {
...
async fn shell_zsh_fork_still_enforces_workspace_write_sandbox() -> Result<()> {
    ...
    let outside_path = "/tmp/brocode-zsh-fork-workspace-write-deny.txt";
    let workspace_write_policy = restrictive_workspace_write_policy();
