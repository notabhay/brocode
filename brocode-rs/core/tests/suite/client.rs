...
use brocode_core::ModelClient;
use brocode_core::ModelProviderAuthInfo;
use brocode_core::ModelProviderInfo;
use brocode_core::NewThread;
use brocode_core::Prompt;
use brocode_core::ResponseEvent;
use brocode_core::ThreadManager;
use brocode_core::WireApi;
use brocode_core::auth::AuthCredentialsStoreMode;
use brocode_core::built_in_model_providers;
use brocode_core::default_client::originator;
...
use brocode_protocol::models::FunctionCallOutputPayload;
use brocode_protocol::models::ImageDetail;
use brocode_protocol::models::LocalShellAction;
use brocode_protocol::models::LocalShellExecAction;
use brocode_protocol::models::LocalShellStatus;
use brocode_protocol::models::MessagePhase;
use brocode_protocol::models::ReasoningItemContent;
use brocode_protocol::models::ReasoningItemReasoningSummary;
use brocode_protocol::models::ResponseItem;
use brocode_protocol::models::WebSearchAction;
...
use brocode_protocol::protocol::SessionMeta;
...
use core_test_support::test_brocode::TestBrocode;
use core_test_support::test_brocode::test_brocode;
...
/// Writes an `auth.json` into the provided `brocode_home` with the specified parameters.
/// Returns the fake JWT string written to `tokens.id_token`.
#[expect(clippy::unwrap_used)]
fn write_auth_json(
    brocode_home: &TempDir,
    ...
    std::fs::write(
        brocode_home.path().join("auth.json"),
...
async fn resume_includes_initial_messages_and_sends_prior_items() {
    ...
    let prior_user = brocode_protocol::models::ResponseItem::Message {
        id: None,
        role: "user".to_string(),
        content: vec![brocode_protocol::models::ContentItem::InputText {
    ...
    let prior_system = brocode_protocol::models::ResponseItem::Message {
        id: None,
        role: "system".to_string(),
        content: vec![brocode_protocol::models::ContentItem::OutputText {
    ...
    let prior_item = brocode_protocol::models::ResponseItem::Message {
        id: None,
    ...
    // Configure Brocode to resume from our file
    let brocode_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_brocode()
        .with_home(brocode_home.clone())
        .with_config(|config| {
            // Ensure user instructions are NOT delivered on resume.
            config.user_instructions = Some("be nice".to_string());
        });
    let test = builder
        .resume(&server, brocode_home, session_path.clone())
        .await
        .expect("resume conversation");
    let brocode = test.brocode.clone();
    ...
    brocode
        ...
        .await
        .unwrap();
    wait_for_event(&brocode, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn resume_replays_legacy_js_repl_image_rollout_shapes() {
    ...
    let brocode_home = Arc::new(TempDir::new().unwrap());
    let mut builder = test_brocode().with_model("gpt-5.1");
    let test = builder
        .resume(&server, brocode_home, session_path.clone())
        .await
        .expect("resume conversation");
...
async fn includes_conversation_id_and_model_headers_in_request() {
    ...
    let mut builder = test_brocode().with_auth(BrocodeAuth::from_api_key("Test API Key"));
    let test = builder
        .build(&server)
        .await
...
async fn send_provider_auth_request(server: &MockServer, auth: ModelProviderAuthInfo) {
    ...
    let brocode_home = TempDir::new().unwrap();
    let mut config = load_default_config_for_test(&brocode_home).await;
    ...
    let model_info =
        brocode_core::test_support::construct_model_info_offline(model.as_str(), &config);
    ...
    let client = ModelClient::new(
        Some(AuthManager::from_auth_for_testing(
            BrocodeAuth::from_api_key("unused-api-key"),
        )),
        conversation_id,
        provider,
        SessionSource::Exec,
...
async fn includes_base_instructions_override_in_request() {
    ...
    let mut builder = test_brocode()
        .with_auth(BrocodeAuth::from_api_key("Test API Key"))
        .with_config(|config| {
...
async fn chatgpt_auth_sends_correct_request() {
    ...
    model_provider.base_url = Some(format!("{}/api/brocode", server.uri()));
    model_provider.supports_websockets = false;
    let mut builder = test_brocode()
        .with_auth(create_dummy_brocode_auth())
    ...
    let request = resp_mock.single_request();
    assert_eq!(request.path(), "/api/brocode/responses");
    let request_authorization = request
        .header("authorization")
        .expect("authorization header");
    let request_originator = request.header("originator").expect("originator header");
...
async fn prefers_apikey_when_config_prefers_apikey_even_with_chatgpt_tokens() {
    ...
    let auth_manager =
        match BrocodeAuth::from_auth_storage(brocode_home.path(), AuthCredentialsStoreMode::File) {
            Ok(Some(auth)) => brocode_core::test_support::auth_manager_from_auth(auth),
            Ok(None) => panic!("No BrocodeAuth found in brocode_home"),
            Err(e) => panic!("Failed to load BrocodeAuth: {e}"),
        };
    let thread_manager = ThreadManager::new(
        ...
        Arc::new(brocode_exec_server::EnvironmentManager::new(
...
async fn skills_append_to_developer_message() {
    ...
    let brocode_home = Arc::new(TempDir::new().unwrap());
    let skill_dir = brocode_home.path().join("skills/demo");
    ...
    let brocode_home_path = brocode_home.path().to_path_buf();
    let mut builder = test_brocode()
        .with_home(brocode_home.clone())
        .with_auth(BrocodeAuth::from_api_key("Test API Key"))
        .with_config(move |config| {
            config.cwd = brocode_home_path.abs();
        });
    let brocode = builder
        .build(&server)
        .await
    ...
    let _brocode_home_guard = brocode_home;
}
...
async fn includes_configured_effort_in_request() -> anyhow::Result<()> {
    ...
    .await;
    let TestBrocode { brocode, .. } = test_brocode()
...
-> anyhow::Result<()> {
    ...
    .await;
    let TestBrocode { brocode, .. } = test_brocode().with_model("gpt-5.1").build(&server).await?;
...
async fn token_count_includes_rate_limits_snapshot() {
    ...
    let response = ResponseTemplate::new(200)
        .insert_header("content-type", "text/event-stream")
        .insert_header("x-brocode-primary-used-percent", "12.5")
        .insert_header("x-brocode-secondary-used-percent", "40.0")
        .insert_header("x-brocode-primary-window-minutes", "10")
        .insert_header("x-brocode-secondary-window-minutes", "60")
        .insert_header("x-brocode-primary-reset-at", "1704069000")
        .insert_header("x-brocode-secondary-reset-at", "1704074400")
    ...
    let mut builder = test_brocode()
        .with_auth(BrocodeAuth::from_api_key("test"))
        .with_config(move |config| {
    ...
    let first_token_event =
        wait_for_event(&brocode, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    pretty_assertions::assert_eq!(
        ...
        json!({
            "info": {
                ...
                // Default model is gpt-5.1-brocode-max in tests → 95% usable context window
    ...
    wait_for_event(&brocode, |msg| matches!(msg, EventMsg::TurnComplete(_))).await;
}
...
async fn usage_limit_error_emits_rate_limit_event() -> anyhow::Result<()> {
    ...
    let response = ResponseTemplate::new(429)
        .insert_header("x-brocode-primary-used-percent", "100.0")
        .insert_header("x-brocode-secondary-used-percent", "87.5")
        .insert_header("x-brocode-primary-over-secondary-limit-percent", "95.0")
        .insert_header("x-brocode-primary-window-minutes", "15")
        .insert_header("x-brocode-secondary-window-minutes", "60")
        .set_body_json(json!({
            "error": {
    ...
    let mut builder = test_brocode();
    let brocode_fixture = builder.build(&server).await?;
    let brocode = brocode_fixture.brocode.clone();
    ...
    let token_event = wait_for_event(&brocode, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&brocode, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn context_window_error_sets_total_tokens_to_model_window() -> anyhow::Result<()> {
    ...
    let token_event = wait_for_event(&brocode, |event| {
        matches!(
            event,
    ...
    let error_event = wait_for_event(&brocode, |ev| matches!(ev, EventMsg::Error(_))).await;
    let expected_context_window_message = BrocodeErr::ContextWindowExceeded.to_string();
    assert!(
        matches!(
            error_event,
...
fn create_dummy_brocode_auth() -> BrocodeAuth {
...
async fn history_dedupes_streamed_and_final_messages_across_turns() {
    // Skip under Brocode sandbox network restrictions (mirrors other tests).
    skip_if_no_network!();
