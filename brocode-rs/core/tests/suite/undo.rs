...
async fn undo_harness() -> Result<TestBrocodeHarness> {
    let builder = test_brocode().with_model("gpt-5.1").with_config(|config| {
    ...
    TestBrocodeHarness::with_builder(builder).await
}
...
fn init_git_repo(path: &Path) -> Result<()> {
    ...
    git(path, &["config", "user.name", "Brocode Tests"])?;
    git(path, &["config", "user.email", "brocode-tests@example.com"])?;

    // Create README.txt
    let readme_path = path.join("README.txt");
    fs::write(&readme_path, "Test repository initialized by Brocode.\n")?;
...
async fn invoke_undo(brocode: &Arc<BrocodeThread>) -> Result<UndoCompletedEvent> {
    brocode.submit(Op::Undo).await?;
...
async fn expect_successful_undo(brocode: &Arc<BrocodeThread>) -> Result<UndoCompletedEvent> {
    let event = invoke_undo(brocode).await?;
    assert!(
        event.success,
...
async fn expect_failed_undo(brocode: &Arc<BrocodeThread>) -> Result<UndoCompletedEvent> {
    let event = invoke_undo(brocode).await?;
    assert!(
        !event.success,
...
async fn undo_removes_new_file_created_during_turn() -> Result<()> {
    ...
    let brocode = Arc::clone(&harness.test().brocode);
    let completed = expect_successful_undo(&brocode).await?;
...
async fn undo_sequential_turns_consumes_snapshots() -> Result<()> {
    ...
    let brocode = Arc::clone(&harness.test().brocode);
    expect_successful_undo(&brocode).await?;
    assert_eq!(fs::read_to_string(&story)?, "turn two\n");
    ...
    expect_failed_undo(&brocode).await?;
