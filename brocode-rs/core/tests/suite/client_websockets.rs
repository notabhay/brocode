...
use brocode_api::WS_REQUEST_HEADER_TRACEPARENT_CLIENT_METADATA_KEY;
use brocode_api::WS_REQUEST_HEADER_TRACESTATE_CLIENT_METADATA_KEY;
use brocode_core::BrocodeAuth;
use brocode_core::ModelClient;
use brocode_core::ModelClientSession;
use brocode_core::ModelProviderInfo;
use brocode_core::Prompt;
use brocode_core::ResponseEvent;
use brocode_core::WireApi;
use brocode_core::X_RESPONSESAPI_INCLUDE_TIMING_METRICS_HEADER;
...
const MODEL: &str = "gpt-5.2-brocode";
...
async fn responses_websocket_usage_limit_error_emits_rate_limit_event() {
    ...
    let usage_limit_error = json!({
        ...
        "headers": {
            "x-brocode-primary-used-percent": "100.0",
            "x-brocode-secondary-used-percent": "87.5",
            "x-brocode-primary-over-secondary-limit-percent": "95.0",
            "x-brocode-primary-window-minutes": "15",
            "x-brocode-secondary-window-minutes": "60"
        }
    });
    ...
    let test = builder
        ...
        .await
        .expect("build websocket brocode");
    ...
    let token_event =
        wait_for_event(&test.brocode, |msg| matches!(msg, EventMsg::TokenCount(_))).await;
    ...
    let error_event = wait_for_event(&test.brocode, |msg| matches!(msg, EventMsg::Error(_))).await;
...
async fn responses_websocket_forwards_turn_metadata_on_initial_and_incremental_create() {
    ...
    let enriched_turn_metadata = r#"{"turn_id":"turn-123","sandbox":"workspace-write","workspaces":[{"root_path":"/tmp/repo","latest_git_commit_hash":"abc123","associated_remote_urls":["git@github.com:openai/brocode.git"],"has_changes":true}]}"#;
    let prompt_one = prompt_with_input(vec![message_item("hello")]);
    let prompt_two = prompt_with_input(vec![
        message_item("hello"),
        assistant_message_item("msg-1", "assistant output"),
    ...
    assert_eq!(first["type"].as_str(), Some("response.create"));
    assert_eq!(
        first["client_metadata"]["x-brocode-turn-metadata"].as_str(),
        Some(first_turn_metadata)
    );
    assert_eq!(second["type"].as_str(), Some("response.create"));
    assert_eq!(second["previous_response_id"].as_str(), Some("resp-1"));
    assert_eq!(
        second["client_metadata"]["x-brocode-turn-metadata"].as_str(),
        Some(enriched_turn_metadata)
    );
...
async fn websocket_harness_with_provider_options(
    ...
    let metrics = MetricsClient::new(
        MetricsConfig::in_memory("test", "brocode-core", env!("CARGO_PKG_VERSION"), exporter)
            .with_runtime_reader(),
    )
