...
async fn interrupt_long_running_tool_emits_turn_aborted() {
    ...
    wait_for_event(&brocode, |ev| matches!(ev, EventMsg::ExecCommandBegin(_))).await;

    brocode.submit(Op::Interrupt).await.unwrap();

    // Expect TurnAborted soon after.
    wait_for_event(&brocode, |ev| matches!(ev, EventMsg::TurnAborted(_))).await;
}
...
async fn interrupt_tool_records_history_entries() {
    ...
    let fixture = test_brocode()
        ...
        .await
        .unwrap();
    let brocode = Arc::clone(&fixture.brocode);
