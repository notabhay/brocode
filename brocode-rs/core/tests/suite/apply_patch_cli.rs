...
use core_test_support::test_brocode::ApplyPatchModelOutput;
...
pub async fn apply_patch_harness() -> Result<TestBrocodeHarness> {
    apply_patch_harness_with(|builder| builder).await
}

async fn apply_patch_harness_with(
    configure: impl FnOnce(TestBrocodeBuilder) -> TestBrocodeBuilder,
) -> Result<TestBrocodeHarness> {
    let builder = configure(test_brocode()).with_config(|config| {
    ...
    Box::pin(TestBrocodeHarness::with_builder(builder)).await
}
...
async fn apply_patch_cli_uses_brocode_self_exe_with_linux_sandbox_helper_alias() -> Result<()> {
    skip_if_no_network!(Ok(()));

    let harness = apply_patch_harness().await?;
    let brocode_linux_sandbox_exe = harness
        .test()
        .config
        .brocode_linux_sandbox_exe
        .as_ref()
        .expect("linux test config should include brocode-linux-sandbox helper");
    assert_eq!(
        brocode_linux_sandbox_exe
            .file_name()
            .and_then(|name| name.to_str()),
        Some(BROCODE_LINUX_SANDBOX_ARG0),
    );
