...
use core_test_support::test_brocode::TestBrocodeBuilder;
use core_test_support::test_brocode::TestBrocodeHarness;
...
fn remote_realtime_test_brocode_builder(
    realtime_server: &responses::WebSocketTestServer,
) -> TestBrocodeBuilder {
    let realtime_base_url = realtime_server.uri().to_string();
    test_brocode()
        .with_auth(BrocodeAuth::from_api_key("dummy"))
...
async fn start_realtime_conversation(brocode: &brocode_core::BrocodeThread) -> Result<()> {
    brocode
    ...
    wait_for_event_match(brocode, |msg| match msg {
...
async fn close_realtime_conversation(brocode: &brocode_core::BrocodeThread) -> Result<()> {
    brocode.submit(Op::RealtimeConversationClose).await?;
    wait_for_event_match(brocode, |msg| match msg {
...
async fn remote_compact_replaces_history_for_followups() -> Result<()> {
    ...
    let harness = TestBrocodeHarness::with_builder(
        test_brocode().with_auth(BrocodeAuth::create_dummy_chatgpt_auth_for_testing()),
    )
    .await?;
    let brocode = harness.test().brocode.clone();
    ...
    brocode.submit(Op::Compact).await?;
    wait_for_event(&brocode, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn remote_compact_runs_automatically() -> Result<()> {
    ...
    let message = wait_for_event_match(&brocode, |event| match event {
...
async fn remote_compact_trim_estimate_uses_session_base_instructions() -> Result<()> {
    ...
    let baseline_harness = TestBrocodeHarness::with_builder(
        test_brocode()
            .with_auth(BrocodeAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config(|config| {
    ...
    .await?;
    let baseline_brocode = baseline_harness.test().brocode.clone();
    ...
    baseline_brocode
        ...
        .await?;
    wait_for_event(&baseline_brocode, |event| {
    ...
    baseline_brocode.submit(Op::Compact).await?;
    wait_for_event(&baseline_brocode, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    let override_harness = TestBrocodeHarness::with_builder(
        test_brocode()
            .with_auth(BrocodeAuth::create_dummy_chatgpt_auth_for_testing())
            .with_config({
    ...
    .await?;
    let override_brocode = override_harness.test().brocode.clone();
    ...
    override_brocode
        ...
        .await?;
    wait_for_event(&override_brocode, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    override_brocode.submit(Op::Compact).await?;
    wait_for_event(&override_brocode, |event| {
        matches!(event, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn remote_compact_persists_replacement_history_in_rollout() -> Result<()> {
    ...
    brocode.submit(Op::Shutdown).await?;
...
async fn remote_compact_and_resume_refresh_stale_developer_instructions() -> Result<()> {
    ...
    initial.brocode.submit(Op::Compact).await?;
    wait_for_event(&initial.brocode, |ev| {
        matches!(ev, EventMsg::TurnComplete(_))
    })
    .await;
    ...
    initial.brocode.submit(Op::Shutdown).await?;
    wait_for_event(&initial.brocode, |ev| {
    ...
    resumed
        ...
        .await?;
    wait_for_event(&resumed.brocode, |ev| {
        matches!(ev, EventMsg::TurnComplete(_))
    })
    .await;
...
async fn remote_compact_refreshes_stale_developer_instructions_without_resume() -> Result<()> {
    ...
    test.brocode
        ...
        .await?;
    wait_for_event(&test.brocode, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    test.brocode.submit(Op::Compact).await?;
    wait_for_event(&test.brocode, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn snapshot_request_shape_remote_pre_turn_compaction_restates_realtime_start() -> Result<()> {
    ...
    let mut builder =
        remote_realtime_test_brocode_builder(&realtime_server).with_config(|config| {
    ...
    start_realtime_conversation(test.brocode.as_ref()).await?;
    ...
    close_realtime_conversation(test.brocode.as_ref()).await?;
...
async fn remote_request_uses_custom_experimental_realtime_start_instructions() -> Result<()> {
    ...
    let mut builder = remote_realtime_test_brocode_builder(&realtime_server).with_config({
...
async fn snapshot_request_shape_remote_manual_compact_restates_realtime_start() -> Result<()> {
    ...
    let server = wiremock::MockServer::start().await;
    let realtime_server = start_remote_realtime_server().await;
    let mut builder = remote_realtime_test_brocode_builder(&realtime_server);
    let test = builder.build(&server).await?;
...
async fn snapshot_request_shape_remote_compact_resume_restates_realtime_end() -> Result<()> {
    ...
    start_realtime_conversation(initial.brocode.as_ref()).await?;
    ...
    close_realtime_conversation(initial.brocode.as_ref()).await?;
