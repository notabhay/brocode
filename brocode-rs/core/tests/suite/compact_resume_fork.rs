...
//! request payload that Brocode would send to the model and assert that the
...
fn network_disabled() -> bool {
    std::env::var(BROCODE_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok()
}
...
async fn compact_resume_and_fork_preserve_model_history_view() {
    ...
    let expected_model = "gpt-5.1-brocode";
...
async fn snapshot_rollback_followup_turn_trims_context_updates() -> Result<()> {
    ...
    const MODEL: &str = "gpt-5.1-brocode";
...
async fn start_test_conversation(
    server: &MockServer,
    model: Option<&str>,
) -> (Arc<TempDir>, Config, Arc<ThreadManager>, Arc<BrocodeThread>) {
    ...
    (test.home, test.config, test.thread_manager, test.brocode)
}

async fn user_turn(conversation: &Arc<BrocodeThread>, text: &str) {
    conversation
        .submit(Op::UserInput {
...
async fn compact_conversation(conversation: &Arc<BrocodeThread>) {
    conversation
        .submit(Op::Compact)
        .await
...
fn fetch_conversation_path(conversation: &Arc<BrocodeThread>) -> std::path::PathBuf {
...
async fn resume_conversation(
    ...
    let auth_manager = brocode_core::test_support::auth_manager_from_auth(
        brocode_core::BrocodeAuth::from_api_key("dummy"),
    );
