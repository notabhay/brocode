...
) -> Result<(TestBrocode, ResponseMock)> {
    let mut builder = test_brocode()
        .with_model("test-gpt-5.1-brocode")
...
async fn code_mode_can_call_hidden_dynamic_tools() -> Result<()> {
    ...
    test.brocode = new_thread.thread;
...
"#;
    ...
    let turn_id = wait_for_event_match(&test.brocode, |event| match event {
        EventMsg::TurnStarted(event) => Some(event.turn_id.clone()),
        _ => None,
    })
    .await;
    let request = wait_for_event_match(&test.brocode, |event| match event {
    ...
    test.brocode
        ...
        .await?;
    wait_for_event(&test.brocode, |event| match event {
