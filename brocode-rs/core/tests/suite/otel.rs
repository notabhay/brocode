...
fn extract_log_field_handles_empty_bare_values() {
    let line = "event.name=\"brocode.tool_result\" mcp_server= mcp_server_origin=";
...
fn extract_log_field_does_not_confuse_similar_keys() {
    let line = "event.name=\"brocode.tool_result\" mcp_server_origin=stdio";
...
async fn responses_api_emits_api_request_event() {
    ...
    let TestBrocode { brocode, .. } = test_brocode().build(&server).await.unwrap();
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| line.contains("brocode.api_request"))
            .map(|_| Ok(()))
            .unwrap_or_else(|| Err("expected brocode.api_request event".to_string()))
    });

    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| line.contains("brocode.conversation_starts"))
            .map(|_| Ok(()))
            .unwrap_or_else(|| Err("expected brocode.conversation_starts event".to_string()))
    });
}
...
async fn process_sse_emits_tracing_for_output_item() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| {
                line.contains("brocode.sse_event")
...
async fn process_sse_emits_failed_event_on_parse_error() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            ...
            .map(|_| Ok(()))
            .unwrap_or(Err("missing brocode.sse_event".to_string()))
    });
}
...
async fn process_sse_failed_event_logs_parse_error() {
    ...
    logs_assert(|lines: &[&str]| {
        lines
            .iter()
            .find(|line| {
                line.contains("brocode.sse_event") && line.contains("event.kind=response.failed")
            })
            .map(|_| Ok(()))
            .unwrap_or(Err("missing brocode.sse_event".to_string()))
    });
...
async fn handle_response_item_records_tool_result_for_custom_tool_call() {
    ...
    wait_for_event(&brocode, |ev| matches!(ev, EventMsg::TokenCount(_))).await;

    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("brocode.tool_result") && line.contains("call_id=custom-tool-call")
            })
            .ok_or_else(|| "missing brocode.tool_result event".to_string())?;
...
async fn handle_response_item_records_tool_result_for_function_call() {
    ...
    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("brocode.tool_result") && line.contains("call_id=function-call")
            })
            .ok_or_else(|| "missing brocode.tool_result event".to_string())?;
...
async fn handle_response_item_records_tool_result_for_local_shell_missing_ids() {
    ...
    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("brocode.tool_result")
...
async fn handle_response_item_records_tool_result_for_local_shell_call() {
    ...
    logs_assert(|lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("brocode.tool_result") && line.contains("call_id=shell-call")
            })
            .ok_or_else(|| "missing brocode.tool_result event".to_string())?;
...
fn tool_decision_assertion<'a>(
    ...
    move |lines: &[&str]| {
        let line = lines
            .iter()
            .find(|line| {
                line.contains("brocode.tool_decision")
                    && line.contains(&format!("call_id={call_id}"))
            })
            .ok_or_else(|| format!("missing brocode.tool_decision event for {call_id}"))?;
...
async fn handle_container_exec_user_approved_records_tool_decision() {
    ...
    let approval_event = wait_for_event(&brocode, |ev| {
