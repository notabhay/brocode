...
use brocode_core::REVIEW_PROMPT;
use brocode_core::config::Config;
use brocode_core::review_format::render_review_output_text;
use brocode_protocol::models::ContentItem;
use brocode_protocol::models::ResponseItem;
use brocode_protocol::protocol::ENVIRONMENT_CONTEXT_OPEN_TAG;
...
use brocode_protocol::protocol::ReviewCodeLocation;
use brocode_protocol::protocol::ReviewFinding;
use brocode_protocol::protocol::ReviewLineRange;
...
async fn review_op_emits_lifecycle_and_review_output() {
    // Skip under Brocode sandbox network restrictions.
    skip_if_no_network!();
    ...
    let _entered =
        wait_for_event(&brocode, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let closed = wait_for_event(&brocode, |ev| matches!(ev, EventMsg::ExitedReviewMode(_))).await;
    ...
    let _complete = wait_for_event(&brocode, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;

    // Also verify that a user message with the header and a formatted finding
    // was recorded back in the parent session's rollout.
    let path = brocode.rollout_path().expect("rollout path");
...
async fn review_filters_agent_message_related_events() {
    ...
    wait_for_event(&brocode, |event| match event {
...
async fn review_uses_custom_review_model_from_config() {
    ...
    let _entered =
        wait_for_event(&brocode, |ev| matches!(ev, EventMsg::EnteredReviewMode(_))).await;
    let _closed = wait_for_event(&brocode, |ev| {
        matches!(
            ev,
...
async fn review_input_isolated_from_parent_history() {
    ...
    let session_file = brocode_home.path().join("resume.jsonl");
    {
        ...
        let user = brocode_protocol::models::ResponseItem::Message {
            id: None,
            role: "user".to_string(),
        ...
        let assistant = brocode_protocol::models::ResponseItem::Message {
            id: None,
    ...
    let brocode =
        resume_conversation_for_server(&server, brocode_home.clone(), session_file.clone(), |_| {})
            .await;
...
async fn review_uses_overridden_cwd_for_base_branch_merge_base() {
    ...
    let brocode = new_conversation_for_server(&server, brocode_home.clone(), move |config| {
    ...
    F: FnOnce(&mut Config) + Send + 'static,
{
    ...
    builder
        .resume(server, brocode_home, resume_path)
        .await
