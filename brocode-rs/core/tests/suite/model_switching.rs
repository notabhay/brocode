...
use brocode_core::config::types::Personality;
...
async fn service_tier_change_is_applied_on_next_http_turn() -> Result<()> {
    ...
    let test = test_brocode().build(&server).await?;
...
async fn model_change_from_image_to_text_strips_prior_image_content() -> Result<()> {
    ...
    assert!(
        second_user_texts
            .iter()
            .any(|text| text == &brocode_protocol::models::image_open_tag_text()),
        "second request should preserve the image open tag text"
    );
    assert!(
        second_user_texts
            .iter()
            .any(|text| text == &brocode_protocol::models::image_close_tag_text()),
...
async fn thread_rollback_after_generated_image_drops_entire_image_turn_history() -> Result<()> {
    ...
    test.brocode
        ...
        .await?;
    wait_for_event(&test.brocode, |ev| {
...
async fn model_switch_to_smaller_model_updates_token_context_window() -> Result<()> {
    ...
    let large_window_event = wait_for_event(&test.brocode, |event| {
        matches!(
            event,
    ...
    let smaller_turn_started_event = wait_for_event(&test.brocode, |event| {
        matches!(
            event,
    ...
    let smaller_window_event = wait_for_event(&test.brocode, |event| {
        matches!(
            event,
            EventMsg::TokenCount(token_count)
                if token_count
