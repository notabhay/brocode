...
use brocode_protocol::openai_models::ModelInstructionsVariables;
use brocode_protocol::openai_models::ModelMessages;
...
async fn personality_does_not_mutate_base_instructions_without_template() {
    ...
    let model_info = brocode_core::test_support::construct_model_info_offline("gpt-5.1", &config);
    assert_eq!(
...
async fn remote_model_friendly_personality_instructions_with_feature() -> anyhow::Result<()> {
    ...
    let remote_slug = "brocode-remote-default-personality";
    ...
    let mut builder = test_brocode()
        .with_auth(brocode_core::BrocodeAuth::create_dummy_chatgpt_auth_for_testing())
        .with_config(|config| {
            config
                .features
                .enable(Feature::Personality)
...
async fn user_turn_personality_remote_model_template_includes_update_message() -> anyhow::Result<()>
{
    ...
    let remote_slug = "brocode-remote-personality";
