...
async fn build_plugin_test_brocode(
    server: &MockServer,
    brocode_home: Arc<TempDir>,
) -> Result<Arc<brocode_core::BrocodeThread>> {
...
async fn build_analytics_plugin_test_brocode(
    server: &MockServer,
    brocode_home: Arc<TempDir>,
) -> Result<Arc<brocode_core::BrocodeThread>> {
...
async fn build_apps_enabled_plugin_test_brocode(
    server: &MockServer,
    brocode_home: Arc<TempDir>,
...
async fn capability_sections_render_in_developer_message_in_order() -> Result<()> {
    ...
    let brocode_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(brocode_home.as_ref());
    write_plugin_app_plugin(brocode_home.as_ref());
    let brocode = build_apps_enabled_plugin_test_brocode(
        &server,
        Arc::clone(&brocode_home),
    ...
    brocode
        .submit(Op::UserInput {
            items: vec![brocode_protocol::user_input::UserInput::Text {
                text: "hello".into(),
...
async fn explicit_plugin_mentions_inject_plugin_guidance() -> Result<()> {
    ...
    write_plugin_skill_plugin(brocode_home.as_ref());
    write_plugin_mcp_plugin(brocode_home.as_ref(), &rmcp_test_server_bin);
    write_plugin_app_plugin(brocode_home.as_ref());

    let brocode =
        build_apps_enabled_plugin_test_brocode(&server, brocode_home, apps_server.chatgpt_base_url)
            .await?;

    brocode
        .submit(Op::UserInput {
            items: vec![brocode_protocol::user_input::UserInput::Mention {
                name: "sample".into(),
    ...
    assert!(
        request_tools
            .iter()
            .any(|name| name == "mcp__brocode_apps__google_calendar_create_event"),
    ...
    let calendar_description = tool_description(
        &request_body,
        "mcp__brocode_apps__google_calendar_create_event",
    )
...
async fn explicit_plugin_mentions_track_plugin_used_analytics() -> Result<()> {
    ...
    let brocode_home = Arc::new(TempDir::new()?);
    write_plugin_skill_plugin(brocode_home.as_ref());
    let brocode = build_analytics_plugin_test_brocode(&server, brocode_home).await?;
    ...
    let analytics_request = loop {
        ...
        if let Some(request) = requests
            .into_iter()
            .find(|request| request.url.path() == "/brocode/analytics-events/events")
        {
            break request;
        }
    ...
    assert_eq!(event["event_type"], "brocode_plugin_used");
    ...
    assert_eq!(
        ...
        serde_json::json!(brocode_core::default_client::originator().value)
    );
...
async fn plugin_mcp_tools_are_listed() -> Result<()> {
    ...
    write_plugin_mcp_plugin(brocode_home.as_ref(), &rmcp_test_server_bin);
    let brocode = build_plugin_test_brocode(&server, brocode_home).await?;

    let tools_ready_deadline = Instant::now() + Duration::from_secs(30);
    loop {
        brocode.submit(Op::ListMcpTools).await?;
