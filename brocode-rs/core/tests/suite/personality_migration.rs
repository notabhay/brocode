...
use brocode_core::SESSIONS_SUBDIR;
use brocode_core::config::ConfigToml;
use brocode_core::personality_migration::PERSONALITY_MIGRATION_FILENAME;
use brocode_core::personality_migration::PersonalityMigrationStatus;
use brocode_core::personality_migration::maybe_migrate_personality;
...
async fn read_config_toml(brocode_home: &Path) -> io::Result<ConfigToml> {
    let contents = tokio::fs::read_to_string(brocode_home.join("config.toml")).await?;
    toml::from_str(&contents).map_err(|err| io::Error::new(io::ErrorKind::InvalidData, err))
}

async fn write_session_with_user_event(brocode_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = brocode_home
...
async fn write_archived_session_with_user_event(brocode_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = brocode_home.join(ARCHIVED_SESSIONS_SUBDIR);
    write_rollout_with_user_event(&dir, thread_id).await
}

async fn write_session_with_meta_only(brocode_home: &Path) -> io::Result<()> {
    let thread_id = ThreadId::new();
    let dir = brocode_home
        .join(SESSIONS_SUBDIR)
        .join("2025")
...
async fn no_marker_sessions_preserves_existing_config_fields() -> io::Result<()> {
    ...
    assert_eq!(status, PersonalityMigrationStatus::Applied);
    let persisted = read_config_toml(temp.path()).await?;
    assert_eq!(persisted.model, Some("gpt-5-brocode".to_string()));
    assert_eq!(persisted.personality, Some(Personality::Pragmatic));
    Ok(())
}
