...
use brocode_core::exec::ExecToolCallOutput;
...
use brocode_sandboxing::get_platform_sandbox;
use tempfile::TempDir;

use brocode_core::error::Result;

fn skip_test() -> bool {
    if std::env::var(BROCODE_SANDBOX_ENV_VAR) == Ok("seatbelt".to_string()) {
        eprintln!("{BROCODE_SANDBOX_ENV_VAR} is set to 'seatbelt', skipping test.");
        return true;
    }
