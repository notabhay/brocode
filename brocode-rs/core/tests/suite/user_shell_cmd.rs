...
async fn user_shell_cmd_ls_and_cat_in_temp_dir() {
    ...
    brocode
        ...
        .await
        .unwrap();
    let msg = wait_for_event(&brocode, |ev| matches!(ev, EventMsg::ExecCommandEnd(_))).await;
...
async fn user_shell_cmd_can_be_interrupted() {
    ...
    let _begin = wait_for_event_match(brocode, |ev| match ev {
    ...
    .await;
    brocode.submit(Op::Interrupt).await.unwrap();
...
async fn user_shell_command_does_not_replace_active_turn() -> anyhow::Result<()> {
    ...
    let _ = wait_for_event_match(&fixture.brocode, |ev| match ev {
    ...
    for _ in 0..200 {
        let event = timeout(Duration::from_secs(20), fixture.brocode.next_event())
            .await
...
async fn user_shell_command_history_is_persisted_and_shared_with_model() -> anyhow::Result<()> {
    ...
    let mut builder = core_test_support::test_brocode::test_brocode().with_config(move |config| {
        config
            .features
    ...
    #[cfg(windows)]
    let command = r#"$val = $env:BROCODE_SANDBOX; if ([string]::IsNullOrEmpty($val)) { $val = 'not-set' } ; [System.Console]::Write($val)"#.to_string();
    #[cfg(not(windows))]
    let command = r#"sh -c "printf '%s' \"${BROCODE_SANDBOX:-not-set}\"""#.to_string();
    ...
    let begin_event = wait_for_event_match(&test.brocode, |ev| match ev {
    ...
    let delta_event = wait_for_event_match(&test.brocode, |ev| match ev {
    ...
    let end_event = wait_for_event_match(&test.brocode, |ev| match ev {
    ...
    let _ = wait_for_event(&test.brocode, |ev| matches!(ev, EventMsg::TurnComplete(_))).await;
...
async fn user_shell_command_does_not_set_network_sandbox_env_var() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let mut builder = core_test_support::test_brocode::test_brocode().with_config(|config| {
        config.permissions.network_sandbox_policy = NetworkSandboxPolicy::Restricted;
    });
    let test = builder.build(&server).await?;

    #[cfg(windows)]
    let command = r#"$val = $env:BROCODE_SANDBOX_NETWORK_DISABLED; if ([string]::IsNullOrEmpty($val)) { $val = 'not-set' } ; [System.Console]::Write($val)"#.to_string();
    #[cfg(not(windows))]
    let command =
        r#"sh -c "printf '%s' \"${BROCODE_SANDBOX_NETWORK_DISABLED:-not-set}\"""#.to_string();
...
async fn user_shell_command_output_is_truncated_in_history() -> anyhow::Result<()> {
    let server = responses::start_mock_server().await;
    let builder = core_test_support::test_brocode::test_brocode();
    let test = builder
        .with_config(|config| {
