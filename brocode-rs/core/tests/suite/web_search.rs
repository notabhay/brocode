...
async fn web_search_mode_cached_sets_external_web_access_false() {
    ...
    let test = builder
        .build(&server)
        .await
        .expect("create test Brocode conversation");
...
async fn web_search_tool_config_from_config_toml_is_forwarded_to_request() {
    ...
    let home = Arc::new(tempfile::TempDir::new().expect("create brocode home"));
    std::fs::write(
...
"#,
    ...
    let mut builder = test_brocode().with_model("gpt-5-brocode").with_home(home);
    let test = builder
        .build(&server)
        .await
        .expect("create test Brocode conversation");
