...
use brocode_core::EventPersistenceMode;
use brocode_core::RolloutRecorder;
use brocode_core::RolloutRecorderParams;
...
fn write_minimal_rollout_with_id_in_subdir(brocode_home: &Path, subdir: &str, id: Uuid) -> PathBuf {
    let sessions = brocode_home.join(subdir).join("2024/01/01");
...
/// provided conversation id in the SessionMeta line. Returns the absolute path.
fn write_minimal_rollout_with_id(brocode_home: &Path, id: Uuid) -> PathBuf {
    write_minimal_rollout_with_id_in_subdir(brocode_home, "sessions", id)
}

async fn upsert_thread_metadata(brocode_home: &Path, thread_id: ThreadId, rollout_path: PathBuf) {
    let runtime = StateRuntime::init(brocode_home.to_path_buf(), "test-provider".to_string())
        .await
        .unwrap();
    ...
    builder.cwd = brocode_home.to_path_buf();
...
#[tokio::test]
async fn find_handles_gitignore_covering_brocode_home_directory() {
    let repo = TempDir::new().unwrap();
    let brocode_home = repo.path().join(".brocode");
    std::fs::create_dir_all(&brocode_home).unwrap();
    std::fs::write(repo.path().join(".gitignore"), ".brocode/**\n").unwrap();
    let id = Uuid::new_v4();
    let expected = write_minimal_rollout_with_id(&brocode_home, id);

    let found = find_thread_path_by_id_str(&brocode_home, &id.to_string())
        .await
        .unwrap();
