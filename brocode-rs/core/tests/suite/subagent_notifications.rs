...
use brocode_core::config::AgentRoleConfig;
...
async fn wait_for_spawned_thread_id(test: &TestBrocode) -> Result<String> {
...
async fn setup_turn_one_with_custom_spawned_child(
    ...
    let mut builder = configure_test(test_brocode().with_config(|config| {
        config
            .features
    ...
    let child_snapshot = spawn_child_and_capture_snapshot(
        ...
        |builder| {
            builder.with_config(|config| {
                let role_path = config.brocode_home.join("custom-role.toml");
                std::fs::write(
                    &role_path,
                    format!(
