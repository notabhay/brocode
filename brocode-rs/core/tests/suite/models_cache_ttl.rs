...
const REMOTE_MODEL: &str = "brocode-test-ttl";
const VERSIONED_MODEL: &str = "brocode-test-versioned";
const MISSING_VERSION_MODEL: &str = "brocode-test-missing-version";
const DIFFERENT_VERSION_MODEL: &str = "brocode-test-different-version";
...
async fn renews_cache_ttl_on_matching_models_etag() -> Result<()> {
    ...
    let brocode = Arc::clone(&test.brocode);
    ...
    let cache_path = config.brocode_home.join(CACHE_FILE);
    ...
    let _ = wait_for_event(&brocode, |event| matches!(event, EventMsg::TurnComplete(_))).await;
    ...
    builder = builder
        .with_pre_build_hook(move |home| {
            let cache = ModelsCache {
                fetched_at: Utc::now(),
                etag: None,
                client_version: Some(brocode_core::models_manager::client_version_to_whole()),
...
async fn refreshes_when_cache_version_differs() -> Result<()> {
    ...
    builder = builder
        .with_pre_build_hook(move |home| {
            let client_version = brocode_core::models_manager::client_version_to_whole();
            let cache = ModelsCache {
                fetched_at: Utc::now(),
                etag: None,
