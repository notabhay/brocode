...
async fn brocode_returns_json_result_for_gpt5() -> anyhow::Result<()> {
    brocode_returns_json_result("gpt-5.1".to_string()).await
}
...
async fn brocode_returns_json_result(model: String) -> anyhow::Result<()> {
    ...
    let match_json_text_param = move |req: &wiremock::Request| {
        ...
        format.get("name") == Some(&serde_json::Value::String("brocode_output_schema".into()))
    ...
    let TestBrocode { brocode, cwd, .. } = test_brocode().build(&server).await?;
    ...
    let message = wait_for_event(&brocode, |ev| matches!(ev, EventMsg::AgentMessage(_))).await;
