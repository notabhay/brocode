...
const CALENDAR_CREATE_TOOL: &str = "mcp__brocode_apps__calendar_create_event";
const CALENDAR_LIST_TOOL: &str = "mcp__brocode_apps__calendar_list_events";
const SEARCH_CALENDAR_NAMESPACE: &str = "mcp__brocode_apps__calendar";
...
fn configured_builder(apps_base_url: String) -> TestBrocodeBuilder {
    test_brocode()
...
async fn tool_search_returns_deferred_tools_without_follow_up_tool_injection() -> Result<()> {
    ...
    let EventMsg::McpToolCallEnd(end) = wait_for_event(&test.brocode, |event| {
    ...
    let apps_tool_call = server
        ...
        .find_map(|request| {
            ...
            (request.url.path() == "/api/brocode/apps"
    ...
    assert_eq!(
        apps_tool_call.pointer("/params/_meta/_brocode_apps"),
        Some(&json!({
            "resource_uri": CALENDAR_CREATE_EVENT_RESOURCE_URI,
            "contains_mcp_source": true,
            "connector_id": "calendar",
    ...
    assert_eq!(
        apps_tool_call.pointer("/params/_meta/x-brocode-turn-metadata/session_id"),
        Some(&json!(test.session_configured.session_id.to_string()))
    );
    assert!(
        apps_tool_call
            .pointer("/params/_meta/x-brocode-turn-metadata/turn_id")
            .and_then(Value::as_str)
