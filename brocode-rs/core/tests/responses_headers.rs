...
async fn responses_stream_includes_subagent_header_on_review() {
    ...
    let brocode_home = TempDir::new().expect("failed to create TempDir");
    ...
    assert_eq!(request.header("x-brocode-sandbox"), None);
}
...
async fn responses_respects_model_info_overrides_from_config() {
    ...
    let conversation_id = ThreadId::new();
    let auth_mode = brocode_core::test_support::auth_manager_from_auth(BrocodeAuth::from_api_key(
        "Test API Key",
    ))
    .auth_mode()
...
async fn responses_stream_includes_turn_metadata_header_for_git_workspace_e2e() {
    ...
    let initial_header = first_request
        .single_request()
        .header("x-brocode-turn-metadata")
        .expect("x-brocode-turn-metadata header should be present");
    let initial_parsed: serde_json::Value = serde_json::from_str(&initial_header)
        .expect("x-brocode-turn-metadata should be valid JSON");
    ...
    assert!(
        ...
        "turn_id should not be empty in x-brocode-turn-metadata"
    );
    assert_eq!(
        initial_parsed
            .get("sandbox")
    ...
    run_git(&[
        ...
        "add",
        "origin",
        "https://github.com/openai/brocode.git",
    ]);
