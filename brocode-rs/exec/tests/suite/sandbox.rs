#![cfg(unix)]
use brocode_core::spawn::StdioPolicy;
...
async fn spawn_command_under_sandbox(
    ...
    use brocode_core::seatbelt::spawn_command_under_seatbelt;
...
async fn spawn_command_under_sandbox(
    ...
    use brocode_core::landlock::spawn_command_under_linux_sandbox;
    let brocode_linux_sandbox_exe = core_test_support::find_brocode_linux_sandbox_exe()
...
#[tokio::test]
async fn sandbox_blocks_first_time_dot_brocode_creation() {
    core_test_support::skip_if_sandbox!();
    #[cfg(target_os = "linux")]
    let sandbox_env = match linux_sandbox_test_env().await {
        Some(env) => env,
    ...
    let mut child = spawn_command_under_sandbox(
        vec![
            "bash".to_string(),
            "-lc".to_string(),
            "mkdir -p .brocode && echo 'sandbox_mode = \"danger-full-access\"' > .brocode/config.toml"
                .to_string(),
        ],
        repo_root.clone(),
        &policy,
    ...
    .await
    .expect("should spawn command creating .brocode");

    let status = child
        .wait()
        .await
        .expect("should wait for .brocode command");
    assert!(
        !status.success(),
        "sandbox unexpectedly allowed first-time .brocode creation: {status:?}"
    );
    let dot_brocode_metadata = tokio::fs::symlink_metadata(&dot_brocode).await;
    if let Ok(metadata) = dot_brocode_metadata {
        assert!(
            !metadata.is_dir(),
            "{} should not be creatable as a directory",
            dot_brocode.display()
        );
    } else if let Err(err) = &dot_brocode_metadata {
        assert_eq!(
            err.kind(),
