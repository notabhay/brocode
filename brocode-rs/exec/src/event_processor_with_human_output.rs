...
use brocode_protocol::num_format::format_with_separators;
    ...
    fn render_item_completed(&mut self, item: ThreadItem) {
        match item {
            ThreadItem::AgentMessage { text, .. } => {
                eprintln!(
                    "{}\n{}",
                    "brocode".style(self.italic).style(self.magenta),
                    text
                );
...
impl EventProcessor for EventProcessorWithHumanOutput {
    fn print_config_summary(
        ...
        eprintln!("OpenAI Brocode v{VERSION} (research preview)\n--------");
    ...
    fn process_server_notification(&mut self, notification: ServerNotification) -> BrocodeStatus {
        match notification {
            ...
            ServerNotification::TurnPlanUpdated(notification) => {
                ...
                for step in notification.plan {
                    match step.status {
                        ...
                        brocode_app_server_protocol::TurnPlanStepStatus::InProgress => {
                            eprintln!("  {} {}", "→".style(self.cyan), step.step);
                        }
                        brocode_app_server_protocol::TurnPlanStepStatus::Pending => {
                            eprintln!(
                                "  {} {}",
                                "•".style(self.dimmed),
            ...
            ServerNotification::TurnStarted(_) => BrocodeStatus::Running,
...
fn summarize_sandbox_policy(sandbox_policy: &SandboxPolicy) -> String {
    match sandbox_policy {
        ...
        SandboxPolicy::ExternalSandbox { network_access } => {
            ...
            if matches!(
                network_access,
                brocode_protocol::protocol::NetworkAccess::Enabled
            ) {
                summary.push_str(" (network access enabled)");
            }
            summary
...
mod tests {
    ...
    fn turn_completed_recovers_final_message_from_turn_items() {
        ...
        assert_eq!(
            status,
            crate::event_processor::BrocodeStatus::InitiateShutdown
        );
