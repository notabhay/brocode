...
use brocode_app_server_client::InProcessServerEvent;
...
use brocode_app_server_protocol::Thread as AppServerThread;
...
use brocode_cloud_requirements::cloud_requirements_loader_for_storage;
use brocode_core::LMSTUDIO_OSS_PROVIDER_ID;
use brocode_core::OLLAMA_OSS_PROVIDER_ID;
use brocode_core::auth::AuthConfig;
use brocode_core::auth::enforce_login_restrictions;
use brocode_core::check_execpolicy_for_warnings;
...
use brocode_core::config::load_config_as_toml_with_cli_overrides;
use brocode_core::config::resolve_oss_provider;
use brocode_core::config_loader::ConfigLoadError;
use brocode_core::config_loader::LoaderOverrides;
use brocode_core::config_loader::format_config_error_with_source;
use brocode_core::format_exec_policy_error_with_source;
use brocode_core::path_utils;
use brocode_feedback::BrocodeFeedback;
use brocode_git_utils::get_git_repo_root;
use brocode_otel::set_parent_from_context;
use brocode_otel::traceparent_context_from_env;
use brocode_protocol::config_types::SandboxMode;
...
use brocode_utils_oss::ensure_oss_provider_ready;
use brocode_utils_oss::get_default_model_for_oss_provider;
pub use cli::Cli;
pub use cli::Command;
...
use crate::event_processor::BrocodeStatus;
use crate::event_processor::EventProcessor;
use brocode_core::default_client::set_default_client_residency_requirement;
use brocode_core::default_client::set_default_originator;
...
enum StdinPromptBehavior {
    ...
    /// `brocode exec` behavior for `brocode exec` with piped input.
    RequiredIfPiped,
    /// Always treat stdin as the prompt, used for the explicit `brocode exec -`
...
pub async fn run_main(cli: Cli, arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    if let Err(err) = set_default_originator("brocode_exec".to_string()) {
        tracing::warn!(
            ?err,
            "Failed to set brocode exec originator override {err:?}"
        );
    }
    ...
    #[allow(clippy::print_stderr)]
    let brocode_home = match find_brocode_home() {
        Ok(brocode_home) => brocode_home,
        Err(err) => {
            eprintln!("Error finding brocode home: {err}");
            std::process::exit(1);
        }
    };
    ...
    let overrides = ConfigOverrides {
        ...
        service_tier: None,
        brocode_self_exe: arg0_paths.brocode_self_exe.clone(),
        brocode_linux_sandbox_exe: arg0_paths.brocode_linux_sandbox_exe.clone(),
    ...
    let otel = match std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        brocode_core::otel_init::build_provider(
            &config,
    ...
    let in_process_start_args = InProcessClientStartArgs {
        ...
        feedback: BrocodeFeedback::new(),
        config_warnings,
        session_source: SessionSource::Exec,
        enable_brocode_api_key_env: true,
        client_name: "brocode-exec".to_string(),
...
async fn run_exec_session(args: ExecRunArgs) -> anyhow::Result<()> {
    ...
    let (initial_operation, prompt_summary) = match (command.as_ref(), prompt, images) {
        (Some(ExecCommand::Review(review_cli)), _, _) => {
            ...
            let summary = brocode_core::review_prompts::user_facing_hint(&review_request.target);
    ...
    // Print the effective configuration and initial request so users can see what Brocode
    // is using.
    event_processor.print_config_summary(&config, &prompt_summary, &session_configured);
    if !json_mode && let Some(message) = brocode_core::config::system_bwrap_warning() {
        event_processor.process_warning(message);
    }

    info!("Brocode initialized with event: {session_configured:?}");
        ...
        match server_event {
            ...
            InProcessServerEvent::ServerNotification(mut notification) => {
                if let ServerNotification::Error(payload) = &notification {
                    ...
                    && matches!(
                        payload.turn.status,
                        brocode_app_server_protocol::TurnStatus::Failed
                            | brocode_app_server_protocol::TurnStatus::Interrupted
                    )
                {
                    error_seen = true;
                }
                ...
                if should_process_notification(
                    ...
                    match event_processor.process_server_notification(notification) {
                        BrocodeStatus::Running => {}
                        BrocodeStatus::InitiateShutdown => {
...
fn sandbox_mode_from_policy(
    ...
    match sandbox_policy {
        ...
        brocode_protocol::protocol::SandboxPolicy::ReadOnly { .. } => {
            Some(brocode_app_server_protocol::SandboxMode::ReadOnly)
        }
        brocode_protocol::protocol::SandboxPolicy::WorkspaceWrite { .. } => {
            Some(brocode_app_server_protocol::SandboxMode::WorkspaceWrite)
        }
        brocode_protocol::protocol::SandboxPolicy::ExternalSandbox { .. } => None,
    }
}
...
fn session_configured_from_thread_response(
    ...
    let session_id = brocode_protocol::ThreadId::from_string(thread_id)
...
mod tests {
    ...
    fn test_tracing_subscriber() -> impl tracing::Subscriber + Send + Sync {
        ...
        let tracer = provider.tracer("brocode-exec-tests");
    ...
    fn exec_root_span_can_be_parented_from_trace_context() {
        ...
        let parent = brocode_protocol::protocol::W3cTraceContext {
    ...
    async fn resume_lookup_model_providers_filters_only_last_lookup() {
        let brocode_home = tempdir().expect("create temp brocode home");
    ...
    fn turn_items_for_thread_returns_matching_turn_items() {
        let thread = AppServerThread {
            ...
            source: brocode_app_server_protocol::SessionSource::Exec,
            agent_nickname: None,
            agent_role: None,
            git_info: None,
            name: None,
            turns: vec![
                brocode_app_server_protocol::Turn {
                    id: "turn-1".to_string(),
                    ...
                    status: brocode_app_server_protocol::TurnStatus::Completed,
                    error: None,
                },
                brocode_app_server_protocol::Turn {
                    id: "turn-2".to_string(),
    ...
    async fn thread_start_params_include_review_policy_when_review_policy_is_manual_only() {
        ...
        assert_eq!(
            ...
            Some(brocode_app_server_protocol::ApprovalsReviewer::User)
        );
    }
    ...
    async fn thread_start_params_include_review_policy_when_auto_review_is_enabled() {
        ...
        assert_eq!(
            params.approvals_reviewer,
            Some(brocode_app_server_protocol::ApprovalsReviewer::GuardianSubagent)
        );
    }
    ...
    fn session_configured_from_thread_response_uses_review_policy_from_response() {
        let response = ThreadStartResponse {
            thread: brocode_app_server_protocol::Thread {
                ...
                source: brocode_app_server_protocol::SessionSource::Cli,
                agent_nickname: None,
                agent_role: None,
                git_info: None,
            ...
            service_tier: None,
            cwd: PathBuf::from("/tmp"),
            approval_policy: brocode_app_server_protocol::AskForApproval::OnRequest,
            approvals_reviewer: brocode_app_server_protocol::ApprovalsReviewer::GuardianSubagent,
            sandbox: brocode_app_server_protocol::SandboxPolicy::WorkspaceWrite {
                writable_roots: vec![],
                read_only_access: brocode_app_server_protocol::ReadOnlyAccess::FullAccess,
