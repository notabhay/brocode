...
impl OllamaClient {
    ...
    async fn try_from_provider_with_base_url(base_url: &str) -> io::Result<Self> {
        let provider = brocode_core::create_oss_provider_with_base_url(
            base_url,
            brocode_core::WireApi::Responses,
        );
        Self::try_from_provider(&provider).await
    }
...
mod tests {
    ...
    async fn test_fetch_models_happy_path() {
        if std::env::var(brocode_core::spawn::BROCODE_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
            tracing::info!(
