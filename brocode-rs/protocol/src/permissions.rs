...
pub enum FileSystemSpecialPath {
    ...
    /// representable so config from a newer Brocode degrades to warn-and-ignore
    /// instead of failing to load. Brocode 0.112.0 rejected unknown values here,
        ...
        if let SandboxPolicy::WorkspaceWrite { writable_roots, .. } = sandbox_policy {
            ...
            if let Ok(cwd_root) = AbsolutePathBuf::from_absolute_path(cwd) {
                for protected_path in default_read_only_subpaths_for_writable_root(
                    &cwd_root, /*protect_missing_dot_brocode*/ true,
                ) {
            ...
            for writable_root in writable_roots {
                for protected_path in default_read_only_subpaths_for_writable_root(
                    writable_root,
                    /*protect_missing_dot_brocode*/ false,
                ) {
                    append_default_read_only_path_if_no_explicit_rule(
                        &mut file_system_policy.entries,
                        protected_path,
    ...
    pub fn get_writable_roots_with_cwd(&self, cwd: &Path) -> Vec<WritableRoot> {
        ...
        .map(|root| {
            ...
            let raw_writable_roots: Vec<&AbsolutePathBuf> = writable_entries
                ...
                .collect();
            let protect_missing_dot_brocode = AbsolutePathBuf::from_absolute_path(cwd)
                .ok()
                .is_some_and(|cwd| normalize_effective_absolute_path(cwd) == root);
            let mut read_only_subpaths: Vec<AbsolutePathBuf> =
                default_read_only_subpaths_for_writable_root(&root, protect_missing_dot_brocode)
                    .into_iter()
            ...
            // Example: if `<root>/.brocode -> <root>/decoy`, bwrap must still see
            // `<root>/.brocode`, not only the resolved `<root>/decoy`.
            ...
            WritableRoot {
                ...
                // `.brocode` so downstream sandboxes can still detect and mask
...
mod tests {
    ...
    #[cfg(unix)]
    const SYMLINKED_TMPDIR_TEST_ENV: &str = "BROCODE_PROTOCOL_TEST_SYMLINKED_TMPDIR";
    ...
    #[cfg(unix)]
    #[test]
    fn writable_roots_proactively_protect_missing_dot_brocode() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let expected_dot_brocode = expected_root
            .join(".brocode")
            .expect("expected .brocode path");
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .contains(&expected_dot_brocode)
        );
    }

    #[cfg(unix)]
    #[test]
    fn writable_roots_skip_default_dot_brocode_when_explicit_user_rule_exists() {
        let cwd = TempDir::new().expect("tempdir");
        let expected_root = AbsolutePathBuf::from_absolute_path(
            cwd.path().canonicalize().expect("canonicalize cwd"),
        )
        .expect("absolute canonical root");
        let explicit_dot_brocode = expected_root
            .join(".brocode")
            .expect("expected .brocode path");
        ...
        let policy = FileSystemSandboxPolicy::restricted(vec![
            ...
            FileSystemSandboxEntry {
                path: FileSystemPath::Path {
                    path: explicit_dot_brocode.clone(),
                },
                access: FileSystemAccessMode::Write,
            },
        ]);
        ...
        assert!(
            !workspace_root
                .read_only_subpaths
                .contains(&explicit_dot_brocode),
            "explicit .brocode rule should win over the default protected carveout"
        );
        assert!(
            policy.can_write_path_with_cwd(
                explicit_dot_brocode
                    .join("config.toml")
                    .expect("config.toml")
                    .as_path(),
                cwd.path()
    ...
    #[test]
    fn legacy_workspace_write_projection_blocks_missing_dot_brocode_writes() {
        let cwd = TempDir::new().expect("tempdir");
        let dot_brocode_config = cwd.path().join(".brocode").join("config.toml");
        ...
        assert!(!file_system_policy.can_write_path_with_cwd(&dot_brocode_config, cwd.path()));
    }

    #[test]
    fn legacy_workspace_write_projection_accepts_relative_cwd() {
        let relative_cwd = Path::new("workspace");
        let expected_dot_brocode = AbsolutePathBuf::from_absolute_path(
            std::env::current_dir()
                .expect("current dir")
                .join(relative_cwd)
                .join(".brocode"),
        )
        .expect("absolute dot brocode");
        let policy = SandboxPolicy::WorkspaceWrite {
            writable_roots: vec![],
            read_only_access: ReadOnlyAccess::Restricted {
                include_platform_defaults: false,
        ...
        assert!(
            !file_system_policy
                .can_write_path_with_cwd(Path::new(".brocode/config.toml"), relative_cwd,)
        );
    }
    ...
    fn effective_runtime_roots_canonicalize_symlinked_paths() {
        ...
        let brocode_dir = real_root.join(".brocode");

        fs::create_dir_all(&blocked).expect("create blocked");
        fs::create_dir_all(&brocode_dir).expect("create .brocode");
        ...
        let expected_brocode = AbsolutePathBuf::from_absolute_path(
            brocode_dir.canonicalize().expect("canonicalize .brocode"),
        )
        .expect("absolute canonical .brocode");
    ...
    fn writable_roots_preserve_symlinked_protected_subpaths() {
        ...
        let decoy = root.join("decoy-brocode");
        let dot_brocode = root.join(".brocode");
        fs::create_dir_all(&decoy).expect("create decoy");
        symlink_dir(&decoy, &dot_brocode).expect("create .brocode symlink");
        ...
        .expect("absolute .brocode symlink");
    ...
    fn tmpdir_special_path_canonicalizes_symlinked_tmpdir() {
        ...
        let brocode_dir = real_tmpdir.join(".brocode");
