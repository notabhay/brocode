...
fn default_read_only_subpaths_for_writable_root(
    ...
    // Make .agents/skills and .brocode/config.toml and related files read-only
    // to the agent, by default.
    for subdir in &[".agents", ".brocode"] {
        #[allow(clippy::expect_used)]
        let top_level_brocode = writable_root.join(subdir).expect("valid relative path");
        if top_level_brocode.as_path().is_dir() {
