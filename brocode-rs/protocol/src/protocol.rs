//! Defines the protocol for a Brocode session between a client and an agent.
//!
...
pub const USER_MESSAGE_BEGIN: &str = "## My request for Brocode:";
...
pub enum Op {
    ...
    /// for a turn of a [`crate::brocode_thread::BrocodeThread`].
    UserTurn {
        /// User input items, see `InputItem`
        items: Vec<UserInput>,
    ...
    /// This is a local-only operation handled by brocode-core; it does not
    /// involve the model.
    SetThreadName { name: String },

    /// Request Brocode to undo a turn (turn are stacked so it is the same effect as CMD + Z).
    Undo,

    /// Request Brocode to drop the last N user turns from in-memory context.
    ///
    ...
    /// Request to shut down brocode instance.
    Shutdown,
...
/// running the command proposed by Brocode.
#[derive(
    Debug,
    Clone,
    Copy,
...
/// privileges of the agent (e.g. `.brocode`, `.git`, notably `.git/hooks`) under
...
fn default_read_only_subpaths_for_writable_root(
    ...
    protect_missing_dot_brocode: bool,
) -> Vec<AbsolutePathBuf> {
    ...
    // Keep top-level project metadata under .brocode read-only to the agent by
    ...
    #[allow(clippy::expect_used)]
    let top_level_brocode = writable_root.join(".brocode").expect("valid relative path");
    if protect_missing_dot_brocode || top_level_brocode.as_path().is_dir() {
        subpaths.push(top_level_brocode);
    }
...
/// Brocode errors that we expose to clients.
...
impl ErrorEvent {
    /// Whether this error should mark the current turn as failed when replaying history.
    pub fn affects_turn_status(&self) -> bool {
        self.brocode_error_info
            .as_ref()
            .is_none_or(BrocodeErrorInfo::affects_turn_status)
    }
}
...
impl SessionSource {
    ...
    pub fn restriction_product(&self) -> Option<Product> {
        match self {
            ...
            | SessionSource::Exec
            | SessionSource::Mcp
            | SessionSource::Unknown => Some(Product::Brocode),
    ...
    pub fn token_budget(&self) -> usize {
        match self {
            TruncationPolicy::Bytes(bytes) => {
                usize::try_from(brocode_utils_string::approx_tokens_from_byte_count(*bytes))
                    .unwrap_or(usize::MAX)
            }
    ...
    pub fn byte_budget(&self) -> usize {
        match self {
            ...
            TruncationPolicy::Tokens(tokens) => {
                brocode_utils_string::approx_bytes_for_tokens(*tokens)
            }
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn session_source_restriction_product_defaults_non_subagent_sources_to_brocode() {
        assert_eq!(
            SessionSource::Cli.restriction_product(),
            Some(Product::Brocode)
        );
        assert_eq!(
    ...
    fn session_source_restriction_product_maps_custom_sources_to_products() {
        ...
        assert_eq!(
            SessionSource::Custom("brocode".to_string()).restriction_product(),
            Some(Product::Brocode)
        );
        assert_eq!(
    ...
    fn session_source_matches_product_restriction() {
        ...
        assert!(
            !SessionSource::Custom("chatgpt".to_string())
                .matches_product_restriction(&[Product::Brocode])
        );
        assert!(SessionSource::VSCode.matches_product_restriction(&[Product::Brocode]));
        assert!(
    ...
    fn restricted_file_system_policy_derives_effective_paths() {
        ...
        std::fs::create_dir_all(cwd.path().join(".brocode")).expect("create .brocode");
        let canonical_cwd = cwd.path().canonicalize().expect("canonicalize cwd");
        let cwd_absolute =
        ...
        let expected_brocode = AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".brocode"))
            .expect("canonical .brocode");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert!(
            writable_roots[0]
                .read_only_subpaths
                .iter()
                .any(|path| path.as_path() == expected_brocode.as_path())
        );
    }
    ...
    fn restricted_file_system_policy_treats_read_entries_as_read_only_subpaths() {
        ...
        let expected_dot_brocode =
            AbsolutePathBuf::from_absolute_path(canonical_cwd.join(".brocode"))
                .expect("canonical .brocode");
        let policy = FileSystemSandboxPolicy::restricted(vec![
            FileSystemSandboxEntry {
                path: FileSystemPath::Special {
        ...
        assert_eq!(
            ...
            vec![
                (
                    canonical_cwd,
                    vec![
                        expected_dot_brocode.to_path_buf(),
    ...
    fn legacy_workspace_write_nested_readable_root_stays_writable() {
        ...
        assert_eq!(
            ...
            vec![(canonical_cwd, vec![expected_dot_brocode.to_path_buf()])]
        );
    }
