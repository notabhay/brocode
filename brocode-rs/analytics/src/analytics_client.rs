use brocode_git_utils::collect_git_info;
use brocode_git_utils::get_git_repo_root;
use brocode_login::AuthManager;
use brocode_login::default_client::create_client;
use brocode_login::default_client::originator;
use brocode_plugin::PluginTelemetryMetadata;
...
enum TrackEventRequest {
    ...
    AppMentioned(BrocodeAppMentionedEventRequest),
    AppUsed(BrocodeAppUsedEventRequest),
    PluginUsed(BrocodePluginUsedEventRequest),
    PluginInstalled(BrocodePluginEventRequest),
    PluginUninstalled(BrocodePluginEventRequest),
    PluginEnabled(BrocodePluginEventRequest),
    PluginDisabled(BrocodePluginEventRequest),
}
...
#[derive(Serialize)]
struct BrocodeAppMentionedEventRequest {
    event_type: &'static str,
    event_params: BrocodeAppMetadata,
}

#[derive(Serialize)]
struct BrocodeAppUsedEventRequest {
    event_type: &'static str,
    event_params: BrocodeAppMetadata,
}

#[derive(Serialize)]
struct BrocodePluginMetadata {
...
#[derive(Serialize)]
struct BrocodePluginUsedMetadata {
    #[serde(flatten)]
    plugin: BrocodePluginMetadata,
    thread_id: Option<String>,
    turn_id: Option<String>,
    model_slug: Option<String>,
}
...
#[derive(Serialize)]
struct BrocodePluginEventRequest {
    event_type: &'static str,
    event_params: BrocodePluginMetadata,
}

#[derive(Serialize)]
struct BrocodePluginUsedEventRequest {
    event_type: &'static str,
    event_params: BrocodePluginUsedMetadata,
}
...
async fn send_track_app_mentioned(
    ...
    let events = mentions
        .into_iter()
        .map(|mention| {
            let event_params = brocode_app_metadata(&tracking, mention);
            TrackEventRequest::AppMentioned(BrocodeAppMentionedEventRequest {
                event_type: "brocode_app_mentioned",
                event_params,
            })
        })
        .collect::<Vec<_>>();
...
async fn send_track_app_used(auth_manager: &AuthManager, base_url: &str, job: TrackAppUsedJob) {
    ...
    let event_params = brocode_app_metadata(&tracking, app);
    let events = vec![TrackEventRequest::AppUsed(BrocodeAppUsedEventRequest {
        event_type: "brocode_app_used",
        event_params,
    })];
...
async fn send_track_plugin_used(
    ...
    let events = vec![TrackEventRequest::PluginUsed(
        BrocodePluginUsedEventRequest {
            event_type: "brocode_plugin_used",
            event_params: brocode_plugin_used_metadata(&tracking, plugin),
        },
    )];
...
async fn send_track_plugin_installed(
    ...
    send_track_plugin_management_event(auth_manager, base_url, job, "brocode_plugin_installed")
        .await;
}
...
async fn send_track_plugin_uninstalled(
    ...
    send_track_plugin_management_event(auth_manager, base_url, job, "brocode_plugin_uninstalled")
        .await;
}
...
async fn send_track_plugin_enabled(
    ...
    send_track_plugin_management_event(auth_manager, base_url, job, "brocode_plugin_enabled").await;
}
...
async fn send_track_plugin_disabled(
    ...
    send_track_plugin_management_event(auth_manager, base_url, job, "brocode_plugin_disabled")
        .await;
}
...
async fn send_track_plugin_management_event(
    ...
    let event_params = brocode_plugin_metadata(plugin);
    let event = BrocodePluginEventRequest {
        event_type,
        event_params,
    };
    let events = vec![match event_type {
        "brocode_plugin_installed" => TrackEventRequest::PluginInstalled(event),
        "brocode_plugin_uninstalled" => TrackEventRequest::PluginUninstalled(event),
        "brocode_plugin_enabled" => TrackEventRequest::PluginEnabled(event),
        "brocode_plugin_disabled" => TrackEventRequest::PluginDisabled(event),
...
fn brocode_app_metadata(tracking: &TrackEventsContext, app: AppInvocation) -> BrocodeAppMetadata {
    BrocodeAppMetadata {
...
fn brocode_plugin_metadata(plugin: PluginTelemetryMetadata) -> BrocodePluginMetadata {
    let capability_summary = plugin.capability_summary;
    BrocodePluginMetadata {
...
fn brocode_plugin_used_metadata(
    tracking: &TrackEventsContext,
    plugin: PluginTelemetryMetadata,
) -> BrocodePluginUsedMetadata {
    BrocodePluginUsedMetadata {
        plugin: brocode_plugin_metadata(plugin),
        thread_id: Some(tracking.thread_id.clone()),
        turn_id: Some(tracking.turn_id.clone()),
        model_slug: Some(tracking.model_slug.clone()),
    }
...
async fn send_track_events(
    ...
    let url = format!("{base_url}/brocode/analytics-events/events");
