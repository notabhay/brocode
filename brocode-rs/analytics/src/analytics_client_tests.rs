...
use super::BrocodeAppMentionedEventRequest;
use super::BrocodeAppUsedEventRequest;
use super::BrocodePluginEventRequest;
use super::BrocodePluginUsedEventRequest;
...
use super::brocode_app_metadata;
use super::brocode_plugin_metadata;
use super::brocode_plugin_used_metadata;
...
fn normalize_path_for_skill_id_repo_scoped_uses_relative_path() {
    ...
    let skill_path = PathBuf::from("/repo/root/.brocode/skills/doc/SKILL.md");
    ...
    assert_eq!(path, ".brocode/skills/doc/SKILL.md");
}

#[test]
fn normalize_path_for_skill_id_user_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/Users/abc/.brocode/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_admin_scoped_uses_absolute_path() {
    let skill_path = PathBuf::from("/etc/brocode/skills/doc/SKILL.md");
...
fn normalize_path_for_skill_id_repo_root_not_in_skill_path_uses_absolute_path() {
    let repo_root = PathBuf::from("/repo/root");
    let skill_path = PathBuf::from("/other/path/.brocode/skills/doc/SKILL.md");
...
fn app_mentioned_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppMentioned(BrocodeAppMentionedEventRequest {
        event_type: "brocode_app_mentioned",
        event_params: brocode_app_metadata(
            &tracking,
            AppInvocation {
...
fn app_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::AppUsed(BrocodeAppUsedEventRequest {
...
fn plugin_used_event_serializes_expected_shape() {
    ...
    let event = TrackEventRequest::PluginUsed(BrocodePluginUsedEventRequest {
        event_type: "brocode_plugin_used",
        event_params: brocode_plugin_used_metadata(&tracking, sample_plugin_metadata()),
    });
...
fn plugin_management_event_serializes_expected_shape() {
    let event = TrackEventRequest::PluginInstalled(BrocodePluginEventRequest {
        event_type: "brocode_plugin_installed",
        event_params: brocode_plugin_metadata(sample_plugin_metadata()),
    });
        ...
        .ingest(
            AnalyticsFact::Custom(CustomAnalyticsFact::SkillInvoked(SkillInvokedInput {
                ...
                invocations: vec![SkillInvocation {
                    ...
                    skill_scope: brocode_protocol::protocol::SkillScope::User,
                    skill_path,
...
async fn reducer_ingests_app_and_plugin_facts() {
    ...
    assert_eq!(payload[0]["event_type"], "brocode_app_mentioned");
    assert_eq!(payload[1]["event_type"], "brocode_app_used");
    assert_eq!(payload[2]["event_type"], "brocode_plugin_used");
}
...
async fn reducer_ingests_plugin_state_changed_fact() {
    ...
    assert_eq!(
        payload,
        json!([{
            "event_type": "brocode_plugin_disabled",
            "event_params": {
                "plugin_id": "sample@test",
                "plugin_name": "sample",
                "marketplace_name": "test",
