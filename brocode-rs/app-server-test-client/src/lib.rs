...
use brocode_app_server_protocol::ClientInfo;
...
use brocode_app_server_protocol::DynamicToolSpec;
...
use brocode_app_server_protocol::InitializeCapabilities;
use brocode_app_server_protocol::InitializeParams;
use brocode_app_server_protocol::InitializeResponse;
use brocode_app_server_protocol::JSONRPCMessage;
use brocode_app_server_protocol::JSONRPCNotification;
use brocode_app_server_protocol::JSONRPCRequest;
use brocode_app_server_protocol::JSONRPCResponse;
...
use brocode_app_server_protocol::ReadOnlyAccess;
use brocode_app_server_protocol::RequestId;
use brocode_app_server_protocol::SandboxPolicy;
...
const OTEL_SERVICE_NAME: &str = "brocode-app-server-test-client";
const TRACE_DISABLED_MESSAGE: &str =
    "Not enabled - enable tracing in $BROCODE_HOME/config.toml to get a trace URL!";

/// Minimal launcher that initializes the Brocode app-server and logs the handshake.
#[derive(Parser)]
#[command(author = "Brocode", version, about = "Bootstrap Brocode app-server", long_about = None)]
struct Cli {
    /// Path to the `brocode` CLI binary. When set, requests use stdio by
    /// spawning `brocode app-server` as a child process.
    #[arg(long, env = "BROCODE_BIN", global = true)]
    brocode_bin: Option<PathBuf>,

    /// Existing websocket server URL to connect to.
    ///
    /// If neither `--brocode-bin` nor `--url` is provided, defaults to
    /// `ws://127.0.0.1:4222`.
    #[arg(long, env = "BROCODE_APP_SERVER_URL", global = true)]
    url: Option<String>,

    /// Forwarded to the `brocode` CLI as `--config key=value`. Repeatable.
    ///
    /// Example:
...
#[derive(Subcommand)]
enum CliCommand {
    /// Start `brocode app-server` on a websocket endpoint in the background.
    ///
    /// Logs are written to:
    ///   `/tmp/brocode-app-server-test-client/`
    Serve {
        /// WebSocket listen URL passed to `brocode app-server --listen`.
    ...
    /// Send a user message through the Brocode app-server.
    SendMessage {
        /// User message to send to Brocode.
        user_message: String,
    },
    ...
    /// Fetch the current account rate limits from the Brocode app-server.
    GetAccountRateLimits,
    /// List the available models from the Brocode app-server.
    #[command(name = "model-list")]
    ModelList,
    /// List stored threads from the Brocode app-server.
    ...
    LiveElicitationTimeoutPause {
        ...
        #[arg(long, env = "BROCODE_E2E_MODEL", default_value = "gpt-5")]
        model: String,
...
pub async fn run() -> Result<()> {
    ...
    match command {
        CliCommand::Serve { listen, kill } => {
            ...
            let brocode_bin = brocode_bin.unwrap_or_else(|| PathBuf::from("brocode"));
            serve(&brocode_bin, &config_overrides, &listen, kill)
        }
        CliCommand::SendMessage { user_message } => {
            ensure_dynamic_tools_unused(&dynamic_tools, "send-message")?;
            let endpoint = resolve_endpoint(brocode_bin, url)?;
        ...
        CliCommand::ThreadIncrementElicitation { thread_id } => {
            ...
            let url =
                resolve_shared_websocket_url(brocode_bin, url, "thread-increment-elicitation")?;
        ...
        CliCommand::ThreadDecrementElicitation { thread_id } => {
            ...
            let url =
                resolve_shared_websocket_url(brocode_bin, url, "thread-decrement-elicitation")?;
...
fn resolve_endpoint(brocode_bin: Option<PathBuf>, url: Option<String>) -> Result<Endpoint> {
    if brocode_bin.is_some() && url.is_some() {
        bail!("--brocode-bin and --url are mutually exclusive");
    }
    if let Some(brocode_bin) = brocode_bin {
        return Ok(Endpoint::SpawnBrocode(brocode_bin));
    }
    if let Some(url) = url {
...
fn resolve_shared_websocket_url(
    ...
    if brocode_bin.is_some() {
        bail!(
            "{command} requires --url or an already-running websocket app-server; --brocode-bin would spawn a private stdio app-server instead"
        );
    }
...
impl BackgroundAppServer {
    fn spawn(brocode_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        ...
        let mut cmd = Command::new(brocode_bin);
        if let Some(brocode_bin_parent) = brocode_bin.parent() {
            let mut path = OsString::from(brocode_bin_parent.as_os_str());
        ...
        let process = cmd
            ...
            .spawn()
            .with_context(|| format!("failed to start `{}` app-server", brocode_bin.display()))?;
...
fn serve(brocode_bin: &Path, config_overrides: &[String], listen: &str, kill: bool) -> Result<()> {
    let runtime_dir = PathBuf::from("/tmp/brocode-app-server-test-client");
    ...
    let mut cmdline = format!(
        "tail -f /dev/null | RUST_BACKTRACE=full RUST_LOG=warn,brocode_=trace {}",
        shell_quote(&brocode_bin.display().to_string())
    );
    for override_kv in config_overrides {
    ...
    println!("started brocode app-server");
...
pub async fn send_message_v2(
    ...
    let endpoint = Endpoint::SpawnBrocode(brocode_bin.to_path_buf());
    send_message_v2_endpoint(
        &endpoint,
        config_overrides,
        user_message,
...
async fn with_client<T>(
    ...
    endpoint: &Endpoint,
    config_overrides: &[String],
    f: impl FnOnce(&mut BrocodeClient) -> Result<T>,
) -> Result<T> {
    ...
    let result = command_span.in_scope(|| {
        let mut client = BrocodeClient::connect(endpoint, config_overrides)?;
        f(&mut client)
    });
...
fn thread_increment_elicitation(url: &str, thread_id: String) -> Result<()> {
    ...
    let mut client = BrocodeClient::connect(&endpoint, &[])?;
...
fn live_elicitation_timeout_pause(
    ...
    let websocket_url = match (brocode_bin, url) {
        (Some(_), Some(_)) => bail!("--brocode-bin and --url are mutually exclusive"),
        (Some(brocode_bin), None) => {
            let server = BackgroundAppServer::spawn(&brocode_bin, config_overrides)?;
    ...
    let app_server_test_client_bin = std::env::current_exe()
        .context("failed to resolve brocode-app-server-test-client binary path")?;
...
impl BrocodeClient {
    fn connect(endpoint: &Endpoint, config_overrides: &[String]) -> Result<Self> {
        match endpoint {
            Endpoint::SpawnBrocode(brocode_bin) => Self::spawn_stdio(brocode_bin, config_overrides),
            Endpoint::ConnectWs(url) => Self::connect_websocket(url),
        }
    }

    fn spawn_stdio(brocode_bin: &Path, config_overrides: &[String]) -> Result<Self> {
        let brocode_bin_display = brocode_bin.display();
        let mut cmd = Command::new(brocode_bin);
        if let Some(brocode_bin_parent) = brocode_bin.parent() {
            let mut path = OsString::from(brocode_bin_parent.as_os_str());
            if let Some(existing_path) = std::env::var_os("PATH") {
        ...
        let mut brocode_app_server = cmd
            .arg("app-server")
            .stdin(Stdio::piped())
            .stdout(Stdio::piped())
            .stderr(Stdio::inherit())
            .spawn()
            .with_context(|| format!("failed to start `{brocode_bin_display}` app-server"))?;

        let stdin = brocode_app_server
            .stdin
            .take()
            .context("brocode app-server stdin unavailable")?;
        let stdout = brocode_app_server
            .stdout
            .take()
            .context("brocode app-server stdout unavailable")?;

        Ok(Self {
            transport: ClientTransport::Stdio {
                child: brocode_app_server,
                stdin: Some(stdin),
                ...
                Err(err) => {
                    if Instant::now() >= deadline {
                        return Err(err).with_context(|| {
                            format!(
                                "failed to connect to websocket app-server at `{url}`; if no server is running, start one with `brocode-app-server-test-client serve --listen {url}`"
                            )
                        });
                    }
        ...
        let request = ClientRequest::Initialize {
            ...
            params: InitializeParams {
                client_info: ClientInfo {
                    name: "brocode-toy-app-server".to_string(),
                    title: Some("Brocode Toy App Server".to_string()),
    ...
    fn login_account_chatgpt(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: brocode_app_server_protocol::LoginAccountParams::Chatgpt,
        };
    ...
    fn login_account_chatgpt_device_code(&mut self) -> Result<LoginAccountResponse> {
        ...
        let request = ClientRequest::LoginAccount {
            request_id: request_id.clone(),
            params: brocode_app_server_protocol::LoginAccountParams::ChatgptDeviceCode,
        };
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdin, .. } => {
                if let Some(stdin) = stdin.as_mut() {
                    ...
                    stdin
                        .flush()
                        .context("failed to flush payload to brocode app-server")?;
                    return Ok(());
                }
                bail!("brocode app-server stdin closed")
            }
        ...
        match &mut self.transport {
            ClientTransport::Stdio { stdout, .. } => {
                ...
                let bytes = stdout
                    ...
                    .context("failed to read from brocode app-server")?;
                if bytes == 0 {
                    bail!("brocode app-server closed stdout");
                }
                Ok(response_line)
            }
...
impl TestClientTracing {
    async fn initialize(config_overrides: &[String]) -> Result<Self> {
        ...
        let otel_provider = brocode_core::otel_init::build_provider(
            &config,
...
impl Drop for BrocodeClient {
    fn drop(&mut self) {
        ...
        if let Ok(Some(status)) = child.try_wait() {
            println!("[brocode app-server exited: {status}]");
            return;
        }
