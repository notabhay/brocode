...
//!
//! If the keyring is not available or fails, we fall back to BROCODE_HOME/.credentials.json which is consistent with other coding CLI agents.
...
const KEYRING_SERVICE: &str = "Brocode MCP Credentials";
...
/// Determine where Brocode should store and read MCP credentials.
...
fn fallback_file_path() -> Result<PathBuf> {
    let mut path = find_brocode_home()?;
...
mod tests {
    ...
    struct TempBrocodeHome {
        _guard: MutexGuard<'static, ()>,
        _dir: tempfile::TempDir,
    }

    impl TempBrocodeHome {
        fn new() -> Self {
            ...
            let dir = tempdir().expect("create BROCODE_HOME temp dir");
            unsafe {
                std::env::set_var("BROCODE_HOME", dir.path());
            }
            Self {
                _guard: guard,
                _dir: dir,
    ...
    impl Drop for TempBrocodeHome {
        fn drop(&mut self) {
            unsafe {
                std::env::remove_var("BROCODE_HOME");
            }
        }
    }
