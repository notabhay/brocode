...
const MEMO_URI: &str = "memo://brocode/example-note";
...
impl TestToolServer {
    ...
    /// Tool intended for manual testing of Brocode TUI rendering for MCP image tool results.
    ///
    ...
    ///
    /// Manual testing approach (Brocode TUI):
    /// - Build this binary: `cargo build -p brocode-rmcp-client --bin test_stdio_server`
    /// - Register it:
    ///   - `brocode mcp add mcpimg -- /abs/path/to/test_stdio_server`
    /// - Then in Brocode TUI, ask it to call:
    ...
    fn memo_template() -> ResourceTemplate {
        let raw = RawResourceTemplate {
            uri_template: "memo://brocode/{slug}".to_string(),
            name: "brocode-memo".to_string(),
            title: Some("Brocode Memo".to_string()),
            description: Some(
                "Template for memo://brocode/{slug} resources used in tests.".to_string(),
            ),
            mime_type: Some("text/plain".to_string()),
            icons: None,
        };
...
/// Scenarios for `image_scenario`, intended to exercise Brocode TUI handling of MCP image outputs.
///
