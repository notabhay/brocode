...
pub async fn run_mac_app_open_or_install(
    ...
    if let Some(app_path) = find_existing_brocode_app_path() {
        eprintln!(
            "Opening Brocode Desktop at {app_path}...",
            app_path = app_path.display()
        );
        open_brocode_app(&app_path, &workspace).await?;
        return Ok(());
    }
    eprintln!("Brocode Desktop not found; downloading installer...");
    let installed_app = download_and_install_brocode_to_user_applications(&download_url)
        .await
        .context("failed to download/install Brocode Desktop")?;
    eprintln!(
        "Launching Brocode Desktop from {installed_app}...",
        installed_app = installed_app.display()
    );
    open_brocode_app(&installed_app, &workspace).await?;
    Ok(())
}

fn find_existing_brocode_app_path() -> Option<PathBuf> {
    candidate_brocode_app_paths()
        .into_iter()
        .find(|candidate| candidate.is_dir())
}

fn candidate_brocode_app_paths() -> Vec<PathBuf> {
    let mut paths = vec![PathBuf::from("/Applications/Brocode.app")];
    if let Some(home) = std::env::var_os("HOME") {
        paths.push(PathBuf::from(home).join("Applications").join("Brocode.app"));
    }
    paths
}

async fn open_brocode_app(app_path: &Path, workspace: &Path) -> anyhow::Result<()> {
    eprintln!(
...
async fn download_and_install_brocode_to_user_applications(
    dmg_url: &str,
) -> anyhow::Result<PathBuf> {
    let temp_dir = Builder::new()
        .prefix("brocode-app-installer-")
        .tempdir()
        .context("failed to create temp dir")?;
    let tmp_root = temp_dir.path().to_path_buf();
    let _temp_dir = temp_dir;

    let dmg_path = tmp_root.join("Brocode.dmg");
    download_dmg(dmg_url, &dmg_path).await?;

    eprintln!("Mounting Brocode Desktop installer...");
    ...
    let result = async {
        let app_in_volume = find_brocode_app_in_mount(&mount_point)
            .context("failed to locate Brocode.app in mounted dmg")?;
        install_brocode_app_bundle(&app_in_volume).await
    }
    .await;
...
async fn install_brocode_app_bundle(app_in_volume: &Path) -> anyhow::Result<PathBuf> {
    for applications_dir in candidate_applications_dirs()? {
        eprintln!(
            "Installing Brocode Desktop into {applications_dir}...",
        ...
        let dest_app = applications_dir.join("Brocode.app");
        if dest_app.is_dir() {
            return Ok(dest_app);
        }
        ...
        match copy_app_bundle(app_in_volume, &dest_app).await {
            ...
            Err(err) => {
                eprintln!(
                    "warning: failed to install Brocode.app to {applications_dir}: {err}",
                    applications_dir = applications_dir.display()
                );
            }
        }
    ...
    anyhow::bail!("failed to install Brocode.app to any applications directory");
}
...
fn find_brocode_app_in_mount(mount_point: &Path) -> anyhow::Result<PathBuf> {
    let direct = mount_point.join("Brocode.app");
    if direct.is_dir() {
        return Ok(direct);
    }
...
mod tests {
    ...
    fn parses_mount_point_from_tab_separated_hdiutil_output() {
        let output = "/dev/disk2s1\tApple_HFS\tBrocode\t/Volumes/Brocode\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
            Some("/Volumes/Brocode")
        );
    }

    #[test]
    fn parses_mount_point_with_spaces() {
        let output = "/dev/disk2s1\tApple_HFS\tBrocode Installer\t/Volumes/Brocode Installer\n";
        assert_eq!(
            parse_hdiutil_attach_mount_point(output).as_deref(),
            Some("/Volumes/Brocode Installer")
        );
    }
}
