...
pub use brocode_core::env::is_wsl;
...
mod tests {
    ...
    fn win_to_wsl_basic() {
        assert_eq!(
            win_path_to_wsl(r"C:\Temp\brocode.zip").as_deref(),
            Some("/mnt/c/Temp/brocode.zip")
        );
        assert_eq!(
            win_path_to_wsl("D:/Work/brocode.tgz").as_deref(),
            Some("/mnt/d/Work/brocode.tgz")
        );
        assert!(win_path_to_wsl("/home/user/brocode").is_none());
    }
