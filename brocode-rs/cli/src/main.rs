...
use brocode_chatgpt::apply_command::ApplyCommand;
use brocode_chatgpt::apply_command::run_apply_command;
use brocode_cli::LandlockCommand;
use brocode_cli::SeatbeltCommand;
use brocode_cli::WindowsCommand;
use brocode_cli::login::read_api_key_from_stdin;
use brocode_cli::login::run_login_status;
use brocode_cli::login::run_login_with_api_key;
use brocode_cli::login::run_login_with_chatgpt;
use brocode_cli::login::run_login_with_device_code;
use brocode_cli::login::run_logout;
use brocode_cloud_tasks::Cli as CloudTasksCli;
use brocode_exec::Cli as ExecCli;
use brocode_exec::Command as ExecCommand;
use brocode_exec::ReviewArgs;
use brocode_execpolicy::ExecPolicyCheckCommand;
use brocode_responses_api_proxy::Args as ResponsesApiProxyArgs;
use brocode_state::StateRuntime;
use brocode_state::state_db_path;
use brocode_tui::AppExitInfo;
use brocode_tui::Cli as TuiCli;
use brocode_tui::ExitReason;
use brocode_tui::update_action::UpdateAction;
...
use brocode_core::config::find_brocode_home;
use brocode_features::FEATURES;
use brocode_features::Stage;
use brocode_features::is_known_feature_key;
...
#[clap(
    ...
    // `brocode-x86_64-unknown-linux-musl`, but the help output should always use
    // the generic `brocode` command name that users run.
    bin_name = "brocode",
    override_usage = "brocode [OPTIONS] [PROMPT]\n       brocode [OPTIONS] <COMMAND> [ARGS]"
)]
struct MultitoolCli {
    #[clap(flatten)]
...
enum Subcommand {
    ...
    /// Manage external MCP servers for Brocode.
    Mcp(McpCli),

    /// Start Brocode as an MCP server (stdio).
    McpServer,

    /// [experimental] Run the app server or related tooling.
    AppServer(AppServerCommand),

    /// Launch the Brocode desktop app (downloads the macOS installer if missing).
    #[cfg(target_os = "macos")]
    ...
    /// Run commands within a Brocode-provided sandbox.
    Sandbox(SandboxArgs),
    ...
    /// Apply the latest diff produced by Brocode agent as a `git apply` to your local working tree.
    ...
    /// [EXPERIMENTAL] Browse tasks from Brocode Cloud and apply changes locally.
...
struct LoginCommand {
    ...
    #[arg(
        long = "with-api-key",
        help = "Read the API key from stdin (e.g. `printenv OPENAI_API_KEY | brocode login --with-api-key`)"
    )]
    with_api_key: bool,
...
struct AppServerCommand {
    ...
    #[arg(
        long = "listen",
        value_name = "URL",
        default_value = brocode_app_server::AppServerTransport::DEFAULT_LISTEN_URL
    )]
    listen: brocode_app_server::AppServerTransport,
    ...
    #[command(flatten)]
    auth: brocode_app_server::AppServerWebsocketAuthArgs,
}
...
enum AppServerSubcommand {
    ...
    /// [internal] Generate internal JSON Schema artifacts for Brocode tooling.
    #[clap(hide = true)]
...
fn format_exit_messages(exit_info: AppExitInfo, color_enabled: bool) -> Vec<String> {
    ...
    if let Some(resume_cmd) =
        brocode_core::util::resume_command(thread_name.as_deref(), conversation_id)
    {
...
fn run_update_action(action: UpdateAction) -> anyhow::Result<()> {
    ...
    println!("Updating Brocode via `{cmd_str}`...");
    ...
    println!("\n🎉 Update ran successfully! Please restart Brocode.");
    Ok(())
}
...
async fn run_debug_app_server_command(cmd: DebugAppServerCommand) -> anyhow::Result<()> {
    match cmd.subcommand {
        DebugAppServerSubcommand::SendMessageV2(cmd) => {
            let brocode_bin = std::env::current_exe()?;
            brocode_app_server_test_client::send_message_v2(
                &brocode_bin,
                &[],
                cmd.user_message,
                &None,
...
async fn cli_main(arg0_paths: Arg0DispatchPaths) -> anyhow::Result<()> {
    ...
    match subcommand {
        ...
        Some(Subcommand::Exec(mut exec_cli)) => {
            ...
            brocode_exec::run_main(exec_cli, arg0_paths.clone()).await?;
        }
        Some(Subcommand::Review(review_args)) => {
            ...
            let mut exec_cli = ExecCli::try_parse_from(["brocode", "exec"])?;
        ...
        Some(Subcommand::McpServer) => {
            ...
            brocode_mcp_server::run_main(arg0_paths.clone(), root_config_overrides).await?;
        }
        ...
        Some(Subcommand::AppServer(app_server_cli)) => {
            ...
            match subcommand {
                None => {
                    ...
                    brocode_app_server::run_main_with_transport(
                        arg0_paths.clone(),
                        root_config_overrides,
                        brocode_core::config_loader::LoaderOverrides::default(),
                        analytics_default_enabled,
                        transport,
                        brocode_protocol::protocol::SessionSource::VSCode,
                        auth,
                    )
                    .await?;
                }
                Some(AppServerSubcommand::GenerateTs(gen_cli)) => {
                    let options = brocode_app_server_protocol::GenerateTsOptions {
                        experimental_api: gen_cli.experimental,
                        ..Default::default()
                    };
                    brocode_app_server_protocol::generate_ts_with_options(
                        &gen_cli.out_dir,
                ...
                Some(AppServerSubcommand::GenerateJsonSchema(gen_cli)) => {
                    brocode_app_server_protocol::generate_json_with_experimental(
                        &gen_cli.out_dir,
                        gen_cli.experimental,
                    )?;
                }
                Some(AppServerSubcommand::GenerateInternalJsonSchema(gen_cli)) => {
                    brocode_app_server_protocol::generate_internal_json_schema(&gen_cli.out_dir)?;
                }
            }
        }
        #[cfg(target_os = "macos")]
            ...
            match login_cli.action {
                ...
                None => {
                    if login_cli.use_device_code {
                        ...
                        eprintln!(
                            "The --api-key flag is no longer supported. Pipe the key instead, e.g. `printenv OPENAI_API_KEY | brocode login --with-api-key`."
                        );
                        std::process::exit(1);
        ...
        Some(Subcommand::Cloud(mut cloud_cli)) => {
            ...
            brocode_cloud_tasks::run_main(cloud_cli, arg0_paths.brocode_linux_sandbox_exe.clone())
                .await?;
        }
        Some(Subcommand::Sandbox(sandbox_args)) => match sandbox_args.cmd {
            SandboxCommand::Macos(mut seatbelt_cli) => {
                ...
                brocode_cli::debug_sandbox::run_command_under_seatbelt(
                    seatbelt_cli,
                    arg0_paths.brocode_linux_sandbox_exe.clone(),
                )
                .await?;
            }
            SandboxCommand::Linux(mut landlock_cli) => {
                ...
                brocode_cli::debug_sandbox::run_command_under_landlock(
                    landlock_cli,
                    arg0_paths.brocode_linux_sandbox_exe.clone(),
                )
                .await?;
            ...
            SandboxCommand::Windows(mut windows_cli) => {
                ...
                brocode_cli::debug_sandbox::run_command_under_windows(
                    windows_cli,
                    arg0_paths.brocode_linux_sandbox_exe.clone(),
                )
                .await?;
        ...
        Some(Subcommand::ResponsesApiProxy(args)) => {
            ...
            tokio::task::spawn_blocking(move || brocode_responses_api_proxy::run_main(args))
                .await??;
        }
        Some(Subcommand::StdioToUds(cmd)) => {
            ...
            tokio::task::spawn_blocking(move || brocode_stdio_to_uds::run(socket_path.as_path()))
                .await??;
        }
...
async fn enable_feature_in_config(interactive: &TuiCli, feature: &str) -> anyhow::Result<()> {
    ...
    let brocode_home = find_brocode_home()?;
    ...
    maybe_print_under_development_feature_warning(&brocode_home, interactive, feature);
    Ok(())
}
...
fn maybe_print_under_development_feature_warning(
    ...
    let config_path = brocode_home.join(brocode_config::CONFIG_TOML_FILE);
    eprintln!(
...
async fn run_debug_clear_memories_command(
    ...
    let memory_root = config.brocode_home.join("memories");
...
fn reject_remote_mode_for_subcommand(
    ...
    if let Some(remote) = remote {
        anyhow::bail!(
            "`--remote {remote}` is only supported for interactive TUI commands, not `brocode {subcommand}`"
        );
    }
    if remote_auth_token_env.is_some() {
        anyhow::bail!(
            "`--remote-auth-token-env` is only supported for interactive TUI commands, not `brocode {subcommand}`"
        );
    }
    Ok(())
}
...
async fn run_interactive_tui(
    ...
    let terminal_info = brocode_terminal_detection::terminal_info();
    if terminal_info.name == TerminalName::Dumb {
        ...
        eprintln!(
            "WARNING: TERM is set to \"dumb\". Brocode's interactive TUI may not work in this terminal."
        );
    ...
    let use_app_server_tui = brocode_tui::should_use_app_server_tui(&interactive).await?;
    let normalized_remote = remote
        .as_deref()
        .map(brocode_tui_app_server::normalize_remote_addr)
        .transpose()
    ...
    if use_app_server_tui {
        let remote_auth_token = remote_auth_token_env
            ...
            .transpose()
            .map_err(std::io::Error::other)?;
        brocode_tui_app_server::run_main(
...
fn into_app_server_tui_cli(cli: TuiCli) -> brocode_tui_app_server::Cli {
    brocode_tui_app_server::Cli {
        prompt: cli.prompt,
        images: cli.images,
...
fn into_legacy_update_action(
    action: brocode_tui_app_server::update_action::UpdateAction,
) -> UpdateAction {
    match action {
        brocode_tui_app_server::update_action::UpdateAction::NpmGlobalLatest => {
            UpdateAction::NpmGlobalLatest
        }
        brocode_tui_app_server::update_action::UpdateAction::BunGlobalLatest => {
            UpdateAction::BunGlobalLatest
        }
        brocode_tui_app_server::update_action::UpdateAction::BrewUpgrade => {
            UpdateAction::BrewUpgrade
        }
    }
}

fn into_legacy_exit_reason(reason: brocode_tui_app_server::ExitReason) -> ExitReason {
    match reason {
        brocode_tui_app_server::ExitReason::UserRequested => ExitReason::UserRequested,
        brocode_tui_app_server::ExitReason::Fatal(message) => ExitReason::Fatal(message),
    }
}

fn into_legacy_app_exit_info(exit_info: brocode_tui_app_server::AppExitInfo) -> AppExitInfo {
    AppExitInfo {
...
/// Build the final `TuiCli` for a `brocode resume` invocation.
fn finalize_resume_interactive(
    ...
    // configuration surface area as `brocode` without additional flags.
...
/// Build the final `TuiCli` for a `brocode fork` invocation.
...
/// Merge flags provided to `brocode resume`/`brocode fork` so they take precedence over any
...
mod tests {
    ...
    fn exec_resume_last_accepts_prompt_positional() {
        let cli =
            MultitoolCli::try_parse_from(["brocode", "exec", "--json", "resume", "--last", "2+2"])
                .expect("parse should succeed");

        let Some(Subcommand::Exec(exec)) = cli.subcommand else {
            panic!("expected exec subcommand");
        };
        let Some(brocode_exec::Command::Resume(args)) = exec.command else {
    ...
    fn format_exit_messages_includes_resume_hint_without_color() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                "To continue this session, run brocode resume 123e4567-e89b-12d3-a456-426614174000"
                    .to_string(),
            ]
        );
    }
    ...
    fn format_exit_messages_prefers_thread_name() {
        ...
        assert_eq!(
            ...
            vec![
                "Token usage: total=2 input=0 output=2".to_string(),
                "To continue this session, run brocode resume my-thread".to_string(),
            ]
        );
    }
    ...
    fn resume_model_flag_applies_when_no_root_flags() {
        let interactive =
            finalize_resume_from_args(["brocode", "resume", "-m", "gpt-5.1-test"].as_ref());
    ...
    fn resume_picker_logic_none_and_not_last() {
        let interactive = finalize_resume_from_args(["brocode", "resume"].as_ref());
        assert!(interactive.resume_picker);
        assert!(!interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
    ...
    fn resume_picker_logic_last() {
        let interactive = finalize_resume_from_args(["brocode", "resume", "--last"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(interactive.resume_last);
        assert_eq!(interactive.resume_session_id, None);
        assert!(!interactive.resume_show_all);
    ...
    fn resume_picker_logic_with_session_id() {
        let interactive = finalize_resume_from_args(["brocode", "resume", "1234"].as_ref());
        assert!(!interactive.resume_picker);
        assert!(!interactive.resume_last);
    ...
    fn resume_all_flag_sets_show_all() {
        let interactive = finalize_resume_from_args(["brocode", "resume", "--all"].as_ref());
        assert!(interactive.resume_picker);
        assert!(interactive.resume_show_all);
    }
    ...
    fn resume_include_non_interactive_flag_sets_source_filter_override() {
        let interactive =
            finalize_resume_from_args(["brocode", "resume", "--include-non-interactive"].as_ref());
    ...
    fn resume_merges_option_flags_and_full_auto() {
        ...
        assert_matches!(
            ...
            Some(brocode_utils_cli::SandboxModeCliArg::WorkspaceWrite)
        );
        assert_matches!(
            interactive.approval_policy,
            Some(brocode_utils_cli::ApprovalModeCliArg::OnRequest)
        );
    ...
    fn fork_picker_logic_none_and_not_last() {
        let interactive = finalize_fork_from_args(["brocode", "fork"].as_ref());
    ...
    fn fork_picker_logic_last() {
        let interactive = finalize_fork_from_args(["brocode", "fork", "--last"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(interactive.fork_last);
        assert_eq!(interactive.fork_session_id, None);
        assert!(!interactive.fork_show_all);
    ...
    fn fork_picker_logic_with_session_id() {
        let interactive = finalize_fork_from_args(["brocode", "fork", "1234"].as_ref());
        assert!(!interactive.fork_picker);
        assert!(!interactive.fork_last);
    ...
    fn fork_all_flag_sets_show_all() {
        let interactive = finalize_fork_from_args(["brocode", "fork", "--all"].as_ref());
        assert!(interactive.fork_picker);
        assert!(interactive.fork_show_all);
    }
    ...
    fn app_server_analytics_default_disabled_without_flag() {
        let app_server = app_server_from_args(["brocode", "app-server"].as_ref());
        assert!(!app_server.analytics_default_enabled);
        assert_eq!(
            app_server.listen,
            brocode_app_server::AppServerTransport::Stdio
        );
    }
    ...
    fn app_server_analytics_default_enabled_with_flag() {
        let app_server =
            app_server_from_args(["brocode", "app-server", "--analytics-default-enabled"].as_ref());
        assert!(app_server.analytics_default_enabled);
    }

    #[test]
    fn remote_flag_parses_for_interactive_root() {
        let cli = MultitoolCli::try_parse_from(["brocode", "--remote", "ws://127.0.0.1:4500"])
            .expect("parse");
    ...
    fn remote_auth_token_env_flag_parses_for_interactive_root() {
        ...
        assert_eq!(
            ...
            Some("BROCODE_REMOTE_AUTH_TOKEN")
        );
    }
    ...
    fn remote_flag_parses_for_resume_subcommand() {
        let cli =
            MultitoolCli::try_parse_from(["brocode", "resume", "--remote", "ws://127.0.0.1:4500"])
                .expect("parse");
    ...
    fn reject_remote_auth_token_env_for_non_interactive_subcommands() {
        let err =
            reject_remote_mode_for_subcommand(None, Some("BROCODE_REMOTE_AUTH_TOKEN"), "exec")
    ...
    fn read_remote_auth_token_from_env_var_reports_missing_values() {
        let err = read_remote_auth_token_from_env_var_with("BROCODE_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn read_remote_auth_token_from_env_var_trims_values() {
        let auth_token =
            read_remote_auth_token_from_env_var_with("BROCODE_REMOTE_AUTH_TOKEN", |_| {
    ...
    fn app_server_listen_websocket_url_parses() {
        let app_server = app_server_from_args(
            ["brocode", "app-server", "--listen", "ws://127.0.0.1:4500"].as_ref(),
        );
        assert_eq!(
            app_server.listen,
            brocode_app_server::AppServerTransport::WebSocket {
    ...
    fn app_server_listen_stdio_url_parses() {
        let app_server =
            app_server_from_args(["brocode", "app-server", "--listen", "stdio://"].as_ref());
        assert_eq!(
            app_server.listen,
            brocode_app_server::AppServerTransport::Stdio
        );
    ...
    fn app_server_listen_invalid_url_fails_to_parse() {
        let parse_result =
            MultitoolCli::try_parse_from(["brocode", "app-server", "--listen", "http://foo"]);
    ...
    fn app_server_capability_token_flags_parse() {
        ...
        assert_eq!(
            ...
            Some(brocode_app_server::WebsocketAuthCliMode::CapabilityToken)
        );
        assert_eq!(
            app_server.auth.ws_token_file,
            Some(PathBuf::from("/tmp/brocode-token"))
        );
    }
    ...
    fn app_server_signed_bearer_flags_parse() {
        ...
        assert_eq!(
            app_server.auth.ws_auth,
            Some(brocode_app_server::WebsocketAuthCliMode::SignedBearerToken)
        );
        assert_eq!(
            app_server.auth.ws_shared_secret_file,
            Some(PathBuf::from("/tmp/brocode-secret"))
        );
    ...
    fn features_enable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["brocode", "features", "enable", "unified_exec"])
            .expect("parse should succeed");
    ...
    fn features_disable_parses_feature_name() {
        let cli = MultitoolCli::try_parse_from(["brocode", "features", "disable", "shell_tool"])
            .expect("parse should succeed");
        let Some(Subcommand::Features(FeaturesCli { sub })) = cli.subcommand else {
            panic!("expected features subcommand");
        };
