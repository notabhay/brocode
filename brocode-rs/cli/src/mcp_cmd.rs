...
use brocode_core::config::load_global_mcp_servers;
...
use brocode_core::mcp::auth::McpOAuthLoginSupport;
use brocode_core::mcp::auth::ResolvedMcpOAuthScopes;
use brocode_core::mcp::auth::compute_auth_statuses;
use brocode_core::mcp::auth::discover_supported_scopes;
use brocode_core::mcp::auth::oauth_login_support;
use brocode_core::mcp::auth::resolve_oauth_scopes;
use brocode_core::mcp::auth::should_retry_without_scopes;
use brocode_core::plugins::PluginsManager;
use brocode_protocol::protocol::McpAuthStatus;
use brocode_rmcp_client::delete_oauth_tokens;
use brocode_rmcp_client::perform_oauth_login;
...
/// - `add`    — add a server launcher entry to `~/.brocode/config.toml`
...
#[derive(Debug, clap::Parser)]
#[command(override_usage = "brocode mcp add [OPTIONS] <NAME> (--url <URL> | -- <COMMAND>...)")]
pub struct AddArgs {
...
async fn perform_oauth_login_retry_without_scopes(
    name: &str,
    url: &str,
    store_mode: brocode_rmcp_client::OAuthCredentialsStoreMode,
...
async fn run_add(config_overrides: &CliConfigOverrides, add_args: AddArgs) -> Result<()> {
    ...
    ConfigEditsBuilder::new(&brocode_home)
        ...
        .apply()
        .await
        .with_context(|| format!("failed to write MCP servers to {}", brocode_home.display()))?;
    ...
    match oauth_login_support(&transport).await {
        ...
        McpOAuthLoginSupport::Unknown(_) => println!(
            "MCP server may or may not require login. Run `brocode mcp login {name}` to login."
        ),
    }
...
async fn run_remove(config_overrides: &CliConfigOverrides, remove_args: RemoveArgs) -> Result<()> {
    ...
    if removed {
        ConfigEditsBuilder::new(&brocode_home)
            ...
            .await
            .with_context(|| {
                format!("failed to write MCP servers to {}", brocode_home.display())
            })?;
    }
...
async fn run_list(config_overrides: &CliConfigOverrides, list_args: ListArgs) -> Result<()> {
    ...
    if entries.is_empty() {
        println!("No MCP servers configured yet. Try `brocode mcp add my-tool -- my-command`.");
        return Ok(());
    }
...
async fn run_get(config_overrides: &CliConfigOverrides, get_args: GetArgs) -> Result<()> {
    ...
    println!("  remove: brocode mcp remove {}", get_args.name);
