...
//! interactive-session layers. Direct `brocode login` intentionally does less: it preserves the
...
//! into a one-shot CLI command while still producing a durable `brocode-login.log` artifact that
...
use brocode_core::auth::logout;
use brocode_core::config::Config;
use brocode_login::ServerOptions;
use brocode_login::run_device_code_login;
...
/// Installs a small file-backed tracing layer for direct `brocode login` flows.
///
...
/// command produce a durable `brocode-login.log` artifact without coupling it to the TUI's broader
/// telemetry and feedback initialization.
fn init_login_file_logging(config: &Config) -> Option<WorkerGuard> {
    let log_dir = match brocode_core::config::log_dir(config) {
        Ok(log_dir) => log_dir,
        Err(err) => {
    ...
    let log_path = log_dir.join("brocode-login.log");
    ...
    let env_filter = EnvFilter::try_from_default_env().unwrap_or_else(|_| {
        EnvFilter::new("brocode_cli=info,brocode_core=info,brocode_login=info")
    });
    ...
    // Direct `brocode login` otherwise relies on ephemeral stderr and browser output.
...
fn print_login_server_start(actual_port: u16, auth_url: &str) {
    eprintln!(
        "Starting local login server on http://localhost:{actual_port}.\nIf your browser did not open, navigate to this URL to authenticate:\n\n{auth_url}\n\nOn a remote or headless machine? Use `brocode login --device-auth` instead."
    );
}
...
pub fn read_api_key_from_stdin() -> String {
    ...
    if stdin.is_terminal() {
        eprintln!(
            "--with-api-key expects the API key on stdin. Try piping it, e.g. `printenv OPENAI_API_KEY | brocode login --with-api-key`."
        );
        std::process::exit(1);
    }
...
/// `brocode login` working in environments where device-code may be disabled/feature-gated.
...
pub async fn run_login_status(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    match BrocodeAuth::from_auth_storage(
        &config.brocode_home,
        config.cli_auth_credentials_store_mode,
    ) {
...
pub async fn run_logout(cli_config_overrides: CliConfigOverrides) -> ! {
    ...
    match logout(&config.brocode_home, config.cli_auth_credentials_store_mode) {
        Ok(true) => {
