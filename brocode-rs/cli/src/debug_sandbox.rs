...
#[cfg(target_os = "macos")]
use brocode_core::spawn::BROCODE_SANDBOX_ENV_VAR;
...
pub async fn run_command_under_seatbelt(
    ...
    _brocode_linux_sandbox_exe: Option<PathBuf>,
) -> anyhow::Result<()> {
...
async fn run_command_under_sandbox(
    ...
    if let SandboxType::Windows = sandbox_type {
        #[cfg(target_os = "windows")]
        {
            ...
            use brocode_windows_sandbox::run_windows_sandbox_capture;
            ...
            let base_dir = config.brocode_home.clone();
    ...
    let mut child = match sandbox_type {
        ...
        SandboxType::Seatbelt => {
            ...
            spawn_debug_sandbox_child(
                ...
                env,
                |env_map| {
                    env_map.insert(BROCODE_SANDBOX_ENV_VAR.to_string(), "seatbelt".to_string());
        ...
        SandboxType::Landlock => {
            ...
            let brocode_linux_sandbox_exe = config
                .brocode_linux_sandbox_exe
                .expect("brocode-linux-sandbox executable not found");
...
async fn spawn_debug_sandbox_child(
    ...
    if !network_sandbox_policy.is_enabled() {
        cmd.env(BROCODE_SANDBOX_NETWORK_DISABLED_ENV_VAR, "1");
    }
...
async fn load_debug_sandbox_config(
    ...
    load_debug_sandbox_config_with_brocode_home(
        cli_overrides,
        brocode_linux_sandbox_exe,
        full_auto,
        /*brocode_home*/ None,
...
async fn load_debug_sandbox_config_with_brocode_home(
    cli_overrides: Vec<(String, TomlValue)>,
    brocode_linux_sandbox_exe: Option<PathBuf>,
    full_auto: bool,
    ...
    let config = build_debug_sandbox_config(
        cli_overrides.clone(),
        ConfigOverrides {
            brocode_linux_sandbox_exe: brocode_linux_sandbox_exe.clone(),
            ..Default::default()
        },
        brocode_home.clone(),
    )
    ...
    if config_uses_permission_profiles(&config) {
        if full_auto {
            anyhow::bail!(
                "`brocode sandbox --full-auto` is only supported for legacy `sandbox_mode` configs; choose a writable `[permissions]` profile instead"
            );
        }
        return Ok(config);
    }
...
async fn build_debug_sandbox_config(
    ...
    if let Some(brocode_home) = brocode_home {
        builder = builder
            .brocode_home(brocode_home.clone())
            .fallback_cwd(Some(brocode_home));
    }
    builder.build().await
}
...
mod tests {
    ...
    fn write_permissions_profile_config(
        ...
        std::fs::write(brocode_home.path().join("config.toml"), config)?;
        Ok(())
    }
    ...
    async fn debug_sandbox_honors_active_permission_profiles() -> anyhow::Result<()> {
        ...
        write_permissions_profile_config(&brocode_home, &docs, &private)?;
