...
use brocode_login::auth::AuthCredentialsStoreMode;
...
async fn end_to_end_login_flow_persists_auth_json() -> Result<()> {
    ...
    let server_home = brocode_home.clone();

    let opts = ServerOptions {
        brocode_home: server_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: brocode_login::CLIENT_ID.to_string(),
        issuer,
        port: 0,
        open_browser: false,
    ...
    // Validate auth.json
    let auth_path = brocode_home.join("auth.json");
...
#[tokio::test]
async fn creates_missing_brocode_home_dir() -> Result<()> {
    skip_if_no_network!(Ok(()));
    ...
    let tmp = tempdir()?;
    let brocode_home = tmp.path().join("missing-subdir"); // does not exist
...
async fn oauth_access_denied_missing_entitlement_blocks_login_with_clear_error() -> Result<()> {
    ...
    let body = resp.text().await?;
    assert!(
        body.contains("You do not have access to Brocode"),
        "error body should clearly explain the Brocode access denial"
    );
    assert!(
    ...
    assert!(
        !body.contains("missing_brocode_entitlement"),
...
async fn oauth_access_denied_unknown_reason_uses_generic_error_page() -> Result<()> {
    ...
    assert!(
        body.contains("Return to Brocode to retry"),
    ...
    assert!(
        !body.contains("get access to Brocode"),
...
async fn cancels_previous_login_server_when_port_is_in_use() -> Result<()> {
    ...
    let first_brocode_home = first_tmp.path().to_path_buf();

    let first_opts = ServerOptions {
        brocode_home: first_brocode_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: brocode_login::CLIENT_ID.to_string(),
        issuer: issuer.clone(),
        port: 0,
    ...
    let second_brocode_home = second_tmp.path().to_path_buf();

    let second_opts = ServerOptions {
        brocode_home: second_brocode_home,
        cli_auth_credentials_store_mode: AuthCredentialsStoreMode::File,
        client_id: brocode_login::CLIENT_ID.to_string(),
        issuer,
        port: login_port,
