...
use brocode_app_server_protocol::AuthMode as ApiAuthMode;
...
use brocode_client::BrocodeHttpClient;
use brocode_protocol::account::PlanType as AccountPlanType;
use serde_json::Value;
use thiserror::Error;

/// Authentication mechanism used by the current user.
#[derive(Debug, Clone)]
pub enum BrocodeAuth {
    ApiKey(ApiKeyAuth),
    Chatgpt(ChatgptAuth),
...
struct ChatgptAuthState {
    ...
    client: BrocodeHttpClient,
}

impl PartialEq for BrocodeAuth {
...
pub const REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR: &str = "BROCODE_REFRESH_TOKEN_URL_OVERRIDE";
...
impl BrocodeAuth {
    fn from_auth_dot_json(
        ...
        match auth_mode {
            ApiAuthMode::Chatgpt => {
                let storage = create_auth_storage(brocode_home.to_path_buf(), storage_mode);
...
impl ChatgptAuth {
    ...
    fn client(&self) -> &BrocodeHttpClient {
        &self.state.client
    }
}

pub const OPENAI_API_KEY_ENV_VAR: &str = "OPENAI_API_KEY";
pub const BROCODE_API_KEY_ENV_VAR: &str = "BROCODE_API_KEY";
...
pub fn read_brocode_api_key_from_env() -> Option<String> {
    env::var(BROCODE_API_KEY_ENV_VAR)
        .ok()
        .map(|value| value.trim().to_string())
        .filter(|value| !value.is_empty())
}

/// Delete the auth.json file inside `brocode_home` if it exists. Returns `Ok(true)`
/// if a file was removed, `Ok(false)` if no auth file was present.
pub fn logout(
    brocode_home: &Path,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<bool> {
    let storage = create_auth_storage(brocode_home.to_path_buf(), auth_credentials_store_mode);
    storage.delete()
}
...
pub fn login_with_api_key(
    ...
    save_auth(brocode_home, &auth_dot_json, auth_credentials_store_mode)
}
...
fn logout_with_message(
    ...
    let removal_result = logout_all_stores(brocode_home, auth_credentials_store_mode);
...
fn logout_all_stores(
    ...
    if auth_credentials_store_mode == AuthCredentialsStoreMode::Ephemeral {
        return logout(brocode_home, AuthCredentialsStoreMode::Ephemeral);
    }
    let removed_ephemeral = logout(brocode_home, AuthCredentialsStoreMode::Ephemeral)?;
    let removed_managed = logout(brocode_home, auth_credentials_store_mode)?;
    Ok(removed_ephemeral || removed_managed)
}

fn load_auth(
    brocode_home: &Path,
    enable_brocode_api_key_env: bool,
    auth_credentials_store_mode: AuthCredentialsStoreMode,
) -> std::io::Result<Option<BrocodeAuth>> {
    let build_auth = |auth_dot_json: AuthDotJson, storage_mode| {
        BrocodeAuth::from_auth_dot_json(brocode_home, auth_dot_json, storage_mode)
    };

    // API key via env var takes precedence over any other auth method.
    if enable_brocode_api_key_env && let Some(api_key) = read_brocode_api_key_from_env() {
        return Ok(Some(BrocodeAuth::from_api_key(api_key.as_str())));
    }
...
impl Debug for CachedAuth {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.debug_struct("CachedAuth")
            .field(
                "auth_mode",
                &self.auth.as_ref().map(BrocodeAuth::api_auth_mode),
            )
            .field(
...
impl UnauthorizedRecovery {
    fn new(manager: Arc<AuthManager>) -> Self {
        ...
        let expected_account_id = cached_auth.as_ref().and_then(BrocodeAuth::get_account_id);
        let mode = if manager.has_external_bearer_auth()
            || cached_auth
                .as_ref()
                .is_some_and(BrocodeAuth::is_external_chatgpt_tokens)
        {
...
/// hands out cloned `BrocodeAuth` values so the rest of the program has a
/// consistent snapshot.
///
...
impl AuthManager {
    ...
    /// Create an AuthManager with a specific BrocodeAuth, for testing only.
    pub fn from_auth_for_testing(auth: BrocodeAuth) -> Arc<Self> {
        ...
        Arc::new(Self {
            brocode_home: PathBuf::from("non-existent"),
    ...
    /// Create an AuthManager with a specific BrocodeAuth and brocode home, for testing only.
    pub fn from_auth_for_testing_with_home(auth: BrocodeAuth, brocode_home: PathBuf) -> Arc<Self> {
        let cached = CachedAuth {
            auth: Some(auth),
            permanent_refresh_failure: None,
        };
    ...
    pub fn auth_cached(&self) -> Option<BrocodeAuth> {
        self.inner.read().ok().and_then(|c| c.auth.clone())
    }

    pub fn refresh_failure_for_auth(&self, auth: &BrocodeAuth) -> Option<RefreshTokenFailedError> {
    ...
    pub async fn auth(&self) -> Option<BrocodeAuth> {
    ...
    fn reload_if_account_id_matches(&self, expected_account_id: Option<&str>) -> ReloadOutcome {
        ...
        let new_auth = self.load_auth_from_storage();
        let new_account_id = new_auth.as_ref().and_then(BrocodeAuth::get_account_id);
    ...
    fn auths_equal_for_refresh(a: Option<&BrocodeAuth>, b: Option<&BrocodeAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
    ...
    fn auths_equal(a: Option<&BrocodeAuth>, b: Option<&BrocodeAuth>) -> bool {
        match (a, b) {
            (None, None) => true,
            (Some(a), Some(b)) => a == b,
            _ => false,
    ...
    fn load_auth_from_storage(&self) -> Option<BrocodeAuth> {
        load_auth(
            &self.brocode_home,
            self.enable_brocode_api_key_env,
    ...
    fn set_cached_auth(&self, new_auth: Option<BrocodeAuth>) -> bool {
        if let Ok(mut guard) = self.inner.write() {
    ...
    pub fn brocode_api_key_env_enabled(&self) -> bool {
        self.enable_brocode_api_key_env
    }
    ...
    async fn resolve_external_bearer_auth(&self) -> Option<BrocodeAuth> {
        let ExternalAuth::Bearer(bearer_auth) = self.external_auth()? else {
            return None;
        };

        match bearer_auth.resolve_access_token().await {
            Ok(access_token) => Some(BrocodeAuth::from_api_key(&access_token)),
            Err(err) => {
    ...
    pub async fn refresh_token(&self) -> Result<(), RefreshTokenError> {
        ...
        if auth_before_reload
            .as_ref()
            .is_some_and(BrocodeAuth::is_api_key_auth)
        {
            return Ok(());
        }
        let expected_account_id = auth_before_reload
            .as_ref()
            .and_then(BrocodeAuth::get_account_id);
    ...
    async fn refresh_token_from_authority_impl(&self) -> Result<(), RefreshTokenError> {
        ...
        let result = match auth {
            BrocodeAuth::ChatgptAuthTokens(_) => {
                self.refresh_external_auth(ExternalAuthRefreshReason::Unauthorized)
                    .await
            }
            BrocodeAuth::Chatgpt(chatgpt_auth) => {
    ...
    pub fn logout(&self) -> std::io::Result<bool> {
        let removed = logout_all_stores(&self.brocode_home, self.auth_credentials_store_mode)?;
    ...
    pub fn get_api_auth_mode(&self) -> Option<ApiAuthMode> {
        ...
        self.auth_cached().as_ref().map(BrocodeAuth::api_auth_mode)
    }
    ...
    pub fn auth_mode(&self) -> Option<crate::AuthMode> {
        ...
        self.auth_cached().as_ref().map(BrocodeAuth::auth_mode)
    }

    fn is_stale_for_proactive_refresh(auth: &BrocodeAuth) -> bool {
        let chatgpt_auth = match auth {
            BrocodeAuth::Chatgpt(chatgpt_auth) => chatgpt_auth,
            _ => return false,
        };
