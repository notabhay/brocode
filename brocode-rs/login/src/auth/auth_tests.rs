...
fn missing_auth_json_returns_none() {
    let dir = tempdir().unwrap();
    let auth = BrocodeAuth::from_auth_storage(dir.path(), AuthCredentialsStoreMode::File)
        .expect("call should succeed");
    assert_eq!(auth, None);
}

#[tokio::test]
#[serial(brocode_api_key)]
...
fn refresh_failure_is_scoped_to_the_matching_auth_snapshot() {
    ...
    let updated_auth = BrocodeAuth::from_auth_dot_json(
        brocode_home.path(),
...
fn write_auth_file(params: AuthFileParams, brocode_home: &Path) -> std::io::Result<String> {
