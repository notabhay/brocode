//! Default Brocode HTTP client: shared `User-Agent`, `originator`, optional residency header, and
//! reqwest/`BrocodeHttpClient` construction.
//!
//! Use [`crate::default_client`] or [`brocode_login::default_client`] from other crates in this
//! workspace.

use brocode_client::BrocodeHttpClient;
pub use brocode_client::BrocodeRequestBuilder;
use brocode_client::BuildCustomCaTransportError;
...
/// Parenthesis will be added by Brocode. This should only specify what goes inside of the parenthesis.
pub static USER_AGENT_SUFFIX: LazyLock<Mutex<Option<String>>> = LazyLock::new(|| Mutex::new(None));
pub const DEFAULT_ORIGINATOR: &str = "brocode_cli_rs";
pub const BROCODE_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR: &str =
    "BROCODE_INTERNAL_ORIGINATOR_OVERRIDE";
pub const RESIDENCY_HEADER_NAME: &str = "x-openai-internal-brocode-residency";
...
fn get_originator_value(provided: Option<String>) -> Originator {
    let value = std::env::var(BROCODE_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR)
        .ok()
        .or(provided)
...
pub fn originator() -> Originator {
    ...
    if std::env::var(BROCODE_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR).is_ok() {
...
pub fn is_first_party_chat_originator(originator_value: &str) -> bool {
    originator_value == "brocode_atlas" || originator_value == "brocode_chatgpt_desktop"
}

pub fn get_brocode_user_agent() -> String {
...
fn sanitize_user_agent(candidate: String, fallback: &str) -> String {
    ...
    if !sanitized.is_empty() && HeaderValue::from_str(sanitized.as_str()).is_ok() {
        tracing::warn!(
            "Sanitized Brocode user agent because provided suffix contained invalid header characters"
        );
        sanitized
    } else if HeaderValue::from_str(fallback).is_ok() {
        tracing::warn!(
            "Falling back to base Brocode user agent because provided suffix could not be sanitized"
        );
        fallback.to_string()
    } else {
        tracing::warn!(
            "Falling back to default Brocode originator because base user agent string is invalid"
        );
        originator().value
    }
}
...
pub fn create_client() -> BrocodeHttpClient {
    let inner = build_reqwest_client();
    BrocodeHttpClient::new(inner)
}

/// Builds the default reqwest client used for ordinary Brocode HTTP traffic.
///
/// This starts from the standard Brocode user agent, default headers, and sandbox-specific proxy
/// policy, then layers in shared custom CA handling from `BROCODE_CA_CERTIFICATE` /
...
/// Tries to build the default reqwest client used for ordinary Brocode HTTP traffic.
///
...
fn is_sandboxed() -> bool {
    std::env::var("BROCODE_SANDBOX").as_deref() == Ok("seatbelt")
}
