                ...
                Ok(tokens) => {
                    ...
                    match tiny_http::Header::from_bytes(&b"Location"[..], success_url.as_bytes()) {
                        ...
                        Err(_) => login_error_response(
                            "Sign-in completed but redirecting back to Brocode failed.",
                            io::ErrorKind::Other,
...
fn build_authorize_url(
    ...
    let mut query = vec![
        ...
        (
            "brocode_cli_simplified_flow".to_string(),
            "true".to_string(),
        ),
...
pub(crate) async fn persist_tokens_async(
    ...
    let brocode_home = brocode_home.to_path_buf();
    tokio::task::spawn_blocking(move || {
        ...
        save_auth(&brocode_home, &auth, auth_credentials_store_mode)
    })
    .await
...
/// Returns true when the OAuth callback represents a missing Brocode entitlement.
fn is_missing_brocode_entitlement_error(error_code: &str, error_description: Option<&str>) -> bool {
    error_code == "access_denied"
        && error_description.is_some_and(|description| {
            description
                .to_ascii_lowercase()
                .contains("missing_brocode_entitlement")
        })
}
...
fn oauth_callback_error_message(error_code: &str, error_description: Option<&str>) -> String {
    if is_missing_brocode_entitlement_error(error_code, error_description) {
        return "Brocode is not enabled for your workspace. Contact your workspace administrator to request access to Brocode.".to_string();
    }
...
fn render_login_error_page(
    ...
    let (title, display_message, display_description, help_text) =
        if is_missing_brocode_entitlement_error(code, error_description) {
            (
                "You do not have access to Brocode".to_string(),
                "This account is not currently authorized to use Brocode in this workspace."
                    .to_string(),
                "Contact your workspace administrator to request access to Brocode.".to_string(),
                "Contact your workspace administrator to get access to Brocode, then return to Brocode and try again."
                    .to_string(),
            )
        } else {
            (
                ...
                "Return to Brocode to retry, switch accounts, or contact your workspace admin if access is restricted."
                    .to_string(),
            )
        };
    template
