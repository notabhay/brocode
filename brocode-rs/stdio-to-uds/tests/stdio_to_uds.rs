...
fn pipes_stdin_and_stdout_through_socket() -> anyhow::Result<()> {
    ...
    let mut child = Command::new(brocode_utils_cargo_bin::cargo_bin("brocode-stdio-to-uds")?)
        .arg(&socket_path)
        .stdin(Stdio::from(stdin))
        .stdout(Stdio::piped())
        .stderr(Stdio::piped())
        .spawn()
        .context("failed to spawn brocode-stdio-to-uds")?;
    ...
    let status = loop {
        ...
        if Instant::now() >= deadline {
            ...
            anyhow::bail!(
                "brocode-stdio-to-uds did not exit in time; server events: {:?}; stderr: {}",
                server_events,
    ...
    assert!(
        status.success(),
        "brocode-stdio-to-uds exited with {status}; server events: {:?}; stderr: {}",
        server_events,
        String::from_utf8_lossy(&stderr).trim_end()
    );
