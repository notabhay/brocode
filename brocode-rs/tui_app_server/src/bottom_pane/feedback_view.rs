...
use brocode_feedback::feedback_diagnostics::FEEDBACK_DIAGNOSTICS_ATTACHMENT_FILENAME;
use brocode_feedback::feedback_diagnostics::FeedbackDiagnostics;
            ...
            Ok(()) => {
                ...
                let mut lines = vec![Line::from(match issue_url.as_ref() {
                    Some(_) if self.feedback_audience == FeedbackAudience::OpenAiEmployee => {
                        format!("{prefix} Please report this in #brocode-feedback:")
                    }
...
fn slack_feedback_url(_thread_id: &str) -> String {
    BROCODE_FEEDBACK_INTERNAL_URL.to_string()
}
...
pub(crate) fn feedback_upload_consent_params(
    ...
    let mut header_lines: Vec<Box<dyn crate::render::renderable::Renderable>> = vec![
        ...
        Line::from(vec!["  • ".into(), "brocode-logs.log".into()]).into(),
    ];
...
mod tests {
    ...
    use crate::app_event::AppEvent;
    use crate::app_event_sender::AppEventSender;
    use brocode_feedback::feedback_diagnostics::FeedbackDiagnostic;
