...
use brocode_core::config::ConstraintResult;
...
use brocode_git_utils::current_branch_name;
use brocode_git_utils::get_git_repo_root;
use brocode_git_utils::local_git_branches;
use brocode_git_utils::recent_commits;
...
use brocode_terminal_detection::terminal_info;
use brocode_utils_absolute_path::AbsolutePathBuf;
use brocode_utils_sleep_inhibitor::SleepInhibitor;
...
use brocode_utils_approval_presets::ApprovalPreset;
...
/// and by sending requests back to brocode-core.
///
...
pub(crate) struct ChatWidget {
    ...
    /// Tracks whether brocode-core currently considers an agent turn to be in progress.
    ///
    ...
    session_network_proxy: Option<brocode_protocol::protocol::SessionNetworkProxyRuntime>,
    // Shared latch so we only warn once about invalid status-line item IDs.
...
fn exec_approval_request_from_params(
    ...
    ExecApprovalRequestEvent {
        ...
        skill_metadata: params.skill_metadata.map(|metadata| {
            brocode_protocol::approvals::ExecApprovalRequestSkillMetadata {
...
impl ChatWidget {
    ...
    fn on_session_configured(&mut self, event: brocode_protocol::protocol::SessionConfiguredEvent) {
        self.bottom_pane
    ...
    fn emit_forked_thread_event(&self, forked_from_id: ThreadId) {
        ...
        tokio::spawn(async move {
            ...
            match find_thread_name_by_id(&brocode_home, &forked_from_id).await {
    ...
    fn on_thread_name_updated(
        &mut self,
        event: brocode_protocol::protocol::ThreadNameUpdatedEvent,
    ) {
    ...
    fn show_feedback_note(
        ...
        category: crate::app_event::FeedbackCategory,
        include_logs: bool,
        snapshot: brocode_feedback::FeedbackSnapshot,
    ) {
    ...
    pub(crate) fn on_rate_limit_snapshot(&mut self, snapshot: Option<RateLimitSnapshot>) {
        if let Some(mut snapshot) = snapshot {
            ...
            let is_brocode_limit = limit_id.eq_ignore_ascii_case("brocode");
            let warnings = if is_brocode_limit {
            ...
            let high_usage = is_brocode_limit
                && (snapshot
                    .secondary
                    .as_ref()
            ...
            match tool {
                ...
                Some("mcp_tool_call") => {
                    ...
                    history_cell::new_guardian_denied_action_request(format!(
                        "brocode to call MCP tool {server}.{tool_name}"
                    ))
                }
                Some("network_access") => {
                    let target = action
    ...
    fn on_patch_apply_end(&mut self, event: brocode_protocol::protocol::PatchApplyEndEvent) {
        let ev2 = event.clone();
        self.defer_or_handle(
    ...
    pub(crate) fn handle_history_entry_response(
        ...
        let brocode_protocol::protocol::GetHistoryEntryResponseEvent {
            offset,
            log_id,
            entry,
        } = event;
    ...
    fn on_hook_started(&mut self, event: brocode_protocol::protocol::HookStartedEvent) {
    ...
    fn on_hook_completed(&mut self, event: brocode_protocol::protocol::HookCompletedEvent) {
        ...
        for entry in event.run.entries {
            let prefix = match entry.kind {
                brocode_protocol::protocol::HookOutputEntryKind::Warning => "warning: ",
                brocode_protocol::protocol::HookOutputEntryKind::Stop => "stop: ",
                brocode_protocol::protocol::HookOutputEntryKind::Feedback => "feedback: ",
                brocode_protocol::protocol::HookOutputEntryKind::Context => "hook context: ",
                brocode_protocol::protocol::HookOutputEntryKind::Error => "error: ",
            };
    ...
    pub(crate) fn handle_patch_apply_end_now(
        &mut self,
        event: brocode_protocol::protocol::PatchApplyEndEvent,
    ) {
        ...
        match cmd {
            ...
            SlashCommand::Rename => {
                self.session_telemetry
                    .counter("brocode.thread.rename", /*inc*/ 1, &[]);
            ...
            SlashCommand::ElevateSandbox => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    self.session_telemetry.counter(
                        "brocode.windows_sandbox.setup_elevated_sandbox_command",
                        /*inc*/ 1,
                        &[],
                    );
                    self.app_event_tx
            ...
            SlashCommand::Logout => {
                if let Err(e) = brocode_core::auth::logout(
            ...
            // SlashCommand::Undo => {
            //     self.app_event_tx.send(AppEvent::BrocodeOp(Op::Undo));
            // }
            SlashCommand::Diff => {
    ...
    fn dispatch_command_with_args(
        ...
        match cmd {
            ...
            SlashCommand::Rename if !trimmed.is_empty() => {
                ...
                let Some(name) = brocode_core::util::normalize_thread_name(&prepared_args) else {
    ...
    fn submit_user_message(&mut self, user_message: UserMessage) {
        ...
        if let Some(apps) = self.connectors_for_mentions() {
            ...
            for app in app_mentions {
                let slug = brocode_core::connectors::connector_mention_slug(&app);
        ...
        match notification {
            ...
            ServerNotification::HookStarted(notification) => {
                if let Some(run) = convert_via_json(notification.run) {
                    self.on_hook_started(brocode_protocol::protocol::HookStartedEvent {
            ...
            ServerNotification::HookCompleted(notification) => {
                if let Some(run) = convert_via_json(notification.run) {
                    self.on_hook_completed(brocode_protocol::protocol::HookCompletedEvent {
                        turn_id: notification.turn_id,
                        run,
                    });
                }
    ...
    fn dispatch_event_msg(
        ...
        match msg {
            ...
            _ => {
                tracing::trace!("handle_brocode_event: {:?}", msg);
            }
        }
        ...
        match msg {
            ...
            EventMsg::ItemCompleted(event) => {
                let item = event.item;
                if !from_replay && let brocode_protocol::items::TurnItem::UserMessage(item) = &item
                {
                ...
                if let brocode_protocol::items::TurnItem::Plan(plan_item) = &item {
                    self.on_plan_item_completed(plan_item.text.clone());
                }
                if let brocode_protocol::items::TurnItem::AgentMessage(item) = item {
    ...
    fn prefetch_connectors_with_options(&mut self, force_refetch: bool) {
        ...
        tokio::spawn(async move {
            ...
            let should_schedule_force_refetch =
                !force_refetch && !accessible_result.brocode_apps_ready;
    ...
    fn is_auto_model(model: &str) -> bool {
        model.starts_with("brocode-auto-")
    }

    fn auto_model_order(model: &str) -> usize {
        match model {
            "brocode-auto-fast" => 0,
            "brocode-auto-balanced" => 1,
            "brocode-auto-thorough" => 2,
            _ => 3,
        }
    }
    ...
    pub(crate) fn open_all_models_popup(&mut self, presets: Vec<ModelPreset>) {
        ...
        let header = self.model_menu_header(
            ...
            "Access legacy models by running brocode -m <model_name> or in your config.toml",
        );
        self.bottom_pane.show_selection_view(SelectionViewParams {
    ...
    pub(crate) fn open_permissions_popup(&mut self) {
        ...
        let show_elevate_sandbox_hint = brocode_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
        ...
        for preset in presets.into_iter() {
            ...
            let default_actions: Vec<SelectionAction> = if requires_confirmation {
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    {
                        let preset_clone = preset.clone();
                        if brocode_core::windows_sandbox::ELEVATED_SANDBOX_NUX_ENABLED
                            && brocode_core::windows_sandbox::sandbox_setup_is_complete(
                                self.config.brocode_home.as_path(),
                            )
                        {
                            vec![Box::new(move |tx| {
    ...
    pub(crate) fn world_writable_warning_details(&self) -> Option<(Vec<String>, usize, bool)> {
        ...
        match brocode_windows_sandbox::apply_world_writable_scan_and_denies(
            self.config.brocode_home.as_path(),
            cwd.as_path(),
            &env_map,
            self.config.permissions.sandbox_policy.get(),
            Some(self.config.brocode_home.as_path()),
        ) {
            Ok(_) => None,
    ...
    pub(crate) fn open_windows_sandbox_enable_prompt(&mut self, preset: ApprovalPreset) {
        ...
        self.session_telemetry.counter(
            "brocode.windows_sandbox.elevated_prompt_shown",
            /*inc*/ 1,
            &[],
        );
        ...
        let items = vec![
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    accept_otel.counter(
                        "brocode.windows_sandbox.elevated_prompt_accept",
                        /*inc*/ 1,
                        &[],
                    );
            ...
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    legacy_otel.counter(
                        "brocode.windows_sandbox.elevated_prompt_use_legacy",
                        /*inc*/ 1,
                        &[],
                    );
            ...
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    quit_otel.counter(
                        "brocode.windows_sandbox.elevated_prompt_quit",
                        /*inc*/ 1,
                        &[],
                    );
        ...
        let items = vec![
            SelectionItem {
                ...
                actions: vec![Box::new({
                    ...
                    move |tx| {
                        otel.counter(
                            "brocode.windows_sandbox.fallback_retry_elevated",
                            /*inc*/ 1,
                            &[],
                        );
                        tx.send(AppEvent::BeginWindowsSandboxElevatedSetup {
            ...
            SelectionItem {
                ...
                actions: vec![Box::new({
                    ...
                    move |tx| {
                        otel.counter(
                            "brocode.windows_sandbox.fallback_use_legacy",
                            /*inc*/ 1,
                            &[],
                        );
                        tx.send(AppEvent::BeginWindowsSandboxLegacySetup {
            ...
            SelectionItem {
                ...
                actions: vec![Box::new(move |tx| {
                    quit_otel.counter(
                        "brocode.windows_sandbox.fallback_prompt_quit",
                        /*inc*/ 1,
                        &[],
                    );
                    tx.send(AppEvent::Exit(ExitMode::ShutdownFirst));
    ...
    pub(crate) fn set_collaboration_mask(&mut self, mut mask: CollaborationModeMask) {
            ...
            && (previous_model != next_model || previous_effort != next_effort)
        {
            ...
            if !next_model.starts_with("brocode-auto-") {
    ...
    fn plugins_for_mentions(&self) -> Option<&[brocode_core::plugins::PluginCapabilitySummary]> {
    ...
    pub(crate) fn refresh_plugin_mentions(&mut self) {
        ...
        let plugins = PluginsManager::new(self.config.brocode_home.clone())
...
impl Notification {
    ...
    fn user_input_request_summary(
        questions: &[brocode_protocol::request_user_input::RequestUserInputQuestion],
    ) -> Option<String> {
...
fn hook_event_label(event_name: brocode_protocol::protocol::HookEventName) -> &'static str {
    match event_name {
        brocode_protocol::protocol::HookEventName::PreToolUse => "PreToolUse",
        brocode_protocol::protocol::HookEventName::PostToolUse => "PostToolUse",
        brocode_protocol::protocol::HookEventName::SessionStart => "SessionStart",
        brocode_protocol::protocol::HookEventName::UserPromptSubmit => "UserPromptSubmit",
        brocode_protocol::protocol::HookEventName::Stop => "Stop",
    }
}
