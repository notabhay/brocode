...
async fn run_session_picker_with_loader(
    ...
    let brocode_home = config.brocode_home.as_path();
...
impl PickerState {
    ...
    async fn update_thread_names(&mut self) {
        ...
        let names = find_thread_names_by_ids(&self.brocode_home, &missing_ids)
            .await
            .unwrap_or_default();
