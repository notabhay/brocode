//! Custom CA handling for Brocode outbound HTTP and websocket clients.
//!
//! Brocode constructs outbound reqwest clients and secure websocket connections in a few crates, but
...
//!
//! - read CA material from `BROCODE_CA_CERTIFICATE`, falling back to `SSL_CERT_FILE`
...
//!   because Brocode itself commonly runs spawned test processes under seatbelt, so this is not just
...
use brocode_utils_rustls_provider::ensure_rustls_crypto_provider;
...
pub const BROCODE_CA_CERT_ENV: &str = "BROCODE_CA_CERTIFICATE";
pub const SSL_CERT_FILE_ENV: &str = "SSL_CERT_FILE";
const CA_CERT_HINT: &str = "If you set BROCODE_CA_CERTIFICATE or SSL_CERT_FILE, ensure it points to a PEM file containing one or more CERTIFICATE blocks, or unset it to use system roots.";
...
/// Builds a reqwest client that honors Brocode custom CA environment variables.
///
/// Callers supply the baseline builder configuration they need, and this helper layers in custom
/// CA handling before finally constructing the client. `BROCODE_CA_CERTIFICATE` takes precedence
...
/// easy mistake to make when adding a new outbound Brocode HTTP path, and the resulting bug only
...
/// Builds a rustls client config when a Brocode custom CA bundle is configured.
///
/// This is the websocket-facing sibling of [`build_reqwest_client_with_custom_ca`]. When
/// `BROCODE_CA_CERTIFICATE` or `SSL_CERT_FILE` selects a CA bundle, the returned config starts from
...
fn maybe_build_rustls_client_config_with_env(
    ...
    // they would get from tungstenite's default rustls connector, then layer in the Brocode custom
...
fn build_reqwest_client_with_env(
    ...
    info!(
        brocode_ca_certificate_configured = false,
...
trait EnvSource {
    ...
    ///
    /// `BROCODE_CA_CERTIFICATE` wins over `SSL_CERT_FILE` because it is the Brocode-specific override.
    ...
    fn configured_ca_bundle(&self) -> Option<ConfiguredCaBundle> {
        self.non_empty_path(BROCODE_CA_CERT_ENV)
            .map(|path| ConfiguredCaBundle {
                source_env: BROCODE_CA_CERT_ENV,
                path,
            })
            .or_else(|| {
...
impl ConfiguredCaBundle {
    ...
    /// Loads every certificate block from a PEM file intended for Brocode CA overrides.
    ///
    /// This accepts a few common real-world variants so Brocode behaves like other CA-aware tooling:
...
impl NormalizedPem {
    ...
    ///
    /// Brocode only needs certificate DER bytes to seed `reqwest`'s root store, but operators may
...
mod tests {
    ...
    #[test]
    fn ca_path_prefers_brocode_env() {
        let env = map_env(&[
            (BROCODE_CA_CERT_ENV, "/tmp/brocode.pem"),
            (SSL_CERT_FILE_ENV, "/tmp/fallback.pem"),
        ]);

        assert_eq!(
            env.configured_ca_bundle().map(|bundle| bundle.path),
            Some(PathBuf::from("/tmp/brocode.pem"))
        );
    }
    ...
    fn rustls_config_uses_custom_ca_bundle_when_configured() {
        ...
        let env = map_env(&[(BROCODE_CA_CERT_ENV, cert_path.to_string_lossy().as_ref())]);
