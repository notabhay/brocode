...
//! detailed explanation of what "hermetic" means here lives in `brocode_client::custom_ca`; these
...
const BROCODE_CA_CERT_ENV: &str = "BROCODE_CA_CERTIFICATE";
...
fn run_probe(envs: &[(&str, &Path)]) -> std::process::Output {
    ...
    cmd.env_remove(BROCODE_CA_CERT_ENV);
...
#[test]
fn uses_brocode_ca_cert_env() {
    let temp_dir = TempDir::new().expect("tempdir");
    let cert_path = write_cert_file(&temp_dir, "ca.pem", TEST_CERT_1);

    let output = run_probe(&[(BROCODE_CA_CERT_ENV, cert_path.as_path())]);
...
#[test]
fn prefers_brocode_ca_cert_over_ssl_cert_file() {
    let temp_dir = TempDir::new().expect("tempdir");
    let cert_path = write_cert_file(&temp_dir, "ca.pem", TEST_CERT_1);
    let bad_path = write_cert_file(&temp_dir, "bad.pem", "");

    let output = run_probe(&[
        (BROCODE_CA_CERT_ENV, cert_path.as_path()),
...
fn rejects_empty_pem_file_with_hint() {
    ...
    assert!(stderr.contains("BROCODE_CA_CERTIFICATE"));
