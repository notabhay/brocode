...
impl LocalSecretsBackend {
    pub fn new(brocode_home: PathBuf, keyring_store: Arc<dyn KeyringStore>) -> Self {
        Self {
            brocode_home,
            keyring_store,
        }
    ...
    fn secrets_dir(&self) -> PathBuf {
        self.brocode_home.join("secrets")
    }
    ...
    fn load_or_create_passphrase(&self) -> Result<SecretString> {
        let account = compute_keyring_account(&self.brocode_home);
        let loaded = self
            .keyring_store
...
mod tests {
    ...
    fn load_file_rejects_newer_schema_versions() -> Result<()> {
        ...
        let backend = LocalSecretsBackend::new(brocode_home.path().to_path_buf(), keyring);
    ...
    fn set_fails_when_keyring_is_unavailable() -> Result<()> {
        let brocode_home = tempfile::tempdir().expect("tempdir");
        let keyring = Arc::new(MockKeyringStore::default());
        let account = compute_keyring_account(brocode_home.path());
        keyring.set_error(
            &account,
