...
impl StateRuntime {
    /// Initialize the state runtime using the provided Brocode home and default provider.
    ///
    /// This opens (and migrates) the SQLite databases under `brocode_home`,
    ...
    pub async fn init(
        ...
        .await;
        let state_path = state_db_path(brocode_home.as_path());
        let logs_path = logs_db_path(brocode_home.as_path());
    ...
    /// Return the configured Brocode home directory for this runtime.
    pub fn brocode_home(&self) -> &Path {
...
pub fn state_db_path(brocode_home: &Path) -> PathBuf {
    brocode_home.join(state_db_filename())
}
...
pub fn logs_db_path(brocode_home: &Path) -> PathBuf {
    brocode_home.join(logs_db_filename())
}
...
async fn remove_legacy_db_files(
    ...
    let mut entries = match tokio::fs::read_dir(brocode_home).await {
        Ok(entries) => entries,
        Err(err) => {
            warn!(
                "failed to read brocode_home for {db_label} db cleanup {}: {err}",
