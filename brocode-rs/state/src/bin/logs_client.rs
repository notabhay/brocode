...
use anyhow::Context;
use brocode_state::LogQuery;
use brocode_state::LogRow;
...
async fn main() -> anyhow::Result<()> {
    ...
    let brocode_home = db_path
        .parent()
        .map(ToOwned::to_owned)
        .unwrap_or_else(|| PathBuf::from("."));
    let runtime = StateRuntime::init(brocode_home, "logs-client".to_string()).await?;
...
fn resolve_db_path(args: &Args) -> anyhow::Result<PathBuf> {
    ...
    let brocode_home = args
        .brocode_home
        .clone()
        .unwrap_or_else(default_brocode_home);
    Ok(brocode_state::logs_db_path(brocode_home.as_path()))
}

fn default_brocode_home() -> PathBuf {
    if let Some(home) = home_dir() {
        return home.join(".brocode");
    }
    PathBuf::from(".brocode")
}
