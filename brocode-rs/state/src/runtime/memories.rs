...
mod tests {
    ...
    async fn stage1_claim_skips_when_up_to_date() {
        let brocode_home = unique_temp_dir();
        let runtime = StateRuntime::init(brocode_home.clone(), "test-provider".to_string())
            .await
            .expect("initialize runtime");

        let thread_id = ThreadId::from_string(&Uuid::new_v4().to_string()).expect("thread id");
        let metadata = test_thread_metadata(&brocode_home, thread_id, brocode_home.join("a"));
        runtime
        ...
        let _ = tokio::fs::remove_dir_all(brocode_home).await;
    }
    ...
    async fn stage1_running_stale_can_be_stolen_but_fresh_running_is_skipped() {
        ...
        let owner_a = ThreadId::from_string(&Uuid::new_v4().to_string()).expect("owner id");
        let owner_b = ThreadId::from_string(&Uuid::new_v4().to_string()).expect("owner id");
        let cwd = brocode_home.join("workspace");
        runtime
            .upsert_thread(&test_thread_metadata(&brocode_home, thread_id, cwd))
            .await
            .expect("upsert thread");
    ...
    async fn claim_stage1_jobs_filters_by_age_idle_and_current_thread() {
        ...
        let mut fresh =
            test_thread_metadata(&brocode_home, fresh_thread_id, brocode_home.join("fresh"));
        ...
        let mut just_under_idle = test_thread_metadata(
            ...
            brocode_home.join("just-under-idle"),
        );
        ...
        let mut eligible_idle = test_thread_metadata(
            ...
            brocode_home.join("eligible-idle"),
        );
        ...
        let mut old = test_thread_metadata(&brocode_home, old_thread_id, brocode_home.join("old"));
    ...
    async fn claim_stage1_jobs_prefilters_threads_with_up_to_date_memory() {
        ...
        let mut stale =
            test_thread_metadata(&brocode_home, stale_thread_id, brocode_home.join("stale"));
    ...
    async fn claim_stage1_jobs_enforces_global_running_cap() {
        ...
        for idx in 0..total_candidates {
            ...
            let mut metadata = test_thread_metadata(
                &brocode_home,
                thread_id,
                brocode_home.join(format!("thread-{idx}")),
            );
  ...
  AND lease_until > ?
    ...
    async fn list_stage1_outputs_for_global_returns_latest_outputs() {
        ...
        runtime
            ...
            .expect("upsert thread a");
        let mut metadata_b =
            test_thread_metadata(&brocode_home, thread_id_b, brocode_home.join("workspace-b"));
        ...
        assert_eq!(outputs[0].cwd, brocode_home.join("workspace-b"));
        ...
        assert_eq!(outputs[1].cwd, brocode_home.join("workspace-a"));
    ...
    async fn list_stage1_outputs_for_global_skips_empty_payloads() {
        ...
        runtime
            .upsert_thread(&test_thread_metadata(
                &brocode_home,
                thread_id_non_empty,
                brocode_home.join("workspace-non-empty"),
            ))
            .await
        ...
        runtime
            .upsert_thread(&test_thread_metadata(
                &brocode_home,
                thread_id_empty,
                brocode_home.join("workspace-empty"),
            ))
            .await
...
VALUES (?, ?, ?, ?, ?)
        ...
        assert_eq!(outputs[0].cwd, brocode_home.join("workspace-non-empty"));
    ...
    async fn get_phase2_input_selection_reports_added_retained_and_removed_rows() {
        ...
        assert_eq!(
            ...
            brocode_home.join(format!("rollout-{thread_id_c}.jsonl"))
        );
        ...
        for (thread_id, updated_at) in [(thread_id_enabled, 100), (thread_id_polluted, 101)] {
            runtime
                .upsert_thread(&test_thread_metadata(
                    &brocode_home,
                    thread_id,
                    brocode_home.join(thread_id.to_string()),
                ))
                .await
                .expect("upsert thread");
