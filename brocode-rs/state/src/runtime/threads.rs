...
mod tests {
    ...
    use brocode_protocol::protocol::GitInfo;
    ...
    async fn apply_rollout_items_restores_memory_mode_from_session_meta() {
        ...
        let metadata = test_thread_metadata(&brocode_home, thread_id, brocode_home.clone());
    ...
    async fn insert_thread_if_absent_preserves_existing_metadata() {
        ...
        let mut existing = test_thread_metadata(&brocode_home, thread_id, brocode_home.clone());
        ...
        let mut fallback = test_thread_metadata(&brocode_home, thread_id, brocode_home.clone());
    ...
    async fn apply_rollout_items_uses_override_updated_at_when_provided() {
        ...
        let items = vec![RolloutItem::EventMsg(EventMsg::TokenCount(
            brocode_protocol::protocol::TokenCountEvent {
                info: Some(brocode_protocol::protocol::TokenUsageInfo {
                    total_token_usage: brocode_protocol::protocol::TokenUsage {
                        input_tokens: 0,
                    ...
                    last_token_usage: brocode_protocol::protocol::TokenUsage::default(),
    ...
    async fn thread_spawn_edges_track_directional_status() {
        let brocode_home = unique_temp_dir();
        let runtime = StateRuntime::init(brocode_home, "test-provider".to_string())
            .await
            .expect("state db should initialize");
