...
This repository uses Bazel to build the Rust workspace under `brocode-rs`.
...
- `rules_rs` imports third-party crates from `brocode-rs/Cargo.toml` and
  `brocode-rs/Cargo.lock` via `crate.from_cargo(...)` and exposes them under
  `@crates`.
- `../defs.bzl` provides `brocode_rust_crate`, which wraps `rust_library`,
...
- Each crate in `brocode-rs/*/BUILD.bazel` typically uses `brocode_rust_crate` and
...
2. Create a `BUILD.bazel` that calls `brocode_rust_crate` (see nearby crates for
   examples).
3. If a dependency needs special handling (compile/runtime data, additional binaries
   ...
   `brocode_rust_crate` to configure it.
