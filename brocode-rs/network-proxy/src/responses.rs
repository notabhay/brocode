...
pub fn blocked_message(reason: &str) -> &'static str {
    match reason {
        REASON_NOT_ALLOWED => {
            "Brocode blocked this request: domain not in allowlist (this is not a denylist block)."
        }
        REASON_NOT_ALLOWED_LOCAL => {
            "Brocode blocked this request: local/private addresses not allowed."
        }
        REASON_DENIED => "Brocode blocked this request: domain denied by policy.",
        REASON_METHOD_NOT_ALLOWED => {
            "Brocode blocked this request: method not allowed in limited mode."
        }
        REASON_MITM_REQUIRED => "Brocode blocked this request: MITM required for limited HTTPS.",
        _ => "Brocode blocked this request by network policy.",
    }
}
