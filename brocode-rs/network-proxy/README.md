# brocode-network-proxy

`brocode-network-proxy` is Brocode's local network policy enforcement proxy. It runs:
...
`brocode-network-proxy` reads from Brocode's merged `config.toml` (via `brocode-core` config loading).
...
mitm = false
# CA cert/key are managed internally under $BROCODE_HOME/proxy/ (ca.pem + ca.key).
...
```bash
cargo run -p brocode-network-proxy --
```
...
`brocode-network-proxy` can be embedded as a library with a thin API:

```rust
use brocode_network_proxy::{NetworkProxy, NetworkDecision, NetworkPolicyRequest};
...
When `brocode-network-proxy` is embedded in managed Brocode runtime, policy decisions emit structured
OTEL-compatible events with `target=brocode_otel.network_proxy`.

Event name:

- `brocode.network_proxy.policy_decision`
...
This section documents the protections implemented by `brocode-network-proxy`, and the boundaries of
