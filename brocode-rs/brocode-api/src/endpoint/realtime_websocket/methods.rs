...
impl RealtimeWebsocketClient {
    ...
    pub async fn connect(
        ...
        // Realtime websocket TLS should honor the same custom-CA env vars as the rest of Brocode's
...
mod tests {
    ...
    use crate::endpoint::realtime_websocket::protocol::RealtimeTranscriptDelta;
    use crate::endpoint::realtime_websocket::protocol::RealtimeTranscriptEntry;
    use brocode_protocol::protocol::RealtimeInputAudioSpeechStarted;
    use brocode_protocol::protocol::RealtimeResponseCancelled;
    use http::HeaderValue;
    ...
    #[tokio::test]
    async fn realtime_v2_session_update_includes_brocode_tool_and_handoff_output_item() {
        let listener = TcpListener::bind("127.0.0.1:0").await.expect("bind");
        let addr = listener.local_addr().expect("local addr");
        ...
        let server = tokio::spawn(async move {
            ...
            assert_eq!(
                ...
                Value::String("brocode".to_string())
            );
            assert_eq!(
