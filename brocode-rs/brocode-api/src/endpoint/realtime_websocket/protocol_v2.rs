...
const BROCODE_TOOL_NAME: &str = "brocode";
    ...
    let item = parsed
        ...
        .find(|item| {
            item.get("type").and_then(Value::as_str) == Some("function_call")
                && item.get("name").and_then(Value::as_str) == Some(BROCODE_TOOL_NAME)
        })?
        .as_object()?;
...
fn parse_handoff_requested_event(item: &JsonMap<String, Value>) -> Option<RealtimeEvent> {
    ...
    if item_type != Some("function_call") || item_name != Some(BROCODE_TOOL_NAME) {
        return None;
    }
