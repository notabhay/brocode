...
use brocode_client::TransportError;
use brocode_client::maybe_build_rustls_client_config_with_custom_ca;
...
const X_BROCODE_TURN_STATE_HEADER: &str = "x-brocode-turn-state";
...
async fn connect_websocket(
    ...
    // If a Brocode-specific CA bundle is configured, build an explicit rustls connector so this
    ...
    if let Some(turn_state) = turn_state
        && let Some(header_value) = response
            .headers()
            .get(X_BROCODE_TURN_STATE_HEADER)
            .and_then(|value| value.to_str().ok())
    {
...
mod tests {
    ...
    fn parse_wrapped_websocket_error_event_maps_to_transport_http() {
        ...
        assert_eq!(
            headers
                .get("x-brocode-primary-used-percent")
                .and_then(|value| value.to_str().ok()),
            Some("100.0")
        );
        assert_eq!(
            headers
                .get("x-brocode-primary-window-minutes")
                .and_then(|value| value.to_str().ok()),
            Some("15")
        );
