...
const SYSTEM_SKILLS_MARKER_FILENAME: &str = ".brocode-system-skills.marker";
const SYSTEM_SKILLS_MARKER_SALT: &str = "v1";

/// Returns the on-disk cache location for embedded system skills.
///
/// This is typically located at `BROCODE_HOME/skills/.system`.
pub fn system_cache_root_dir(brocode_home: &Path) -> PathBuf {
    AbsolutePathBuf::try_from(brocode_home)
        .and_then(|brocode_home| system_cache_root_dir_abs(&brocode_home))
...
fn system_cache_root_dir_abs(brocode_home: &AbsolutePathBuf) -> std::io::Result<AbsolutePathBuf> {
    brocode_home
        .join(SKILLS_DIR_NAME)?
        .join(SYSTEM_SKILLS_DIR_NAME)
}

/// Installs embedded system skills into `BROCODE_HOME/skills/.system`.
///
...
/// install is skipped.
pub fn install_system_skills(brocode_home: &Path) -> Result<(), SystemSkillsError> {
    let brocode_home = AbsolutePathBuf::try_from(brocode_home)
        .map_err(|source| SystemSkillsError::io("normalize brocode home dir", source))?;
    let skills_root_dir = brocode_home
        .join(SKILLS_DIR_NAME)
    ...
    let dest_system = system_cache_root_dir_abs(&brocode_home)
