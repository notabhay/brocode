...
fn main() {
    ...
    println!("cargo:rerun-if-env-changed=BROCODE_BWRAP_SOURCE_DIR");
...
fn try_build_vendored_bwrap() -> Result<(), String> {
    ...
    std::fs::write(
        &config_h,
        r#"#pragma once
#define PACKAGE_STRING "bubblewrap built at brocode build-time"
"#,
    )
...
///
/// Priority:
/// 1. `BROCODE_BWRAP_SOURCE_DIR` points at an existing bubblewrap checkout.
/// 2. The vendored bubblewrap tree under `brocode-rs/vendor/bubblewrap`.
fn resolve_bwrap_source_dir(manifest_dir: &Path) -> Result<PathBuf, String> {
    if let Ok(path) = env::var("BROCODE_BWRAP_SOURCE_DIR") {
        let src_dir = PathBuf::from(path);
        if src_dir.exists() {
            return Ok(src_dir);
        }
        return Err(format!(
            "BROCODE_BWRAP_SOURCE_DIR was set but does not exist: {}",
            src_dir.display()
        ));
    }
...
Set BROCODE_BWRAP_SOURCE_DIR to an existing checkout or vendor bubblewrap under brocode-rs/vendor.",
        vendor_dir.display()
    ))
}
