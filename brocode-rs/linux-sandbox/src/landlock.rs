...
pub(crate) fn apply_sandbox_policy_to_current_thread(
    ...
    if apply_landlock_fs && !sandbox_policy.has_full_disk_write_access() {
        if !sandbox_policy.has_full_disk_read_access() {
            return Err(BrocodeErr::UnsupportedOperation(
...
///
/// # Errors
/// Returns [`BrocodeErr::Sandbox`] variants when the ruleset fails to apply.
///
...
fn install_filesystem_landlock_rules_on_current_thread(
    ...
    if status.ruleset == landlock::RulesetStatus::NotEnforced {
        return Err(BrocodeErr::Sandbox(SandboxErr::LandlockRestrict));
    }
