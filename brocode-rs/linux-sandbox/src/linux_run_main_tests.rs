...
use super::*;
#[cfg(test)]
use brocode_protocol::protocol::FileSystemSandboxPolicy;
#[cfg(test)]
use brocode_protocol::protocol::NetworkSandboxPolicy;
#[cfg(test)]
...
fn inserts_bwrap_argv0_before_command_separator() {
    ...
    apply_inner_command_argv0_for_launcher(
        &mut argv,
        /*supports_argv0*/ true,
        "/tmp/brocode-arg0-session/brocode-linux-sandbox".to_string(),
    );
    assert_eq!(
        argv,
        vec![
            ...
            "/proc".to_string(),
            "--argv0".to_string(),
            "brocode-linux-sandbox".to_string(),
            "--".to_string(),
            "/bin/true".to_string(),
        ]
    );
...
fn rewrites_inner_command_path_when_bwrap_lacks_argv0() {
    ...
    assert!(
        argv.windows(2)
            .any(|window| { window == ["--", "/tmp/brocode-arg0-session/brocode-linux-sandbox"] })
    );
}
...
fn rewrites_bwrap_helper_command_not_nested_user_command_when_current_exe_appears_later() {
    ...
    let mut argv = vec![
        ...
        "--brocode-run-as-apply-patch".to_string(),
        "patch".to_string(),
    ];
...
fn split_only_filesystem_policy_requires_direct_runtime_enforcement() {
    ...
    let policy = FileSystemSandboxPolicy::restricted(vec![
        ...
        brocode_protocol::permissions::FileSystemSandboxEntry {
            path: brocode_protocol::permissions::FileSystemPath::Path { path: docs },
