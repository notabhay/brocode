# brocode-linux-sandbox

This crate is responsible for producing:

- a `brocode-linux-sandbox` standalone executable for Linux that is bundled with the Node.js version of the Brocode CLI
- a lib crate that exposes the business logic of the executable as `run_main()` so that
  - the `brocode-exec` CLI can check if its arg0 is `brocode-linux-sandbox` and, if so, execute as if it were `brocode-linux-sandbox`
  - this should also be true of the `brocode` multitool CLI
...
this binary.
Brocode also surfaces a startup warning when `/usr/bin/bwrap` is missing so users
...
- If `/usr/bin/bwrap` is missing, the helper falls back to the vendored
  bubblewrap path.
- If `/usr/bin/bwrap` is missing, Brocode also surfaces a startup warning instead
...
- When the default bubblewrap pipeline is active, protected subpaths under writable roots (for
  example `.git`,
  resolved `gitdir:`, and `.brocode`) are re-applied as read-only via `--ro-bind`.
...
**Notes**
- The CLI surface still uses legacy names like `brocode debug landlock`.
