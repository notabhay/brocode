...
use brocode_core::config::types::ShellEnvironmentPolicy;
use brocode_core::error::BrocodeErr;
use brocode_core::error::Result;
use brocode_core::error::SandboxErr;
use brocode_core::exec::ExecCapturePolicy;
use brocode_core::exec::ExecParams;
use brocode_core::exec::process_exec_tool_call;
...
async fn run_cmd_output(
    ...
    timeout_ms: u64,
) -> brocode_core::exec::ExecToolCallOutput {
...
async fn run_cmd_result_with_writable_roots(
    ...
    network_access: bool,
) -> Result<brocode_core::exec::ExecToolCallOutput> {
...
async fn run_cmd_result_with_policies(
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_brocode-linux-sandbox");
    let brocode_linux_sandbox_exe = Some(PathBuf::from(sandbox_program));
...
fn is_bwrap_unavailable_output(output: &brocode_core::exec::ExecToolCallOutput) -> bool {
...
async fn should_skip_bwrap_tests() -> bool {
    ...
    .await
    {
        ...
        Err(BrocodeErr::Sandbox(SandboxErr::Denied { output, .. })) => {
        ...
        Err(BrocodeErr::Sandbox(SandboxErr::Timeout { .. })) => true,
        Err(err) => panic!("bwrap availability probe failed unexpectedly: {err:?}"),
    }
}

fn expect_denied(
    result: Result<brocode_core::exec::ExecToolCallOutput>,
    context: &str,
) -> brocode_core::exec::ExecToolCallOutput {
    match result {
        Ok(output) => {
        ...
        Err(BrocodeErr::Sandbox(SandboxErr::Denied { output, .. })) => *output,
...
async fn assert_network_blocked(cmd: &[&str]) {
    ...
    let sandbox_program = env!("CARGO_BIN_EXE_brocode-linux-sandbox");
    let brocode_linux_sandbox_exe: Option<PathBuf> = Some(PathBuf::from(sandbox_program));
...
#[tokio::test]
async fn sandbox_blocks_git_and_brocode_writes_inside_writable_root() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let dot_brocode = tmpdir.path().join(".brocode");
    std::fs::create_dir_all(&dot_git).expect("create .git");
    std::fs::create_dir_all(&dot_brocode).expect("create .brocode");

    let git_target = dot_git.join("config");
    let brocode_target = dot_brocode.join("config.toml");
    ...
    let brocode_output = expect_denied(
        run_cmd_result_with_writable_roots(
            &[
                "bash",
                "-lc",
                &format!("echo denied > {}", brocode_target.to_string_lossy()),
            ],
            &[tmpdir.path().to_path_buf()],
            LONG_TIMEOUT_MS,
            false,
        ...
        .await,
        ".brocode write should be denied under bubblewrap",
    );
    assert_ne!(git_output.exit_code, 0);
    assert_ne!(brocode_output.exit_code, 0);
}

#[tokio::test]
async fn sandbox_blocks_brocode_symlink_replacement_attack() {
    if should_skip_bwrap_tests().await {
        eprintln!("skipping bwrap test: bwrap sandbox prerequisites are unavailable");
        return;
    }
    ...
    let tmpdir = tempfile::tempdir().expect("tempdir");
    let decoy = tmpdir.path().join("decoy-brocode");
    std::fs::create_dir_all(&decoy).expect("create decoy dir");

    let dot_brocode = tmpdir.path().join(".brocode");
    symlink(&decoy, &dot_brocode).expect("create .brocode symlink");
    ...
    let brocode_output = expect_denied(
        ...
        .await,
        ".brocode symlink replacement should be denied",
    );
    assert_ne!(brocode_output.exit_code, 0);
}
...
async fn sandbox_blocks_explicit_split_policy_carveouts_under_bwrap() {
    ...
    let sandbox_helper_dir = PathBuf::from(env!("CARGO_BIN_EXE_brocode-linux-sandbox"))
        .parent()
