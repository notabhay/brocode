...
pub async fn append_thread_name(
    ...
    append_session_index_entry(brocode_home, &entry).await
}
...
pub async fn append_session_index_entry(
    ...
    let path = session_index_path(brocode_home);
...
pub async fn find_thread_path_by_name_str(
    ...
    let Some(thread_id) = find_thread_id_by_name(brocode_home, name).await? else {
        return Ok(None);
    };
    super::list::find_thread_path_by_id_str(brocode_home, &thread_id.to_string()).await
}

fn session_index_path(brocode_home: &Path) -> PathBuf {
    brocode_home.join(SESSION_INDEX_FILE)
}
