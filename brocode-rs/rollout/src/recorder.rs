//! Persist Brocode session rollouts (.jsonl) so sessions can be replayed or inspected later.
...
use brocode_utils_string::truncate_middle_chars;
...
use brocode_protocol::protocol::GitInfo as ProtocolGitInfo;
use brocode_protocol::protocol::InitialHistory;
use brocode_protocol::protocol::ResumedHistory;
...
use brocode_utils_path as path_utils;
...
///
/// ```ignore
/// $ jq -C . ~/.brocode/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// $ fx ~/.brocode/sessions/rollout-2025-05-07T17-24-21-5973b6c0-94b8-487b-a530-2aeb6098ae0e.jsonl
/// ```
#[derive(Clone)]
...
impl RolloutRecorder {
    /// List threads (rollout files) under the provided Brocode home directory.
    ...
    async fn list_threads_with_db_fallback(
        ...
        let brocode_home = config.brocode_home();
        ...
        let fs_page = if archived {
            let root = brocode_home.join(ARCHIVED_SESSIONS_SUBDIR);
            get_threads_in_root(
                root,
                fs_page_size,
                cursor,
...
fn precompute_log_file_info(
    ...
    // Resolve ~/.brocode/sessions/YYYY/MM/DD path.
    let timestamp = OffsetDateTime::now_local()
        .map_err(|e| IoError::other(format!("failed to get local time: {e}")))?;
    let mut dir = config.brocode_home().to_path_buf();
...
async fn sync_thread_state_after_write(
    ...
    if new_thread_memory_mode.is_some()
        || items
            .iter()
            .any(brocode_state::rollout_item_affects_thread_metadata)
    {
...
impl From<brocode_state::ThreadsPage> for ThreadsPage {
    fn from(db_page: brocode_state::ThreadsPage) -> Self {
        let items = db_page
            .items
            .into_iter()
            .map(|item| ThreadItem {
...
async fn select_resume_path_from_db_page(
    page: &brocode_state::ThreadsPage,
    filter_cwd: Option<&Path>,
    default_provider: &str,
) -> Option<PathBuf> {
    match filter_cwd {
