...
use brocode_state::BackfillState;
use brocode_state::BackfillStats;
use brocode_state::BackfillStatus;
use brocode_state::DB_ERROR_METRIC;
use brocode_state::DB_METRIC_BACKFILL;
use brocode_state::DB_METRIC_BACKFILL_DURATION_MS;
use brocode_state::ExtractionOutcome;
use brocode_state::ThreadMetadataBuilder;
use brocode_state::apply_rollout_item;
use chrono::DateTime;
...
pub(crate) async fn backfill_sessions(
    runtime: &brocode_state::StateRuntime,
    config: &impl RolloutConfigView,
) {
    let metric_client = brocode_otel::metrics::global();
    ...
    let sessions_root = config.brocode_home().join(SESSIONS_SUBDIR);
    let archived_root = config.brocode_home().join(ARCHIVED_SESSIONS_SUBDIR);
        ...
        match collect_rollout_paths(&root).await {
            Ok(paths) => {
                rollout_paths.extend(paths.into_iter().map(|path| BackfillRolloutPath {
                    watermark: backfill_watermark_for_path(config.brocode_home(), &path),
                    path,
                    archived,
                }));
            }
...
fn backfill_watermark_for_path(brocode_home: &Path, path: &Path) -> String {
    path.strip_prefix(brocode_home)
        .unwrap_or(path)
        .to_string_lossy()
        .replace('\\', "/")
}
