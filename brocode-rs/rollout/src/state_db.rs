...
pub use brocode_state::LogEntry;
use brocode_state::ThreadMetadataBuilder;
use brocode_utils_path::normalize_for_path_comparison;
use chrono::DateTime;
...
pub type StateDbHandle = Arc<brocode_state::StateRuntime>;
...
pub async fn init(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    ...
    let runtime = match brocode_state::StateRuntime::init(
    ...
    let backfill_state = match runtime.get_backfill_state().await {
        ...
        Err(err) => {
            warn!(
                ...
                config.brocode_home.display()
            );
            return None;
        }
    };
    if backfill_state.status != brocode_state::BackfillStatus::Complete {
...
pub async fn get_state_db(config: &impl RolloutConfigView) -> Option<StateDbHandle> {
    let state_path = brocode_state::state_db_path(config.sqlite_home());
...
pub async fn open_if_present(brocode_home: &Path, default_provider: &str) -> Option<StateDbHandle> {
    let db_path = brocode_state::state_db_path(brocode_home);
    if !tokio::fs::try_exists(&db_path).await.unwrap_or(false) {
        return None;
    }
    let runtime =
        brocode_state::StateRuntime::init(brocode_home.to_path_buf(), default_provider.to_string())
            .await
            .ok()?;
    require_backfill_complete(runtime, brocode_home).await
}
...
async fn require_backfill_complete(
    ...
    match runtime.get_backfill_state().await {
        Ok(state) if state.status == brocode_state::BackfillStatus::Complete => Some(runtime),
        Ok(state) => {
            warn!(
...
fn cursor_to_anchor(cursor: Option<&Cursor>) -> Option<brocode_state::Anchor> {
    let cursor = cursor?;
    ...
    .with_nanosecond(0)?;
    Some(brocode_state::Anchor { ts, id })
}
...
pub async fn list_thread_ids_db(
    context: Option<&brocode_state::StateRuntime>,
    brocode_home: &Path,
    page_size: usize,
    ...
    let ctx = context?;
    if ctx.brocode_home() != brocode_home {
        warn!(
            "state db brocode_home mismatch: expected {}, got {}",
            ctx.brocode_home().display(),
            brocode_home.display()
        );
    }
    ...
    match ctx
        .list_thread_ids(
            ...
            anchor.as_ref(),
            match sort_key {
                ThreadSortKey::CreatedAt => brocode_state::SortKey::CreatedAt,
                ThreadSortKey::UpdatedAt => brocode_state::SortKey::UpdatedAt,
            },
...
) -> Option<brocode_state::ThreadsPage> {
    let ctx = context?;
    if ctx.brocode_home() != brocode_home {
        warn!(
            "state db brocode_home mismatch: expected {}, got {}",
