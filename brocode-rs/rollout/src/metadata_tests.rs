...
fn test_config(brocode_home: PathBuf) -> RolloutConfig {
    RolloutConfig {
        sqlite_home: brocode_home.clone(),
        cwd: brocode_home.clone(),
        brocode_home,
...
async fn backfill_sessions_resumes_from_watermark_and_marks_complete() {
    let dir = tempdir().expect("tempdir");
    let brocode_home = dir.path().to_path_buf();
    ...
    let runtime =
        brocode_state::StateRuntime::init(brocode_home.clone(), "test-provider".to_string())
            .await
            .expect("initialize runtime");
    let first_watermark = backfill_watermark_for_path(brocode_home.as_path(), first_path.as_path());
    ...
    let config = test_config(brocode_home.clone());
...
async fn backfill_sessions_normalizes_cwd_before_upsert() {
    ...
    let brocode_home = dir.path().to_path_buf();
    let thread_uuid = Uuid::new_v4();
    let session_cwd = brocode_home.join(".");
...
fn write_rollout_in_sessions_with_cwd(
    ...
    let sessions_dir = brocode_home.join("sessions");
