...
use crate::state_db;
use brocode_file_search as file_search;
...
impl From<brocode_state::Anchor> for Cursor {
    fn from(anchor: brocode_state::Anchor) -> Self {
...
pub async fn get_threads(
    ...
    let root = brocode_home.join(SESSIONS_SUBDIR);
    get_threads_in_root(
        root,
        page_size,
        cursor,
...
///
/// Directory layout: `~/.brocode/sessions/YYYY/MM/DD/rollout-YYYY-MM-DDThh-mm-ss-<uuid>.jsonl`
...
async fn find_thread_path_by_id_str_in_subdir(
    ...
    let state_db_ctx = state_db::open_if_present(brocode_home, "").await;
    ...
    let mut root = brocode_home.to_path_buf();
    root.push(subdir);
    if !root.exists() {
        return Ok(None);
    }
...
pub async fn find_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(brocode_home, SESSIONS_SUBDIR, id_str).await
}
...
pub async fn find_archived_thread_path_by_id_str(
    ...
    find_thread_path_by_id_str_in_subdir(brocode_home, ARCHIVED_SESSIONS_SUBDIR, id_str).await
}
