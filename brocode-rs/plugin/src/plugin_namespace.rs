...
pub const PLUGIN_MANIFEST_PATH: &str = ".brocode-plugin/plugin.json";
...
/// plugin manifest (same `name` rules as full manifest loading in brocode-core).
...
mod tests {
    ...
    fn uses_manifest_name() {
        ...
        fs::create_dir_all(plugin_root.join(".brocode-plugin")).expect("mkdir manifest");
        fs::write(
