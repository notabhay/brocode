...
- The legacy rule matcher lives in `brocode-execpolicy-legacy`.
...
- From the Brocode CLI, run `brocode execpolicy check` subcommand with one or more policy files (for example `src/default.rules`) to check a command:

```bash
brocode execpolicy check --rules path/to/policy.rules git status
```

- To opt into basename fallback for absolute program paths, pass `--resolve-host-executables`:

```bash
brocode execpolicy check \
...
```bash
cargo run -p brocode-execpolicy -- check --rules path/to/policy.rules git status
```
