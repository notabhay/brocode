...
use crate::brocode_message_processor::BrocodeMessageProcessor;
use crate::brocode_message_processor::BrocodeMessageProcessorArgs;
...
use brocode_app_server_protocol::AppListUpdatedNotification;
use brocode_app_server_protocol::ChatgptAuthTokensRefreshParams;
use brocode_app_server_protocol::ChatgptAuthTokensRefreshReason;
use brocode_app_server_protocol::ChatgptAuthTokensRefreshResponse;
...
use brocode_app_server_protocol::ExperimentalApi;
use brocode_app_server_protocol::ExperimentalFeatureEnablementSetParams;
use brocode_app_server_protocol::ExternalAgentConfigDetectParams;
use brocode_app_server_protocol::ExternalAgentConfigImportParams;
...
use brocode_app_server_protocol::experimental_required_message;
...
use brocode_core::default_client::DEFAULT_ORIGINATOR;
use brocode_core::default_client::SetOriginatorError;
use brocode_core::default_client::USER_AGENT_SUFFIX;
use brocode_core::default_client::get_brocode_user_agent;
...
use brocode_login::auth::ExternalAuthRefreshContext;
use brocode_login::auth::ExternalAuthRefreshReason;
use brocode_login::auth::ExternalAuthRefresher;
use brocode_login::auth::ExternalAuthTokens;
...
const TUI_APP_SERVER_CLIENT_NAME: &str = "brocode-tui";
...
pub(crate) struct MessageProcessor {
    outgoing: Arc<OutgoingMessageSender>,
    brocode_message_processor: BrocodeMessageProcessor,
...
pub(crate) struct MessageProcessorArgs {
    ...
    pub(crate) enable_brocode_api_key_env: bool,
}
...
impl MessageProcessor {
    ...
    pub(crate) fn new(args: MessageProcessorArgs) -> Self {
        ...
        let brocode_message_processor = BrocodeMessageProcessor::new(BrocodeMessageProcessorArgs {
        ...
        let external_agent_config_api = ExternalAgentConfigApi::new(config.brocode_home.clone());
        let fs_api = FsApi::default();
        let fs_watch_manager = FsWatchManager::new(outgoing.clone());

        Self {
            outgoing,
            brocode_message_processor,
            config_api,
    ...
    pub(crate) async fn process_request(
        ...
        Self::run_request_with_context(
            ...
            async {
                ...
                let brocode_request = match serde_json::from_value::<ClientRequest>(request_json) {
                    Ok(brocode_request) => brocode_request,
                    Err(err) => {
                        let error = JSONRPCErrorError {
                            code: INVALID_REQUEST_ERROR_CODE,
                            message: format!("Invalid request: {err}"),
    ...
    pub(crate) fn thread_created_receiver(&self) -> broadcast::Receiver<ThreadId> {
        self.brocode_message_processor.thread_created_receiver()
    }
    ...
    pub(crate) async fn connection_initialized(&self, connection_id: ConnectionId) {
        self.brocode_message_processor
    ...
    pub(crate) async fn cancel_active_login(&self) {
        self.brocode_message_processor.cancel_active_login().await;
    }
    ...
    pub(crate) async fn shutdown_threads(&self) {
        self.brocode_message_processor.shutdown_threads().await;
    }
    ...
    async fn handle_client_request(
        ...
        match brocode_request {
            // Handle Initialize internally so BrocodeMessageProcessor does not have to concern
            ...
            ClientRequest::Initialize { request_id, params } => {
                ...
                let originator = if name == TUI_APP_SERVER_CLIENT_NAME {
                    ...
                    // need to retain the legacy TUI `brocode_cli_rs` originator behavior.
                ...
                if let Err(error) = set_default_originator(originator) {
                    match error {
                        ...
                        SetOriginatorError::AlreadyInitialized => {
                            ...
                            // TODO(owen): Once we remove support for BROCODE_INTERNAL_ORIGINATOR_OVERRIDE,
                ...
                let user_agent = get_brocode_user_agent();
                let brocode_home = match self.config.brocode_home.clone().try_into() {
        ...
        if let Some(reason) = brocode_request.experimental_reason()
        ...
        match brocode_request {
            ...
            other => {
                ...
                // inline the full `BrocodeMessageProcessor::process_request` future, which
    ...
    async fn handle_config_value_write(
        ...
        match self.config_api.write_value(params).await {
            Ok(response) => {
                self.brocode_message_processor.clear_plugin_related_caches();
                self.brocode_message_processor
