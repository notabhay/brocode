...
use brocode_app_server_protocol::TextPosition as AppTextPosition;
use brocode_app_server_protocol::TextRange as AppTextRange;
use brocode_core::ExecPolicyError;
use brocode_core::check_execpolicy_for_warnings;
use brocode_core::config_loader::ConfigLoadError;
use brocode_core::config_loader::TextRange as CoreTextRange;
...
fn project_config_warning(config: &Config) -> Option<ConfigWarningNotification> {
    ...
    for layer in config.config_layer_stack.get_layers(
        ...
        if let ConfigLayerSource::Project { dot_brocode_folder } = &layer.name {
...
pub async fn run_main_with_transport(
        ...
        .await
    {
        Ok(config) => {
            ...
            match effective_toml.try_into() {
                Ok(config_toml) => {
                    if let Err(err) =
                        brocode_core::personality_migration::maybe_migrate_personality(
                            &config.brocode_home,
                            &config_toml,
                        )
                        .await
    ...
    if let Some(warning) = brocode_core::config::system_bwrap_warning() {
        config_warnings.push(ConfigWarningNotification {
            summary: warning,
            details: None,
            path: None,
    ...
    let feedback = BrocodeFeedback::new();

    let otel = brocode_core::otel_init::build_provider(
        &config,
        env!("CARGO_PKG_VERSION"),
        Some("brocode-app-server"),
        default_analytics_enabled,
    )
    .map_err(|e| {
        std::io::Error::new(
    ...
    let log_db = brocode_state::StateRuntime::init(
