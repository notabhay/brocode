...
use brocode_app_server_protocol::CommandExecOutputDeltaNotification;
use brocode_app_server_protocol::CommandExecOutputStream;
use brocode_app_server_protocol::CommandExecResizeParams;
use brocode_app_server_protocol::CommandExecResizeResponse;
use brocode_app_server_protocol::CommandExecResponse;
use brocode_app_server_protocol::CommandExecTerminalSize;
use brocode_app_server_protocol::CommandExecTerminateParams;
use brocode_app_server_protocol::CommandExecTerminateResponse;
use brocode_app_server_protocol::CommandExecWriteParams;
use brocode_app_server_protocol::CommandExecWriteResponse;
use brocode_app_server_protocol::JSONRPCErrorError;
use brocode_app_server_protocol::ServerNotification;
use brocode_core::bytes_to_string_smart;
use brocode_core::config::StartedNetworkProxy;
use brocode_core::exec::DEFAULT_EXEC_COMMAND_TIMEOUT_MS;
use brocode_core::exec::ExecExpiration;
use brocode_core::exec::IO_DRAIN_TIMEOUT_MS;
use brocode_core::sandboxing::ExecRequest;
use brocode_sandboxing::SandboxType;
use brocode_utils_pty::DEFAULT_OUTPUT_BYTES_CAP;
use brocode_utils_pty::ProcessHandle;
use brocode_utils_pty::SpawnedProcess;
use brocode_utils_pty::TerminalSize;
    ...
    pub(crate) async fn start(
        ...
        if matches!(exec_request.sandbox, SandboxType::WindowsRestrictedToken) {
            ...
            tokio::spawn(async move {
                ...
                match brocode_core::sandboxing::execute_env(
                    exec_request,
                    /*stdout_stream*/ None,
                )
                .await
        ...
        let spawned = if tty {
            brocode_utils_pty::spawn_pty_process(
                program,
                args,
                cwd.as_path(),
                &env,
            ...
            brocode_utils_pty::spawn_pipe_process(program, args, cwd.as_path(), &env, &arg0).await
        } else {
            brocode_utils_pty::spawn_pipe_process_no_stdin(
                program,
                args,
                cwd.as_path(),
                &env,
...
mod tests {
    ...
    fn windows_sandbox_exec_request() -> ExecRequest {
        ...
        ExecRequest::new(
            ...
            /*network*/ None,
            ExecExpiration::DefaultTimeout,
            brocode_core::exec::ExecCapturePolicy::ShellTool,
        ...
        let err = manager
            .start(StartCommandExecParams {
                ...
                request_id: ConnectionRequestId {
                    ...
                    request_id: brocode_app_server_protocol::RequestId::Integer(42),
                },
    ...
    async fn windows_sandbox_non_streaming_exec_uses_execution_path() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: brocode_app_server_protocol::RequestId::Integer(99),
        };
    ...
    async fn cancellation_expiration_keeps_process_alive_until_terminated() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: brocode_app_server_protocol::RequestId::Integer(100),
        };
        let sandbox_policy = SandboxPolicy::ReadOnly {
            access: ReadOnlyAccess::FullAccess,
            network_access: false,
    ...
    async fn windows_sandbox_process_ids_reject_write_requests() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: brocode_app_server_protocol::RequestId::Integer(1),
        };
    ...
    async fn windows_sandbox_process_ids_reject_terminate_requests() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: brocode_app_server_protocol::RequestId::Integer(2),
        };
        let process_id = ConnectionProcessId {
            connection_id: request_id.connection_id,
    ...
    async fn dropped_control_request_is_reported_as_not_running() {
        ...
        let request_id = ConnectionRequestId {
            ...
            request_id: brocode_app_server_protocol::RequestId::Integer(3),
        };
