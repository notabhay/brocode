...
use brocode_app_server_protocol::ConfigReadResponse;
use brocode_app_server_protocol::ConfigRequirements;
use brocode_app_server_protocol::ConfigRequirementsReadResponse;
use brocode_app_server_protocol::ConfigValueWriteParams;
use brocode_app_server_protocol::ConfigWriteErrorCode;
use brocode_app_server_protocol::ConfigWriteResponse;
use brocode_app_server_protocol::ExperimentalFeatureEnablementSetParams;
use brocode_app_server_protocol::ExperimentalFeatureEnablementSetResponse;
use brocode_app_server_protocol::JSONRPCErrorError;
use brocode_app_server_protocol::NetworkDomainPermission;
use brocode_app_server_protocol::NetworkRequirements;
use brocode_app_server_protocol::NetworkUnixSocketPermission;
use brocode_app_server_protocol::SandboxMode;
use brocode_core::AnalyticsEventsClient;
use brocode_core::ThreadManager;
use brocode_core::config::Config;
use brocode_core::config::ConfigService;
use brocode_core::config::ConfigServiceError;
...
use brocode_core::config_loader::ResidencyRequirement as CoreResidencyRequirement;
use brocode_core::config_loader::SandboxModeRequirement as CoreSandboxModeRequirement;
use brocode_core::plugins::PluginId;
use brocode_core::plugins::collect_plugin_enabled_candidates;
use brocode_core::plugins::installed_plugin_telemetry_metadata;
use brocode_features::canonical_feature_for_key;
use brocode_features::feature_for_key;
...
impl ConfigApi {
    ...
    pub(crate) async fn load_latest_config(
        ...
        let mut config = brocode_core::config::ConfigBuilder::default()
            .brocode_home(self.brocode_home.clone())
    ...
    fn emit_plugin_toggle_events(&self, pending_changes: std::collections::BTreeMap<String, bool>) {
        for (plugin_id, enabled) in pending_changes {
            ...
            let metadata =
                installed_plugin_telemetry_metadata(self.brocode_home.as_path(), &plugin_id);
            if enabled {
...
pub(crate) fn protected_feature_keys(
    config_layer_stack: &brocode_core::config_loader::ConfigLayerStack,
) -> BTreeSet<String> {
...
fn map_requirements_toml_to_api(requirements: ConfigRequirementsToml) -> ConfigRequirements {
    ConfigRequirements {
        allowed_approval_policies: requirements.allowed_approval_policies.map(|policies| {
            policies
                .into_iter()
                .map(brocode_app_server_protocol::AskForApproval::from)
                .collect()
        }),
...
) -> brocode_app_server_protocol::ResidencyRequirement {
    match residency {
        CoreResidencyRequirement::Us => brocode_app_server_protocol::ResidencyRequirement::Us,
    }
}

fn map_network_requirements_to_api(
    network: brocode_core::config_loader::NetworkRequirementsToml,
) -> NetworkRequirements {
    let allowed_domains = network
        .domains
        .as_ref()
        .and_then(brocode_core::config_loader::NetworkDomainPermissionsToml::allowed_domains);
    let denied_domains = network
        .domains
        .as_ref()
        .and_then(brocode_core::config_loader::NetworkDomainPermissionsToml::denied_domains);
    let allow_unix_sockets = network
        .unix_sockets
        .as_ref()
        .map(brocode_core::config_loader::NetworkUnixSocketPermissionsToml::allow_unix_sockets)
...
fn map_network_domain_permission_to_api(
    permission: brocode_core::config_loader::NetworkDomainPermissionToml,
) -> NetworkDomainPermission {
    match permission {
        brocode_core::config_loader::NetworkDomainPermissionToml::Allow => {
            NetworkDomainPermission::Allow
        }
        brocode_core::config_loader::NetworkDomainPermissionToml::Deny => {
...
fn map_network_unix_socket_permission_to_api(
    permission: brocode_core::config_loader::NetworkUnixSocketPermissionToml,
) -> NetworkUnixSocketPermission {
    match permission {
        brocode_core::config_loader::NetworkUnixSocketPermissionToml::Allow => {
            NetworkUnixSocketPermission::Allow
        }
        brocode_core::config_loader::NetworkUnixSocketPermissionToml::None => {
...
mod tests {
    ...
    use brocode_core::config_loader::NetworkDomainPermissionToml as CoreNetworkDomainPermissionToml;
    use brocode_core::config_loader::NetworkDomainPermissionsToml as CoreNetworkDomainPermissionsToml;
    use brocode_core::config_loader::NetworkRequirementsToml as CoreNetworkRequirementsToml;
    use brocode_core::config_loader::NetworkUnixSocketPermissionToml as CoreNetworkUnixSocketPermissionToml;
    use brocode_core::config_loader::NetworkUnixSocketPermissionsToml as CoreNetworkUnixSocketPermissionsToml;
    ...
    fn map_requirements_toml_to_api_converts_core_enums() {
        let requirements = ConfigRequirementsToml {
            ...
            allowed_web_search_modes: Some(vec![
                brocode_core::config_loader::WebSearchModeRequirement::Cached,
            ]),
            guardian_developer_instructions: None,
            feature_requirements: Some(brocode_core::config_loader::FeatureRequirementsToml {
        ...
        assert_eq!(
            ...
            Some(vec![
                brocode_app_server_protocol::AskForApproval::Never,
                brocode_app_server_protocol::AskForApproval::OnRequest,
            ])
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            Some(brocode_app_server_protocol::ResidencyRequirement::Us),
        );
        assert_eq!(
            mapped.network,
    ...
    async fn apply_runtime_feature_enablement_keeps_cli_overrides_above_config_and_runtime() {
        let brocode_home = TempDir::new().expect("create temp dir");
        std::fs::write(
        ...
        let mut config = brocode_core::config::ConfigBuilder::default()
            ...
            .fallback_cwd(Some(brocode_home.path().to_path_buf()))
            .cli_overrides(vec![(
            ...
            .cloud_requirements(CloudRequirementsLoader::new(async {
                Ok(Some(ConfigRequirementsToml {
                    feature_requirements: Some(
                        brocode_core::config_loader::FeatureRequirementsToml {
    ...
    async fn batch_write_reloads_user_config_when_requested() {
        let brocode_home = TempDir::new().expect("create temp dir");
        let user_config_path = brocode_home.path().join("config.toml");
        ...
        let analytics_config = Arc::new(
            brocode_core::config::ConfigBuilder::default()
                .build()
                .await
                .expect("load analytics config"),
        );
        let auth_manager = AuthManager::from_auth_for_testing(BrocodeAuth::from_api_key("test"));
        ...
        let response = config_api
            .batch_write(ConfigBatchWriteParams {
                edits: vec![brocode_app_server_protocol::ConfigEdit {
                    key_path: "model".to_string(),
                    value: json!("gpt-5"),
                    merge_strategy: brocode_app_server_protocol::MergeStrategy::Replace,
                }],
        ...
        assert_eq!(
            response,
            ConfigWriteResponse {
                status: brocode_app_server_protocol::WriteStatus::Ok,
                version: response.version.clone(),
                file_path: brocode_utils_absolute_path::AbsolutePathBuf::try_from(
