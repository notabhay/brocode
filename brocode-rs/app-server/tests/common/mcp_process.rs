...
use brocode_app_server_protocol::ClientNotification;
...
use brocode_app_server_protocol::ConfigReadParams;
use brocode_app_server_protocol::ConfigValueWriteParams;
use brocode_app_server_protocol::ExperimentalFeatureListParams;
use brocode_app_server_protocol::FeedbackUploadParams;
use brocode_app_server_protocol::FsCopyParams;
use brocode_app_server_protocol::FsCreateDirectoryParams;
use brocode_app_server_protocol::FsGetMetadataParams;
use brocode_app_server_protocol::FsReadDirectoryParams;
use brocode_app_server_protocol::FsReadFileParams;
use brocode_app_server_protocol::FsRemoveParams;
use brocode_app_server_protocol::FsUnwatchParams;
use brocode_app_server_protocol::FsWatchParams;
use brocode_app_server_protocol::FsWriteFileParams;
...
use brocode_app_server_protocol::JSONRPCError;
...
use brocode_core::default_client::BROCODE_INTERNAL_ORIGINATOR_OVERRIDE_ENV_VAR;
...
pub const DEFAULT_CLIENT_NAME: &str = "brocode-app-server-tests";

impl McpProcess {
    pub async fn new(brocode_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env_and_args(brocode_home, &[], &[]).await
    }

    pub async fn new_with_args(brocode_home: &Path, args: &[&str]) -> anyhow::Result<Self> {
        Self::new_with_env_and_args(brocode_home, &[], args).await
    }
    ...
    pub async fn new_with_env(
        ...
        Self::new_with_env_and_args(brocode_home, env_overrides, &[]).await
    }
    ...
    async fn new_with_env_and_args(
        ...
        let program = brocode_utils_cargo_bin::cargo_bin("brocode-app-server")
            .context("should find binary for brocode-app-server")?;
        ...
        let mut process = cmd
            .kill_on_drop(true)
            .spawn()
            .context("brocode-mcp-server proc should start")?;
        let stdin = process
            .stdin
            .take()
    ...
    pub async fn send_experimental_feature_enablement_set_request(
        &mut self,
        params: brocode_app_server_protocol::ExperimentalFeatureEnablementSetParams,
    ) -> anyhow::Result<i64> {
        let params = Some(serde_json::to_value(params)?);
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `brocode-app-server` child process.
        //
