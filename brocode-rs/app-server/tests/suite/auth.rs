...
fn create_config_toml(brocode_home: &Path) -> std::io::Result<()> {
    let config_toml = brocode_home.join("config.toml");
    std::fs::write(
        config_toml,
        r#"
...
async fn get_auth_status_no_auth() -> Result<()> {
    ...
    create_config_toml(brocode_home.path())?;
...
async fn get_auth_status_with_api_key_when_auth_not_required() -> Result<()> {
    let brocode_home = TempDir::new()?;
    create_config_toml_custom_provider(brocode_home.path(), /*requires_openai_auth*/ false)?;
...
async fn login_api_key_rejected_when_forced_chatgpt() -> Result<()> {
    let brocode_home = TempDir::new()?;
    create_config_toml_forced_login(brocode_home.path(), "chatgpt")?;
