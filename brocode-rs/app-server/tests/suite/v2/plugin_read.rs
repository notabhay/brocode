...
[plugins."demo-plugin@brocode-curated"]
enabled = true
"#,
    )?;
    write_installed_plugin(&brocode_home, "brocode-curated", "demo-plugin")?;
    ...
    assert_eq!(response.plugin.marketplace_name, "brocode-curated");
    assert_eq!(response.plugin.marketplace_path, marketplace_path);
    assert_eq!(response.plugin.summary.id, "demo-plugin@brocode-curated");
...
async fn plugin_read_returns_app_needs_auth() -> Result<()> {
    ...
    let brocode_home = TempDir::new()?;
    write_connectors_config(brocode_home.path(), &server_url)?;
    write_chatgpt_auth(
        brocode_home.path(),
...
async fn plugin_read_accepts_legacy_string_default_prompt() -> Result<()> {
  ...
  "interface": {
    ...
    write_plugins_enabled_config(&brocode_home)?;
...
async fn plugin_read_returns_invalid_request_when_plugin_manifest_is_missing() -> Result<()> {
  ...
  "plugins": [
    ...
    assert!(
        err.error
            .message
            .contains("missing or invalid .brocode-plugin/plugin.json")
    );
    Ok(())
}
...
fn write_plugins_enabled_config(brocode_home: &TempDir) -> Result<()> {
    std::fs::write(
        brocode_home.path().join("config.toml"),
        r#"[features]
plugins = true
...
async fn start_apps_server(
    ...
    let router = Router::new()
        ...
        .with_state(state)
        .nest_service("/api/brocode/apps", mcp_service);
...
fn write_connectors_config(brocode_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
