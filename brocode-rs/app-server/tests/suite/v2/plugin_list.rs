...
use brocode_core::config::set_project_trust_level;
...
fn write_plugins_enabled_config(brocode_home: &std::path::Path) -> std::io::Result<()> {
    std::fs::write(
        brocode_home.join("config.toml"),
        r#"[features]
plugins = true
"#,
    )
...
async fn plugin_list_skips_invalid_marketplace_file_and_reports_error() -> Result<()> {
    ...
    write_plugins_enabled_config(brocode_home.path())?;
    let marketplace_path =
        AbsolutePathBuf::try_from(repo_root.path().join(".agents/plugins/marketplace.json"))?;
    std::fs::write(marketplace_path.as_path(), "{not json")?;

    let home = brocode_home.path().to_string_lossy().into_owned();
...
async fn plugin_list_keeps_valid_marketplaces_when_another_marketplace_fails_to_load() -> Result<()>
{
    ...
    std::fs::create_dir_all(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.brocode-plugin"),
    )?;
  ...
  "plugins": [
    ...
    std::fs::write(
        valid_repo_root
            .path()
            .join("plugins/valid-plugin/.brocode-plugin/plugin.json"),
        r#"{"name":"valid-plugin"}"#,
    )?;
...
async fn plugin_list_accepts_omitted_cwds() -> Result<()> {
    let brocode_home = TempDir::new()?;
    std::fs::create_dir_all(brocode_home.path().join(".agents/plugins"))?;
    write_plugins_enabled_config(brocode_home.path())?;
    std::fs::write(
        brocode_home.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "brocode-curated",
  "plugins": [
    {
      "name": "home-plugin",
      "source": {
...
async fn plugin_list_includes_install_and_enabled_state_from_config() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    write_installed_plugin(&brocode_home, "brocode-curated", "enabled-plugin")?;
    write_installed_plugin(&brocode_home, "brocode-curated", "disabled-plugin")?;
    std::fs::write(
...
[plugins."enabled-plugin@brocode-curated"]
enabled = true

[plugins."disabled-plugin@brocode-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(marketplace.name, "brocode-curated");
    assert_eq!(
        marketplace
            .interface
            .as_ref()
    ...
    assert_eq!(marketplace.plugins[0].id, "enabled-plugin@brocode-curated");
    ...
    assert_eq!(marketplace.plugins[1].id, "disabled-plugin@brocode-curated");
    ...
    assert_eq!(
        ...
        "uninstalled-plugin@brocode-curated"
    );
...
async fn plugin_list_uses_home_config_for_enabled_state() -> Result<()> {
    let brocode_home = TempDir::new()?;
    std::fs::create_dir_all(brocode_home.path().join(".agents/plugins"))?;
    write_installed_plugin(&brocode_home, "brocode-curated", "shared-plugin")?;
    std::fs::write(
        brocode_home.path().join(".agents/plugins/marketplace.json"),
        r#"{
  "name": "brocode-curated",
...
[plugins."shared-plugin@brocode-curated"]
enabled = true
"#,
    )?;
  ...
  "plugins": [
    ...
    std::fs::create_dir_all(workspace_enabled.path().join(".brocode"))?;
    std::fs::write(
        workspace_enabled.path().join(".brocode/config.toml"),
        r#"[plugins."shared-plugin@brocode-curated"]
enabled = false
"#,
    )?;
    ...
    assert_eq!(shared_plugin.id, "shared-plugin@brocode-curated");
...
async fn plugin_list_returns_plugin_interface_with_absolute_asset_paths() -> Result<()> {
    ...
    std::fs::create_dir_all(repo_root.path().join(".git"))?;
    std::fs::create_dir_all(repo_root.path().join(".agents/plugins"))?;
    std::fs::create_dir_all(plugin_root.join(".brocode-plugin"))?;
    write_plugins_enabled_config(brocode_home.path())?;
    std::fs::write(
        repo_root.path().join(".agents/plugins/marketplace.json"),
        r#"{
  ...
  "interface": {
    ...
    assert_eq!(plugin.id, "demo-plugin@brocode-curated");
...
async fn plugin_list_force_remote_sync_returns_remote_sync_error_on_fail_open() -> Result<()> {
    let brocode_home = TempDir::new()?;
    write_plugin_sync_config(brocode_home.path(), "https://chatgpt.com/backend-api/")?;
    write_openai_curated_marketplace(brocode_home.path(), &["linear"])?;
    write_installed_plugin(&brocode_home, "openai-curated", "linear")?;
...
async fn plugin_list_force_remote_sync_reconciles_curated_plugin_state() -> Result<()> {
    ...
    write_openai_curated_marketplace(brocode_home.path(), &["linear", "gmail", "calendar"])?;
    write_installed_plugin(&brocode_home, "openai-curated", "linear")?;
    write_installed_plugin(&brocode_home, "openai-curated", "gmail")?;
    write_installed_plugin(&brocode_home, "openai-curated", "calendar")?;
  ...
  {"id":"2","name":"gmail","marketplace_name":"openai-curated","version":"1.0.0","enabled":false}
    ...
    Mock::given(method("GET"))
        ...
        .and(query_param("platform", "brocode"))
        .and(header("authorization", "Bearer chatgpt-token"))
        .and(header("chatgpt-account-id", "account-123"))
        .respond_with(
...
async fn plugin_list_fetches_featured_plugin_ids_without_chatgpt_auth() -> Result<()> {
    ...
    write_openai_curated_marketplace(brocode_home.path(), &["linear", "gmail"])?;
...
fn write_installed_plugin(
    ...
    let plugin_root = brocode_home
        .path()
        .join("plugins/cache")
        .join(marketplace_name)
        .join(plugin_name)
        .join("local/.brocode-plugin");
...
fn write_plugin_sync_config(brocode_home: &std::path::Path, base_url: &str) -> std::io::Result<()> {
    std::fs::write(
        brocode_home.join("config.toml"),
        format!(
            r#"
...
fn write_openai_curated_marketplace(
    ...
    let curated_root = brocode_home.join(".tmp/plugins");
...
{plugins}
    ...
    for plugin_name in plugin_names {
        let plugin_root = curated_root.join(format!("plugins/{plugin_name}/.brocode-plugin"));
        std::fs::create_dir_all(&plugin_root)?;
        std::fs::write(
            plugin_root.join("plugin.json"),
            format!(r#"{{"name":"{plugin_name}"}}"#),
    ...
    std::fs::create_dir_all(brocode_home.join(".tmp"))?;
    std::fs::write(
        brocode_home.join(".tmp/plugins.sha"),
