...
use brocode_login::login_with_api_key;
...
const LOGIN_ISSUER_ENV_VAR: &str = "BROCODE_APP_SERVER_LOGIN_ISSUER";
...
fn create_config_toml(brocode_home: &Path, params: CreateConfigTomlParams) -> std::io::Result<()> {
...
async fn logout_account_removes_auth_and_notifies() -> Result<()> {
    ...
    create_config_toml(brocode_home.path(), CreateConfigTomlParams::default())?;
    ...
    assert!(brocode_home.path().join("auth.json").exists());

    let mut mcp =
        McpProcess::new_with_env(brocode_home.path(), &[("OPENAI_API_KEY", None)]).await?;
    ...
    assert!(
        !brocode_home.path().join("auth.json").exists(),
...
async fn set_auth_token_updates_account_and_notifies() -> Result<()> {
    ...
    write_models_cache(brocode_home.path())?;
...
async fn external_auth_refreshes_on_unauthorized() -> Result<()> {
    ...
    let thread_req = mcp
        .send_thread_start_request(brocode_app_server_protocol::ThreadStartParams {
    ...
    .await??;
    let thread = to_response::<brocode_app_server_protocol::ThreadStartResponse>(thread_resp)?;

    let turn_req = mcp
        .send_turn_start_request(brocode_app_server_protocol::TurnStartParams {
            thread_id: thread.thread.id,
            input: vec![brocode_app_server_protocol::UserInput::Text {
...
async fn login_account_chatgpt_device_code_succeeds_and_notifies() -> Result<()> {
    ...
    assert_eq!(verification_url, format!("{issuer}/brocode/device"));
