...
use brocode_app_server_protocol::AppConfig;
use brocode_app_server_protocol::AppToolApproval;
use brocode_app_server_protocol::AppsConfig;
use brocode_app_server_protocol::AskForApproval;
use brocode_app_server_protocol::ConfigBatchWriteParams;
use brocode_app_server_protocol::ConfigEdit;
...
use brocode_app_server_protocol::ToolsV2;
use brocode_app_server_protocol::WriteStatus;
use brocode_core::config::set_project_trust_level;
use brocode_protocol::config_types::TrustLevel;
use brocode_protocol::config_types::WebSearchContextSize;
use brocode_protocol::config_types::WebSearchLocation;
...
fn write_config(brocode_home: &TempDir, contents: &str) -> Result<()> {
    Ok(std::fs::write(
...
"#,
    ...
    let brocode_home_path = brocode_home.path().canonicalize()?;
    let user_file = AbsolutePathBuf::try_from(brocode_home_path.join("config.toml"))?;
...
async fn config_read_includes_project_layers_for_cwd() -> Result<()> {
    let brocode_home = TempDir::new()?;
    write_config(&brocode_home, r#"model = "gpt-user""#)?;
...
"#,
    ...
    set_project_trust_level(brocode_home.path(), workspace.path(), TrustLevel::Trusted)?;
    ...
    assert_eq!(
        ...
        ConfigLayerSource::Project {
            dot_brocode_folder: project_config
        }
    );
...
"#,
    ...
    let mut mcp = McpProcess::new_with_env(
        brocode_home.path(),
        &[(
            "BROCODE_APP_SERVER_MANAGED_CONFIG_PATH",
...
async fn config_value_write_replaces_value() -> Result<()> {
    ...
    let brocode_home = temp_dir.path().canonicalize()?;
    write_config(
        &temp_dir,
        r#"
model = "gpt-old"
"#,
    ...
    let expected_file_path =
        AbsolutePathBuf::resolve_path_against_base("config.toml", brocode_home)?;
...
async fn config_batch_write_applies_multiple_edits() -> Result<()> {
    ...
    let brocode_home = tmp_dir.path().canonicalize()?;
    ...
    let batch_id = mcp
        .send_config_batch_write_request(ConfigBatchWriteParams {
            file_path: Some(brocode_home.join("config.toml").display().to_string()),
            edits: vec![
                ConfigEdit {
...
fn assert_layers_user_then_optional_system(
    layers: &[brocode_app_server_protocol::ConfigLayer],
