...
use brocode_utils_cargo_bin::cargo_bin;
...
async fn initialize_uses_client_info_name_as_originator() -> Result<()> {
    ...
    let expected_brocode_home = AbsolutePathBuf::try_from(brocode_home.path().canonicalize()?)?;
    create_config_toml(brocode_home.path(), &server.uri(), "never")?;
    let mut mcp = McpProcess::new(brocode_home.path()).await?;

    let message = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp.initialize_with_client_info(ClientInfo {
            name: "brocode_vscode".to_string(),
    ...
    let InitializeResponse {
        user_agent,
        brocode_home: response_brocode_home,
        platform_family,
        platform_os,
    } = to_response::<InitializeResponse>(response)?;

    assert!(user_agent.starts_with("brocode_vscode/"));
    assert_eq!(response_brocode_home, expected_brocode_home);
...
async fn initialize_respects_originator_override_env_var() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_with_env(
        ...
        &[(
            ...
            Some("brocode_originator_via_env_var"),
        )],
    )
    .await?;
    ...
    assert!(user_agent.starts_with("brocode_originator_via_env_var/"));
    assert_eq!(response_brocode_home, expected_brocode_home);
    assert_eq!(platform_family, std::env::consts::FAMILY);
    assert_eq!(platform_os, std::env::consts::OS);
    Ok(())
...
async fn initialize_rejects_invalid_client_name() -> Result<()> {
    ...
    let mut mcp = McpProcess::new_with_env(
        brocode_home.path(),
        &[("BROCODE_INTERNAL_ORIGINATOR_OVERRIDE", None)],
    )
    .await?;
...
async fn turn_start_notify_payload_includes_initialize_client_name() -> Result<()> {
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    let brocode_home = TempDir::new()?;
    let notify_file = brocode_home.path().join("notify.json");
    let notify_capture = cargo_bin("brocode-app-server-test-notify-capture")?;
...
fn create_config_toml(
    ...
    create_config_toml_with_extra(brocode_home, server_uri, approval_policy, "")
}
