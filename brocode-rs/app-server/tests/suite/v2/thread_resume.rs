...
use brocode_app_server_protocol::SessionSource;
...
use brocode_app_server_protocol::UserInput;
use brocode_core::auth::AuthCredentialsStoreMode;
use brocode_core::auth::REFRESH_TOKEN_URL_OVERRIDE_ENV_VAR;
...
use brocode_protocol::protocol::SessionSource as RolloutSessionSource;
use brocode_protocol::protocol::TurnStartedEvent;
use brocode_protocol::user_input::ByteRange;
...
const BROCODE_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT: &str = "You are Brocode, a coding agent based on GPT-5. You and the user share the same workspace and collaborate to achieve the user's goals.";
...
async fn thread_resume_rejects_unmaterialized_thread() -> Result<()> {
    ...
    create_config_toml(brocode_home.path(), &server.uri())?;
...
async fn thread_resume_prefers_persisted_git_metadata_for_local_threads() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let brocode_home = TempDir::new()?;
    let config_toml = brocode_home.path().join("config.toml");
    std::fs::write(
        &config_toml,
        format!(
            r#"
...
"#,
    ...
    let repo_path = brocode_home.path().join("repo");
    ...
    let rollout_path = rollout_path(brocode_home.path(), "2025-01-05T12-00-00", &thread_id);
...
async fn thread_resume_without_overrides_does_not_change_updated_at_or_mtime() -> Result<()> {
    let server = create_mock_responses_server_repeating_assistant("Done").await;
    let brocode_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(brocode_home.path(), &server.uri())?;
...
async fn thread_resume_keeps_in_flight_turn_streaming() -> Result<()> {
    ...
    let mut primary = McpProcess::new(brocode_home.path()).await?;
    ...
    let mut secondary = McpProcess::new(brocode_home.path()).await?;
...
async fn thread_resume_replays_pending_file_change_request_approval() -> Result<()> {
    let tmp = TempDir::new()?;
    let brocode_home = tmp.path().join("brocode_home");
    std::fs::create_dir(&brocode_home)?;
...
"#;
    ...
    let server = create_mock_responses_server_sequence_unchecked(responses).await;
    create_config_toml(&brocode_home, &server.uri())?;

    let mut primary = McpProcess::new(&brocode_home).await?;
    timeout(DEFAULT_READ_TIMEOUT, primary.initialize()).await??;
...
async fn thread_resume_with_overrides_defers_updated_at_until_turn_start() -> Result<()> {
    ...
    let RestartedThreadFixture {
        ...
        thread_id,
        rollout_file_path,
    } = start_materialized_thread_and_restart(brocode_home.path(), "materialize").await?;
...
async fn thread_resume_fails_when_required_mcp_server_fails_to_initialize() -> Result<()> {
    ...
    let brocode_home = TempDir::new()?;
    let rollout = setup_rollout_fixture(brocode_home.path(), &server.uri())?;
    create_config_toml_with_required_broken_mcp(brocode_home.path(), &server.uri())?;
...
async fn thread_resume_supports_history_and_overrides() -> Result<()> {
    ...
    let RestartedThreadFixture {
        mut mcp, thread_id, ..
    } = start_materialized_thread_and_restart(brocode_home.path(), "seed history").await?;
...
async fn start_materialized_thread_and_restart(
    ...
    let mut first_mcp = McpProcess::new(brocode_home).await?;
    ...
    let mut second_mcp = McpProcess::new(brocode_home).await?;
...
async fn thread_resume_accepts_personality_override() -> Result<()> {
    ...
    assert!(
        instructions_text.contains(BROCODE_5_2_INSTRUCTIONS_TEMPLATE_DEFAULT),
...
fn create_config_toml(brocode_home: &std::path::Path, server_uri: &str) -> std::io::Result<()> {
    let config_toml = brocode_home.join("config.toml");
    std::fs::write(
        config_toml,
        format!(
            r#"
...
command = "brocode-definitely-not-a-real-binary"
required = true
"#
        ),
    )
...
fn setup_rollout_fixture(brocode_home: &Path, server_uri: &str) -> Result<RolloutFixture> {
    create_config_toml(brocode_home, server_uri)?;
