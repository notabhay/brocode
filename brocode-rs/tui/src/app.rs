...
use brocode_ansi_escape::ansi_escape_line;
use brocode_app_server_client::AppServerRequestHandle;
use brocode_app_server_client::TypedRequestError;
...
use brocode_app_server_protocol::ThreadRollbackResponse;
use brocode_app_server_protocol::Turn;
use brocode_app_server_protocol::TurnError as AppServerTurnError;
use brocode_app_server_protocol::TurnStatus;
use brocode_core::config::Config;
use brocode_core::config::ConfigBuilder;
...
use brocode_core::config::types::ApprovalsReviewer;
use brocode_core::config::types::ModelAvailabilityNuxConfig;
use brocode_core::config_loader::ConfigLayerStackOrdering;
use brocode_core::message_history;
use brocode_core::models_manager::collaboration_mode_presets::CollaborationModesConfig;
use brocode_core::models_manager::model_presets::HIDE_GPT_5_1_BROCODE_MAX_MIGRATION_PROMPT_CONFIG;
use brocode_core::models_manager::model_presets::HIDE_GPT5_1_MIGRATION_PROMPT_CONFIG;
#[cfg(target_os = "windows")]
use brocode_core::windows_sandbox::WindowsSandboxLevelExt;
use brocode_features::Feature;
use brocode_otel::SessionTelemetry;
use brocode_protocol::ThreadId;
use brocode_protocol::approvals::ExecApprovalRequestEvent;
use brocode_protocol::config_types::Personality;
#[cfg(target_os = "windows")]
use brocode_protocol::config_types::WindowsSandboxLevel;
use brocode_protocol::openai_models::ModelAvailabilityNux;
use brocode_protocol::openai_models::ModelPreset;
use brocode_protocol::openai_models::ModelUpgrade;
use brocode_protocol::openai_models::ReasoningEffort as ReasoningEffortConfig;
use brocode_protocol::protocol::AskForApproval;
use brocode_protocol::protocol::FinalOutput;
use brocode_protocol::protocol::GetHistoryEntryResponseEvent;
use brocode_protocol::protocol::ListSkillsResponseEvent;
#[cfg(test)]
use brocode_protocol::protocol::McpAuthStatus;
use brocode_protocol::protocol::Op;
use brocode_protocol::protocol::SandboxPolicy;
use brocode_protocol::protocol::SessionSource;
use brocode_protocol::protocol::SkillErrorInfo;
...
fn command_execution_decision_to_review_decision(
    decision: brocode_app_server_protocol::CommandExecutionApprovalDecision,
) -> brocode_protocol::protocol::ReviewDecision {
    match decision {
...
fn default_exec_approval_decisions(
    network_approval_context: Option<&brocode_protocol::protocol::NetworkApprovalContext>,
    proposed_execpolicy_amendment: Option<&brocode_protocol::approvals::ExecPolicyAmendment>,
    proposed_network_policy_amendments: Option<
        &[brocode_protocol::approvals::NetworkPolicyAmendment],
    >,
    additional_permissions: Option<&brocode_protocol::models::PermissionProfile>,
) -> Vec<brocode_protocol::protocol::ReviewDecision> {
...
fn session_summary(
    ...
    let resume_command = brocode_core::util::resume_command(thread_name.as_deref(), thread_id);
    Some(SessionSummary {
        usage_line,
        resume_command,
    })
...
fn list_skills_response_to_core(response: SkillsListResponse) -> ListSkillsResponseEvent {
    ListSkillsResponseEvent {
        skills: response
            .data
            .into_iter()
            .map(|entry| brocode_protocol::protocol::SkillsListEntry {
                cwd: entry.cwd,
                skills: entry
                    .skills
                    .into_iter()
                    .map(|skill| brocode_protocol::protocol::SkillMetadata {
                        name: skill.name,
                        description: skill.description,
                        short_description: skill.short_description,
                        interface: skill.interface.map(|interface| {
                            brocode_protocol::protocol::SkillInterface {
                        ...
                        dependencies: skill.dependencies.map(|dependencies| {
                            brocode_protocol::protocol::SkillDependencies {
                                tools: dependencies
                                    .tools
                                    .into_iter()
                                    .map(|tool| brocode_protocol::protocol::SkillToolDependency {
                                        r#type: tool.r#type,
                                        value: tool.value,
                        ...
                        scope: match skill.scope {
                            brocode_app_server_protocol::SkillScope::User => {
                                brocode_protocol::protocol::SkillScope::User
                            }
                            brocode_app_server_protocol::SkillScope::Repo => {
                                brocode_protocol::protocol::SkillScope::Repo
                            }
                            brocode_app_server_protocol::SkillScope::System => {
                                brocode_protocol::protocol::SkillScope::System
                            }
                            brocode_app_server_protocol::SkillScope::Admin => {
                                brocode_protocol::protocol::SkillScope::Admin
                            }
                        },
                ...
                errors: entry
                    .errors
                    .into_iter()
                    .map(|error| brocode_protocol::protocol::SkillErrorInfo {
                        path: error.path,
...
fn emit_project_config_warnings(app_event_tx: &AppEventSender, config: &Config) {
    ...
    for layer in config.config_layer_stack.get_layers(
        ...
        let ConfigLayerSource::Project { dot_brocode_folder } = &layer.name else {
            continue;
        };
        ...
        disabled_folders.push((
            dot_brocode_folder.as_path().display().to_string(),
            layer
                .disabled_reason
                .as_ref()
...
fn emit_system_bwrap_warning(app_event_tx: &AppEventSender) {
    let Some(message) = brocode_core::config::system_bwrap_warning() else {
        return;
    };
...
fn migration_prompt_hidden(config: &Config, migration_config_key: &str) -> bool {
    match migration_config_key {
        HIDE_GPT_5_1_BROCODE_MAX_MIGRATION_PROMPT_CONFIG => config
            .notices
            .hide_gpt_5_1_brocode_max_migration_prompt
            .unwrap_or(false),
...
async fn prepare_startup_tooltip_override(
    ...
    if let Err(err) = ConfigEditsBuilder::new(&config.brocode_home)
...
fn active_turn_not_steerable_turn_error(error: &TypedRequestError) -> Option<AppServerTurnError> {
    ...
    matches!(
        turn_error.brocode_error_info,
        Some(AppServerBrocodeErrorInfo::ActiveTurnNotSteerable { .. })
    )
...
impl App {
    pub fn chatwidget_init_for_forked_or_resumed_thread(
        &self,
        tui: &mut tui::Tui,
        cfg: brocode_core::config::Config,
    ...
    async fn update_feature_flags(&mut self, updates: Vec<(Feature, bool)>) {
        ...
        let mut builder = ConfigEditsBuilder::new(&self.config.brocode_home)
    ...
    fn clear_ui_header_lines(&self, width: u16) -> Vec<Line<'static>> {
        self.clear_ui_header_lines_with_version(width, BROCODE_CLI_VERSION)
    }
    ...
    /// `$BROCODE_HOME/history.jsonl`, bypassing the app-server RPC layer.
                    ...
                    app_event_tx.send(AppEvent::ThreadHistoryEntryResponse {
                        ...
                        event: GetHistoryEntryResponseEvent {
                            ...
                            entry: entry_opt.map(|entry| {
                                brocode_protocol::message_history::HistoryEntry {
        ...
        match op.view() {
            ...
            AppCommandView::ListSkills { cwds, force_reload } => {
                let response = app_server
                    .skills_list(brocode_app_server_protocol::SkillsListParams {
                        cwds: cwds.to_vec(),
                        force_reload,
            ...
            .await
        {
            Ok(thread) => {
                self.upsert_agent_picker_thread(
                    ...
                    matches!(
                        thread.status,
                        brocode_app_server_protocol::ThreadStatus::NotLoaded
                    ),
                );
                true
            }
    ...
    async fn session_state_for_thread_read(
        &self,
        thread_id: ThreadId,
        thread: &brocode_app_server_protocol::Thread,
    ) -> ThreadSessionState {
    ...
    pub async fn run(
        ...
        feedback: brocode_feedback::BrocodeFeedback,
        is_first_run: bool,
        ...
        let session_telemetry = SessionTelemetry::new(
            ...
            auth_mode,
            brocode_core::default_client::originator().value,
        ...
        {
            session_telemetry.counter("brocode.status_line", /*inc*/ 1, &[]);
        }
        ...
        #[cfg(target_os = "windows")]
        {
            ...
            if should_check {
                ...
                let logs_base_dir = app.config.brocode_home.clone();
    ...
    async fn handle_event(
        ...
        match event {
            ...
            AppEvent::OpenWindowsSandboxFallbackPrompt { preset } => {
                self.session_telemetry.counter(
                    "brocode.windows_sandbox.fallback_prompt_shown",
                    /*inc*/ 1,
                    &[],
                );
                ...
                if let Some(started_at) = self.windows_sandbox.setup_started_at.take() {
                    self.session_telemetry.record_duration(
                        "brocode.windows_sandbox.elevated_setup_duration_ms",
                        started_at.elapsed(),
                        &[("result", "failure")],
                    );
                }
            ...
            AppEvent::BeginWindowsSandboxElevatedSetup { preset } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    if brocode_core::windows_sandbox::sandbox_setup_is_complete(
                    ...
                    tokio::task::spawn_blocking(move || {
                        let result = brocode_core::windows_sandbox::run_elevated_setup(
                            &policy,
                            policy_cwd.as_path(),
                            command_cwd.as_path(),
                            &env_map,
                        ...
                        let event = match result {
                            Ok(()) => {
                                session_telemetry.counter(
                                    "brocode.windows_sandbox.elevated_setup_success",
                                    /*inc*/ 1,
                                    &[],
                                );
                            ...
                            Err(err) => {
                                ...
                                if let Some((code, message)) =
                                    brocode_core::windows_sandbox::elevated_setup_failure_details(
                                        &err,
                                    )
                                {
                                    code_tag = Some(code);
                                ...
                                session_telemetry.counter(
                                    brocode_core::windows_sandbox::elevated_setup_failure_metric_name(
                                        &err,
                                    ),
                                    /*inc*/ 1,
                                    &tags,
            ...
            AppEvent::BeginWindowsSandboxLegacySetup { preset } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    self.chat_widget.show_windows_sandbox_setup_status();
                    tokio::task::spawn_blocking(move || {
                        if let Err(err) = brocode_core::windows_sandbox::run_legacy_setup_preflight(
                            &policy,
                            policy_cwd.as_path(),
                            command_cwd.as_path(),
                            &env_map,
                            ...
                            session_telemetry.counter(
                                "brocode.windows_sandbox.legacy_setup_preflight_failed",
                                /*inc*/ 1,
                                &[],
                            );
                            tracing::warn!(
            ...
            AppEvent::BeginWindowsSandboxGrantReadRoot { path } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    tokio::task::spawn_blocking(move || {
                        ...
                        let event = match brocode_core::windows_sandbox_read_grants::grant_read_root_non_elevated(
                            &policy,
                            policy_cwd.as_path(),
                            command_cwd.as_path(),
                            &env_map,
            ...
            AppEvent::EnableWindowsSandboxForAgentMode { preset, mode } => {
                #[cfg(target_os = "windows")]
                {
                    ...
                    let builder = ConfigEditsBuilder::new(&self.config.brocode_home)
                        ...
                        Ok(()) => {
                                ...
                                self.chat_widget.world_writable_warning_details()
                            {
                                ...
                                self.chat_widget.add_plain_history_lines(vec![
                                    ...
                                    Line::from(vec![
                                        "  ".into(),
                                        "Brocode can now safely edit files and execute commands in your computer"
                                            .dark_gray(),
                                    ]),
                                ]);
                            }
            ...
            AppEvent::PersistModelSelection { model, effort } => {
                let profile = self.active_profile.as_deref();
                match ConfigEditsBuilder::new(&self.config.brocode_home)
                    .with_profile(profile)
            ...
            AppEvent::PersistRealtimeAudioDeviceSelection { kind, name } => {
                let builder = match kind {
                    RealtimeAudioDeviceKind::Microphone => {
                        ConfigEditsBuilder::new(&self.config.brocode_home)
                ...
                #[cfg(target_os = "windows")]
                {
                    ...
                    if should_check {
                        ...
                        let env_map: std::collections::HashMap<String, String> =
                            std::env::vars().collect();
                        let tx = self.app_event_tx.clone();
                        let logs_base_dir = self.config.brocode_home.clone();
            ...
            AppEvent::UpdateApprovalsReviewer(policy) => {
                ...
                if let Err(err) = ConfigEditsBuilder::new(&self.config.brocode_home)
                    .with_profile(profile)
            ...
            AppEvent::StatusLineSetup { items } => {
                ...
                let edit = brocode_core::config::edit::status_line_items_edit(&ids);
                let apply_result = ConfigEditsBuilder::new(&self.config.brocode_home)
                    .with_edits([edit])
                    .apply()
                    .await;
                match apply_result {
            ...
            AppEvent::TerminalTitleSetup { items } => {
                let ids = items.iter().map(ToString::to_string).collect::<Vec<_>>();
                let edit = brocode_core::config::edit::terminal_title_items_edit(&ids);
                let apply_result = ConfigEditsBuilder::new(&self.config.brocode_home)
                    .with_edits([edit])
                    .apply()
                    .await;
            ...
            AppEvent::SyntaxThemeSelected { name } => {
                let edit = brocode_core::config::edit::syntax_theme_edit(&name);
                let apply_result = ConfigEditsBuilder::new(&self.config.brocode_home)
                    .with_edits([edit])
                    .apply()
                    .await;
    ...
    fn reasoning_label_for(
        ...
        (!model.starts_with("brocode-auto-")).then(|| Self::reasoning_label(reasoning_effort))
    }

    pub(crate) fn token_usage(&self) -> brocode_protocol::protocol::TokenUsage {
        self.chat_widget.token_usage()
    }
        ...
        let editor_cmd = match external_editor::resolve_editor_command() {
            ...
            Err(external_editor::EditorError::MissingEditor) => {
                self.chat_widget
                    ...
                    "Cannot open external editor: set $VISUAL or $EDITOR before starting Brocode."
                        .to_string(),
                ));
    ...
    fn spawn_world_writable_scan(
        ...
        sandbox_policy: brocode_protocol::protocol::SandboxPolicy,
        tx: AppEventSender,
    ) {
        tokio::task::spawn_blocking(move || {
            let result = brocode_windows_sandbox::apply_world_writable_scan_and_denies(
                &logs_base_dir,
                &cwd,
                &env_map,
                &sandbox_policy,
...
type McpInventoryMaps = (
    HashMap<String, brocode_protocol::mcp::Tool>,
    HashMap<String, Vec<brocode_protocol::mcp::Resource>>,
    HashMap<String, Vec<brocode_protocol::mcp::ResourceTemplate>>,
    ...
    for status in statuses {
        ...
        auth_statuses.insert(
            ...
            match status.auth_status {
                brocode_app_server_protocol::McpAuthStatus::Unsupported => {
                    McpAuthStatus::Unsupported
                }
                brocode_app_server_protocol::McpAuthStatus::NotLoggedIn => {
                    McpAuthStatus::NotLoggedIn
                }
                brocode_app_server_protocol::McpAuthStatus::BearerToken => {
                    McpAuthStatus::BearerToken
                }
                brocode_app_server_protocol::McpAuthStatus::OAuth => McpAuthStatus::OAuth,
            },
        );
...
mod tests {
    ...
    use brocode_app_server_protocol::AdditionalNetworkPermissions;
    use brocode_app_server_protocol::AdditionalPermissionProfile;
    use brocode_app_server_protocol::AgentMessageDeltaNotification;
    use brocode_app_server_protocol::CommandExecutionRequestApprovalParams;
    use brocode_app_server_protocol::ConfigWarningNotification;
    use brocode_app_server_protocol::HookCompletedNotification;
    use brocode_app_server_protocol::HookEventName as AppServerHookEventName;
    use brocode_app_server_protocol::HookExecutionMode as AppServerHookExecutionMode;
    use brocode_app_server_protocol::HookHandlerType as AppServerHookHandlerType;
    use brocode_app_server_protocol::HookOutputEntry as AppServerHookOutputEntry;
    use brocode_app_server_protocol::HookOutputEntryKind as AppServerHookOutputEntryKind;
    use brocode_app_server_protocol::HookRunStatus as AppServerHookRunStatus;
    use brocode_app_server_protocol::HookRunSummary as AppServerHookRunSummary;
    use brocode_app_server_protocol::HookScope as AppServerHookScope;
    use brocode_app_server_protocol::HookStartedNotification;
    use brocode_app_server_protocol::JSONRPCErrorError;
    use brocode_app_server_protocol::NetworkApprovalContext as AppServerNetworkApprovalContext;
    use brocode_app_server_protocol::NetworkApprovalProtocol as AppServerNetworkApprovalProtocol;
    use brocode_app_server_protocol::NetworkPolicyAmendment as AppServerNetworkPolicyAmendment;
    use brocode_app_server_protocol::NetworkPolicyRuleAction as AppServerNetworkPolicyRuleAction;
    use brocode_app_server_protocol::NonSteerableTurnKind as AppServerNonSteerableTurnKind;
    use brocode_app_server_protocol::RequestId as AppServerRequestId;
    use brocode_app_server_protocol::ServerNotification;
    use brocode_app_server_protocol::ServerRequest;
    ...
    use brocode_app_server_protocol::ThreadTokenUsageUpdatedNotification;
    use brocode_app_server_protocol::TokenUsageBreakdown;
    use brocode_app_server_protocol::Turn;
    use brocode_app_server_protocol::TurnCompletedNotification;
    use brocode_app_server_protocol::TurnError as AppServerTurnError;
    use brocode_app_server_protocol::TurnStartedNotification;
    use brocode_app_server_protocol::TurnStatus;
    use brocode_app_server_protocol::UserInput as AppServerUserInput;
    use brocode_core::config::ConfigBuilder;
    use brocode_core::config::ConfigOverrides;
    use brocode_core::config::types::ModelAvailabilityNuxConfig;
    use brocode_otel::SessionTelemetry;
    ...
    use brocode_protocol::config_types::CollaborationModeMask;
    use brocode_protocol::config_types::ModeKind;
    use brocode_protocol::config_types::Settings;
    use brocode_protocol::mcp::Tool;
    ...
    use brocode_protocol::protocol::SessionSource;
    use brocode_protocol::protocol::TurnContextItem;
    ...
    fn mcp_inventory_maps_prefix_tool_names_by_server() {
        let statuses = vec![
            McpServerStatus {
                ...
                auth_status: brocode_app_server_protocol::McpAuthStatus::Unsupported,
            },
            McpServerStatus {
    ...
    -> Result<()> {
        ...
        let model = brocode_core::test_support::get_model_offline(config.model.as_deref());
        app.chat_widget = ChatWidget::new_with_app_event(ChatWidgetInit {
            ...
            feedback: brocode_feedback::BrocodeFeedback::new(),
            is_first_run: false,
        ...
        while let Ok(event) = app_event_rx.try_recv() {
            match event {
                ...
                AppEvent::BrocodeOp(Op::UserTurn { items, .. }) => {
                    submitted_items = Some(items);
                }
                _ => {}
            }
    ...
    async fn update_feature_flags_enabling_guardian_selects_guardian_approvals() -> Result<()> {
        ...
        let brocode_home = tempdir()?;
        app.config.brocode_home = brocode_home.path().to_path_buf();
    ...
    -> Result<()> {
        ...
        let brocode_home = tempdir()?;
        app.config.brocode_home = brocode_home.path().to_path_buf();
        let config_toml_path = brocode_home.path().join("config.toml").abs();
...
"#;
    ...
    async fn inactive_thread_exec_approval_preserves_context() {
        ...
        assert_eq!(
            ...
            vec![
                ...
                brocode_protocol::protocol::ReviewDecision::ApprovedForSession,
                brocode_protocol::protocol::ReviewDecision::NetworkPolicyAmendment {
                    network_policy_amendment: brocode_protocol::approvals::NetworkPolicyAmendment {
                        host: "example.com".to_string(),
                        action: brocode_protocol::approvals::NetworkPolicyRuleAction::Allow,
                    },
                },
                brocode_protocol::protocol::ReviewDecision::Abort,
            ]
        ...
        app.enqueue_thread_notification(
            ...
            ServerNotification::ThreadStarted(ThreadStartedNotification {
                thread: Thread {
                    ...
                    created_at: 1,
                    updated_at: 2,
                    status: brocode_app_server_protocol::ThreadStatus::Idle,
                    path: Some(rollout_path.clone()),
                    cwd: PathBuf::from("/tmp/agent"),
                    cli_version: "0.0.0".to_string(),
                    source: brocode_app_server_protocol::SessionSource::Unknown,
                    agent_nickname: Some("Robie".to_string()),
                    agent_role: Some("explorer".to_string()),
                    git_info: None,
    ...
    async fn active_non_primary_shutdown_target_returns_none_for_non_shutdown_event() -> Result<()>
    {
        ...
        assert_eq!(
            app.active_non_primary_shutdown_target(&ServerNotification::SkillsChanged(
                brocode_app_server_protocol::SkillsChangedNotification {},
            )),
            None
        );
        Ok(())
    ...
    async fn clear_ui_header_shows_fast_status_only_for_gpt54() {
        ...
        app.chat_widget
            .set_service_tier(Some(brocode_protocol::config_types::ServiceTier::Fast));
    ...
    fn thread_event_store_rebase_preserves_resolved_request_state() {
        ...
        store.push_notification(ServerNotification::ServerRequestResolved(
            brocode_app_server_protocol::ServerRequestResolvedNotification {
                request_id: AppServerRequestId::Integer(1),
                thread_id: thread_id.to_string(),
            },
        ));
    ...
    fn test_session_telemetry(config: &Config, model: &str) -> SessionTelemetry {
        let model_info = brocode_core::test_support::construct_model_info_offline(model, config);
    ...
    fn active_turn_not_steerable_turn_error_extracts_structured_server_error() {
        let turn_error = AppServerTurnError {
            ...
            brocode_error_info: Some(AppServerBrocodeErrorInfo::ActiveTurnNotSteerable {
    ...
    async fn model_migration_prompt_skips_when_target_missing_or_hidden() {
        ...
        let mut current = available
            .iter()
            .find(|preset| preset.model == "gpt-5-brocode")
            .cloned()
        ...
        available.retain(|preset| preset.model != "gpt-5-brocode");
        ...
        let target = with_hidden_target
            .iter_mut()
            .find(|preset| preset.model == "gpt-5.2-brocode")
            .expect("target preset present");
        ...
        assert!(target_preset_for_upgrade(&with_hidden_target, "gpt-5.2-brocode").is_none());
    }

    #[tokio::test]
    async fn model_migration_prompt_shows_for_hidden_model() {
        let brocode_home = tempdir().expect("temp brocode home");
        let config = ConfigBuilder::default()
            .brocode_home(brocode_home.path().to_path_buf())
            .build()
            .await
            .expect("config");
        ...
        let current = available_models
            .iter()
            .find(|preset| preset.model == "gpt-5.1-brocode")
            .cloned()
            .expect("gpt-5.1-brocode preset present");
        assert!(
            !current.show_in_picker,
            "expected gpt-5.1-brocode to be hidden from picker for this test"
        );
    ...
    async fn refresh_in_memory_config_from_disk_loads_latest_apps_state() -> Result<()> {
        ...
        ConfigEditsBuilder::new(&app.config.brocode_home)
            .with_edits([
                ConfigEdit::SetPath {
    ...
    -> Result<()> {
        ...
        let brocode_home = tempdir()?;
        app.config.brocode_home = brocode_home.path().to_path_buf();
        std::fs::write(brocode_home.path().join("config.toml"), "[broken")?;
    ...
    async fn refresh_in_memory_config_from_disk_uses_active_chat_widget_cwd() -> Result<()> {
        ...
        app.chat_widget.handle_brocode_event(Event {
            id: String::new(),
    ...
    async fn fresh_session_config_uses_current_service_tier() {
        ...
        assert_eq!(
            config.service_tier,
            Some(brocode_protocol::config_types::ServiceTier::Fast)
        );
    }
    ...
    async fn replace_chat_widget_reseeds_collab_agent_metadata_for_replay() {
        ...
        app.replay_thread_snapshot(
            ThreadEventSnapshot {
                ...
                turns: Vec::new(),
                events: vec![ThreadBufferedEvent::Notification(
                    ServerNotification::ItemStarted(brocode_app_server_protocol::ItemStartedNotification {
                        thread_id: "thread-1".to_string(),
                        turn_id: "turn-1".to_string(),
                        item: ThreadItem::CollabAgentToolCall {
                            id: "wait-1".to_string(),
                            tool: brocode_app_server_protocol::CollabAgentTool::Wait,
                            status: brocode_app_server_protocol::CollabAgentToolCallStatus::InProgress,
    ...
    async fn session_summary_includes_resume_hint() {
        ...
        assert_eq!(
            ...
            Some("brocode resume 123e4567-e89b-12d3-a456-426614174000".to_string())
        );
    }
    ...
    async fn session_summary_prefers_name_over_id() {
        ...
        assert_eq!(
            summary.resume_command,
            Some("brocode resume my-session".to_string())
        );
    }
}
