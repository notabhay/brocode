...
use brocode_app_server_client::DEFAULT_IN_PROCESS_CHANNEL_CAPACITY;
use brocode_app_server_client::InProcessAppServerClient;
use brocode_app_server_client::InProcessClientStartArgs;
...
use brocode_core::models_manager::manager::RefreshStrategy;
...
async fn start_plugin_request_client(
    ...
    InProcessAppServerClient::start(InProcessClientStartArgs {
        ...
        client_name: "brocode-tui".to_string(),
...
async fn emit_custom_prompt_deprecation_notice(app_event_tx: &AppEventSender, brocode_home: &Path) {
    let prompts_dir = brocode_home.join("prompts");
    let prompt_count = brocode_core::custom_prompts::discover_prompts_in(&prompts_dir)
        .await
        .len();
    if prompt_count == 0 {
        return;
...
impl App {
    ...
    async fn select_agent_thread(&mut self, tui: &mut tui::Tui, thread_id: ThreadId) -> Result<()> {
        ...
        let brocode_op_tx = if let Some(thread) = live_thread {
            crate::chatwidget::spawn_op_forwarder(thread)
        } else {
            let (tx, _rx) = unbounded_channel();
            tx
        };
        self.replace_chat_widget(ChatWidget::new_with_op_sender(init, brocode_op_tx));
    ...
    async fn drain_active_thread_events(&mut self, tui: &mut tui::Tui) -> Result<()> {
        ...
        loop {
            match rx.try_recv() {
                Ok(event) => self.handle_brocode_event_now(event),
    ...
    fn replay_thread_snapshot(
        ...
        if let Some(event) = snapshot.session_configured {
            self.handle_brocode_event_replay(event);
        }
        self.chat_widget
    ...
    pub async fn run(
        ...
        emit_custom_prompt_deprecation_notice(&app_event_tx, &config.brocode_home).await;
        ...
        let feedback_audience = if auth_ref
            .and_then(BrocodeAuth::get_account_email)
        ...
        let session_telemetry = SessionTelemetry::new(
            ...
            model.as_str(),
            model.as_str(),
            auth_ref.and_then(BrocodeAuth::get_account_id),
            auth_ref.and_then(BrocodeAuth::get_account_email),
            auth_mode,
    ...
    fn handle_brocode_event_now(&mut self, event: Event) {
        ...
        self.chat_widget.handle_brocode_event(event);
    ...
    fn handle_brocode_event_replay(&mut self, event: Event) {
        self.chat_widget.handle_brocode_event_replay(event);
    }
    ...
    async fn handle_active_thread_event(&mut self, tui: &mut tui::Tui, event: Event) -> Result<()> {
        ...
        self.handle_brocode_event_now(event);
        if self.backtrack_render_pending {
            tui.frame_requester().schedule_frame();
        }
        Ok(())
...
mod tests {
    ...
    use brocode_protocol::protocol::TurnAbortedEvent;
    ...
    async fn startup_custom_prompt_deprecation_notice_emits_when_prompts_exist() -> Result<()> {
        ...
        let prompts_dir = brocode_home.path().join("prompts");
        ...
        emit_custom_prompt_deprecation_notice(&app_event_tx, brocode_home.path()).await;
    ...
    async fn startup_custom_prompt_deprecation_notice_skips_empty_prompts_dir() -> Result<()> {
        let brocode_home = tempdir()?;
        std::fs::create_dir_all(brocode_home.path().join("prompts"))?;
        let (tx_raw, mut rx) = unbounded_channel();
        let app_event_tx = AppEventSender::new(tx_raw);
    ...
    -> Result<()> {
        ...
        let approval_event = Event {
            ...
            msg: EventMsg::ExecApprovalRequest(
                brocode_protocol::protocol::ExecApprovalRequestEvent {
        ...
        app.handle_brocode_event_now(first_event);
        app.handle_brocode_event_now(second_event);
        app.chat_widget
    ...
    async fn replayed_turn_complete_submits_restored_queued_follow_up() {
        ...
        app.chat_widget
            .handle_brocode_event(session_configured.clone());
        ...
        let (chat_widget, _app_event_tx, _rx, mut new_op_rx) =
            make_chatwidget_manual_with_sender().await;
        app.chat_widget = chat_widget;
        app.chat_widget.handle_brocode_event(session_configured);
    ...
    async fn replace_chat_widget_keeps_replacement_terminal_title_cache_when_present() {
        ...
        replacement.last_terminal_title = Some("brocode".to_string());

        app.replace_chat_widget(replacement);

        assert_eq!(
            app.chat_widget.last_terminal_title,
            Some("brocode".to_string())
        );
    }
    ...
    async fn live_turn_started_refreshes_status_line_with_runtime_context_window() {
        ...
        app.handle_brocode_event_now(Event {
            id: "turn-started".to_string(),
            msg: EventMsg::TurnStarted(TurnStartedEvent {
                turn_id: "turn-1".to_string(),
...
"#;
    ...
    async fn make_test_app() -> App {
        ...
        let server = Arc::new(
            brocode_core::test_support::thread_manager_with_models_provider(
                BrocodeAuth::from_api_key("Test API Key"),
                config.model_provider.clone(),
            ),
        );
        let auth_manager = brocode_core::test_support::auth_manager_from_auth(
            BrocodeAuth::from_api_key("Test API Key"),
        );
        let file_search = FileSearchManager::new(config.cwd.to_path_buf(), app_event_tx.clone());
    ...
    async fn replayed_initial_messages_apply_rollback_in_queue_order() {
        ...
        app.handle_brocode_event_replay(Event {
            id: String::new(),
            msg: EventMsg::SessionConfigured(SessionConfiguredEvent {
                session_id,
                forked_from_id: None,
