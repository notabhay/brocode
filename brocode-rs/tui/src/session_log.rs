...
pub(crate) fn maybe_init(config: &Config) {
    let enabled = std::env::var("BROCODE_TUI_RECORD_SESSION")
    ...
    let path = if let Ok(path) = std::env::var("BROCODE_TUI_SESSION_LOG_PATH") {
        PathBuf::from(path)
    } else {
        let mut p = match brocode_core::config::log_dir(config) {
            Ok(dir) => dir,
...
pub(crate) fn log_inbound_app_event(event: &AppEvent) {
    ...
    match event {
        AppEvent::BrocodeEvent(ev) => {
            write_record("to_tui", "brocode_event", ev);
        }
        AppEvent::NewSession => {
            let value = json!({
                "ts": now_ts(),
