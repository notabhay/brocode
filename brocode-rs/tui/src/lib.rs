...
// The standalone `brocode-tui` binary prints a short help message before the
...
use brocode_app_server_client::RemoteAppServerClient;
use brocode_app_server_client::RemoteAppServerConnectArgs;
use brocode_app_server_protocol::AuthMode as AppServerAuthMode;
...
use brocode_app_server_protocol::ThreadSortKey as AppServerThreadSortKey;
...
use brocode_protocol::config_types::AltScreenMode;
...
mod voice {
    ...
    use brocode_core::config::Config;
    use brocode_protocol::protocol::RealtimeAudioFrame;
    use std::sync::Arc;
...
pub(crate) async fn start_app_server_for_picker(
    ...
    let app_server = start_app_server(
        ...
        brocode_feedback::BrocodeFeedback::new(),
    )
    .await?;
    ...
    Fut: Future<Output = std::io::Result<InProcessAppServerClient>>,
{
    ...
    let client = start_client(InProcessClientStartArgs {
        ...
        feedback,
        config_warnings,
        session_source: brocode_protocol::protocol::SessionSource::Cli,
...
pub async fn run_main(
    ...
    // `oss` model provider.
    let overrides_cli = brocode_utils_cli::CliConfigOverrides { raw_overrides };
    ...
    let brocode_home = match find_brocode_home() {
        Ok(brocode_home) => brocode_home.to_path_buf(),
        Err(err) => {
    ...
    if let Err(err) =
        brocode_core::personality_migration::maybe_migrate_personality(&brocode_home, &config_toml)
            .await
    {
    ...
    let model_provider_override = if cli.oss {
        ...
        if let Some(provider) = resolved {
            ...
            let provider = oss_selection::select_oss_provider(&brocode_home).await?;
    ...
    let log_dir = brocode_core::config::log_dir(&config)?;
    ...
    // Brocode CLI is officially supported on Windows.
    #[cfg(unix)]
    {
    ...
    let log_file = log_file_opts.open(log_dir.join("brocode-tui.log"))?;

    // Wrap file in non‑blocking writer.
    let (non_blocking, _guard) = non_blocking(log_file);

    // use RUST_LOG env var, default to info for brocode crates.
    let env_filter = || {
        EnvFilter::try_from_default_env().unwrap_or_else(|_| {
            EnvFilter::new("brocode_core=info,brocode_tui=info,brocode_rmcp_client=info")
        })
    };
    ...
    let feedback = brocode_feedback::BrocodeFeedback::new();
    ...
    let log_db_layer = brocode_core::state_db::get_state_db(&config)
        .await
            ...
            match update_prompt::run_update_prompt_if_needed(&mut tui, &initial_config).await? {
                ...
                UpdatePromptOutcome::RunUpdate(action) => {
                    ...
                    return Ok(AppExitInfo {
                        token_usage: brocode_protocol::protocol::TokenUsage::default(),
                        thread_id: None,
                        thread_name: None,
    ...
    let config = if should_show_onboarding {
        ...
        if show_login_screen && !remote_mode {
            cloud_requirements = cloud_requirements_loader_for_storage(
                initial_config.brocode_home.clone(),
                /*enable_brocode_api_key_env*/ false,
    ...
    let mut missing_session_exit = |id_str: &str, action: &str| {
        ...
        Ok(AppExitInfo {
            ...
            exit_reason: ExitReason::Fatal(format!(
                "No saved session found with ID {id_str}. Run `brocode {action}` without an ID to choose from existing sessions."
            )),
        })
    };
...
mod tests {
    ...
    use brocode_core::config::ConfigBuilder;
    use brocode_core::config::ConfigOverrides;
    use brocode_core::config::ProjectConfig;
    ...
    -> color_eyre::Result<()> {
        ...
        let state_runtime = brocode_state::StateRuntime::init(
        ...
        let mut builder = brocode_state::ThreadMetadataBuilder::new(
            thread_id,
            rollout_path.clone(),
            created_at,
            SessionSource::Cli,
        ...
        brocode_core::append_thread_name(&config.brocode_home, thread_id, "saved-session").await?;

        let mut app_server =
            AppServerSession::new(brocode_app_server_client::AppServerClient::InProcess(
    ...
    async fn untrusted_project_skips_trust_prompt() -> std::io::Result<()> {
        use brocode_protocol::config_types::TrustLevel;
        let temp_dir = TempDir::new()?;
        let mut config = build_config(&temp_dir).await?;
    ...
    fn build_turn_context(config: &Config, cwd: PathBuf) -> TurnContextItem {
        ...
        TurnContextItem {
            ...
            summary: config
                ...
                .unwrap_or(brocode_protocol::config_types::ReasoningSummary::Auto),
    ...
    async fn config_rebuild_changes_trust_defaults_with_cwd() -> std::io::Result<()> {
        let temp_dir = TempDir::new()?;
        let brocode_home = temp_dir.path().to_path_buf();
...
"#
    ...
    async fn read_session_cwd_prefers_sqlite_when_thread_id_present() -> std::io::Result<()> {
        ...
        let runtime = brocode_state::StateRuntime::init(
            config.brocode_home.clone(),
            config.model_provider_id.clone(),
        )
        .await
