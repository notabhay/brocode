...
use brocode_core::INTERACTIVE_SESSION_SOURCES;
use brocode_core::RolloutRecorder;
use brocode_core::ThreadSortKey;
use brocode_core::auth::AuthConfig;
use brocode_core::auth::AuthMode;
...
use brocode_core::find_thread_path_by_name_str;
...
async fn run_ratatui_app(
    ...
    let session_selection = if use_fork {
        if let Some(id_str) = cli.fork_session_id.as_deref() {
            ...
            let path = if is_uuid {
                find_thread_path_by_id_str(&config.brocode_home, id_str).await?
            } else {
                find_thread_path_by_name_str(&config.brocode_home, id_str).await?
            };
            match path {
                Some(path) => {
                    let thread_id =
                        ...
                        .await
                        {
                            ...
                            None => {
                                ...
                                return Ok(AppExitInfo {
                                    ...
                                    update_action: None,
                                    exit_reason: ExitReason::Fatal(format!(
                                        "Found latest saved session at {rollout_path}, but failed to read its metadata. Run `brocode fork` to choose from existing sessions."
                                    )),
                                });
                            }
                        }
                ...
                match resolve_session_thread_id(path.as_path(), /*id_str_if_uuid*/ None).await {
                    ...
                    None => {
                        ...
                        return Ok(AppExitInfo {
                            ...
                            update_action: None,
                            exit_reason: ExitReason::Fatal(format!(
                                "Found latest saved session at {rollout_path}, but failed to read its metadata. Run `brocode resume` to choose from existing sessions."
                            )),
                        });
                    }
                }
...
fn get_login_status(config: &Config) -> LoginStatus {
    if config.model_provider.requires_openai_auth {
        ...
        let brocode_home = config.brocode_home.clone();
        match BrocodeAuth::from_auth_storage(&brocode_home, config.cli_auth_credentials_store_mode)
        {
