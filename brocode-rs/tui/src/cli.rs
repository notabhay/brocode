use brocode_utils_cli::ApprovalModeCliArg;
...
pub struct Cli {
    ...
    // Internal controls set by the top-level `brocode resume` subcommand.
    // These are not exposed as user flags on the base `brocode` command.
    #[clap(skip)]
    ...
    /// top-level `brocode resume <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    // Internal controls set by the top-level `brocode fork` subcommand.
    // These are not exposed as user flags on the base `brocode` command.
    #[clap(skip)]
    ...
    /// top-level `brocode fork <SESSION_ID>` wrapper; not exposed as a public flag.
    #[clap(skip)]
    ...
    pub sandbox_mode: Option<brocode_utils_cli::SandboxModeCliArg>,
