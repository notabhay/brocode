...
/// Set the user-configured syntax theme override and brocode home path.
///
/// Call this with the **final resolved config** (after onboarding, resume, and
/// fork reloads complete). The first call persists `name` and `brocode_home` in
...
pub(crate) fn set_theme_override(
    ...
    if !override_set_ok || !brocode_home_set_ok {
...
fn custom_theme_path(name: &str, brocode_home: &Path) -> PathBuf {
    brocode_home.join("themes").join(format!("{name}.tmTheme"))
}

/// Try to load a custom `.tmTheme` file from `{brocode_home}/themes/{name}.tmTheme`.
fn load_custom_theme(name: &str, brocode_home: &Path) -> Option<Theme> {
    ThemeSet::get_theme(custom_theme_path(name, brocode_home)).ok()
}
...
fn resolve_theme_with_override(name: Option<&str>, brocode_home: Option<&Path>) -> Theme {
...
pub(crate) fn resolve_theme_by_name(name: &str, brocode_home: Option<&Path>) -> Option<Theme> {
    let ts = two_face::theme::extra();
    // Bundled theme?
...
/// found in `{brocode_home}/themes/`.
pub(crate) fn list_available_themes(brocode_home: Option<&Path>) -> Vec<ThemeEntry> {
