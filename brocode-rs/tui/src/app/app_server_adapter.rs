...
#[cfg(test)]
use brocode_protocol::items::ContextCompactionItem;
#[cfg(test)]
use brocode_protocol::items::ImageGenerationItem;
#[cfg(test)]
...
#[cfg(test)]
use brocode_protocol::protocol::ExecCommandStatus;
#[cfg(test)]
use brocode_protocol::protocol::ExecOutputStream;
#[cfg(test)]
use brocode_protocol::protocol::ItemCompletedEvent;
#[cfg(test)]
use brocode_protocol::protocol::ItemStartedEvent;
#[cfg(test)]
use brocode_protocol::protocol::PlanDeltaEvent;
#[cfg(test)]
...
#[cfg(test)]
use brocode_protocol::protocol::RealtimeConversationStartedEvent;
#[cfg(test)]
        ...
        ServerNotification::Error(notification) => Some((
            ...
            vec![Event {
                ...
                msg: EventMsg::Error(ErrorEvent {
                    ...
                    brocode_error_info: notification
                        .error
                        .brocode_error_info
                        .and_then(app_server_brocode_error_info_to_core),
                }),
            }],
        )),
...
fn token_usage_from_app_server(
    value: brocode_app_server_protocol::TokenUsageBreakdown,
) -> TokenUsage {
    TokenUsage {
            ...
            memory_citation: memory_citation.clone().map(|citation| {
                brocode_protocol::memory_citation::MemoryCitation {
                    entries: citation
                        ...
                        .into_iter()
                        .map(
                            |entry| brocode_protocol::memory_citation::MemoryCitationEntry {
                                path: entry.path,
...
fn command_execution_completed_event(turn_id: &str, item: &ThreadItem) -> Option<Vec<Event>> {
    ...
    let status = match status {
        brocode_app_server_protocol::CommandExecutionStatus::InProgress => return Some(Vec::new()),
        brocode_app_server_protocol::CommandExecutionStatus::Completed => {
            ExecCommandStatus::Completed
        }
        brocode_app_server_protocol::CommandExecutionStatus::Failed => ExecCommandStatus::Failed,
...
) -> Option<brocode_protocol::models::WebSearchAction> {
    match action {
        brocode_app_server_protocol::WebSearchAction::Search { query, queries } => {
            Some(brocode_protocol::models::WebSearchAction::Search { query, queries })
        }
        brocode_app_server_protocol::WebSearchAction::OpenPage { url } => {
            Some(brocode_protocol::models::WebSearchAction::OpenPage { url })
        }
        brocode_app_server_protocol::WebSearchAction::FindInPage { url, pattern } => {
            Some(brocode_protocol::models::WebSearchAction::FindInPage { url, pattern })
        }
        brocode_app_server_protocol::WebSearchAction::Other => {
            Some(brocode_protocol::models::WebSearchAction::Other)
        }
    }
}

#[cfg(test)]
fn app_server_brocode_error_info_to_core(
    value: brocode_app_server_protocol::BrocodeErrorInfo,
) -> Option<brocode_protocol::protocol::BrocodeErrorInfo> {
...
mod tests {
    ...
    use brocode_app_server_protocol::BrocodeErrorInfo;
    ...
    fn bridges_thread_snapshot_turns_for_resume_restore() {
        ...
        assert_eq!(
            error.brocode_error_info,
            Some(brocode_protocol::protocol::BrocodeErrorInfo::Other)
        );
            ...
            &Turn {
                ...
                items: vec![
                    ...
                    ThreadItem::WebSearch {
                        ...
                        action: Some(brocode_app_server_protocol::WebSearchAction::Other),
                    },
                    ThreadItem::ImageGeneration {
