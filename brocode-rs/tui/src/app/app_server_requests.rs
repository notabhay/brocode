...
use brocode_app_server_protocol::GrantedPermissionProfile;
...
use brocode_protocol::mcp::RequestId as McpRequestId;
                    ...
                    Ok::<AppServerRequestResolution, String>(AppServerRequestResolution {
                        ...
                        result: serde_json::to_value(McpServerElicitationRequestResponse {
                            action: match decision {
                                brocode_protocol::approvals::ElicitationAction::Accept => {
                                    McpServerElicitationAction::Accept
                                }
                                brocode_protocol::approvals::ElicitationAction::Decline => {
                                    McpServerElicitationAction::Decline
                                }
                                brocode_protocol::approvals::ElicitationAction::Cancel => {
...
mod tests {
    ...
    use brocode_app_server_protocol::PermissionGrantScope;
    use brocode_app_server_protocol::PermissionsRequestApprovalParams;
    use brocode_app_server_protocol::PermissionsRequestApprovalResponse;
    use brocode_app_server_protocol::RequestId as AppServerRequestId;
    use brocode_app_server_protocol::ServerRequest;
    use brocode_app_server_protocol::ToolRequestUserInputAnswer;
    use brocode_app_server_protocol::ToolRequestUserInputParams;
    use brocode_app_server_protocol::ToolRequestUserInputResponse;
    use brocode_protocol::approvals::ElicitationAction;
    ...
    fn resolves_permissions_and_user_input_through_app_server_request_id() {
        ...
        let permissions = pending
            .take_resolution(&Op::RequestPermissionsResponse {
                ...
                response: brocode_protocol::request_permissions::RequestPermissionsResponse {
                    ...
                    .expect("valid permissions"),
                    scope: brocode_protocol::request_permissions::PermissionGrantScope::Session,
                },
            })
        ...
        let user_input = pending
            .take_resolution(&Op::UserInputAnswer {
                id: "turn-2".to_string(),
                response: brocode_protocol::request_user_input::RequestUserInputResponse {
    ...
    fn rejects_dynamic_tool_calls_as_unsupported() {
        ...
        let unsupported = pending
            .note_server_request(&ServerRequest::DynamicToolCall {
                ...
                params: brocode_app_server_protocol::DynamicToolCallParams {
                    thread_id: "thread-1".to_string(),
                    turn_id: "turn-1".to_string(),
    ...
    fn does_not_mark_chatgpt_auth_refresh_as_unsupported() {
        ...
        assert_eq!(
            pending.note_server_request(&ServerRequest::ChatgptAuthTokensRefresh {
                ...
                params: brocode_app_server_protocol::ChatgptAuthTokensRefreshParams {
                    reason:
                        brocode_app_server_protocol::ChatgptAuthTokensRefreshReason::Unauthorized,
