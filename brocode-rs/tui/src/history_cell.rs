//! Transcript/history cells for the Brocode TUI.
//!
...
#[cfg(test)]
use brocode_core::mcp::qualified_mcp_tool_name_prefix;
#[cfg(test)]
use brocode_core::plugins::PluginsManager;
use brocode_core::web_search::web_search_detail;
use brocode_otel::RuntimeMetricsSummary;
use brocode_protocol::account::PlanType;
use brocode_protocol::config_types::ServiceTier;
#[cfg(test)]
use brocode_protocol::mcp::Resource;
#[cfg(test)]
use brocode_protocol::mcp::ResourceTemplate;
...
use brocode_protocol::request_user_input::RequestUserInputAnswer;
use brocode_protocol::request_user_input::RequestUserInputQuestion;
use brocode_protocol::user_input::TextElement;
use brocode_utils_cli::format_env_display::format_env_display;
...
pub(crate) trait HistoryCell: std::fmt::Debug + Send + Sync + Any {
    ...
    /// Whether this cell should render in the main chat viewport.
...
impl HistoryCell for UpdateAvailableHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let update_instruction = if let Some(update_action) = self.update_action {
            ...
            line![
                "See ",
                "https://github.com/openai/brocode".cyan().underlined(),
        ...
        let content = text![
            line![
                ...
                " ",
                format!("{BROCODE_CLI_VERSION} -> {}", self.latest_version).bold(),
            ],
            update_instruction,
            "",
            "See full release notes:",
            "https://github.com/openai/brocode/releases/latest"
                .cyan()
                .underlined(),
        ];
...
pub fn new_approval_decision_cell(
    command: Vec<String>,
    decision: brocode_protocol::protocol::ReviewDecision,
        ...
        ApprovedExecpolicyAmendment {
            ...
            (
                ...
                vec![
                    actor.subject().into(),
                    "approved".bold(),
                    " brocode to always run commands that start with ".into(),
                    snippet,
                ],
            )
        }
        ...
        NetworkPolicyAmendment {
            ...
            NetworkPolicyRuleAction::Allow => (
                ...
                vec![
                    actor.subject().into(),
                    "persisted".bold(),
                    " Brocode network access to ".into(),
    ...
    if is_first_event {
        ...
        let help_lines: Vec<Line<'static>> = vec![
            ...
            Line::from(vec![
                "  ".into(),
                "/init".into(),
                " - create an AGENTS.md file with instructions for Brocode".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/status".into(),
            ...
            Line::from(vec![
                "  ".into(),
                "/permissions".into(),
                " - choose what Brocode is allowed to do".dim(),
            ]),
            Line::from(vec![
                "  ".into(),
                "/model".into(),
...
impl HistoryCell for SessionHeaderHistoryCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        let title = Line::from(vec![
            Span::from(">_ ").dim(),
            Span::from("brocode").bold(),
            Span::from(" ").dim(),
            Span::from(format!("v{}", self.version)).dim(),
        ]);
        let wrapped = adaptive_wrap_line(&title, RtOptions::new(inner_width.max(1)));
        with_border(wrapped.iter().map(line_to_static).collect())
    }
}
...
pub(crate) struct McpToolCallCell {
    ...
    result: Option<Result<brocode_protocol::mcp::CallToolResult, String>>,
...
impl McpToolCallCell {
    ...
    pub(crate) fn complete(
        &mut self,
        duration: Duration,
        result: Result<brocode_protocol::mcp::CallToolResult, String>,
...
impl HistoryCell for McpToolCallCell {
    fn display_lines(&self, width: u16) -> Vec<Line<'static>> {
        ...
        if let Some(result) = &self.result {
            match result {
                Ok(brocode_protocol::mcp::CallToolResult { content, .. }) => {
                    if !content.is_empty() {
...
///
/// Manual testing tip:
/// - Run the rmcp stdio test server (`brocode-rs/rmcp-client/src/bin/test_stdio_server.rs`) and
///   register it as an MCP server via `brocode mcp add`.
...
pub(crate) fn empty_mcp_output() -> PlainHistoryCell {
    let lines: Vec<Line<'static>> = vec![
        ...
        Line::from(vec![
            "    See the ".into(),
            "\u{1b}]8;;https://developers.openai.com/brocode/mcp\u{7}MCP docs\u{1b}]8;;\u{7}"
                .underlined(),
...
pub(crate) fn new_mcp_tools_output(
    config: &Config,
    tools: HashMap<String, brocode_protocol::mcp::Tool>,
    ...
    let mcp_manager = McpManager::new(Arc::new(PluginsManager::new(config.brocode_home.clone())));
...
mod tests {
    ...
    use brocode_core::config::types::McpServerConfig;
    use brocode_core::config::types::McpServerDisabledReason;
    use brocode_otel::RuntimeMetricTotals;
    use brocode_otel::RuntimeMetricsSummary;
    ...
    #[test]
    fn unified_exec_interaction_cell_is_hidden_from_main_history() {
        let cell = new_unified_exec_interaction(/*command_display*/ None, String::new());
        assert!(!cell.show_in_main_history());
        assert!(!cell.show_in_transcript_history());
    }

    #[test]
    fn patch_history_cell_is_hidden_from_main_history() {
    ...
    fn prefixed_wrapped_history_cell_indents_wrapped_lines() {
        ...
        assert_eq!(
            rendered,
            vec![
                "✔ You approved brocode".to_string(),
    ...
    #[test]
    fn session_header_renders_compact_title_only() {
    ...
    #[test]
    fn session_header_omits_model_and_directory_details() {
        let cell = SessionHeaderHistoryCell::new(
            "gpt-4o".to_string(),
            Some(ReasoningEffortConfig::High),
            /*show_fast_status*/ true,
        ...
        assert!(lines.iter().all(|line| !line.contains("directory:")));
        assert!(lines.iter().all(|line| !line.contains("/model")));
    }
