...
//! under `{BROCODE_HOME}/themes/`.  It provides:
//!
...
fn theme_picker_subtitle(brocode_home: Option<&Path>, terminal_width: Option<u16>) -> String {
    let themes_dir = brocode_home.map(|home| home.join("themes"));
...
pub(crate) fn build_theme_picker_params(
    ...
    brocode_home: Option<&Path>,
    terminal_width: Option<u16>,
) -> SelectionViewParams {
    // Snapshot the current theme so we can restore on cancel.
    let original_theme = highlight::current_syntax_theme();

    let entries = highlight::list_available_themes(brocode_home);
    let brocode_home_owned = brocode_home.map(Path::to_path_buf);
    ...
    let preview_home = brocode_home_owned.clone();
...
mod tests {
    ...
    fn theme_picker_uses_half_width_with_stacked_fallback_preview() {
        let params = build_theme_picker_params(
            /*current_name*/ None, /*brocode_home*/ None, /*terminal_width*/ None,
        );
    ...
    #[test]
    fn subtitle_uses_tilde_path_when_brocode_home_under_home_directory() {
        let home = dirs::home_dir().expect("home directory should be available");
        let brocode_home = home.join(".brocode");

        let subtitle = theme_picker_subtitle(Some(&brocode_home), Some(200));
    ...
    fn subtitle_falls_back_when_tilde_path_subtitle_is_too_wide() {
        ...
        let brocode_home = home.join(long_segment).join(".brocode");

        let subtitle = theme_picker_subtitle(Some(&brocode_home), Some(140));
    ...
    fn subtitle_falls_back_to_preview_instructions_without_tilde_path() {
        let subtitle =
            theme_picker_subtitle(/*brocode_home*/ None, /*terminal_width*/ None);
        assert_eq!(subtitle, PREVIEW_FALLBACK_SUBTITLE);
    }
    ...
    fn subtitle_falls_back_for_94_column_terminal_side_by_side_layout() {
        ...
        let subtitle = theme_picker_subtitle(Some(&brocode_home), Some(94));
