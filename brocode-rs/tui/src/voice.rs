...
fn send_realtime_audio_chunk(
    ...
    tx.send(AppEvent::BrocodeOp(Op::RealtimeConversationAudio(
...
async fn resolve_auth() -> Result<TranscriptionAuthContext, String> {
    let brocode_home =
        find_brocode_home().map_err(|e| format!("failed to find brocode home: {e}"))?;
    let auth = BrocodeAuth::from_auth_storage(&brocode_home, AuthCredentialsStoreMode::Auto)
        .map_err(|e| format!("failed to read auth.json: {e}"))?
        .ok_or_else(|| "No brocode auth is configured; please run `brocode login`".to_string())?;
    ...
    let (endpoint, request) =
        if matches!(auth.mode, AuthMode::Chatgpt | AuthMode::ChatgptAuthTokens) {
            ...
            let mut req = client
                ...
                .multipart(form)
                .header("User-Agent", get_brocode_user_agent());
