...
use brocode_app_server_client::AppServerClient;
use brocode_app_server_client::AppServerEvent;
...
use brocode_app_server_protocol::ConfigBatchWriteParams;
use brocode_app_server_protocol::ConfigWriteResponse;
...
use brocode_app_server_protocol::Model as ApiModel;
...
use brocode_app_server_protocol::ReviewDelivery;
...
use brocode_protocol::protocol::SessionNetworkProxyRuntime;
...
pub(crate) struct AppServerBootstrap {
    ...
    pub(crate) plan_type: Option<brocode_protocol::account::PlanType>,
...
pub(crate) struct ThreadSessionState {
    ...
    pub(crate) service_tier: Option<brocode_protocol::config_types::ServiceTier>,
    pub(crate) approval_policy: AskForApproval,
    pub(crate) approvals_reviewer: brocode_protocol::config_types::ApprovalsReviewer,
    pub(crate) sandbox_policy: SandboxPolicy,
    pub(crate) cwd: PathBuf,
    pub(crate) reasoning_effort: Option<brocode_protocol::openai_models::ReasoningEffort>,
...
impl AppServerSession {
    ...
    pub(crate) async fn turn_start(
        &mut self,
        thread_id: ThreadId,
        items: Vec<brocode_protocol::user_input::UserInput>,
        cwd: PathBuf,
        approval_policy: AskForApproval,
        approvals_reviewer: brocode_protocol::config_types::ApprovalsReviewer,
        sandbox_policy: SandboxPolicy,
        model: String,
        effort: Option<brocode_protocol::openai_models::ReasoningEffort>,
        summary: Option<brocode_protocol::config_types::ReasoningSummary>,
        service_tier: Option<Option<brocode_protocol::config_types::ServiceTier>>,
        collaboration_mode: Option<brocode_protocol::config_types::CollaborationMode>,
        personality: Option<brocode_protocol::config_types::Personality>,
...
pub(crate) fn status_account_display_from_auth_mode(
    ...
    plan_type: Option<brocode_protocol::account::PlanType>,
...
fn approvals_reviewer_override_from_config(
    config: &Config,
) -> Option<brocode_app_server_protocol::ApprovalsReviewer> {
...
) -> Option<brocode_app_server_protocol::SandboxMode> {
    match policy {
        SandboxPolicy::DangerFullAccess => {
            Some(brocode_app_server_protocol::SandboxMode::DangerFullAccess)
        }
        SandboxPolicy::ReadOnly { .. } => Some(brocode_app_server_protocol::SandboxMode::ReadOnly),
        SandboxPolicy::WorkspaceWrite { .. } => {
            Some(brocode_app_server_protocol::SandboxMode::WorkspaceWrite)
        }
...
) -> brocode_app_server_protocol::ReviewTarget {
    match target {
        CoreReviewTarget::UncommittedChanges => {
            brocode_app_server_protocol::ReviewTarget::UncommittedChanges
        }
        CoreReviewTarget::BaseBranch { branch } => {
            brocode_app_server_protocol::ReviewTarget::BaseBranch { branch }
        }
        CoreReviewTarget::Commit { sha, title } => {
            brocode_app_server_protocol::ReviewTarget::Commit { sha, title }
        }
        CoreReviewTarget::Custom { instructions } => {
            brocode_app_server_protocol::ReviewTarget::Custom { instructions }
        }
    }
}
...
async fn thread_session_state_from_thread_response(
    ...
    service_tier: Option<brocode_protocol::config_types::ServiceTier>,
    approval_policy: AskForApproval,
    approvals_reviewer: brocode_protocol::config_types::ApprovalsReviewer,
    sandbox_policy: SandboxPolicy,
    cwd: PathBuf,
    reasoning_effort: Option<brocode_protocol::openai_models::ReasoningEffort>,
    config: &Config,
) -> Result<ThreadSessionState, String> {
...
pub(crate) fn app_server_rate_limit_snapshot_to_core(
    snapshot: brocode_app_server_protocol::RateLimitSnapshot,
) -> RateLimitSnapshot {
    RateLimitSnapshot {
...
fn app_server_rate_limit_window_to_core(
    window: brocode_app_server_protocol::RateLimitWindow,
) -> RateLimitWindow {
    RateLimitWindow {
...
fn app_server_credits_snapshot_to_core(
    snapshot: brocode_app_server_protocol::CreditsSnapshot,
) -> CreditsSnapshot {
    CreditsSnapshot {
...
mod tests {
    ...
    async fn build_config(temp_dir: &TempDir) -> Config {
        ConfigBuilder::default()
            .brocode_home(temp_dir.path().to_path_buf())
            .build()
            .await
    ...
    async fn resume_response_restores_turns_from_thread_items() {
        ...
        let response = ThreadResumeResponse {
            thread: brocode_app_server_protocol::Thread {
                ...
                source: brocode_protocol::protocol::SessionSource::Cli.into(),
                agent_nickname: None,
                agent_role: None,
                git_info: None,
                name: None,
                turns: vec![Turn {
                    id: "turn-1".to_string(),
                    items: vec![
                        brocode_app_server_protocol::ThreadItem::UserMessage {
                            id: "user-1".to_string(),
                            content: vec![brocode_app_server_protocol::UserInput::Text {
                                text: "hello from history".to_string(),
                                text_elements: Vec::new(),
                            }],
                        },
                        brocode_app_server_protocol::ThreadItem::AgentMessage {
            ...
            service_tier: None,
            cwd: PathBuf::from("/tmp/project"),
            approval_policy: brocode_protocol::protocol::AskForApproval::Never.into(),
            approvals_reviewer: brocode_app_server_protocol::ApprovalsReviewer::User,
            sandbox: brocode_protocol::protocol::SandboxPolicy::new_read_only_policy().into(),
    ...
    async fn session_configured_populates_history_metadata() {
        ...
        let session = thread_session_state_from_thread_response(
            ...
            brocode_protocol::config_types::ApprovalsReviewer::User,
    ...
    fn status_account_display_from_auth_mode_uses_remapped_plan_labels() {
        let business = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(brocode_protocol::account::PlanType::EnterpriseCbpUsageBased),
        );
        assert!(matches!(
            business,
            Some(StatusAccountDisplay::ChatGpt {
        ...
        let team = status_account_display_from_auth_mode(
            Some(AuthMode::Chatgpt),
            Some(brocode_protocol::account::PlanType::SelfServeBusinessUsageBased),
        );
        assert!(matches!(
            team,
            Some(StatusAccountDisplay::ChatGpt {
