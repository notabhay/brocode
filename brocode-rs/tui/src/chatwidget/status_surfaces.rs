...
impl ChatWidget {
    ...
    /// Clears the terminal title brocode most recently wrote, if any.
    ///
        ...
        self.config
            ...
            .find_map(|layer| match &layer.name {
                ConfigLayerSource::Project { dot_brocode_folder } => {
                    dot_brocode_folder.as_path().parent().map(Path::to_path_buf)
                }
                _ => None,
            })
    }
    ...
    pub(super) fn status_line_value_for_item(&mut self, item: &StatusLineItem) -> Option<String> {
        match item {
            ...
            StatusLineItem::BrocodeVersion => Some(BROCODE_CLI_VERSION.to_string()),
    ...
    fn terminal_title_value_for_item(
        ...
        match item {
            TerminalTitleItem::AppName => Some("brocode".to_string()),
