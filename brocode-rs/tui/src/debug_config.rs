...
use brocode_core::config_loader::ConfigLayerEntry;
use brocode_core::config_loader::ConfigLayerStack;
use brocode_core::config_loader::ConfigLayerStackOrdering;
use brocode_core::config_loader::NetworkConstraints;
use brocode_core::config_loader::NetworkDomainPermissionToml;
use brocode_core::config_loader::NetworkUnixSocketPermissionToml;
use brocode_core::config_loader::RequirementSource;
use brocode_core::config_loader::ResidencyRequirement;
use brocode_core::config_loader::SandboxModeRequirement;
use brocode_core::config_loader::WebSearchModeRequirement;
    ...
    if let Some(proxy) = session_network_proxy {
        ...
        let all_proxy = session_all_proxy_url(
            ...
            config
                ...
                .network
                .as_ref()
                .is_some_and(brocode_core::config::NetworkProxySpec::socks_enabled),
        );
...
fn format_config_layer_source(source: &ConfigLayerSource) -> String {
    match source {
        ...
        ConfigLayerSource::Project { dot_brocode_folder } => {
            format!(
                "project ({}/config.toml)",
                dot_brocode_folder.as_path().display()
            )
        }
...
mod tests {
    ...
    use brocode_core::config_loader::ConstrainedWithSource;
    use brocode_core::config_loader::McpServerIdentity;
    use brocode_core::config_loader::McpServerRequirement;
    use brocode_core::config_loader::NetworkConstraints;
    use brocode_core::config_loader::NetworkDomainPermissionToml;
    use brocode_core::config_loader::NetworkDomainPermissionsToml;
    use brocode_core::config_loader::NetworkUnixSocketPermissionToml;
    use brocode_core::config_loader::NetworkUnixSocketPermissionsToml;
    use brocode_core::config_loader::RequirementSource;
    use brocode_core::config_loader::ResidencyRequirement;
    use brocode_core::config_loader::SandboxModeRequirement;
    use brocode_core::config_loader::Sourced;
    use brocode_core::config_loader::WebSearchModeRequirement;
    ...
    fn debug_config_output_lists_all_layers_including_disabled() {
        let system_file = if cfg!(windows) {
            absolute_path("C:\\etc\\brocode\\config.toml")
        } else {
            absolute_path("/etc/brocode/config.toml")
        };
        let project_folder = if cfg!(windows) {
            absolute_path("C:\\repo\\.brocode")
        } else {
            absolute_path("/repo/.brocode")
        };
        ...
        let layers = vec![
            ...
            ConfigLayerEntry::new_disabled(
                ConfigLayerSource::Project {
                    dot_brocode_folder: project_folder,
                },
                empty_toml_table(),
    ...
    fn debug_config_output_lists_requirement_sources() {
        let requirements_file = if cfg!(windows) {
            absolute_path("C:\\ProgramData\\OpenAI\\Brocode\\requirements.toml")
        } else {
            absolute_path("/etc/brocode/requirements.toml")
        };
        ...
        let user_file = if cfg!(windows) {
            absolute_path("C:\\users\\alice\\.brocode\\config.toml")
        } else {
            absolute_path("/home/alice/.brocode/config.toml")
        };
        let stack = ConfigLayerStack::new(
    ...
    fn debug_config_output_formats_unix_socket_permissions() {
        ...
        let rendered = render_to_text(&render_debug_config_lines(&stack));
        assert!(rendered.contains(
            "experimental_network: unix_sockets={/tmp/blocked.sock=none, /tmp/brocode.sock=allow} (source: cloud requirements)"
        ));
    }
