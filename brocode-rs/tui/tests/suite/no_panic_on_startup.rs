...
async fn malformed_rules_should_not_panic() -> anyhow::Result<()> {
    ...
    let brocode_home = tmp.path();
    std::fs::write(
        brocode_home.join("rules"),
...
struct BrocodeCliOutput {
    exit_code: i32,
    output: String,
}

async fn run_brocode_cli(
    brocode_home: impl AsRef<Path>,
    cwd: impl AsRef<Path>,
) -> anyhow::Result<BrocodeCliOutput> {
    ...
    let spawned = brocode_utils_pty::spawn_pty_process(
        brocode_cli.to_string_lossy().as_ref(),
        &args,
        cwd.as_ref(),
        &env,
        &None,
    ...
    let exit_code = match exit_code_result {
        ...
        Err(_) => {
            session.terminate();
            anyhow::bail!("timed out waiting for brocode CLI to exit");
        }
    };
