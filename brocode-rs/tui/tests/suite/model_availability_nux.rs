...
async fn resume_startup_does_not_consume_model_availability_nux_count() -> Result<()> {
    // run_brocode_cli() does not work on Windows due to PTY limitations.
    if cfg!(windows) {
        return Ok(());
    }
    ...
    let source_catalog_path = brocode_utils_cargo_bin::find_resource!("../core/models.json")?;
    ...
    let custom_catalog_path = brocode_home.path().join("catalog.json");
    std::fs::write(
        &custom_catalog_path,
...
"#
    ...
    std::fs::write(brocode_home.path().join("config.toml"), config_contents)?;

    let fixture_path =
        brocode_utils_cargo_bin::find_resource!("../core/tests/cli_responses_fixture.sse")?;
    let brocode = if let Ok(path) = brocode_utils_cargo_bin::cargo_bin("brocode") {
        path
    } else {
        let fallback = repo_root.join("brocode-rs/target/debug/brocode");
        if fallback.is_file() {
            fallback
        } else {
            eprintln!("skipping integration test because brocode binary is unavailable");
            return Ok(());
        }
    };

    let exec_output = std::process::Command::new(&brocode)
        .arg("exec")
        ...
        .output()
        .context("failed to execute brocode exec")?;
    anyhow::ensure!(
    ...
    let spawned = brocode_utils_pty::spawn_pty_process(
        brocode.to_string_lossy().as_ref(),
        &args,
        &repo_root,
        &env,
        &None,
        brocode_utils_pty::TerminalSize::default(),
    )
    .await?;

    let mut output = Vec::new();
    let brocode_utils_pty::SpawnedProcess {
        session,
        stdout_rx,
        stderr_rx,
        exit_rx,
    } = spawned;
    let mut output_rx = brocode_utils_pty::combine_output_receivers(stdout_rx, stderr_rx);
    ...
    let exit_code = match exit_code_result {
        ...
        Err(_) => {
            session.terminate();
            anyhow::bail!("timed out waiting for brocode resume to exit");
        }
    };
    ...
    anyhow::ensure!(
        ...
        "unexpected exit code from brocode resume: {exit_code}; output: {output_text}",
    );

    let config_contents = std::fs::read_to_string(brocode_home.path().join("config.toml"))?;
