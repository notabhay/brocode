...
pub enum Command {
    /// Submit a new Brocode Cloud task without launching the TUI.
    Exec(ExecCommand),
    /// Show the status of a Brocode Cloud task.
    Status(StatusCommand),
    /// List Brocode Cloud tasks.
    List(ListCommand),
    /// Apply the diff for a Brocode Cloud task locally.
    Apply(ApplyCommand),
    /// Show the unified diff for a Brocode Cloud task.
    Diff(DiffCommand),
}

#[derive(Debug, Args)]
pub struct ExecCommand {
    /// Task prompt to run in Brocode Cloud.
    #[arg(value_name = "QUERY")]
    pub query: Option<String>,

    /// Target environment identifier (see `brocode cloud` to browse).
    ...
    /// Git branch to run in Brocode Cloud (defaults to current branch).
...
pub struct StatusCommand {
    /// Brocode Cloud task identifier to inspect.
...
pub struct ApplyCommand {
    /// Brocode Cloud task identifier to apply.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
...
pub struct DiffCommand {
    /// Brocode Cloud task identifier to display.
    #[arg(value_name = "TASK_ID")]
    pub task_id: String,
