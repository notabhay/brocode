...
use crate::ToolSpec;
use brocode_code_mode::CodeModeToolKind;
use brocode_code_mode::ToolDefinition as CodeModeToolDefinition;
...
pub fn augment_tool_spec_for_code_mode(spec: ToolSpec) -> ToolSpec {
    let Some(description) = code_mode_tool_definition_for_spec(&spec)
        .map(brocode_code_mode::augment_tool_definition)
...
pub fn tool_spec_to_code_mode_tool_definition(spec: &ToolSpec) -> Option<CodeModeToolDefinition> {
    ...
    brocode_code_mode::is_code_mode_nested_tool(&definition.name)
        .then(|| brocode_code_mode::augment_tool_definition(definition))
}
...
pub fn create_wait_tool() -> ToolSpec {
    ...
    ToolSpec::Function(ResponsesApiTool {
        name: brocode_code_mode::WAIT_TOOL_NAME.to_string(),
        description: format!(
            "Waits on a yielded `{}` cell and returns new output or completion.\n{}",
            brocode_code_mode::PUBLIC_TOOL_NAME,
...
"#;
    ...
    ToolSpec::Freeform(FreeformTool {
        name: brocode_code_mode::PUBLIC_TOOL_NAME.to_string(),
