...
mod windows_impl {
    ...
    /// Creates the sandbox user's Brocode home directory if it does not already exist.
    fn ensure_brocode_home_exists(p: &Path) -> Result<()> {
    ...
    /// Resolves the command runner path, preferring BROCODE_HOME/.sandbox/bin.
    fn find_runner_exe(brocode_home: &Path, log_dir: Option<&Path>) -> PathBuf {
        resolve_helper_for_launch(HelperExecutable::CommandRunner, brocode_home, log_dir)
    }
    ...
    pub fn run_windows_sandbox_capture(
        ...
        let sandbox_base = brocode_home.join(".sandbox");
        ensure_brocode_home_exists(&sandbox_base)?;
        ...
        let caps = load_or_create_cap_sids(brocode_home)?;
        let (psid_to_use, cap_sids) = match &policy {
            ...
            SandboxPolicy::WorkspaceWrite { .. } => (
                ...
                vec![
                    ...
                    crate::cap::workspace_cap_sid_for_cwd(brocode_home, cwd)?,
                ],
            ),
            SandboxPolicy::DangerFullAccess | SandboxPolicy::ExternalSandbox { .. } => {
        ...
        let runner_exe = find_runner_exe(brocode_home, logs_base_dir);
        let runner_cmdline = runner_exe
            .to_str()
            .map(|s| s.to_string())
            .unwrap_or_else(|| "brocode-command-runner.exe".to_string());
            ...
            let spawn_request = FramedMessage {
                ...
                message: Message::SpawnRequest {
                    payload: Box::new(SpawnRequest {
                        ...
                        brocode_home: sandbox_base.clone(),
                        real_brocode_home: brocode_home.to_path_buf(),
                        cap_sids,
                        timeout_ms,
                        tty: false,
                        stdin_open: false,
