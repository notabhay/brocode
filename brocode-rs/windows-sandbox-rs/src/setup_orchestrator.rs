...
fn run_setup_refresh_inner(
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| brocode_home.to_path_buf());
    log_note(
        &format!(
...
pub fn run_elevated_setup(
    ...
    let sbx_dir = sandbox_dir(brocode_home);
    ...
    run_setup_exe(&payload, needs_elevation, brocode_home)
}
...
fn build_payload_roots(
    ...
    let write_roots = filter_sensitive_write_roots(write_roots, brocode_home);
    let mut read_roots = if let Some(roots) = read_roots_override {
        canonical_existing(&roots)
    } else {
        gather_read_roots(command_cwd, policy, brocode_home)
    };
