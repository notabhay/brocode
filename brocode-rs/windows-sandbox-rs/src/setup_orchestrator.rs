...
pub const OFFLINE_USERNAME: &str = "BrocodeSandboxOffline";
pub const ONLINE_USERNAME: &str = "BrocodeSandboxOnline";
...
pub fn sandbox_dir(brocode_home: &Path) -> PathBuf {
    brocode_home.join(".sandbox")
}

pub fn sandbox_bin_dir(brocode_home: &Path) -> PathBuf {
    brocode_home.join(".sandbox-bin")
}

pub fn sandbox_secrets_dir(brocode_home: &Path) -> PathBuf {
    brocode_home.join(".sandbox-secrets")
}

pub fn setup_marker_path(brocode_home: &Path) -> PathBuf {
    sandbox_dir(brocode_home).join("setup_marker.json")
}

pub fn sandbox_users_path(brocode_home: &Path) -> PathBuf {
    sandbox_secrets_dir(brocode_home).join("sandbox_users.json")
}
...
pub fn run_setup_refresh_with_extra_read_roots(
    ...
    let mut read_roots = gather_read_roots(command_cwd, policy, brocode_home);
...
fn run_setup_refresh_inner(
    ...
    let payload = ElevationPayload {
        ...
        brocode_home: request.brocode_home.to_path_buf(),
    ...
    let cwd = std::env::current_dir().unwrap_or_else(|_| request.brocode_home.to_path_buf());
    log_note(
        &format!(
        ...
        Some(&sandbox_dir(request.brocode_home)),
    );
    let status = cmd
        .status()
        .map_err(|e| {
...
fn gather_helper_read_roots(brocode_home: &Path) -> Vec<PathBuf> {
    ...
    let helper_dir = helper_bin_dir(brocode_home);
...
fn gather_legacy_full_read_roots(
    ...
    let mut roots = gather_helper_read_roots(brocode_home);
    roots.extend(
...
pub(crate) fn gather_read_roots(
    ...
    if policy.has_full_disk_read_access() {
        gather_legacy_full_read_roots(command_cwd, policy, brocode_home)
    } else {
        gather_restricted_read_roots(command_cwd, policy, brocode_home)
    }
}
...
const ALLOW_LOCAL_BINDING_ENV_KEY: &str = "BROCODE_NETWORK_ALLOW_LOCAL_BINDING";
...
fn find_setup_exe() -> PathBuf {
    if let Ok(exe) = std::env::current_exe()
        && let Some(dir) = exe.parent()
    {
        let candidate = dir.join("brocode-windows-sandbox-setup.exe");
        if candidate.exists() {
            return candidate;
        }
    }
    PathBuf::from("brocode-windows-sandbox-setup.exe")
}
...
fn report_helper_failure(
    ...
    match read_setup_error_report(brocode_home) {
...
fn run_setup_exe(
    ...
    let cleared_report = match clear_setup_error_report(brocode_home) {
        Ok(()) => true,
        Err(err) => {
            log_note(
                &format!(
    ...
    if !needs_elevation {
        ...
        if let Err(err) = clear_setup_error_report(brocode_home) {
            log_note(
                &format!(
...
pub fn run_elevated_setup(
    ...
    let sbx_dir = sandbox_dir(request.brocode_home);
    ...
    run_setup_exe(&payload, needs_elevation, request.brocode_home)
}
...
fn build_payload_roots(
    ...
    let write_roots = filter_sensitive_write_roots(write_roots, request.brocode_home);
    let mut read_roots = if let Some(roots) = overrides.read_roots {
        canonical_existing(&roots)
    } else {
        gather_read_roots(request.command_cwd, request.policy, request.brocode_home)
    };
...
fn filter_sensitive_write_roots(mut roots: Vec<PathBuf>, brocode_home: &Path) -> Vec<PathBuf> {
    // Never grant capability write access to BROCODE_HOME or anything under BROCODE_HOME/.sandbox,
    // BROCODE_HOME/.sandbox-bin, or BROCODE_HOME/.sandbox-secrets. These locations contain sandbox
    // control/state and helper binaries and must remain tamper-resistant.
    let brocode_home_key = canonical_path_key(brocode_home);
    let sbx_dir_key = canonical_path_key(&sandbox_dir(brocode_home));
    let sbx_dir_prefix = format!("{}/", sbx_dir_key.trim_end_matches('/'));
    let sbx_bin_dir_key = canonical_path_key(&sandbox_bin_dir(brocode_home));
    let sbx_bin_dir_prefix = format!("{}/", sbx_bin_dir_key.trim_end_matches('/'));
    let secrets_dir_key = canonical_path_key(&sandbox_secrets_dir(brocode_home));
    let secrets_dir_prefix = format!("{}/", secrets_dir_key.trim_end_matches('/'));

    roots.retain(|root| {
        let key = canonical_path_key(root);
        key != brocode_home_key
            && key != sbx_dir_key
...
mod tests {
    ...
    fn offline_proxy_settings_ignore_proxy_env_when_online_identity_selected() {
        ...
        env.insert(
            "BROCODE_NETWORK_ALLOW_LOCAL_BINDING".to_string(),
            "1".to_string(),
        );
    ...
    fn gather_read_roots_includes_helper_bin_dir() {
        let tmp = TempDir::new().expect("tempdir");
        let brocode_home = tmp.path().join("brocode-home");
        let command_cwd = tmp.path().join("workspace");
        fs::create_dir_all(&command_cwd).expect("create workspace");
        let policy = SandboxPolicy::new_read_only_policy();

        let roots = gather_read_roots(&command_cwd, &policy, &brocode_home);
        let expected =
            dunce::canonicalize(helper_bin_dir(&brocode_home)).expect("canonical helper dir");
    ...
    fn full_read_roots_preserve_legacy_platform_defaults() {
        ...
        let roots = gather_legacy_full_read_roots(&command_cwd, &policy, &brocode_home);
