...
pub fn require_logon_sandbox_creds(
    ...
    // NOTE: Do not add BROCODE_HOME/.sandbox to `needed_write`; it must remain non-writable by the
    ...
    let mut identity = match load_marker(brocode_home)? {
        Some(marker) if marker.version_matches() => {
                ...
                marker.request_mismatch_reason(network_identity, &desired_offline_proxy_settings)
            {
                ...
                let selected = select_identity(network_identity, brocode_home)?;
                if selected.is_none() {
                    setup_reason =
    ...
    if identity.is_none() {
        ...
        identity = select_identity(network_identity, brocode_home)?;
    }
