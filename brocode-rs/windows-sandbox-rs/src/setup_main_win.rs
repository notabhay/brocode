...
use brocode_windows_sandbox::ensure_allow_write_aces;
use brocode_windows_sandbox::extract_setup_failure;
use brocode_windows_sandbox::hide_newly_created_users;
use brocode_windows_sandbox::is_command_cwd_root;
use brocode_windows_sandbox::load_or_create_cap_sids;
use brocode_windows_sandbox::log_note;
use brocode_windows_sandbox::path_mask_allows;
use brocode_windows_sandbox::protect_workspace_agents_dir;
use brocode_windows_sandbox::protect_workspace_brocode_dir;
use brocode_windows_sandbox::sandbox_bin_dir;
use brocode_windows_sandbox::sandbox_dir;
use brocode_windows_sandbox::sandbox_secrets_dir;
use brocode_windows_sandbox::string_from_sid_bytes;
...
fn real_main() -> Result<()> {
    ...
    let sbx_dir = sandbox_dir(&payload.brocode_home);
    ...
    if let Err(err) = &result {
        ...
        if let Err(write_err) = write_setup_error_report(&payload.brocode_home, &report) {
            let _ = log_line(
                &mut log,
...
fn run_setup_full(payload: &Payload, log: &mut File, sbx_dir: &Path) -> Result<()> {
    ...
    let caps = load_or_create_cap_sids(&payload.brocode_home).map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
    ...
    let workspace_sid_str = workspace_cap_sid_for_cwd(&payload.brocode_home, &payload.command_cwd)?;
    ...
    lock_sandbox_dir(
        &sandbox_bin_dir(&payload.brocode_home),
        &payload.real_user,
        &sandbox_group_sid,
        GRANT_ACCESS,
        FILE_GENERIC_READ | FILE_GENERIC_EXECUTE,
    ...
    .map_err(|err| {
        anyhow::Error::new(SetupFailure::new(
            ...
            format!(
                ...
                sandbox_bin_dir(&payload.brocode_home).display()
            ),
        ))
    })?;
    ...
    if !refresh_only {
        lock_sandbox_dir(
            &sandbox_dir(&payload.brocode_home),
            &payload.real_user,
            &sandbox_group_sid,
            GRANT_ACCESS,
            FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_dir(&payload.brocode_home).display()
                ),
            ))
        })?;
        lock_sandbox_dir(
            &sandbox_secrets_dir(&payload.brocode_home),
            &payload.real_user,
            &sandbox_group_sid,
            DENY_ACCESS,
            FILE_GENERIC_READ | FILE_GENERIC_WRITE | FILE_GENERIC_EXECUTE | DELETE,
        ...
        .map_err(|err| {
            anyhow::Error::new(SetupFailure::new(
                ...
                format!(
                    ...
                    sandbox_secrets_dir(&payload.brocode_home).display()
                ),
            ))
        })?;
        let legacy_users = sandbox_dir(&payload.brocode_home).join("sandbox_users.json");
        if legacy_users.exists() {
            let _ = std::fs::remove_file(&legacy_users);
        }
    }

    // Protect the current workspace's `.brocode` and `.agents` directories from tampering
    ...
    match unsafe { protect_workspace_brocode_dir(&payload.command_cwd, workspace_psid) } {
        Ok(true) => {
            let cwd_brocode = payload.command_cwd.join(".brocode");
            log_line(
                log,
                &format!(
                    "applied deny ACE to protect workspace .brocode {}",
                    cwd_brocode.display()
                ),
            )?;
        }
        ...
        Err(err) => {
            ...
            log_line(
                log,
                &format!("deny ACE failed on {}: {err}", cwd_brocode.display()),
            )?;
        }
    }
