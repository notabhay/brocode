...
# Run a suite of smoke tests against the Windows sandbox via the Brocode CLI
...
def _resolve_brocode_cmd() -> List[str]:
    """Resolve the Brocode CLI to invoke `brocode sandbox windows`.

    Prefer local builds (debug first), then fall back to PATH.
    Returns the argv prefix to run Brocode.
    """
    ...
    candidates = [
        ws_root / "target" / "debug" / "brocode.exe",
        ws_root / "target" / "release" / "brocode.exe",
    ]
    if cargo_target:
        cargo_base = Path(cargo_target)
        candidates.extend([
            cargo_base / "debug" / "brocode.exe",
            cargo_base / "release" / "brocode.exe",
        ])
    ...
    if shutil.which("brocode"):
        return ["brocode"]

    raise FileNotFoundError(
        "Brocode CLI not found. Build it first, e.g.\n"
        "  cargo build -p brocode-cli --release\n"
        "or for debug:\n"
        "  cargo build -p brocode-cli\n"
    )

BROCODE_CMD = _resolve_brocode_cmd()
print(BROCODE_CMD)
TIMEOUT_SEC = 20
...
def run_sbx(
    ...
    # Map policy to brocode CLI flags
    ...
    argv = [*BROCODE_CMD, "sandbox", "windows", *policy_flags, *overrides, "--", *cmd_argv]
    print(cmd_argv)
...
def main() -> int:
    ...
    if have("curl"):
        with start_loopback_proxy_fixture() as (target_port, proxy_port):
            ...
            rc_direct, _out_direct, err_direct = run_sbx(
                ...
                direct_cmd,
                WS_ROOT,
                env_extra={"BROCODE_HOME": str(proxy_home)},
            )
    ...
    rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo hi > \\\\.\\pipe\\brocode_testpipe"], WS_ROOT)
    ...
    # 34. WS: policy tamper (.brocode artifacts) denied
    brocode_home = Path(os.environ["USERPROFILE"]) / ".brocode"
    cap_sid_target = brocode_home / "cap_sid"
    rc, out, err = run_sbx(
        "workspace-write",
        ["cmd", "/c", f"echo tamper > \"{cap_sid_target}\""],
        WS_ROOT,
    )
    rc2, out2, err2 = run_sbx("workspace-write", ["cmd", "/c", "echo tamper > .brocode\\policy.json"], WS_ROOT)
    add("WS: .brocode cap_sid tamper denied", rc != 0, f"rc={rc}, err={err}")
    add("WS: .brocode policy tamper denied", rc2 != 0, f"rc={rc2}, err={err2}")
    ...
    if make_symlink(fake_root, Path("C:/")):
        rc, out, err = run_sbx("workspace-write", ["cmd", "/c", "echo owned > brocode_escape.txt"], fake_root)
    ...
    rc, out, err = run_sbx(
        ...
        [
            ...
            "-NoProfile",
            "-Command",
            "Start-Process 'https://brocode-invalid.local/smoke'",
        ],
        WS_ROOT,
    )
    add(
