# brocode-package-manager

`brocode-package-manager` is the shared installer used for versioned runtime bundles and other cached artifacts in `brocode-rs`.
...
```text
<brocode_home>/<default_cache_root_relative>
```
...
```rust,ignore
let config = PackageManagerConfig::new(brocode_home, MyPackage::new(...));
