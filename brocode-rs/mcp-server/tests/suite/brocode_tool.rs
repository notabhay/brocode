...
use brocode_core::spawn::BROCODE_SANDBOX_NETWORK_DISABLED_ENV_VAR;
use brocode_mcp_server::BrocodeToolCallParam;
use brocode_mcp_server::ExecApprovalElicitRequestParams;
use brocode_mcp_server::ExecApprovalResponse;
use brocode_mcp_server::PatchApprovalElicitRequestParams;
use brocode_mcp_server::PatchApprovalResponse;
use brocode_protocol::protocol::FileChange;
use brocode_protocol::protocol::ReviewDecision;
use brocode_shell_command::parse_command;
...
async fn test_shell_command_approval_triggers_elicitation() {
    if env::var(BROCODE_SANDBOX_NETWORK_DISABLED_ENV_VAR).is_ok() {
        println!(
            "Skipping test because it cannot execute when network is disabled in a Brocode sandbox."
        );
        return;
    }
...
async fn shell_command_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "brocode" tool request, which should hit the responses endpoint.
    // In turn, it should reply with a tool call, which the MCP should forward
    // as an elicitation.
    let brocode_request_id = mcp_process
        .send_brocode_tool_call(BrocodeToolCallParam {
    ...
    assert_eq!(
        ...
        Some(create_expected_elicitation_request_params(
            ...
            brocode_request_id.to_string(),
            params.brocode_event_id.clone(),
            params.thread_id,
        )?)
    );
    ...
    // Verify the original `brocode` tool call completes and that the file was created.
    let brocode_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(brocode_request_id)),
    )
    .await??;
    assert_eq!(
        JsonRpcResponse {
            jsonrpc: JsonRpcVersion2_0,
            id: RequestId::Number(brocode_request_id),
            result: json!({
                "content": [
                    {
                        "text": "File created!",
...
fn create_expected_elicitation_request_params(
    command: Vec<String>,
    workdir: &Path,
    brocode_mcp_tool_call_id: String,
    brocode_event_id: String,
    thread_id: brocode_protocol::ThreadId,
) -> anyhow::Result<serde_json::Value> {
    let expected_message = format!(
        "Allow Brocode to run `{}` in `{}`?",
        shlex::try_join(command.iter().map(std::convert::AsRef::as_ref))?,
        workdir.to_string_lossy()
    );
    let brocode_parsed_cmd = parse_command::parse_command(&command);
    let params_json = serde_json::to_value(ExecApprovalElicitRequestParams {
        message: expected_message,
        requested_schema: json!({"type":"object","properties":{}}),
        thread_id,
        brocode_elicitation: "exec-approval".to_string(),
        brocode_mcp_tool_call_id,
        brocode_event_id,
        brocode_command: command,
        brocode_cwd: workdir.to_path_buf(),
        brocode_call_id: "call1234".to_string(),
        brocode_parsed_cmd,
    })?;
    Ok(params_json)
}
...
async fn patch_approval_triggers_elicitation() -> anyhow::Result<()> {
    ...
    // Send a "brocode" tool request that will trigger the apply_patch command
    let brocode_request_id = mcp_process
        .send_brocode_tool_call(BrocodeToolCallParam {
    ...
    // Verify the original `brocode` tool call completes
    let brocode_response = timeout(
        DEFAULT_READ_TIMEOUT,
        mcp_process.read_stream_until_response_message(RequestId::Number(brocode_request_id)),
    )
...
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn test_brocode_tool_passes_base_instructions() {
    skip_if_no_network!();

    // Apparently `#[tokio::test]` must return `()`, so we create a helper
    // function that returns `Result` so we can use `?` in favor of `unwrap`.
    if let Err(err) = brocode_tool_passes_base_instructions().await {
        panic!("failure: {err}");
    }
}

async fn brocode_tool_passes_base_instructions() -> anyhow::Result<()> {
    ...
    // Run `brocode mcp` with a specific config.toml.
    let brocode_home = TempDir::new()?;
    create_config_toml(brocode_home.path(), &server.uri())?;
    let mut mcp_process = McpProcess::new(brocode_home.path()).await?;
    ...
    .await??;
    assert_eq!(brocode_response.jsonrpc, JsonRpcVersion2_0);
    assert_eq!(brocode_response.id, RequestId::Number(brocode_request_id));
    assert_eq!(
        brocode_response.result,
        json!({
            "content": [
                {
                    "text": "Enjoy!",
            ...
            "structuredContent": {
                "threadId": brocode_response
                    .result
                    .get("structuredContent")
                    .and_then(|v| v.get("threadId"))
                    .and_then(serde_json::Value::as_str)
                    .expect("brocode tool response should include structuredContent.threadId"),
                "content": "Enjoy!"
            }
        })
    );
...
/// Create a Brocode config that uses the mock server as the model provider.
/// It also uses `approval_policy = "untrusted"` so that we exercise the
/// elicitation code path for shell commands.
fn create_config_toml(brocode_home: &Path, server_uri: &str) -> std::io::Result<()> {
