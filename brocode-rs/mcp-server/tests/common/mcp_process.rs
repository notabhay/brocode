...
impl McpProcess {
    pub async fn new(brocode_home: &Path) -> anyhow::Result<Self> {
        Self::new_with_env(brocode_home, &[]).await
    }
    ...
    pub async fn new_with_env(
        ...
        let program = brocode_utils_cargo_bin::cargo_bin("brocode-mcp-server")
            .context("should find binary for brocode-mcp-server")?;
    ...
    pub async fn initialize(&mut self) -> anyhow::Result<()> {
        ...
        let originator = brocode_core::default_client::originator().value;
    ...
    pub async fn send_brocode_tool_call(
        &mut self,
        params: BrocodeToolCallParam,
    ) -> anyhow::Result<i64> {
        let brocode_tool_call_params = CallToolRequestParams {
            meta: None,
            name: "brocode".into(),
        ...
        self.send_request(
            "tools/call",
            Some(serde_json::to_value(brocode_tool_call_params)?),
        )
        .await
    }
    ...
    /// Method "brocode/event" with params.msg.type == "task_complete".
        ...
        loop {
            ...
            match message {
                JsonRpcMessage::Notification(notification) => {
                    let is_match = if notification.notification.method == "brocode/event" {
...
impl Drop for McpProcess {
    fn drop(&mut self) {
        // These tests spawn a `brocode-mcp-server` child process.
        //
