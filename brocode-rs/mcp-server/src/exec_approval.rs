...
pub struct ExecApprovalElicitRequestParams {
    ...
    // correlate the request with the brocode tool call.
    ...
    pub brocode_command: Vec<String>,
    pub brocode_cwd: PathBuf,
    pub brocode_parsed_cmd: Vec<ParsedCommand>,
}
...
pub(crate) async fn handle_exec_approval_request(
    ...
    call_id: String,
    approval_id: String,
    brocode_parsed_cmd: Vec<ParsedCommand>,
    thread_id: ThreadId,
) {
    let escaped_command =
        shlex::try_join(command.iter().map(String::as_str)).unwrap_or_else(|_| command.join(" "));
    let message = format!(
        "Allow Brocode to run `{escaped_command}` in `{cwd}`?",
    ...
    // Listen for the response on a separate task so we don't block the main agent loop.
    {
        ...
        tokio::spawn(async move {
            on_exec_approval_response(approval_id, event_id, on_response, brocode).await;
        });
    }
}
