...
use crate::brocode_tool_config::BrocodeToolCallParam;
use crate::brocode_tool_config::BrocodeToolCallReplyParam;
use crate::brocode_tool_config::create_tool_for_brocode_tool_call_param;
use crate::brocode_tool_config::create_tool_for_brocode_tool_call_reply_param;
...
pub(crate) struct MessageProcessor {
    ...
    running_requests_id_to_brocode_uuid: Arc<Mutex<HashMap<RequestId, ThreadId>>>,
}
...
impl MessageProcessor {
    ...
    pub(crate) fn new(
        ...
        Self {
            ...
            arg0_paths,
            thread_manager,
            running_requests_id_to_brocode_uuid: Arc::new(Mutex::new(HashMap::new())),
        }
    }
    ...
    async fn handle_initialize(
        ...
        let server_info = Implementation {
            name: "brocode-mcp-server".to_string(),
            title: Some("Brocode".to_string()),
        ...
        // Preserve Brocode's existing non-spec `serverInfo.user_agent` field.
        ...
        if let serde_json::Value::Object(ref mut obj) = server_info_value {
            obj.insert("user_agent".to_string(), json!(get_brocode_user_agent()));
        }
    ...
    async fn handle_list_tools(
        ...
        let result = rmcp::model::ListToolsResult {
            meta: None,
            tools: vec![
                create_tool_for_brocode_tool_call_param(),
                create_tool_for_brocode_tool_call_reply_param(),
            ],
            next_cursor: None,
        };
    ...
    async fn handle_call_tool(&self, id: RequestId, params: CallToolRequestParams) {
        ...
        match name.as_ref() {
            "brocode" => self.handle_tool_call_brocode(id, arguments).await,
            "brocode-reply" => {
                self.handle_tool_call_brocode_session_reply(id, arguments)
                    .await
            }
            _ => {
    ...
    async fn handle_tool_call_brocode(
        &self,
        id: RequestId,
        ...
        let (initial_prompt, config): (String, Config) = match arguments {
            Some(json_val) => match serde_json::from_value::<BrocodeToolCallParam>(json_val) {
                Ok(tool_cfg) => match tool_cfg.into_config(self.arg0_paths.clone()).await {
                    Ok(cfg) => cfg,
                    Err(e) => {
                        let result = CallToolResult {
                            content: vec![rmcp::model::Content::text(format!(
                                "Failed to load Brocode configuration from overrides: {e}"
                            ))],
                            structured_content: None,
                            is_error: Some(true),
                            meta: None,
                ...
                Err(e) => {
                    let result = CallToolResult {
                        content: vec![rmcp::model::Content::text(format!(
                            "Failed to parse configuration for Brocode tool: {e}"
                        ))],
                        structured_content: None,
                        is_error: Some(true),
                        meta: None,
            ...
            None => {
                let result = CallToolResult {
                    content: vec![rmcp::model::Content::text(
                        "Missing arguments for brocode tool-call; the `prompt` field is required.",
                    )],
                    structured_content: None,
                    is_error: Some(true),
                    meta: None,
        ...
        let running_requests_id_to_brocode_uuid = self.running_requests_id_to_brocode_uuid.clone();

        // Spawn an async task to handle the Brocode session so that we do not
        // block the synchronous message-processing loop.
        task::spawn(async move {
            // Run the Brocode session and stream events back to the client.
            crate::brocode_tool_runner::run_brocode_tool_session(
                id,
                initial_prompt,
                config,
                outgoing,
                thread_manager,
                running_requests_id_to_brocode_uuid,
            )
            .await;
        });
    }

    async fn handle_tool_call_brocode_session_reply(
        &self,
        ...
        // parse arguments
        let brocode_tool_call_reply_param: BrocodeToolCallReplyParam = match arguments {
            Some(json_val) => match serde_json::from_value::<BrocodeToolCallReplyParam>(json_val) {
                Ok(params) => params,
                Err(e) => {
                    tracing::error!("Failed to parse Brocode tool call reply parameters: {e}");
                    let result = CallToolResult {
                        content: vec![rmcp::model::Content::text(format!(
                            "Failed to parse configuration for Brocode tool: {e}"
                        ))],
            ...
            None => {
                tracing::error!(
                    "Missing arguments for brocode-reply tool-call; the `thread_id` and `prompt` fields are required."
                );
                let result = CallToolResult {
                    content: vec![rmcp::model::Content::text(
                        "Missing arguments for brocode-reply tool-call; the `thread_id` and `prompt` fields are required.",
        ...
        let thread_id = match brocode_tool_call_reply_param.get_thread_id() {
            Ok(id) => id,
            Err(e) => {
        ...
        let brocode = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                let result = crate::brocode_tool_runner::create_call_tool_result_with_thread_id(
                    thread_id,
        ...
        let prompt = brocode_tool_call_reply_param.prompt.clone();
        tokio::spawn({
            let outgoing = outgoing.clone();
            let running_requests_id_to_brocode_uuid = running_requests_id_to_brocode_uuid.clone();

            async move {
                crate::brocode_tool_runner::run_brocode_tool_session_reply(
                    thread_id,
                    brocode,
                    outgoing,
                    request_id,
    ...
    async fn handle_cancelled_notification(&self, params: rmcp::model::CancelledNotificationParam) {
        ...
        let thread_id = {
            let map_guard = self.running_requests_id_to_brocode_uuid.lock().await;
        ...
        // Obtain the Brocode thread from the server.
        let brocode_arc = match self.thread_manager.get_thread(thread_id).await {
            Ok(c) => c,
            Err(_) => {
                tracing::warn!("Session not found for thread_id: {thread_id}");
                return;
        ...
        // Submit interrupt to Brocode.
        if let Err(e) = brocode_arc
            .submit_with_id(Submission {
                id: request_id_string,
                op: brocode_protocol::protocol::Op::Interrupt,
                trace: None,
            })
            .await
        {
            tracing::error!("Failed to submit interrupt to Brocode: {e}");
            return;
        }
        // unregister the id so we don't keep it in the map
        self.running_requests_id_to_brocode_uuid
            .lock()
            .await
            .remove(&request_id);
    }
