...
mod brocode_tool_config;
mod brocode_tool_runner;
mod exec_approval;
...
pub use crate::brocode_tool_config::BrocodeToolCallParam;
pub use crate::brocode_tool_config::BrocodeToolCallReplyParam;
...
const OTEL_SERVICE_NAME: &str = "brocode_mcp_server";
...
mod tests {
    ...
    use brocode_core::config::types::OtelExporterKind;
    ...
    async fn mcp_server_builds_otel_provider_with_logs_traces_and_metrics() -> anyhow::Result<()> {
        ...
        let provider = brocode_core::otel_init::build_provider(
            &config,
            "0.0.0-test",
            Some(OTEL_SERVICE_NAME),
            DEFAULT_ANALYTICS_ENABLED,
