//! Asynchronous worker that executes a **Brocode** tool-call inside a spawned
...
use brocode_core::config::Config as BrocodeConfig;
...
/// To adhere to MCP `tools/call` response format, include the Brocode
...
/// Run a complete Brocode session and stream events back to the client.
///
/// On completion (success or error) the function sends the appropriate
/// `tools/call` response so the LLM can continue the conversation.
pub async fn run_brocode_tool_session(
    id: RequestId,
    initial_prompt: String,
    config: BrocodeConfig,
        ...
        Err(e) => {
            let result = CallToolResult {
                content: vec![Content::text(format!(
                    "Failed to start Brocode session: {e}"
                ))],
                is_error: Some(true),
    ...
    // Use the original MCP request ID as the `sub_id` for the Brocode submission so that
    ...
    if let Err(e) = thread.submit_with_id(submission).await {
        ...
        running_requests_id_to_brocode_uuid.lock().await.remove(&id);
        return;
    }

    run_brocode_tool_session_inner(
        thread_id,
        thread,
        outgoing,
        id,
...
pub async fn run_brocode_tool_session_reply(
    thread_id: ThreadId,
    thread: Arc<BrocodeThread>,
    outgoing: Arc<OutgoingMessageSender>,
...
async fn run_brocode_tool_session_inner(
    thread_id: ThreadId,
    thread: Arc<BrocodeThread>,
    outgoing: Arc<OutgoingMessageSender>,
    request_id: RequestId,
    ...
    loop {
        match thread.next_event().await {
            Ok(event) => {
                ...
                match event.msg {
                    ...
                    | EventMsg::DeprecationNotice(_) => {
                        ...
                        // events. Note that
                        // send(brocode_event_to_notification(&event)) above has
            ...
            Err(e) => {
                let result = create_call_tool_result_with_thread_id(
                    thread_id,
                    format!("Brocode runtime error: {e}"),
                    Some(true),
                );
                outgoing.send_response(request_id.clone(), result).await;
                break;
