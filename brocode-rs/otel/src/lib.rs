...
pub use brocode_utils_string::sanitize_metric_tag_value;
...
/// Maps to API/auth `AuthMode` to avoid a circular dependency on brocode-core.
...
impl From<brocode_app_server_protocol::AuthMode> for TelemetryAuthMode {
    fn from(mode: brocode_app_server_protocol::AuthMode) -> Self {
        match mode {
            brocode_app_server_protocol::AuthMode::ApiKey => Self::ApiKey,
            brocode_app_server_protocol::AuthMode::Chatgpt
            | brocode_app_server_protocol::AuthMode::ChatgptAuthTokens => Self::Chatgpt,
        }
    }
}
