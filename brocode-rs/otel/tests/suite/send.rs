...
fn send_builds_payload_with_tags_and_histograms() -> Result<()> {
    ...
    let counter = find_metric(&resource_metrics, "brocode.turns").expect("counter metric missing");
    ...
    let expected_counter_attributes = BTreeMap::from([
        ("service".to_string(), "brocode-cli".to_string()),
    ...
    let (bounds, bucket_counts, sum, count) =
        histogram_data(&resource_metrics, "brocode.tool_latency");
    ...
    let histogram_attrs = attributes_to_map(
        match find_metric(&resource_metrics, "brocode.tool_latency").and_then(|metric| match metric
            .data()
        {
...
fn send_merges_default_tags_per_line() -> Result<()> {
    ...
    let resource_metrics = latest_metrics(&exporter);
    let alpha_metric =
        find_metric(&resource_metrics, "brocode.alpha").expect("brocode.alpha metric missing");
    ...
    let beta_metric =
        find_metric(&resource_metrics, "brocode.beta").expect("brocode.beta metric missing");
