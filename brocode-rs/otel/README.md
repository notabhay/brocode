...
`brocode-otel` is the OpenTelemetry integration crate for Brocode. It provides:

- Provider wiring for log/trace/metric exporters (`brocode_otel::OtelProvider`
  and `brocode_otel::provider`).
- Session-scoped business event emission via `brocode_otel::SessionTelemetry`.
- Low-level metrics APIs via `brocode_otel::metrics`.
- Trace-context helpers via `brocode_otel::trace_context` and crate-root re-exports.
...
```rust
use brocode_otel::config::OtelExporter;
use brocode_otel::config::OtelHttpProtocol;
use brocode_otel::config::OtelSettings;
use brocode_otel::OtelProvider;
use tracing_subscriber::prelude::*;

let settings = OtelSettings {
    environment: "dev".to_string(),
    service_name: "brocode-cli".to_string(),
    service_version: env!("CARGO_PKG_VERSION").to_string(),
    brocode_home: std::path::PathBuf::from("/tmp"),
...
Brocode-specific session events. Rich session/business events should go through
...
`brocode-otel` also provides `OtelExporter::Statsig`, a shorthand for exporting OTLP/HTTP JSON metrics
to Statsig using Brocode-internal defaults.

Statsig ingestion (OTLP/HTTP JSON) example:

```rust
use brocode_otel::config::{OtelExporter, OtelHttpProtocol};
...
metrics.counter("brocode.session_started", 1, &[("source", "tui")])?;
metrics.histogram("brocode.request_latency", 83, &[("route", "chat")])?;
```
