...
pub(crate) struct ConfiguredHandler {
    pub event_name: brocode_protocol::protocol::HookEventName,
...
impl ConfiguredHandler {
    ...
    fn event_name_label(&self) -> &'static str {
        match self.event_name {
            brocode_protocol::protocol::HookEventName::PreToolUse => "pre-tool-use",
            brocode_protocol::protocol::HookEventName::PostToolUse => "post-tool-use",
            brocode_protocol::protocol::HookEventName::SessionStart => "session-start",
            brocode_protocol::protocol::HookEventName::UserPromptSubmit => "user-prompt-submit",
    ...
    pub(crate) fn new(
        ...
        if cfg!(windows) {
            return Self {
                handlers: Vec::new(),
                warnings: vec![
                    "Disabled `brocode_hooks` for this session because `hooks.json` lifecycle hooks are not supported on Windows yet."
                        .to_string(),
                ],
                shell,
            };
