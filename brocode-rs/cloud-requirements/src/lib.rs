//! Cloud-hosted config requirements for Brocode.
//!
...
//! requirements cannot be loaded for those accounts, Brocode fails configuration loading rather than
...
use brocode_core::auth::BrocodeAuth;
use brocode_core::auth::RefreshTokenError;
...
const CLOUD_REQUIREMENTS_FETCH_ATTEMPT_METRIC: &str = "brocode.cloud_requirements.fetch_attempt";
const CLOUD_REQUIREMENTS_FETCH_FINAL_METRIC: &str = "brocode.cloud_requirements.fetch_final";
const CLOUD_REQUIREMENTS_LOAD_METRIC: &str = "brocode.cloud_requirements.load";
...
const CLOUD_REQUIREMENTS_CACHE_WRITE_HMAC_KEY: &[u8] =
    b"brocode-cloud-requirements-cache-v3-064f8542-75b4-494c-a294-97d3ce597271";
...
fn auth_identity(auth: &BrocodeAuth) -> (Option<String>, Option<String>) {
...
impl CloudRequirementsService {
    fn new(
        ...
        Self {
            auth_manager,
            fetcher,
            cache_path: brocode_home.join(CLOUD_REQUIREMENTS_CACHE_FILENAME),
            timeout,
        }
    }
    ...
    async fn fetch_with_timeout(
        ...
        let _timer =
            brocode_otel::start_global_timer("brocode.cloud_requirements.fetch.duration_ms", &[]);
...
pub fn cloud_requirements_loader_for_storage(
    ...
    cloud_requirements_loader(auth_manager, chatgpt_base_url, brocode_home)
}
...
mod tests {
    ...
    fn write_auth_json(brocode_home: &Path, value: serde_json::Value) -> std::io::Result<()> {
        std::fs::write(
...
"#,
        ...
        assert_eq!(
            result,
            Some(ConfigRequirementsToml {
                apps: Some(brocode_core::config_loader::AppsRequirementsToml {
                    apps: BTreeMap::from([(
                        "connector_5f3c8c41a1e54ad7a76272c89e2554fa".to_string(),
                        brocode_core::config_loader::AppRequirementToml {
                            enabled: Some(false),
                        },
                    )]),
                }),
    ...
    async fn fetch_cloud_requirements_recovers_after_unauthorized_reload_updates_cache_identity() {
        ...
        let path = brocode_home.path().join(CLOUD_REQUIREMENTS_CACHE_FILENAME);
