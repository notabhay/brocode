...
fn write_user_skill(brocode_home: &TempDir, dir: &str, name: &str, description: &str) {
    let skill_dir = brocode_home.path().join("skills").join(dir);
...
fn user_config_layer(brocode_home: &TempDir, config_toml: &str) -> ConfigLayerEntry {
    let config_path = AbsolutePathBuf::try_from(brocode_home.path().join(CONFIG_TOML_FILE))
...
fn config_stack(brocode_home: &TempDir, user_config_toml: &str) -> ConfigLayerStack {
    ConfigLayerStack::new(
        vec![user_config_layer(brocode_home, user_config_toml)],
        Default::default(),
...
fn config_stack_with_session_flags(
    ...
    ConfigLayerStack::new(
        vec![
            user_config_layer(brocode_home, user_config_toml),
            ConfigLayerEntry::new(
...
fn new_with_disabled_bundled_skills_removes_stale_cached_system_skills() {
    ...
    let stale_system_skill_dir = brocode_home.path().join("skills/.system/stale-skill");
    ...
    let _skills_manager = SkillsManager::new(brocode_home.path().to_path_buf(), false);

    assert!(
        !brocode_home.path().join("skills/.system").exists(),
...
async fn skills_for_config_reuses_cache_for_same_effective_config() {
    ...
    let config_layer_stack = config_stack(&brocode_home, "");
    let skills_manager = SkillsManager::new(brocode_home.path().to_path_buf(), true);

    write_user_skill(&brocode_home, "a", "skill-a", "from a");
    ...
    write_user_skill(&brocode_home, "b", "skill-b", "from b");
...
async fn skills_for_config_excludes_bundled_skills_when_disabled_in_config() {
    let brocode_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let bundled_skill_dir = brocode_home.path().join("skills/.system/bundled-skill");
    ...
    let config_layer_stack = config_stack(&brocode_home, "[skills.bundled]\nenabled = false\n");
    let skills_manager = SkillsManager::new(brocode_home.path().to_path_buf(), false);
...
async fn skills_for_config_ignores_cwd_cache_when_session_flags_reenable_skill() {
    let brocode_home = tempfile::tempdir().expect("tempdir");
    let cwd = tempfile::tempdir().expect("tempdir");
    let skill_dir = brocode_home.path().join("skills").join("demo");
    ...
    let parent_stack = config_stack(&brocode_home, &disabled_skill_config);
