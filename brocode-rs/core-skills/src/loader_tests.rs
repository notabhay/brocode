...
use brocode_config::ConfigLayerEntry;
...
const REPO_ROOT_CONFIG_DIR_NAME: &str = ".brocode";
...
async fn make_config(brocode_home: &TempDir) -> TestConfig {
    make_config_for_cwd(brocode_home, brocode_home.path().to_path_buf()).await
}
...
fn project_layers_for_cwd(cwd: &Path) -> Vec<ConfigLayerEntry> {
    ...
    layers
        .into_iter()
        .filter_map(|dir| {
            let dot_brocode = dir.join(REPO_ROOT_CONFIG_DIR_NAME);
            dot_brocode.is_dir().then(|| {
                ConfigLayerEntry::new(
                    ConfigLayerSource::Project {
                        dot_brocode_folder: AbsolutePathBuf::from_absolute_path(dot_brocode)
                            .expect("project .brocode path should be absolute"),
                    },
...
async fn make_config_for_cwd(brocode_home: &TempDir, cwd: PathBuf) -> TestConfig {
    let user_config_path = brocode_home.path().join(CONFIG_TOML_FILE);
    let system_config_path = brocode_home.path().join("etc/brocode/config.toml");
    fs::create_dir_all(
        system_config_path
            .parent()
...
-> anyhow::Result<()> {
    ...
    let system_folder = tmp.path().join("etc/brocode");
    let home_folder = tmp.path().join("home");
    let user_folder = home_folder.join("brocode");
...
fn skill_roots_from_layer_stack_includes_disabled_project_layers() -> anyhow::Result<()> {
    ...
    fs::create_dir_all(&dot_brocode)?;

    let user_file = AbsolutePathBuf::from_absolute_path(user_folder.join("config.toml"))?;
    let project_dot_brocode = AbsolutePathBuf::from_absolute_path(&dot_brocode)?;
    ...
    let layers = vec![
        ...
        ConfigLayerEntry::new_disabled(
            ConfigLayerSource::Project {
                dot_brocode_folder: project_dot_brocode,
            },
            TomlValue::Table(toml::map::Map::new()),
            "marked untrusted",
        ),
    ...
    assert_eq!(
        got,
        vec![
            (SkillScope::Repo, dot_brocode.join("skills")),
            (SkillScope::User, user_folder.join("skills")),
            (
                SkillScope::User,
                home_folder.join(AGENTS_DIR_NAME).join(SKILLS_DIR_NAME)
...
fn write_skill(brocode_home: &TempDir, dir: &str, name: &str, description: &str) -> PathBuf {
    write_skill_at(&brocode_home.path().join("skills"), dir, name, description)
}

fn write_system_skill(brocode_home: &TempDir, dir: &str, name: &str, description: &str) -> PathBuf {
    write_skill_at(
        &brocode_home.path().join("skills/.system"),
        dir,
        name,
        description,
    )
...
async fn loads_skill_dependencies_metadata_from_yaml() {
    ...
    let skill_path = write_skill(&brocode_home, "demo", "dep-skill", "from json");
...
async fn loads_skill_policy_from_yaml() {
    let brocode_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&brocode_home, "demo", "policy-skill", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn empty_skill_policy_defaults_to_allow_implicit_invocation() {
    let brocode_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&brocode_home, "demo", "policy-empty", "from json");
    let skill_dir = skill_path.parent().expect("skill dir");
...
async fn loads_skill_policy_products_from_yaml() {
    let brocode_home = tempfile::tempdir().expect("tempdir");
    let skill_path = write_skill(&brocode_home, "demo", "policy-products", "from yaml");
    let skill_dir = skill_path.parent().expect("skill dir");
...
"#,
    ...
    assert_eq!(
        ...
        Some(SkillPolicy {
            allow_implicit_invocation: None,
            products: vec![Product::Brocode, Product::Chatgpt, Product::Atlas],
        })
    );
}
...
async fn loads_skills_via_symlinked_subdir_for_user_scope() {
    ...
    fs::create_dir_all(brocode_home.path().join("skills")).unwrap();
    symlink_dir(shared.path(), &brocode_home.path().join("skills/shared"));
...
async fn loads_skills_via_symlinked_subdir_for_repo_scope() {
    ...
    let cfg = make_config_for_cwd(&brocode_home, repo_dir.path().to_path_buf()).await;
    let outcome = load_skills_for_test(&cfg);
...
async fn system_scope_ignores_symlinked_subdir() {
    ...
    let system_root = brocode_home.path().join("skills/.system");
...
async fn respects_max_scan_depth_for_user_scope() {
    ...
    let skills_root = brocode_home.path().join("skills");
    let outcome = load_skills_from_roots([SkillRoot {
        path: skills_root,
        scope: SkillScope::User,
    }]);
...
async fn namespaces_plugin_skills_using_plugin_name() {
    ...
    fs::create_dir_all(plugin_root.join(".brocode-plugin")).unwrap();
    fs::write(
        plugin_root.join(".brocode-plugin/plugin.json"),
        r#"{"name":"sample"}"#,
    )
    .unwrap();
...
async fn skips_hidden_and_invalid() {
    let brocode_home = tempfile::tempdir().expect("tempdir");
    let hidden_dir = brocode_home.path().join("skills/.hidden");
    ...
    let invalid_dir = brocode_home.path().join("skills/invalid");
...
async fn enforces_length_limits() {
    ...
    write_skill(&brocode_home, "max-len", "max-len", &max_desc);
    let cfg = make_config(&brocode_home).await;
    ...
    write_skill(&brocode_home, "too-long", "too-long", &too_long_desc);
    let outcome = load_skills_for_test(&cfg);
    assert_eq!(outcome.skills.len(), 1);
    assert_eq!(outcome.errors.len(), 1);
    assert!(
...
#[tokio::test]
async fn loads_skills_from_agents_dir_without_brocode_dir() {
    let brocode_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
...
#[tokio::test]
async fn loads_skills_from_all_brocode_dirs_under_project_root() {
    let brocode_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
    ...
    let cfg = make_config_for_cwd(&brocode_home, nested_dir).await;
...
#[tokio::test]
async fn loads_skills_from_brocode_dir_when_not_git_repo() {
    let brocode_home = tempfile::tempdir().expect("tempdir");
    ...
    let cfg = make_config_for_cwd(&brocode_home, work_dir.path().to_path_buf()).await;
...
async fn keeps_duplicate_names_from_repo_and_user() {
    ...
    let user_skill_path = write_skill(&brocode_home, "user", "dupe-skill", "from user");
...
#[tokio::test]
async fn keeps_duplicate_names_from_nested_brocode_dirs() {
    let brocode_home = tempfile::tempdir().expect("tempdir");
    let repo_dir = tempfile::tempdir().expect("tempdir");
    mark_as_git_repo(repo_dir.path());
...
async fn repo_skills_search_does_not_escape_repo_root() {
    ...
    let cfg = make_config_for_cwd(&brocode_home, repo_dir).await;
...
async fn loads_skills_when_cwd_is_file_in_repo() {
    ...
    let cfg = make_config_for_cwd(&brocode_home, file_path).await;
...
async fn loads_skills_from_system_cache_when_present() {
    ...
    let skill_path = write_system_skill(&brocode_home, "system", "system-skill", "from system");
