...
use brocode_config::ConfigLayerStackOrdering;
use brocode_config::default_project_root_markers;
use brocode_config::merge_toml_values;
use brocode_config::project_root_markers_from_config;
...
use brocode_utils_plugins::plugin_namespace_for_skill_path;
use dirs::home_dir;
    ...
    for layer in config_layer_stack.get_layers(
        ...
        match &layer.name {
            ...
            ConfigLayerSource::User { .. } => {
                // Deprecated user skills location (`$BROCODE_HOME/skills`), kept for backward
                // compatibility.
                roots.push(SkillRoot {
                    path: config_folder.as_path().join(SKILLS_DIR_NAME),
                    scope: SkillScope::User,
                ...
                // Embedded system skills are cached under `$BROCODE_HOME/skills/.system` and are a
            ...
            ConfigLayerSource::System { .. } => {
                // The system config layer lives under `/etc/brocode/` on Unix, so treat
                // `/etc/brocode/skills` as admin-scoped skills.
                roots.push(SkillRoot {
                    path: config_folder.as_path().join(SKILLS_DIR_NAME),
...
fn discover_skills_under_root(root: &Path, scope: SkillScope, outcome: &mut SkillLoadOutcome) {
    ...
    // Follow symlinked directories for user, admin, and repo skills. System skills are written by Brocode itself.
