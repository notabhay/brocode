...
use brocode_login::default_client::build_reqwest_client;
...
fn as_query_product_surface(product_surface: RemoteSkillProductSurface) -> &'static str {
    match product_surface {
        ...
        RemoteSkillProductSurface::Brocode => "brocode",
        RemoteSkillProductSurface::Api => "api",
        RemoteSkillProductSurface::Atlas => "atlas",
    }
}

fn ensure_chatgpt_auth(auth: Option<&BrocodeAuth>) -> Result<&BrocodeAuth> {
...
pub async fn export_remote_skill(
    ...
    let output_dir = brocode_home.join("skills").join(skill_id);
