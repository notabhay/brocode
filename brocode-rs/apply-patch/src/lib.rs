...
/// Special argv[1] flag used when the Brocode executable self-invokes to run the
/// internal `apply_patch` path.
///
/// Although this constant lives in `brocode-apply-patch` (to avoid forcing
/// `brocode-arg0` to depend on `brocode-core`), it is part of the "brocode core"
/// process-invocation contract between the apply-patch runtime and the arg0
/// dispatcher.
pub const BROCODE_CORE_APPLY_PATCH_ARG1: &str = "--brocode-run-as-apply-patch";
