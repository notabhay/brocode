use crate::landlock::BROCODE_LINUX_SANDBOX_ARG0;
...
pub struct SandboxTransformRequest<'a> {
    ...
    pub brocode_linux_sandbox_exe: Option<&'a PathBuf>,
    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            Self::MissingLinuxSandboxExecutable => {
                write!(f, "missing brocode-linux-sandbox executable path")
            }
            #[cfg(not(target_os = "macos"))]
    ...
    pub fn transform(
        ...
        let (argv, arg0_override) = match sandbox {
            ...
            SandboxType::LinuxSeccomp => {
                let exe = brocode_linux_sandbox_exe
...
fn linux_sandbox_arg0_override(exe: &Path) -> String {
    if exe.file_name().and_then(|name| name.to_str()) == Some(BROCODE_LINUX_SANDBOX_ARG0) {
