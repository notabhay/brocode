...
fn system_bwrap_warning_for_lookup(system_bwrap_path: Option<PathBuf>) -> Option<String> {
    match system_bwrap_path {
        Some(_) => None,
        None => Some(
            "Brocode could not find system bubblewrap on PATH. Please install bubblewrap with your package manager. Brocode will use the vendored bubblewrap in the meantime."
                .to_string(),
        ),
    }
}
