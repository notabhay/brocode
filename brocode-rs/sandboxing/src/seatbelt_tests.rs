...
fn explicit_unreadable_paths_are_excluded_from_full_disk_read_and_write_access() {
    let unreadable = absolute_path("/tmp/brocode-unreadable");
    ...
    assert_eq!(
        ...
        vec![
            ...
            "-DWRITABLE_ROOT_0_EXCLUDED_0=/.brocode".to_string(),
...
fn explicit_unreadable_paths_are_excluded_from_readable_roots() {
    let root = absolute_path("/tmp/brocode-readable");
    let unreadable = absolute_path("/tmp/brocode-readable/private");
    let file_system_policy = FileSystemSandboxPolicy::restricted(vec![
        FileSystemSandboxEntry {
    ...
    assert!(
        args.iter()
            .any(|arg| arg.starts_with("-DWRITABLE_ROOT_0_EXCLUDED_")
                && arg.ends_with("/workspace/.brocode")),
        "expected proactive .brocode carveout for cwd root: {args:#?}",
    );
}
...
#[test]
fn create_seatbelt_args_with_read_only_git_and_brocode_subpaths() {
    // Create a temporary workspace with two writable roots: one containing
    // top-level .git and .brocode directories and one without them.
    let tmp = TempDir::new().expect("tempdir");
    let PopulatedTmp {
        vulnerable_root,
        vulnerable_root_canonical,
        dot_git_canonical,
        dot_brocode_canonical,
        empty_root,
    ...
    // write to .brocode/config.toml in the vulnerable root.
    ...
    assert!(
        ...
        "expected cwd .brocode carveout in policy:\n{policy_text}",
    );
    assert!(
        policy_text.contains("WRITABLE_ROOT_1_EXCLUDED_0")
            && policy_text.contains("WRITABLE_ROOT_1_EXCLUDED_1"),
        "expected explicit writable root .git/.brocode carveouts in policy:\n{policy_text}",
    );
    assert!(
    ...
    let expected_definitions = [
        ...
        format!(
            ...
            dot_brocode_canonical.to_string_lossy()
        ),
        format!(
            "-DWRITABLE_ROOT_2={}",
    ...
    // Verify that .brocode/config.toml cannot be modified under the generated
    // Seatbelt policy.
    let config_toml = dot_brocode_canonical.join("config.toml");
    ...
    // Verify that writing a file to the folder containing .git and .brocode is allowed.
...
#[test]
fn create_seatbelt_args_block_first_time_dot_brocode_creation_with_exact_and_descendant_carveouts()
{
    let tmp = TempDir::new().expect("tempdir");
    ...
    let dot_brocode = repo_root.join(".brocode");
    let config_toml = dot_brocode.join("config.toml");
    let policy = SandboxPolicy::WorkspaceWrite {
    ...
    let shell_command: Vec<String> = [
        ...
        "bash",
        dot_brocode.to_string_lossy().as_ref(),
    ...
    assert!(
        ...
        "expected exact .brocode carveout in policy:\n{policy_text}"
    );
    assert!(
        policy_text.contains("(require-not (subpath (param \"WRITABLE_ROOT_0_EXCLUDED_1\")))"),
        "expected descendant .brocode carveout in policy:\n{policy_text}"
    );
}
...
fn create_seatbelt_args_for_cwd_as_git_repo() {
    ...
    // `.brocode` checks are done properly for cwd.
    let policy = SandboxPolicy::WorkspaceWrite {
        writable_roots: vec![],
        read_only_access: Default::default(),
        network_access: false,
    ...
    // - write access to WRITABLE_ROOT_0 (but not its .git or .brocode), WRITABLE_ROOT_1, and cwd as WRITABLE_ROOT_2.
...
struct PopulatedTmp {
    /// Path containing a .git and .brocode subfolder.
    ...
    /// ran `git commit` themselves, or modified .brocode/config.toml to
    ...
    dot_brocode_canonical: PathBuf,

    /// Path without .git or .brocode subfolders.
    empty_root: PathBuf,
...
fn populate_tmpdir(tmp: &Path) -> PopulatedTmp {
    ...
    fs::create_dir_all(vulnerable_root.join(".brocode")).expect("create .brocode");
    fs::write(
        vulnerable_root.join(".brocode").join("config.toml"),
        "sandbox_mode = \"read-only\"\n",
    )
    .expect("write .brocode/config.toml");
    ...
    let dot_brocode_canonical = vulnerable_root_canonical.join(".brocode");
