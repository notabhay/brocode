    ...
    pub(crate) async fn record_conversation_items(
...

        {
            ...
            if should_check_image_request_limits {
                ...
                candidate_history
                    .record_items(items_to_record.iter(), turn_context.truncation_policy);
                let prompt_input =
                    candidate_history.for_prompt(&turn_context.model_info.input_modalities);
                if let Some(error) =
                    inline_image_request_limit_error(&prompt_input, &turn_context.model_info)
                {
