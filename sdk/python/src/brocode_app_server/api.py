...
class Brocode:
    ...
    # BEGIN GENERATED: Brocode.flat_methods
    def thread_start(
        self,
        *,
    ...
    # END GENERATED: Brocode.flat_methods
...
class AsyncBrocode:
    """Async mirror of :class:`Brocode`.

    Prefer ``async with AsyncBrocode()`` so initialization and shutdown are
    ...
    async def __aenter__(self) -> "AsyncBrocode":
    ...
    async def _ensure_initialized(self) -> None:
        ...
        async with self._init_lock:
            ...
            try:
                ...
                self._init = Brocode._validate_initialize(payload)
    ...
    def metadata(self) -> InitializeResponse:
        if self._init is None:
            raise RuntimeError(
                "AsyncBrocode is not initialized yet. Prefer `async with AsyncBrocode()`; "
    ...
    # BEGIN GENERATED: AsyncBrocode.flat_methods
    ...
    # END GENERATED: AsyncBrocode.flat_methods
...
class AsyncThread:
    ...
    async def turn(
        ...
        *,
        ...
        await self._brocode._ensure_initialized()
        wire_input = _to_wire_input(input)
        params = TurnStartParams(
            thread_id=self.id,
            input=wire_input,
        ...
        turn = await self._brocode._client.turn_start(
            self.id,
            wire_input,
            params=params,
        )
        return AsyncTurnHandle(self._brocode, self.id, turn.turn.id)
    # END GENERATED: AsyncThread.flat_methods

    async def read(self, *, include_turns: bool = False) -> ThreadReadResponse:
        await self._brocode._ensure_initialized()
        return await self._brocode._client.thread_read(self.id, include_turns=include_turns)

    async def set_name(self, name: str) -> ThreadSetNameResponse:
        await self._brocode._ensure_initialized()
        return await self._brocode._client.thread_set_name(self.id, name)

    async def compact(self) -> ThreadCompactStartResponse:
        await self._brocode._ensure_initialized()
        return await self._brocode._client.thread_compact(self.id)
...
class AsyncTurnHandle:
    ...
    async def steer(self, input: Input) -> TurnSteerResponse:
        await self._brocode._ensure_initialized()
        return await self._brocode._client.turn_steer(
            self.thread_id,
            self.id,
    ...
    async def interrupt(self) -> TurnInterruptResponse:
        await self._brocode._ensure_initialized()
        return await self._brocode._client.turn_interrupt(self.thread_id, self.id)

    async def stream(self) -> AsyncIterator[Notification]:
        await self._brocode._ensure_initialized()
        # TODO: replace this client-wide experimental guard with per-turn event demux.
        self._brocode._client.acquire_turn_consumer(self.id)
        try:
            while True:
                event = await self._brocode._client.next_notification()
                yield event
                if (
                    event.method == "turn/completed"
                    and isinstance(event.payload, TurnCompletedNotification)
            ...
            self._brocode._client.release_turn_consumer(self.id)
