...
RUNTIME_PKG_NAME = "brocode-cli-bin"
...
def _installed_brocode_path() -> Path:
    try:
        from brocode_cli_bin import bundled_brocode_path
    except ImportError as exc:
        raise FileNotFoundError(
            "Unable to locate the pinned Brocode runtime. Install the published SDK build "
            f"with its {RUNTIME_PKG_NAME} dependency, or set AppServerConfig.brocode_bin "
            "explicitly."
        ) from exc

    return bundled_brocode_path()


@dataclass(frozen=True)
class BrocodeBinResolverOps:
    installed_brocode_path: Callable[[], Path]
    path_exists: Callable[[Path], bool]


def _default_brocode_bin_resolver_ops() -> BrocodeBinResolverOps:
    return BrocodeBinResolverOps(
        installed_brocode_path=_installed_brocode_path,
        path_exists=lambda path: path.exists(),
    )


def resolve_brocode_bin(config: "AppServerConfig", ops: BrocodeBinResolverOps) -> Path:
    if config.brocode_bin is not None:
        brocode_bin = Path(config.brocode_bin)
        if not ops.path_exists(brocode_bin):
            raise FileNotFoundError(
                f"Brocode binary not found at {brocode_bin}. Set AppServerConfig.brocode_bin "
                "to a valid binary path."
            )
        return brocode_bin

    return ops.installed_brocode_path()


def _resolve_brocode_bin(config: "AppServerConfig") -> Path:
    return resolve_brocode_bin(config, _default_brocode_bin_resolver_ops())


@dataclass(slots=True)
class AppServerConfig:
    brocode_bin: str | None = None
    launch_args_override: tuple[str, ...] | None = None
    config_overrides: tuple[str, ...] = ()
    cwd: str | None = None
    env: dict[str, str] | None = None
    client_name: str = "brocode_python_sdk"
    client_title: str = "Brocode Python SDK"
    client_version: str = "0.2.0"
    experimental_api: bool = True


class AppServerClient:
    """Synchronous typed JSON-RPC client for `brocode app-server` over stdio."""
    ...
    def start(self) -> None:
        ...
        if self.config.launch_args_override is not None:
            ...
            brocode_bin = _resolve_brocode_bin(self.config)
            args = [str(brocode_bin)]
...
def default_brocode_home() -> str:
    return str(Path.home() / ".brocode")
