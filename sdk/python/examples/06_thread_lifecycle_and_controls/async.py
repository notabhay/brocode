...
async def main() -> None:
    async with AsyncBrocode(config=runtime_config()) as brocode:
        ...
        reopened = await brocode.thread_resume(thread.id)
        listing_active = await brocode.thread_list(limit=20, archived=False)
        reading = await reopened.read(include_turns=True)

        _ = await reopened.set_name("sdk-lifecycle-demo")
        _ = await brocode.thread_archive(reopened.id)
        listing_archived = await brocode.thread_list(limit=20, archived=True)
        unarchived = await brocode.thread_unarchive(reopened.id)

        resumed_info = "n/a"
        try:
            resumed = await brocode.thread_resume(
                unarchived.id,
                model="gpt-5.4",
        ...
        forked_info = "n/a"
        try:
            forked = await brocode.thread_fork(unarchived.id, model="gpt-5.4")
