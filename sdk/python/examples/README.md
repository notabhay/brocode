...
- `sync.py` (public sync surface: `Brocode`)
- `async.py` (public async surface: `AsyncBrocode`)

All examples intentionally use only public SDK exports from `brocode_app_server`.
...
uses the installed `brocode-cli-bin` runtime package.

If the pinned `brocode-cli-bin` runtime is not already installed, the bootstrap
will download the matching GitHub release artifact, stage a temporary local
`brocode-cli-bin` package, install it into your active interpreter, and clean up
...
active interpreter and an installed `brocode-cli-bin` runtime package (either
