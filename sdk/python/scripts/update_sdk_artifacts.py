...
def schema_root_dir() -> Path:
    return repo_root() / "brocode-rs" / "app-server-protocol" / "schema" / "json"
...
def runtime_binary_name() -> str:
    return "brocode.exe" if _is_windows() else "brocode"


def staged_runtime_bin_path(root: Path) -> Path:
    return root / "src" / "brocode_cli_bin" / "bin" / runtime_binary_name()
...
def _rewrite_sdk_runtime_dependency(pyproject_text: str, runtime_version: str) -> str:
    ...
    raw_items = [item for item in raw_items if "brocode-cli-bin" not in item]
    raw_items.append(f'"brocode-cli-bin=={runtime_version}"')
...
def stage_python_sdk_package(
    ...
    sdk_bin_dir = staging_dir / "src" / "brocode_app_server" / "bin"
...
def generate_v2_all() -> None:
    out_path = sdk_root() / "src" / "brocode_app_server" / "generated" / "v2_all.py"
...
def _notification_specs() -> list[tuple[str, str]]:
    ...
    generated_source = (
        sdk_root() / "src" / "brocode_app_server" / "generated" / "v2_all.py"
    ).read_text()
...
def _render_async_brocode_block(
    thread_start_fields: list[PublicFieldSpec],
    thread_list_fields: list[PublicFieldSpec],
    resume_fields: list[PublicFieldSpec],
    fork_fields: list[PublicFieldSpec],
...
def generate_public_api_flat_methods() -> None:
    ...
    public_api_path = src_dir / "brocode_app_server" / "api.py"
    ...
    thread_start_fields = _load_public_fields(
        "brocode_app_server.generated.v2_all",
        "ThreadStartParams",
    )
    ...
    source = _replace_generated_block(
        source,
        "Brocode.flat_methods",
        _render_brocode_block(
            thread_start_fields,
            thread_list_fields,
    ...
    source = _replace_generated_block(
        source,
        "AsyncBrocode.flat_methods",
        _render_async_brocode_block(
            thread_start_fields,
            thread_list_fields,
            thread_resume_fields,
            thread_fork_fields,
...
def build_parser() -> argparse.ArgumentParser:
    ...
    stage_sdk_parser.add_argument(
        "--runtime-version",
        required=True,
        help="Pinned brocode-cli-bin version for the staged SDK package",
    )
    stage_sdk_parser.add_argument(
        "--sdk-version",
    ...
    stage_runtime_parser.add_argument(
        "runtime_binary",
        type=Path,
        help="Path to the brocode binary to package for this platform",
    )
    stage_runtime_parser.add_argument(
        "--runtime-version",
        required=True,
