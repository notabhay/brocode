...
- `brocode-rs/tui/src/bottom_pane/chat_composer.rs`

Paste-burst detector:

- `brocode-rs/tui/src/bottom_pane/paste_burst.rs`
...
- **Persistent history** (cross-session, fetched from `~/.brocode/history.jsonl`): text-only. It
...
(`brocode-rs/tui/src/bottom_pane/chat_composer.rs`). The default config preserves current chat
behavior.
...
`brocode-rs/tui/src/bottom_pane/slash_commands.rs` and reused by both the composer and the command
