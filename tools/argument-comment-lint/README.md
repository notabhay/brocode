...
```bash
./tools/argument-comment-lint/run.sh -p brocode-core
```

Run the lint against `brocode-rs` from the repo root:

```bash
./tools/argument-comment-lint/run-prebuilt-linter.sh -p brocode-core
...
`brocode-rs` workspace with `--workspace --no-deps`.
...
```bash
./tools/argument-comment-lint/run-prebuilt-linter.sh -p brocode-core -- --all-targets
```
